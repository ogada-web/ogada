...
#### 4-6-1. NHIS `UNMATCHED` 수동 매칭 (API 구현됨, UI 후속)

자동 매칭(인정번호·이름·생년월일)에 실패한 공단 엑셀 행을 이용자와 연결하는 절차입니다.

1. **청구·정산** → **공단 import 이력**에서 해당 배치를 엽니다 (`GET /api/v1/billing/imports/nhis/{batchId}`).
2. **`UNMATCHED`** 상태 행을 확인합니다.
3. **후보 검색**: `GET /api/v1/billing/imports/nhis/{batchId}/candidates?q=홍길동`으로 같은 지점 소속 이용자를 검색합니다.
4. 올바른 이용자를 선택해 **수동 연결**합니다 (`PATCH /api/v1/billing/imports/nhis/rows/{rowId}/match`, `{ "clientId": "uuid" }`).
