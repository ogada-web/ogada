...
### Q77. Must 기능 API는 있는데 화면이 없어요. 어떻게 쓰나요?

**A.** 2026-06-06 기준 **백엔드 Must API는 구현 완료**이나, 프론트엔드는 역할별 **홈 라우팅 골격**만 있습니다.

| Must 화면 (미구현) | 대응 API | 우선 구현 순서(CHANGELOG) |
|-------------------|----------|-------------------------|
| `/clients` 이용자 CRUD | `GET/POST/PATCH /api/v1/clients*` | ② |
| `/attendance/checkin` 수기 출석 | `POST /api/v1/attendance/check-in` | ③ |
| `/health` 건강 기록 | `POST /api/v1/clients/{id}/health/*` | ④ |
| `/billing` 청구·NHIS import | `GET/POST /api/v1/billing/*` | ⑤ |
| `/dashboard` 실데이터 | `GET /api/v1/dashboard/branch` | ⑥ |
| `/guardian/checkin` QR | `POST /api/v1/attendance/qr/scan` | ⑦ |

파일럿 현장 검증은 **Swagger UI** 또는 Postman 등 API 클라이언트로 진행할 수 있습니다. UI 구현은 coder 단계 후속 작업입니다.
