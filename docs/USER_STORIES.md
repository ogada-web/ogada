...
> **작성**: planner 에이전트  
> **최초 작성일**: 2026-06-05  
> **최종 갱신**: 2026-06-06 (벤치마크 2차 조사 반영 — NHIS reconciliation·갭 로드맵)  
...
[월말] 센터장 → 출석·등급 기반 청구서 확인 → 공단 엑셀 import → 본인부담금 명세서 생성
...
- [ ] **공단 포털 직접 전송 기능 없음** — UI에 「공단 longtermcare에서 전송 후 엑셀 다운로드」 안내 문구 표시
...
| P7 | 공단 엑셀 import 후 대조 | 센터장 | US-G04 |
...
## 14. 미포함 (v1 이후) — 벤치마크 갭 로드맵 (2026-06-06)

> `BENCHMARK_REPORT.md` §5-2 / REQUIREMENTS §1-5-1 갭과 정합. **시점**은 우선순위 — 일정 무관(결정 18) 기조 유지.

| 갭 | 경쟁사 | 시점 | 후보 Epic·스토리 |
|----|--------|------|-----------------|
| G1 — 보호자 풀 포털·알림(SMS·알림톡) | 케어포 가족돌봄앱, 이지케어 문자 | **v1.1** | Epic J(신규) — `guardian_notifications`, 일일 케어 리포트 |
| G2 — 본인부담금 **CMS·간편결제** | 케어포 부가, 엔젤 CMS | **v2** | Epic K(신규) — 결제·자동이체 |
...
| G7 — 공단 엑셀 컬럼 스펙 실측 | 업계 공통 | **즉시** | US-G04/G06 인수 조건 강화 (파일럿 샘플 확보 후) |
