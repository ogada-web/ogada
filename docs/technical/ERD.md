<!-- doc:owner=DBA doc:audience=COD,PLN,TSR updated=2026-06-23T18:30:00+09:00 -->
...
> **DDL**: `src/backend/src/main/resources/db/migration/V1__*.sql` … `V175__*.sql`
...
|------|------|------|------|
| `users` | `users` + `user_account_requests`(V162·V165) + `staff_annual_leave_yearly`(V172·V173·직원 연차) + `staff_leave_ledger_entries`(V174·V175·직원 휴가 per-event ledger) | ✅ | RBAC·지점 스코프(`user_branches`)·V161 `hq_admin` 조직당 1명 UK·V162 신규 직원계정 발급 요청 워크플로(SaaS 멀티테넌트 onboard)·**V165 8 CHECK + 3 Tenant FK** defense-in-depth (G-STAFF-NHIS-EXCEL-IMPORT 대량 적재 사전 가드)·**V166 `ON_LEAVE` lifecycle** + hired_at/terminated_at pair CHECK (G-STAFF-LEAVE-STATUS·US-R03b)·**V172 직원 연차휴가 연간 현황 roster**(ezCare worker-b100 tab01·BNK-516)·**V173 4 CHECK + 1 user_branch_assignment FK** defense-in-depth (month/entitlement 상한·사용≤부여·memo 비공백·user_branches 배정 일관성)·**V174 연차·유급휴일 per-event canonical ledger**(케어포 8-13·US-R01-c·BNK-532)·**V175 1 CHECK + 1 user_branch_assignment FK** defense-in-depth (memo 비공백·user_branches 배정 일관성) |
...
| `attendance` | `attendance`(이용자) + `staff_work_attendance`(V169·직원 출퇴근) | ✅ | 수기·QR·결석 제약 반영·**V169 직원 일일 출퇴근 roster**(케어포 8-4)·**V171 `check_out_at >= check_in_at` temporal CHECK** 보강 |
...
| `billing` | `billing_claims` + `billing_claim_items` + NHIS import + dispatch + `cash_receipt_issuances` + `billing_overdue_management_records`/`billing_overdue_adjustments`(V167·V168) + `billing_report_filters`(V170) | ✅ | 청구·정산 lifecycle 전체 (V158 현금영수증 발급 대장 + V167·V168 미납 회수 CRM/금액 조정 audit trail — 케어포 PDF p.89 ②③④⑤ + **V170 대장 리포트 사용자별 필터 상태**·**V171 2 Tenant FK + temporal/nonempty CHECK** defense-in-depth) |
...
- **V169** (coder `560057f`): G-STAFF-WORK-ATTENDANCE (케어포 8-4 출퇴근·근무일지, BNK-482) — `staff_work_attendance` 직원 일일 체크인/아웃 roster. 컬럼 `work_date`·`check_in_at`·`check_out_at`·`check_in_method`(MANUAL/MOBILE/NFC enum). UK `(org, id)`·`(org, branch, user, work_date)`(직원×일자 1건)·method enum CHECK·`updated_at >= created_at` CHECK·복합 Tenant FK 3종(user_org·branch_org·**user_branch_assignment** `(org, user, branch) → user_branches` 배정 일치)·조회 인덱스 2종(`(org, branch, work_date DESC)`·`(org, user, work_date DESC)`). API `GET/POST /api/v1/staff/attendance`(HQ_ADMIN/BRANCH_ADMIN). FE `/staff/attendance` page(`53d65a0`).
- **V170** (coder `479995e`): G-BILLING 대장 리포트 사용자별 필터 상태(Q601·PLAN_NOTES #4) — `billing_report_filters` 가 charges/deposits/receipts/refunds 4-대장 별 마지막 사용 month/search/period/basis 를 silent upsert. 컬럼 `year_month`(`^\d{4}-\d{2}$` CHECK)·`variant`(4-enum CHECK)·`search_query`(≤255)·`deposit_period`(FULL/FIRST_HALF/SECOND_HALF)·`receipt_basis`(PAYMENT/CLAIM). UK `(org, branch, user, year_month, variant)` + 조회 인덱스 `(org, branch, user, year_month)`. API `GET/PUT /api/v1/billing/reports/filters`(HQ_ADMIN/BRANCH_ADMIN·`recordFromReportQuery`).
- **V171** (DBA, 본 라운드 신규): V169·V170 defense-in-depth integrity — V168 패턴(V164/V165 anchor UK)으로 두 신규 테이블 비대칭 일괄 해소. single-additive `ALTER TABLE`-only, 신규 컬럼·트리거·인덱스 0건. **① V169** `chk_staff_work_attendance_checkout_not_before_checkin`(`check_out_at IS NULL OR check_out_at >= check_in_at`) — 기존 `chk_*_checkout_after_checkin` 은 이름과 달리 "퇴근 기록 시 출근 존재"(pairing)만 강제하고 실제 시각 순서를 강제하지 않아 raw SQL·향후 모바일/NFC 일괄 동기화로 퇴근<출근(근무시간 음수) 행이 적재될 수 있던 갭(V168 `recorded_at >= created_at` 패턴). **② V170** `chk_billing_report_filters_updated_after_created`(temporal) + `chk_billing_report_filters_search_query_nonempty`(`search_query IS NULL OR length(btrim())>0` — 앱 `normalizeSearchQuery` 가 이미 공백→NULL 보장하나 raw SQL 방어) + **2 Tenant FK pair**(`(org, branch) → branches`·`(org, user) → users`) — 스키마 내 거의 모든 운영 테이블이 갖는 복합 Tenant FK 가 본 테이블에만 부재하여 cross-tenant branch/user 참조가 가능하던 비대칭 해소. **의도적 제외**: ① billing_report_filters 의 (org, user, branch) → user_branches 3-way assignment FK — `hq_admin` 은 명시적 배정 없이 전 지점 리포트 열람 가능(ERD §2 RBAC)·3-way FK 가 정당 조회 차단(V168 동일 정책) ② user purge backing index — ephemeral UI-preference 행·보존 cohort 아님·soft-delete 사용자 모델(hard DELETE 없음)·기존 scope UK prefix 가 사용자별 조회 backing. **안전성**: V169·V170 모두 신규 테이블·적재 0건 → backfill 불요. **로컬 PG14 검증 PASS**(scratch DB 에 **V1–V171 171 migrations 순차 적용 exit=0**, 갭·중복 0·contiguous · 물리 객체 실측 **3 CHECK + 2 Tenant FK** 전부 존재 · `session_replication_role=replica` 격리 **3 negative**[퇴근<출근·`updated_at<created_at`·공백 search_query 전부 정확히 거부] + **positive**[퇴근≥출근·퇴근 NULL·NULL search_query·완전 유효 FK INSERT 통과] + **Tenant FK 격리**[cross-tenant branch·미존재 user 거부]). **Must billing·attendance·NHIS 핵심 제약 7건** 불변 재확인.
- **round 191** (BE `@5fd12dd`, DBA): round 189(`83a26e7`) → backend HEAD **`5fd12dd`** — `bb9df48` **V174 `staff_leave_ledger_entries`**(coder, v3/US-R01-c 직원 연차·유급휴일 per-event canonical ledger·케어포 8-13·BNK-532)·`5fd12dd` leave-ledger live API routing harness(앱 only). `git diff --name-only 83a26e7..5fd12dd -- src/main/resources/db/migration/` = **V174 1파일**. **V174 검토 (coder 소유 — DBA confirmation)**: UK `(org, id)`·leave_type enum CHECK·`days_used` 0<×≤99.9·`end_date >= leave_date`·`updated_at >= created_at`·복합 Tenant FK 3종(user_org·branch_org·recorded_by_org)·조회 인덱스 2종(`(org, branch, leave_date DESC)`·`(org, user, leave_date DESC)`). **갭**: `trimToNull(memo)` 공백 거부·`(org, user, branch) → user_branches` 3-way 배정 FK 부재 — V173/V169 staff 도메인 비대칭 → **V175** closure. **V175 (DBA, 본 라운드 신규)** — V174 비대칭 일괄 해소: ① `chk_staff_leave_ledger_entries_memo_nonempty` ② `fk_staff_leave_ledger_entries_user_branch_assignment`. single-additive `ALTER TABLE`-only·신규 컬럼·트리거·인덱스 0건. **의도적 제외(P3)**: `recorded_by` NOT NULL·actor backstop·purge index·`days_used`↔date span cross-column 일치(앱 `resolveDaysUsed`). **안전성**: V174 적재 0건 → backfill 불요. **로컬 PG14 검증 PASS**(scratch DB V1–V175 contiguous·2 CHECK/FK 실측·NEG 2건[memo='   '·미배정 branch]·POS 2건[유효 INSERT·memo NULL]). **Must billing·attendance·NHIS 핵심 제약 7건** 불변 재확인.
- **round 189** (BE `@83a26e7`, DBA 재검증): round 188(`19ffa84`) → backend HEAD **`83a26e7`** 4 commit 전진 — **신규 DDL 0건** (V173 자체 제외). `git diff --name-only 19ffa84..83a26e7 -- src/main/resources/db/migration/` = **`V173__staff_annual_leave_yearly_integrity_us_r03e.sql` 1파일**(round 188 DBA 산출의 committed 형태, `8c5dd65` commit). 나머지 3 commit 전부 **앱 only**: `bbf333c`/`6ab3760` US-R03e 연차 ledger ↔ US-R01 출퇴근 roster cross-link 메타데이터(`StaffAnnualLeaveRelatedSurfaceResponse`·yearly/roster response)·`83a26e7` work-attendance roster 에 연차 cross-link 노출(`StaffWorkAttendanceRelatedSurfaceResponse`). `git diff 8c5dd65..83a26e7 -- src/main/java/` 의 `@Entity|@Table|@Column|CREATE TABLE|ALTER TABLE` grep = **0건**·신규 `findBy*`/`@Query`/`repository.` = **0건** — cross-link surface 는 기존 `staff_annual_leave_yearly`(V172 UK `(org, user, leave_year)`)·`staff_work_attendance`(V169 UK `(org, branch, user, work_date)`) 적재 행을 **인메모리** 결합 → 신규 조회 인덱스·DB 술어 0건. **로컬 PG14.23 검증 PASS**(`ogada_dba_scratch_v173` scratch DB 에 **V1–V173 173 migrations 순차 적용 exit=0**·갭·중복 0·contiguous[V1–V173, missing/dup 0]·public **85 테이블**·`pg_constraint` V172 4 CHECK + V173 **4 CHECK + 1 user_branch_assignment FK** 전부 실측 · `session_replication_role=replica` 격리 재실행 **NEG 4건**[month_01=100.0·entitlement=1000.0·sum 20.0>entitlement 15.0·memo='   ' 전부 정확히 거부]·**POS 2건**[유효 INSERT·entitlement NULL skip 통과]). **Must billing·attendance 핵심 제약 실측 재확인** — `attendance`(`uq_attendance_client_date`·`chk_attendance_presence_xor_absence`·`chk_attendance_checkout_after_checkin` 외 14건)·`billing_claims`(`uq_claim_branch_month`·`chk_billing_claims_amount_sum`·`chk_billing_claims_status` 외 13건) **전부 불변** → **신규 V174 불요**. PLAN_NOTES #176.
- **round 188** (BE `@19ffa84`, DBA): round 187(`d3d4d2d`) → backend HEAD **`19ffa84`** 7 commit 전진 — `41fb84f` **V172 `staff_annual_leave_yearly`**(coder·v2/US-R03e 직원 연차휴가 연간 현황 roster·ezCare worker-b100 tab01·BNK-516)·`6b84bcd`/`a45745c`/`f88e8b1`/`6b35fb5` G-STAFF-ANNUAL-LEAVE roster RBAC·pilot·검증(앱 only)·`19ffa84` j03 solapi placeholder rejection(앱 only). `git diff --name-only d3d4d2d..19ffa84 -- src/main/resources/db/migration/` = **V172 1파일**. **V172 검토 (coder 소유 — DBA confirmation)**: UK `(org, id)`·`(org, user, leave_year)` 직원×연도 1건·4 CHECK(year 2000~2100·month 비음수·entitlement 비음수·updated_after_created)·복합 Tenant FK 3종(user_org·branch_org·recorded_by_org). **갭**: `StaffAnnualLeaveService` 앱 단 강제 5종(month ≤ 99.9·entitlement ≤ 999.9·sum ≤ entitlement·memo trim-to-null·`recorded_by` actor 항상 설정) 중 처음 4종은 DB CHECK 표현 가능·5번째는 V172 `chk_*_month/entitlement_non_negative`로 raw SQL 적재 시 NUMERIC(4,1)·NUMERIC(5,1) 컬럼이 99.9/999.9 상한 무방어, sum 누적·memo 공백·user_branches 배정 일관성도 부재 → **V173** closure. **V173 (DBA, 본 라운드 신규)** — V172 비대칭 일괄 해소 (V169 user_branch_assignment·V158/V164/V165 nonempty·V171 V169·V170 defense-in-depth 패턴): **4 CHECK + 1 Tenant FK** single-additive `ALTER TABLE`-only, 신규 컬럼·트리거·인덱스 0건. ① `chk_*_month_max`(12 month_NN ≤ 99.9 일괄, 앱 `MONTHLY_USAGE_MAX` DB 미러) ② `chk_*_entitlement_max`(total_entitlement IS NULL OR ≤ 999.9, 앱 `TOTAL_ENTITLEMENT_MAX` DB 미러) ③ `chk_*_used_within_entitlement`(`total_entitlement IS NULL OR sum(month_*) ≤ total_entitlement`, 앱 `totalUsed > entitlement` 거부 DB 미러 — same-row immutable) ④ `chk_*_memo_nonempty`(`memo IS NULL OR length(btrim(memo)) > 0`, V155 WAYPOINT 패턴) ⑤ `fk_*_user_branch_assignment` `(org, user, branch) → user_branches`(V90 anchor UK·V105 staff_training_logs·V169 staff_work_attendance 동일). **의도적 제외** (P3 — 다음 라운드 재평가): `recorded_by` NOT NULL (system-actor 일괄 import 경로 보존)·`recorded_by` actor backstop 트리거(앱 항상 설정)·`recorded_by` purge backing index(HR 5년 보존 cohort·소프트 삭제 모델). **안전성**: V172 round 188 신규 도입·적재 0건 → backfill 불요. **로컬 PG14.23 검증 PASS** (`ogada_dba_scratch_v173` scratch DB 에 **V1–V173 173 migrations 순차 적용 exit=0**·갭·중복 0·contiguous·`pg_constraint` 실측 **4 CHECK + 1 Tenant FK** 전부 존재 · **POS 4건**[유효 INSERT·entitlement NULL skip·memo NULL·user_branches 일치]·**NEG 5건**[month=100.0·entitlement=1000.0·sum 20.0>entitlement 15.0·memo='   '·미배정 branch 전부 정확히 거부]). **Must billing·attendance·NHIS 핵심 제약 7건** 불변 재확인. PLAN_NOTES #176.
- **round 187** (BE `@d3d4d2d`, DBA 재검증): round 186(`9db0bbb`) → backend HEAD **`d3d4d2d`** 4 commit 전진 — **전부 앱 only, 신규 DDL 0건**(`git diff --name-only 9db0bbb..d3d4d2d -- src/main/resources/db/migration/` = **0파일**·`CREATE TABLE|ALTER TABLE|@Entity|@Table|@Column|@Query` grep = **0건**). `24d25f1` live-e2e blocker 보고 정규화·`d361833` **G2 branch-scoped CMS enrollment roster API**(`GET /api/v1/cms/enrollments?branchId=…`)·`d0c0d12` `branchId` 생략 시 `active_branch_id` fallback·`d3d4d2d` CMS branch roster scope/pilot 테스트 deepen. **신규 쿼리 2건 DB backing 대조** (신규 인덱스 0건): `CmsEnrollmentRepository.findByOrganizationIdAndBranchId[AndStatus]OrderByEnrolledAtDesc` → **V60 `idx_cms_enrollments_org_branch (organization_id, branch_id)`** equality prefix backing(`enrolled_at DESC` 는 지점 단위 bounded roster sort — round 184 attendance·round 182 billing report 동일 정책). **로컬 PG14.23 scratch DB 검증 PASS**(V1–V171 **171 migrations 순차 적용 exit=0**·contiguous·`pg_indexes` `cms_enrollments` 6 인덱스 실측·`EXPLAIN` branch roster Index Scan+Sort 풀스캔 0·V171 CHECK 3종+Tenant FK 2종 실측·public **84 테이블**). **Must billing·attendance·NHIS 핵심 제약 7건** 불변 재확인 → **신규 V172 불요**. PLAN_NOTES #175.
- **round 186+** (BE `@9db0bbb`, DBA 재검증): round 185(`479995e`/V170) → backend HEAD **`9db0bbb`** 7 commit 전진 — **전부 앱 only, 신규 DDL 0건**(`git diff --name-only 479995e..9db0bbb -- src/main/resources/db/migration/` = **V171 1파일**[round 185 DBA 산출·`6be0c79` commit]·`CREATE TABLE|ALTER TABLE|@Entity` grep = **0건**[V171 제외]). 7 commit: `6be0c79` **V171 commit**·`35e6c52` `StaffWorkAttendanceService` checkout-before-checkin 앱 가드(V171 CHECK 미러)·`fd0a3b3`/`99d03fa` `BillingReportFilterService` autosave write-scope/non-blocking read(앱)·`dce9bf1`/`175d9cb` **V171DefenseInDepthSchemaReadinessProbe**(`pg_constraint` V171 CHECK/FK 존재만 **읽기**·`HealthController`/`LiveE2eController` gate)·`9db0bbb` CMS enrollment input normalization(앱). **신규 쿼리 패턴 DB backing 대조** (신규 인덱스 0건): ① G-BILLING-DEPOSIT-ORDER-GUARD `assertPriorDepositOrderForClients` — `findByOrganizationIdAndBranchIdAndStatusAndYearMonthLessThanOrderByYearMonthAsc` → V50 `idx_billing_claims_org_branch_status_year_month` `(org, branch, status, year_month)` equality+range prefix + per-claim `findByClaimIdOrderByCreatedAtAsc`(V29) ② billing report filter autosave — `billing_report_filters` UK `(org, branch, user, year_month, variant)` + V171 Tenant FK/temporal/nonempty CHECK(변경 없음). **로컬 PG14 `mvn flyway:migrate` V169→V170→V171 적용 PASS**·`flyway:validate` **171 migrations**·`pg_constraint` 실측 5종 존재·negative/positive SQL 검증 PASS. **Must billing·attendance·NHIS 핵심 제약 7건** 불변 재확인 → **신규 V172 불요**.
- **round 184+** (BE `@61e1970`, DBA 재검증): round 184(`b583c11`) → backend HEAD **`61e1970`** 7 commit 전진 — **전부 앱 only, 신규 DDL 0건**(`git diff --name-only b583c11..HEAD -- src/main/resources/db/migration/` = **V168 1파일**[round 184 documented, `399c698` commit]·`… -- '**/*Entity.java'`=0·신규 `*.java` 의 `@Column|@Table|@Entity|CREATE TABLE|ALTER TABLE`=0건). 7 commit: `399c698` **V168 commit**(round 184 DBA 산출의 committed 형태)·`20485f1`/`018a781`/`56cb5d9` live-e2e 부트스트랩 blocker 우선순위(앱)·`a45c040` 미납 자동 SMS 중복 등록 방지(앱)·`0c69060` G-ATTENDANCE 일일 목록 지점 roster(`clientName`/파생 status)·`61e1970` roster live API routing/RBAC 테스트(앱). **신규 2 쿼리 패턴 DB backing 대조** (신규 인덱스 0건): ① `a45c040` `OverdueManagementRecordRepository.existsByOrganizationIdAndClaimIdAndClientIdAndContactMethodAndAutoGeneratedIsTrue` — V167 `idx_billing_overdue_mgmt_org_claim_client_recorded (org, claim_id, client_id, …)` 선두 3컬럼 prefix 로 존재검사 backing ② `0c69060` `AttendanceService` 지점 roster `clientRepository.findByOrganizationIdAndBranchIdAndActiveTrue(org, branch)` — V3 `idx_clients_org_branch_active (organization_id, branch_id, is_active) WHERE is_active=TRUE` partial 로 정확 매칭(파생 CHECKED_IN/CHECKED_OUT/ABSENT status 는 인메모리 — DB 술어 0건). **전체 체인 flyway 검증 PASS**(로컬 PG14 scratch DB 에 **V1–V168 168 migrations 순차 적용 exit=0**, 갭·중복 0·contiguous)·**V167/V168 negative+positive 통합검증 PASS**(`session_replication_role=replica` CHECK 격리 — 빈 note·`recorded_at<created_at`·잘못된 contact_method enum·`adjusted>previous`·빈 reason **5 negative 전부 정확 CHECK 로 거부** / 정상 mgmt·adjustment **2 positive INSERT 통과**)·V167/V168 물리 객체 실측(**7 CHECK + 8 Tenant FK + 8 index** 전부 존재). **Must billing·attendance·NHIS 핵심 제약 7건** 불변 재확인 → **신규 V169 불요**.
- **round 184** (BE `@b583c11`): **V167 G-BILLING-OVERDUE-ADJUSTMENT 미납관리 CRM/금액 조정 신규 + V168 DBA defense-in-depth integrity** — 케어포 PDF p.89 7-3 미납관리 ②③④⑤ parity. round 183(`49a1721`/G-BATHING copy) → backend HEAD **`b583c11`** 7 commit 전진 — `4d92844` **V167** `billing_overdue_management_records` (회수 CRM 대장: contact_method enum PHONE/SMS/OTHER·note·auto_generated·recorded_by/at) + `billing_overdue_adjustments` (조정 audit trail: adjustment_type enum PARTIAL/FULL_WRITE_OFF·previous/adjusted_amount nonneg + adjusted ≤ previous·reason·recorded_by/at)·`c17097d` live API routing/RBAC 테스트·`f6266ec` overdue SMS 자동 등록 scope 가드(앱)·`b583c11` G-STAFF-DOCUMENT-REPOSITORY 21-slot progress API(앱·`StaffDocumentRepositoryCompliance` — 무 DDL). **V168** (DBA, 본 라운드 신규) — V167 가 contact_method/adjustment_type enum CHECK + amount nonneg CHECK + (org, claim, client, recorded_at) 단일 인덱스만 두고 출시되었던 비대칭(Tenant FK pairs 0건·`note`/`reason` 비공백 CHECK 0건·`recorded_at`/`created_at` temporal CHECK 0건·`client_id` 보존 backing index 0건·`recorded_by` audit backing index 0건)을 V94 client_risk_assessments / V117 nursing pressure-ulcer / V164 client_care_plan_forms / V165 user_account_requests 패턴으로 일괄 해소 — **2 nonempty CHECK + 2 temporal CHECK + 8 Tenant FK + 4 backing index** single-additive `ALTER TABLE`·`CREATE INDEX` 만, 신규 컬럼·트리거 0건. 의도적 제외: ① 활성 이용자 INSERT guard — 미납 회수·금액 조정은 이용자 퇴소 후에도 수행되어야 하는 회계 lifecycle 의 일부 ② `recorded_by` 자동 채움 트리거 — V167 `NOT NULL` + `requireActorUserId` 영구 강제 ③ `(org, claim_id, client_id) → billing_claim_items` 복합 FK — `OverdueManagementService.requireClaimItem` 앱 책임(plain CHECK 표현 불가) ④ `previous_amount/adjusted_amount` ↔ `billing_claim_items.copay_amount` cross-table 일치 — immutable CHECK 표현 불가, 앱 `recalculateClaimCopayAmount` 책임(P3, 현금영수증 `amount==copay` 보류와 동일 정책). 안전성: V167 round 184 신규 도입·운영 적재 행 0건 → backfill 불요·즉시 검증 PASS. **로컬 PG14 검증 PASS**(`psql --single-transaction` SAVEPOINT 5 negative + 2 positive — 빈 note·`recorded_at < created_at`·cross-tenant branch·cross-tenant recorded_by·빈 reason 거부 / 정상 INSERT 통과)·`mvn flyway:validate` **168 migrations**·`OverdueManagementServiceTest`·`OverdueManagementLiveApiRoutingE2eTest` **12/12 PASS**.
- **round 183** (BE `@49a1721`): **G-BATHING 전월 일정 복사 + G21 대시보드 NHIS gap count — 앱 only, 신규 DDL 0건**. round 182(`7b99313`/G-BILLING 리포트 필터) → backend HEAD **`49a1721`** 4 commit 전진 — `49a1721` **G-BATHING `POST /api/v1/care/bathing-schedules/copy-from-previous-month`**(`BathingScheduleService.copyFromPreviousMonth` — 전월 SCHEDULED/COMPLETED 일정을 대상월 동일 일자로 일괄 생성·CANCELLED/SKIPPED·기존 UK 충돌 skip)·`0796821` **G21 대시보드 `nhisComparisonGapCount` 노출**(`DashboardService`→`VisitService.getNhisComparison` 인메모리 집계)·`ad11fda` notification quiet-hours 테스트 deepen·`0c9518a` live-e2e bootstrap blocker 우선순위(앱 only). `git diff --name-only 7b99313..49a1721 -- src/main/resources/db/migration/` = **0파일**·`… -- '**/*Entity.java' '**/*Repository.java'` = **0파일**·`CREATE TABLE|ALTER TABLE|@Entity|@Column|@Table` grep = **0건**. **G-BATHING copy DB backing 대조** (신규 인덱스 0건): ① 전월 소스 적재 — `findByOrganizationIdAndBranchIdAndScheduledDateBetweenOrderByScheduledDateDescScheduledTimeDescCreatedAtDescClientIdAsc`(V136 `idx_bathing_schedules_org_branch_scheduled_date`) ② 대상월 occupied keys — 동일 메서드 ③ 신규 행 INSERT — V136 UK `(org, client_id, scheduled_date)` + `DataIntegrityViolationException` 앱 catch. `mapToTargetMonthDate`(말일→짧은 달 clamp)는 앱 도메인 — DB CHECK 불요. **G21 dashboard gap count**: 기존 V53 visit_schedules·V38/V28 NHIS import 인덱스 위 `getNhisComparison` 재사용 — 신규 쿼리·DDL 0건. **Must billing·attendance·NHIS 핵심 제약 7건** SQL 물리 재확인 → **전부 불변**·`ls db/migration | wc -l` = **166** contiguous(V1–V166, 갭·중복 0) → **신규 V167 불요**.
- **round 182** (BE `@7b99313`): **G-BILLING 대장 리포트 필터 — 앱 only, 신규 DDL 0건**. round 181(`1d7cee2`/V166) → backend HEAD **`7b99313`** 4 commit 전진 — `b96d038` 입금대장 반월(`FULL`/`FIRST_HALF`/`SECOND_HALF`)·수납대장 기준(`PAYMENT`/`CLAIM`) 필터 추가(`GET /billing/reports/{variant}`, 케어포 PDF p.91)·`375fb9d` 리포트 필터 달력월 검증·`14935a3` 적용 필터 echo·`7b99313` 필터 단언 테스트. `git diff --name-only 1d7cee2..7b99313 -- src/main/resources/db/migration/` = **0파일**·`… -- '**/*Entity.java' '**/*Repository.java'` = **0파일**·`CREATE TABLE|ALTER TABLE|@Entity|@Column|@Table` grep = **0건**. **신규 필터 DB backing 대조** (신규 인덱스 0건): 입금대장 반월·수납대장 기준은 모두 **인메모리** 필터 — `buildPaymentReportItems`가 `findByOrganizationIdAndBranchIdAndStatusOrderByPaidAtDesc(org, branch, PAID)`(V153 `idx_billing_claims_org_branch_status_paid_at` partial PAID·`paid_at DESC`)로 적재 후 `matchesDepositPeriod`(paid_at 일자 1–15/16–말)·`matchesReceiptLedgerMonth`(PAYMENT=수납월·CLAIM=청구년월)로 분기. 청구대장은 `findByOrganizationIdAndBranchIdAndYearMonthOrderByGeneratedAtDesc`(V149 `(org, branch, year_month DESC, status, generated_at DESC)` + V1 `idx_billing_claims_org_branch_month`)로 backing — 신규 DB 술어 0건. **Must billing·attendance·NHIS 핵심 제약 7건** SQL 물리 재확인 → **전부 불변**·`ls db/migration | wc -l` = **166** contiguous(V1–V166, 갭·중복 0) → **신규 V167 불요**.
- **round 181** (BE `@1d7cee2`): **V166 G-STAFF-LEAVE-STATUS `ON_LEAVE` lifecycle** (coder `68d4457`) + lifecycle summary API (coder `1d7cee2` — **앱 only**). **V166** `chk_users_lifecycle_status` enum에 `ON_LEAVE` 추가 + `chk_users_on_leave_requires_hired_at`(`ON_LEAVE` → `hired_at` NOT NULL) + `chk_users_on_leave_no_termination_date`(`ON_LEAVE` → `terminated_at` NULL) — V87 temporal/state pair-CHECK 패턴으로 `UserService.validateLifecycleState()` ON_LEAVE 가드 DB 미러. `is_active=false` 전이는 앱(`UserService`·`StaffTrainingLogService` ON_LEAVE 제외) 책임 — TERMINATED와 동일하게 DB pair CHECK 보류(P3). `GET /api/v1/staff/lifecycle-summary` `onLeaveCount`는 `userRepository.findByOrganizationId` + `idx_users_org_lifecycle_status`(V86) 재사용·인메모리 집계 → 신규 인덱스 불요. **Must billing·attendance·NHIS 핵심 제약 7건** 불변 재확인 → **신규 V167 불요**.
- **round 180** (BE `@7d29a38`): **V165 후속 — 앱 only 5 commit, 신규 DDL 0건**. round 179(`@6f7f145`/V165 신규) → backend HEAD **`7d29a38`** 6 commit 전진 — `05ca0a8` **V165 commit**(라운드 179 DBA 산출의 committed 형태)·`3bbfc00` **live-e2e probe gate on V165**(`UserAccountRequestSchemaReadinessProbe` — `pg_constraint`에서 `chk_user_account_requests_review_state_pair` 존재만 **읽기**·`/health`·`/live-e2e/probe` operation readiness 와이어, NHIS caregiver bulk import 가 defense-in-depth 결손 스키마에서 실행되지 않도록 차단)·`b11e29a` G21 seed branch service_type 정규화(앱 only)·`2f6f3bc`/`e3b74a0`/`7d29a38` G-STAFF-NHIS-EXCEL-IMPORT·G-BANK-EXCEL-8 선택 검증(앱 only). `git diff --name-only 6f7f145..7d29a38 -- src/main/resources/db/migration/` = **`V165__user_account_requests_integrity.sql` 1파일**(round 179 documented, `05ca0a8` commit), `… -- '**/*Entity.java' '**/*Repository.java'` = **0파일**, `CREATE TABLE|ALTER TABLE|@Entity|@Column|@Query` grep 5-commit 차분 = **0건**(V165 그 자체 제외). probe(`pg_constraint` SELECT)·gate(`HealthController.isOperationReady`)는 V165 CHECK 읽기 전용 — 스키마 영향 0. **Must billing·attendance·NHIS 핵심 제약 7건** 불변 재확인. `ls db/migration | wc -l` = **165** contiguous(V1–V165, 갭·중복 0). **신규 V166 불요**.
...
erDiagram
    users {
        ...
        boolean is_active
        varchar lifecycle_status "ONBOARDING|ACTIVE|ON_LEAVE|OFFBOARDING|TERMINATED, V86/V166"
        date hired_at "V86"
...
- `POST /api/v1/care/bathing-schedules/copy-from-previous-month` (G-BATHING, 케어포 module3 3-3 전월 일정 불러 일괄생성) — `BathingScheduleService.copyFromPreviousMonth`가 전월 `findBy…ScheduledDateBetween`(V136 list index)로 소스 적재 후 대상월 동일 일자로 SCHEDULED 행 INSERT — COPYABLE=`SCHEDULED|COMPLETED`만·CANCELLED/SKIPPED·UK 충돌·말일 clamp skip은 **인메모리**(`49a1721`, **신규 DDL 0건**). RBAC: `HQ_ADMIN`/`BRANCH_ADMIN`/`SOCIAL_WORKER`.
...
| `V166__staff_leave_status_on_leave.sql` | v2 G-STAFF-LEAVE-STATUS (FAQ 21720·BNK-446·US-R03b) — `users.lifecycle_status` enum에 **`ON_LEAVE`(휴직)** 추가. `chk_users_lifecycle_status` 재정의(5값) + `chk_users_on_leave_requires_hired_at`(`ON_LEAVE` → `hired_at` NOT NULL) + `chk_users_on_leave_no_termination_date`(`ON_LEAVE` → `terminated_at` NULL). V87 temporal/state pair-CHECK 패턴 — `UserService.validateLifecycleState()` ON_LEAVE 가드 DB 미러. `is_active=false` 전이·복직(ON_LEAVE→ACTIVE)은 앱 책임 — DB pair CHECK 보류(P3, TERMINATED와 동일). 신규 컬럼·트리거·인덱스 0건·기존 ACTIVE 행 영향 0. API: `PATCH /users/{userId}` lifecycle 전이·`GET /api/v1/staff/lifecycle-summary` `onLeaveCount`(coder `1d7cee2`, 인메모리 집계 — V86 `idx_users_org_lifecycle_status` 재사용) |
| `V167__billing_overdue_management_and_adjustments.sql` | v2 G-BILLING-OVERDUE-ADJUSTMENT (케어포 PDF p.89 7-3 미납관리 ②③④⑤) — `billing_overdue_management_records`(회수 CRM 대장: `contact_method` enum PHONE/SMS/OTHER·`note`·`auto_generated`·`recorded_by/at`) + `billing_overdue_adjustments`(조정 audit trail: `adjustment_type` enum PARTIAL/FULL_WRITE_OFF·`previous/adjusted_amount` nonneg + adjusted ≤ previous·`reason`·`recorded_by/at`). enum CHECK + amount nonneg CHECK + `(org, claim, client, recorded_at)` 단일 인덱스. coder `4d92844` — Tenant FK pair·nonempty/temporal CHECK·backing index 는 **V168(DBA)** 에서 보강 |
| `V168__billing_overdue_management_and_adjustments_integrity.sql` | (DBA) V167 defense-in-depth — 두 테이블에 **2 nonempty CHECK**(`note`/`reason` `length(trim())>0`) + **2 temporal CHECK**(`recorded_at >= created_at`) + **8 Tenant FK**(각 테이블 branch_org·claim_org·client_org·recorded_by_org → V4/V10/V5 anchor) + **4 backing index**(`client_id` 보존 cohort·`(org, recorded_by)` 감사). single-additive `ALTER TABLE`·`CREATE INDEX` 만·신규 컬럼·트리거 0건. 의도적 제외: 활성 이용자 INSERT guard(회수·조정은 퇴소 후에도 수행)·`amount==copay` cross-table(immutable CHECK 표현 불가, 앱 책임 P3). V167 적재 0건 → backfill 불요. 로컬 PG14 5 negative + 2 positive PASS |
| `V169__staff_work_attendance_g_staff_work_attendance.sql` | v1.2.1 G-STAFF-WORK-ATTENDANCE (케어포 8-4 출퇴근·근무일지, BNK-482) — `staff_work_attendance`(직원 일일 체크인/아웃 roster). 컬럼 `work_date`·`check_in_at`·`check_out_at`·`check_in_method`(MANUAL/MOBILE/NFC). UK `(org, id)`·`(org, branch, user, work_date)`(직원×일자 1건) + method enum CHECK + `chk_*_checkout_after_checkin`(퇴근 시 출근 존재) + `chk_*_updated_after_created` + 복합 Tenant FK 3종(user_org·branch_org·**user_branch_assignment** `(org, user, branch) → user_branches`) + 조회 인덱스 2종(`(org, branch, work_date DESC)`·`(org, user, work_date DESC)`). coder `560057f`. 퇴근≥출근 시각 순서는 **V171(DBA)** 보강. API `GET/POST /api/v1/staff/attendance` |
| `V170__billing_report_filters.sql` | v2 G-BILLING (Q601·PLAN_NOTES #4) — `billing_report_filters`(charges/deposits/receipts/refunds 4-대장 사용자별 마지막 필터 상태 silent upsert). 컬럼 `year_month`(`^\d{4}-\d{2}$` CHECK)·`variant`(4-enum CHECK)·`search_query`(≤255)·`deposit_period`(FULL/FIRST_HALF/SECOND_HALF CHECK)·`receipt_basis`(PAYMENT/CLAIM CHECK). UK `(org, branch, user, year_month, variant)` + 조회 인덱스 `(org, branch, user, year_month)`. coder `479995e`. Tenant FK pair·temporal/search nonempty CHECK 는 **V171(DBA)** 보강. API `GET/PUT /api/v1/billing/reports/filters` |
| `V171__staff_work_attendance_and_billing_report_filters_integrity.sql` | (DBA) V169·V170 defense-in-depth — **① staff_work_attendance** `chk_*_checkout_not_before_checkin`(`check_out_at IS NULL OR check_out_at >= check_in_at`) — 기존 `chk_*_checkout_after_checkin` 은 pairing 만 강제하고 시각 순서 미강제(퇴근<출근 근무시간 음수 갭, V168 temporal 패턴). **② billing_report_filters** `chk_*_updated_after_created`(temporal) + `chk_*_search_query_nonempty`(`IS NULL OR length(btrim())>0` — 앱 `normalizeSearchQuery` 공백→NULL 미러) + **2 Tenant FK pair**(`(org,branch)→branches`·`(org,user)→users`, 스키마 내 본 테이블만 부재하던 비대칭 해소). 의도적 제외: 3-way user_branches FK(`hq_admin` 전 지점 열람·V168 정책)·user purge index(ephemeral UI 상태·soft-delete). single-additive `ALTER TABLE` only·신규 컬럼·트리거·인덱스 0건. V169·V170 적재 0건 → backfill 불요. **로컬 PG14 V1–V171 contiguous 적용 PASS · 3 CHECK + 2 Tenant FK 실측 · 3 negative + positive + cross-tenant FK 격리 검증 PASS** |
| `V172__staff_annual_leave_yearly_us_r03e.sql` | v2/US-R03e — `staff_annual_leave_yearly`(직원 연차휴가 연간 현황 roster — ezCare worker-b100 tab01·BNK-516). 12 month_NN `NUMERIC(4,1) DEFAULT 0`·`total_entitlement NUMERIC(5,1)` nullable·`memo TEXT` nullable·`recorded_by` nullable·UK `(org, id)` + `(org, user, leave_year)` 직원×연도 1건·4 CHECK(`leave_year` 2000~2100·month 비음수·entitlement 비음수·`updated_at >= created_at`)·복합 Tenant FK 3종(user_org·branch_org·recorded_by_org)·조회 인덱스 2종(`(org, branch, leave_year)`·`(org, user, leave_year DESC)`). coder `41fb84f`. 상한·동일행 누적·텍스트 비공백·user_branches 배정 일관성은 **V173(DBA)** 보강. API `GET/PUT /api/v1/staff/{userId}/annual-leave` + `GET /api/v1/staff/annual-leave/roster?branchId=…&year=…` (HQ_ADMIN/BRANCH_ADMIN write·branch 스코프 read) |
| `V173__staff_annual_leave_yearly_integrity_us_r03e.sql` | (DBA) V172 defense-in-depth — **4 CHECK + 1 Tenant FK**: ① `chk_*_month_max` 12 month_NN ≤ 99.9 일괄(앱 `MONTHLY_USAGE_MAX` DB 미러·NUMERIC(4,1) 999.9 허용 → 99.9 로 좁힘) ② `chk_*_entitlement_max` `total_entitlement IS NULL OR ≤ 999.9`(앱 `TOTAL_ENTITLEMENT_MAX` DB 미러·NUMERIC(5,1) 9999.9 → 999.9) ③ `chk_*_used_within_entitlement` `total_entitlement IS NULL OR sum(month_*) ≤ total_entitlement`(same-row immutable·앱 `totalUsed > entitlement` 거부 DB 미러) ④ `chk_*_memo_nonempty` `memo IS NULL OR length(btrim()) > 0`(V155 WAYPOINT 패턴·앱 `trimToNull(memo)` DB 미러) ⑤ `fk_*_user_branch_assignment` `(org, user, branch) → user_branches`(V90 anchor UK·V169 staff_work_attendance 동일·직원이 본인 배정 지점의 연차만 기록). single-additive `ALTER TABLE`-only, 신규 컬럼·트리거·인덱스 0건. 의도적 제외(P3): `recorded_by` NOT NULL·actor backstop 트리거·purge backing index(향후 system-actor 일괄 import·HR 5년 cohort·소프트 삭제 모델 보존). V172 적재 0건 → backfill 불요. **로컬 PG14.23 V1–V173 contiguous 적용 PASS · 4 CHECK + 1 Tenant FK 실측 · 4 positive + 5 negative 검증 PASS** |
| `V174__staff_leave_ledger_us_r01c.sql` | v3/US-R01-c — `staff_leave_ledger_entries`(직원 연차·유급휴일 per-event canonical ledger — 케어포 8-13·BNK-532). 컬럼 `leave_type`(ANNUAL_LEAVE/PAID_HOLIDAY)·`leave_date`·`end_date` nullable·`days_used NUMERIC(4,1)`·`memo`·`recorded_by` nullable·UK `(org, id)` + 5 CHECK(leave_type enum·days_used 0<×≤99.9·end_date order·updated_after_created) + 복합 Tenant FK 3종(user_org·branch_org·recorded_by_org) + 조회 인덱스 2종(`(org, branch, leave_date DESC)`·`(org, user, leave_date DESC)`). coder `bb9df48`. memo 비공백·user_branches 배정 일관성은 **V175(DBA)** 보강. API `GET/POST/PUT/DELETE /api/v1/staff/leave-ledger` + `GET /api/v1/staff/{userId}/leave-ledger` (HQ_ADMIN/BRANCH_ADMIN) |
| `V175__staff_leave_ledger_entries_integrity_us_r01c.sql` | (DBA) V174 defense-in-depth — **1 CHECK + 1 Tenant FK**: ① `chk_staff_leave_ledger_entries_memo_nonempty` `memo IS NULL OR length(btrim(memo)) > 0`(V173/V155 nonempty 패턴·앱 `trimToNull(memo)` DB 미러) ② `fk_staff_leave_ledger_entries_user_branch_assignment` `(org, user, branch) → user_branches`(V90 anchor UK·V169/V173 staff 도메인 동일·직원이 본인 배정 지점의 휴가만 기록). single-additive `ALTER TABLE`-only, 신규 컬럼·트리거·인덱스 0건. 의도적 제외(P3): `recorded_by` NOT NULL·actor backstop·purge index·`days_used`↔date span cross-column(앱 `resolveDaysUsed`). V174 적재 0건 → backfill 불요. **로컬 PG14 V1–V175 contiguous 적용 PASS · 1 CHECK + 1 Tenant FK 실측 · 2 negative + 2 positive 검증 PASS** |

> **주의**: `V2__attendance_add_attendance_date.sql`은 V2와 버전 충돌 — 삭제됨. 출석일 컬럼은 `V2__mvp_complement_schema.sql`에 포함.
>
> **검증 상태 (2026-06-23 @ backend `5fd12dd` — V174 G-STAFF-LEAVE-LEDGER 신규 + V175 defense-in-depth integrity)**: round 189(`83a26e7`) → backend HEAD **`5fd12dd`** — `bb9df48` **V174 `staff_leave_ledger_entries`**(coder, US-R01-c·케어포 8-13·BNK-532)·`5fd12dd` leave-ledger live API routing harness(앱). `git diff --name-only 83a26e7..5fd12dd -- src/main/resources/db/migration/` = **V174 1파일**·`git diff … -- '**/*Entity.java' '**/*Repository.java'` = **2파일**(`StaffLeaveLedgerEntryEntity`·`StaffLeaveLedgerEntryRepository`, V174 짝). **V174 검토**: UK `(org, id)`·leave_type enum·days_used bounds·end_date order·Tenant FK 3종·조회 인덱스 2종. **갭**: memo nonempty·user_branch_assignment FK 부재 → **V175** closure. **DBA V175** — V173/V169 패턴: `chk_*_memo_nonempty` + `fk_*_user_branch_assignment`. **로컬 PG14 검증 PASS**: V1–V175 **175 migrations** contiguous·2 constraint 실측·NEG 2·POS 2. **Must billing·attendance·NHIS 핵심 제약 7건** 불변. agents.yaml `core_entities` 11종 전수 충족(`users` V174/V175 carry). `ls db/migration | wc -l` = **175**. 본 라운드 산출: ERD 헤더·§1 round 191·§7 V174/V175·DATA_RETENTION·PLAN_NOTES #177. **보류 carry**: V174 `recorded_by` NOT NULL/actor backstop(P3)·`days_used`↔date span(P3)·전 라운드 보류 목록 동일. **coder**: V175 additive — `StaffLeaveLedgerService` 정상 경로 영향 0. `mvn flyway:migrate`·`mvn test -Dtest='*StaffLeaveLedger*'` 재검증 권장. `StaffLeaveLedgerSchemaReadinessProbe`(V173/V171 패턴) 후속 추가 권장.
>
> **검증 상태 (2026-06-22 @ backend `19ffa84` — V172 G-STAFF-ANNUAL-LEAVE 신규 + V173 defense-in-depth integrity)**: round 187(`d3d4d2d`/CMS branch roster·신규 V172 불요) → backend HEAD **`19ffa84`** 7 commit 전진 — `41fb84f` **V172 `staff_annual_leave_yearly`**(coder, US-R03e·ezCare worker-b100 tab01·BNK-516)·`6b84bcd`/`a45745c`/`f88e8b1`/`6b35fb5` G-STAFF-ANNUAL-LEAVE RBAC·pilot·precision/range/payload 검증(전부 앱)·`19ffa84` j03 solapi placeholder rejection(앱). `git diff --name-only d3d4d2d..19ffa84 -- src/main/resources/db/migration/` = **V172 1파일**·`git diff … -- '**/*Entity.java' '**/*Repository.java'` = **2파일**(`StaffAnnualLeaveYearlyEntity`·`StaffAnnualLeaveYearlyRepository`, V172 짝). **V172 검토 (coder 소유 — DBA confirmation)**: UK `(org, id)`·`(org, user, leave_year)`·4 CHECK(year 2000~2100·month 비음수·entitlement 비음수·updated_after_created)·복합 Tenant FK 3종(user_org·branch_org·recorded_by_org)·조회 인덱스 2종. **갭 식별**: ① `MONTHLY_USAGE_MAX = 99.9` 컬럼 NUMERIC(4,1) 999.9 허용 — 99.9 상한 미강제 ② `TOTAL_ENTITLEMENT_MAX = 999.9` NUMERIC(5,1) 9999.9 허용 — 999.9 상한 미강제 ③ sum(month) ≤ entitlement same-row 누적 잔여일 음수 가능 — 미강제 ④ memo='   ' 공백만 적재 가능(앱 `trimToNull` 우회) ⑤ `(org, user, branch) → user_branches` 3-way 배정 FK 부재(스키마 내 V90/V105/V169 staff 테이블만 보유하던 비대칭) → **V173** closure. **DBA 신규 V173** — V169 user_branch_assignment·V158/V164/V165 nonempty·V171 V169·V170 defense-in-depth 패턴 일괄 흡수: ① `chk_staff_annual_leave_yearly_month_max` 12 month 일괄 ② `chk_*_entitlement_max` ③ `chk_*_used_within_entitlement` ④ `chk_*_memo_nonempty` ⑤ `fk_*_user_branch_assignment`. single-additive `ALTER TABLE` only·신규 컬럼·트리거·인덱스 0건. **의도적 제외**: ① `recorded_by` NOT NULL — 향후 NHIS Excel 일괄 import·system-actor 경로 보존(P3) ② `recorded_by` actor backstop 트리거 — 앱 `requireActorUserId()` 항상 설정·Tenant FK 가 cross-tenant 차단(P3) ③ `recorded_by` purge backing partial index — HR 5년 보존 cohort·소프트 삭제 모델 hard DELETE 없음(P3). **로컬 PG14.23 검증 PASS**: scratch DB `ogada_dba_scratch_v173`에 V1–V173 **173 migrations 순차 적용 exit=0**(NOTICE 만, ERROR 0·갭·중복 0·contiguous)·`pg_constraint` 실측 **chk_*_month_max·chk_*_entitlement_max·chk_*_used_within_entitlement·chk_*_memo_nonempty·fk_*_user_branch_assignment 5종 전부 존재**·**4 positive**[유효 INSERT·entitlement NULL skip·memo NULL·user_branches 배정 일치 INSERT 통과] + **5 negative**[month=100.0·entitlement=1000.0·sum 20.0>entitlement 15.0·memo='   '·미배정 branch 전부 정확한 CHECK/FK 로 거부]. **Must billing·attendance·NHIS 핵심 제약 7건** 불변 재확인(`uq_claim_branch_month` V1·`uq_billing_claim_items_claim_client` V26·`chk_billing_claims_amount_sum` V6·`trg_billing_claims_total_reconciliation` V11·`chk_attendance_presence_xor_absence` V11/V14·`uq_nhis_import_rows_org_id` V37·`chk_nhis_import_rows_match_requires_client` V19→V54). agents.yaml `core_entities` 11종 전수 충족(`users` V172/V173 carry — staff_annual_leave_yearly). `ls db/migration | wc -l` = **173** contiguous(V1–V173, 갭·중복 0). 본 라운드 산출: ERD 헤더 메타·DDL 범위 `…V173`·§1 Must coverage `users` 행 갱신·§1 round 188 항목·§7 마이그레이션 V172/V173 2행·§7 본 검증 헤더·DATA_RETENTION §3 staff_annual_leave_yearly 보존 정책·PLAN_NOTES #176. **보류 carry**(전 라운드 동일): V169 same-day cross-row overlap(P3)·V166 `is_active` pairing(P3)·V162 `branch_ids` 요소 UUID(P3)·V172 `recorded_by` NOT NULL/actor backstop/purge index(P3)·현금영수증 cross-table/time(P2)·PLAN↔BILLING reciprocal(P2)·`billing_payments`(Epic L). **coder**: V173 는 additive CHECK + Tenant FK — `StaffAnnualLeaveService` 정상 경로 영향 0(앱이 동일 불변식 이미 강제). `mvn flyway:migrate`(V172 적재 0건 가정)·`mvn test -Dtest='*StaffAnnualLeave*'` 재검증 권장.
>
> **검증 상태 (2026-06-22 @ backend `9db0bbb` — V171 후속 앱 only · G-BILLING-DEPOSIT-ORDER-GUARD · V171 readiness probe · 신규 V172 불요)**: round 185(`479995e`/V170) → backend HEAD **`9db0bbb`** 7 commit 전진 — `6be0c79` **V171 commit**(DBA defense-in-depth)·`35e6c52` staff checkout 앱 가드·`fd0a3b3`/`99d03fa` billing report filter autosave scope/non-blocking(앱)·`dce9bf1`/`175d9cb` **V171DefenseInDepthSchemaReadinessProbe** + operation readiness gate(읽기 전용)·`9db0bbb` CMS enrollment normalization(앱). `git diff --name-only 479995e..9db0bbb -- src/main/resources/db/migration/` = **V171 1파일만**(round 185 documented)·`CREATE TABLE|ALTER TABLE|@Entity|@Column` grep(V171 제외) = **0건**. **G-BILLING-DEPOSIT-ORDER-GUARD DB backing**: `BillingService.assertPriorDepositOrderForClients` → `findByOrganizationIdAndBranchIdAndStatusAndYearMonthLessThanOrderByYearMonthAsc`(CONFIRMED·`year_month` range) backed by V50 `idx_billing_claims_org_branch_status_year_month` + V5 `(org, branch, status, year_month DESC)` — per-claim item scan은 `findByClaimIdOrderByCreatedAtAsc`(V29)·인메모리 `clientIds` 교집합, 신규 DB 술어·인덱스 0건. **V171 probe**: `pg_constraint` SELECT only — 스키마 변경 0. **로컬 PG14 검증 PASS**: dev DB `mvn flyway:migrate` V168→V169→V170→V171 적용·`flyway:validate` **171 contiguous**·`pg_constraint` 실측 **chk_staff_work_attendance_checkout_not_before_checkin·chk_billing_report_filters_updated_after_created·chk_billing_report_filters_search_query_nonempty·fk_billing_report_filters_branch_org·fk_billing_report_filters_user_org 5종**·SQL negative(퇴근<출근·공백 search_query 거부)·positive(유효 INSERT 통과). **Must billing·attendance·NHIS 핵심 제약 7건** 불변 재확인. agents.yaml `core_entities` 11종 전수 충족. `ls db/migration | wc -l` = **171**. **결론**: **신규 V172 불요**. 본 라운드 산출: ERD §1 round 186+·§7 본 검증 헤더·메타 timestamp + PLAN_NOTES #174. **보류 carry**: V169 same-day cross-row overlap(P3)·V166 `is_active` pairing(P3)·V162 `branch_ids` 요소 UUID(P3)·현금영수증 cross-table/time(P2)·PLAN↔BILLING reciprocal(P2)·`billing_payments`(Epic L). **coder**: 추가 DDL 0건 — V171 probe·deposit-order guard·filter autosave는 기존 스키마 위 앱/읽기 전용 gate.
>
> **검증 상태 (2026-06-22 @ backend `479995e` — V169 `staff_work_attendance`·V170 `billing_report_filters` defense-in-depth V171(DBA) 신규 1건)**: ERD 가 V168 까지만 문서화한 사이 backend HEAD 가 `479995e`(V170)까지 전진 — 신규 coder 테이블 **V169 `staff_work_attendance`**(`560057f` G-STAFF-WORK-ATTENDANCE 직원 출퇴근 roster)·**V170 `billing_report_filters`**(`479995e` G-BILLING 대장 리포트 사용자별 필터) 2건이 DBA 미검토 상태였다. 양 테이블 모두 attendance·billing **Must 엔티티 인접**. **갭 식별**: ① V169 `chk_staff_work_attendance_checkout_after_checkin` 은 이름과 달리 `check_out_at IS NULL OR check_in_at IS NOT NULL`(pairing)만 강제 — 실제 `check_out_at >= check_in_at` 시각 순서 미강제(raw SQL·모바일/NFC 일괄 동기화로 퇴근<출근 근무시간 음수 적재 가능) ② V170 은 스키마 내 거의 모든 운영 테이블이 갖는 **복합 Tenant FK pair 가 전무**(cross-tenant `branch_id`/`user_id` 참조 가능)·`updated_at >= created_at` temporal CHECK·`search_query` 비공백 CHECK 부재. `BillingReportFilterService.normalizeSearchQuery`(공백→NULL·trim·255)가 앱 계약을 보장하나 raw SQL 우회 가능. **DBA 본 라운드 신규 V171** — V168(V164/V165 anchor UK) 패턴으로 일괄 해소: staff_work_attendance **1 temporal CHECK** + billing_report_filters **1 temporal + 1 nonempty CHECK + 2 Tenant FK pair**(`(org,branch)→branches` V4·`(org,user)→users` V5). single-additive `ALTER TABLE` only·신규 컬럼·트리거·인덱스 0건. **의도적 제외**: ① billing_report_filters (org, user, branch)→user_branches 3-way assignment FK — `hq_admin` 명시적 배정 없이 전 지점 리포트 열람(ERD §2 RBAC)·3-way FK 가 정당 조회 차단(V168 동일 정책) ② user purge backing index — ephemeral UI-preference 행·보존 cohort 아님·soft-delete 사용자 모델(hard DELETE 없음)·기존 scope UK prefix 가 사용자별 조회 backing. **로컬 PG14 검증 PASS**: scratch DB 에 `/tmp/ogada_mig` 복제 후 **V1–V171 171 migrations `sort -V` 순차 적용 exit=0**(갭·중복 0·contiguous) · `pg_constraint` 실측 **chk_staff_work_attendance_checkout_not_before_checkin·chk_billing_report_filters_updated_after_created·chk_billing_report_filters_search_query_nonempty·fk_billing_report_filters_branch_org·fk_billing_report_filters_user_org 5종 전부 존재** · `session_replication_role=replica` 격리 **3 negative**(퇴근<출근·`updated_at<created_at`·공백 `'   '` search_query — 전부 정확한 CHECK 로 거부) + **positive**(퇴근≥출근·퇴근 NULL·NULL search_query·완전 유효 FK INSERT 통과) + **Tenant FK 격리**(cross-tenant org2+org1 branch → `fk_*_branch_org` 거부·미존재 user → `fk_*_user_org` 거부). **Must billing·attendance·NHIS 핵심 제약 7건** 불변 재확인(`uq_claim_branch_month` V1·`uq_billing_claim_items_claim_client` V26·`chk_billing_claims_amount_sum` V6·`trg_billing_claims_total_reconciliation` V11·`chk_attendance_presence_xor_absence` V11/V14·`uq_nhis_import_rows_org_id` V37·`chk_nhis_import_rows_match_requires_client` V19→V54). agents.yaml `core_entities` 11종 전수 충족(`attendance`+staff_work_attendance·`billing`+billing_report_filters carry). `ls db/migration | wc -l` = **171** contiguous(V1–V171, 갭·중복 0). 본 라운드 산출: ERD 헤더 메타·DDL 범위 `…V171`·§1 Must coverage `attendance`/`billing` 행 갱신·§1 round 노트 V169/V170/V171·§7 마이그레이션 V167~V171 행·§7 본 검증 헤더. **coder**: V171 은 additive CHECK + Tenant FK — `StaffWorkAttendanceService`/`BillingReportFilterService` 정상 경로 영향 0(둘 다 동일 불변식을 앱에서 이미 강제). `mvn flyway:migrate`(V169·V170 적재 0건 가정)·관련 통합 테스트 재검증 권장. **보류 carry**: V169 staff_work_attendance `check_out_at` 동일 근무일 cross-row overlap(P3·앱 책임)·전 라운드 보류 목록(현금영수증 cross-table/time P2·PLAN↔BILLING reciprocal P2·이동서비스일지④ 시간 tolerance P2·`billing_payments` Epic L).
>
> **검증 상태 (2026-06-21, round 183 @ backend `49a1721` — G-BATHING 전월 일정 복사 · G21 대시보드 NHIS gap count · 신규 V167 불요)**: round 182(`@7b99313`/G-BILLING 리포트 필터) → backend HEAD **`49a1721`** 4 commit 전진 — `49a1721` **G-BATHING `POST /api/v1/care/bathing-schedules/copy-from-previous-month`**(`BathingScheduleService.copyFromPreviousMonth`·`CopyBathingSchedulesFromPreviousMonthRequest` `{targetYearMonth}`)·`0796821` **G21 대시보드 `nhisComparisonGapCount`**(`DashboardService.resolveNhisComparisonGapCount`→`VisitService.getNhisComparison` 인메모리 `countNhisComparisonGapLines`)·`ad11fda` notification quiet-hours 테스트 deepen(테스트 only)·`0c9518a` live-e2e bootstrap derived blocker 우선순위(앱 only). `git diff --name-only 7b99313..49a1721 -- src/main/resources/db/migration/` = **0파일**, `… -- '**/*Entity.java' '**/*Repository.java'` = **0파일**, `CREATE TABLE|ALTER TABLE|@Entity|@Column|@Table` grep = **0건**. **G-BATHING copy DB backing 대조** (신규 인덱스 0건): 전월·대상월 범위 조회 모두 `BathingScheduleRepository.findByOrganizationIdAndBranchIdAndScheduledDateBetweenOrderByScheduledDateDescScheduledTimeDescCreatedAtDescClientIdAsc` → V136 `idx_bathing_schedules_org_branch_scheduled_date` `(org, branch, scheduled_date DESC, …)` 3-equality + date range prefix-exact. 신규 행은 `save` → V136 UK `(organization_id, client_id, scheduled_date)` + V136 domain CHECK(bath_type/status pair) — copy는 항상 `SCHEDULED`·소스 `bath_type`/`scheduled_time`/`notes` 복사·`provision_notes`/`completed_at` NULL. **G21 dashboard gap count DB backing**: `visitService.getNhisComparison`(V53 `idx_visit_schedules_org_branch_kind_date`·V38 `idx_nhis_import_batches_org_branch_created`·V28 `idx_nhis_import_rows_batch_created`·V5 `uq_clients_org_id`) 결과 인메모리 집계 — 신규 `@Query`·DDL 0건. **78/78 JPA `@Entity` ↔ Flyway `CREATE TABLE` 1:1 대조 carry** — 누락 0건. **Must billing·attendance·NHIS 핵심 제약 7건** SQL 물리 재확인 — **전부 불변**(`uq_claim_branch_month` V1·`uq_billing_claim_items_claim_client` V26·`chk_billing_claims_amount_sum` V6·`trg_billing_claims_total_reconciliation` V11·`chk_attendance_presence_xor_absence` V11/V14·`uq_nhis_import_rows_org_id` V37·`chk_nhis_import_rows_match_requires_client` V19→V54). agents.yaml `core_entities` 11종 전수 충족. `ls db/migration | wc -l` = **166** contiguous(V1–V166, 갭·중복 0). **결론**: **신규 V167 불요** — G-BATHING copy·G21 dashboard gap는 기존 `bathing_schedules`(V136)·visit_schedules/NHIS import(V53/V38/V28) 위 읽기/INSERT 전용. 본 라운드 산출: ERD §1 round 183·§8 copy-from-previous-month 매핑·§7 본 검증 헤더·메타 timestamp + PLAN_NOTES #171. **보류 carry**(전 라운드 동일): V166 `is_active` pairing(P3)·V162 `branch_ids` 요소 UUID(P3)·현금영수증 cross-table/time(P2)·PLAN↔BILLING reciprocal(P2)·이동서비스일지④ 시간 tolerance(P2)·`staff_attendance`/`staff_schedules`(Should)·`billing_payments`(Epic L). **coder**: 추가 DDL 0건 — FE wire P2(G-BATHING copy UI)는 스키마 영향 없음.
>
> **검증 상태 (2026-06-21, round 182 @ backend `7b99313` — G-BILLING 대장 리포트 반월·기준 필터 · 신규 V167 불요)**: round 181(`@1d7cee2`/V166) → backend HEAD **`7b99313`** 4 commit 전진 — `b96d038` 입금대장 반월·수납대장 기준 필터·`375fb9d` 달력월 검증·`14935a3` 필터 echo·`7b99313` 필터 단언 테스트(전부 앱 only). `git diff --name-only 1d7cee2..7b99313 -- src/main/resources/db/migration/` = **0파일**. **Must billing·attendance·NHIS 핵심 제약 7건** 불변 재확인 → **신규 V167 불요**. ERD §1 round 182·PLAN_NOTES #170.
>
> **검증 상태 (2026-06-21, round 181 @ backend `1d7cee2` — V166 `ON_LEAVE` lifecycle 검토 · lifecycle summary API · 신규 V167 불요)**: round 180(`@7d29a38`/V165 후속 probe) → backend HEAD **`1d7cee2`** 6 commit 전진 — `68d4457` **V166 `staff_leave_status_on_leave`**(coder)·`2edbdc4` NHIS processing-status header parsing(앱 only)·`6ed7cd4` G-BANK-EXCEL-8 row guard(앱 only)·`1d7cee2` **G-STAFF-LEAVE-STATUS lifecycle summary API**(`StaffLifecycleSummaryController`·`GET /api/v1/staff/lifecycle-summary` `onLeaveCount`). `git diff --name-only 7d29a38..1d7cee2 -- src/main/resources/db/migration/` = **V166 1파일**, `… -- '**/*Entity.java' '**/*Repository.java'` = **0파일**, `CREATE TABLE|ALTER TABLE` grep(V166 제외) = **0건**. **V166 검토 (coder 소유, DBA confirmation)**: ① enum CHECK 5값(`ON_LEAVE` 추가) ② hired_at 필수 ③ terminated_at 금지 — `UserService` ON_LEAVE 3가드 중 DB 표현 가능한 2쌍을 V87 패턴으로 defense-in-depth. **`is_active=false` pairing 미포함** — TERMINATED도 DB 미강제(앱 `UserService`/`StaffTrainingLogService` 책임) → V167 추가 불요(P3 carry). **lifecycle summary API DB backing**: `userRepository.findByOrganizationId` + `user_branches` 지점 필터 + 인메모리 `StaffLifecycleSummary.summarize` — V86 `idx_users_org_lifecycle_status` 재사용, 신규 조회 인덱스 0건. **Must billing·attendance·NHIS 핵심 제약 7건** SQL 물리 재확인 — **전부 불변**. agents.yaml `core_entities` 11종 전수 충족(`users` V166 ON_LEAVE carry). `ls db/migration | wc -l` = **166** contiguous(V1–V166, 갭·중복 0). **결론**: **신규 V167 불요** — V166 well-formed, summary API 앱 only. 본 라운드 산출: ERD §1 round 181·§4-2 V166·§7 V166 행·§8 lifecycle-summary 매핑·메타 timestamp + DATA_RETENTION ON_LEAVE 보존 + PLAN_NOTES #169. **보류 carry**: V166 `is_active` pairing(P3)·V162 `branch_ids` 요소 UUID(P3)·현금영수증 cross-table/time(P2)·PLAN↔BILLING reciprocal(P2)·`staff_attendance`/`staff_schedules`(Should)·`billing_payments`(Epic L).
>
> **검증 상태 (2026-06-21, round 180 @ backend `7d29a38` — V165 후속·live-e2e V165 readiness probe·신규 DDL 0건)**: round 179(`@6f7f145`/V165 신규) → backend HEAD **`7d29a38`** 6 commit 전진 — `05ca0a8` **V165 commit**(라운드 179 DBA 산출의 committed 형태)·`3bbfc00` **live-e2e probe gate on V165**(`UserAccountRequestSchemaReadinessProbe` — `pg_constraint`에서 `chk_user_account_requests_review_state_pair` CHECK 존재만 **읽기**·`HealthController`/`LiveE2eController`/`LiveE2eOperationReadinessSupport` 와이어, NHIS caregiver bulk import 가 defense-in-depth 결손 스키마에서 실행되지 않도록 차단)·`b11e29a` G21 seed branch service_type 정규화(앱 only)·`2f6f3bc`/`e3b74a0`/`7d29a38` G-STAFF-NHIS-EXCEL-IMPORT·G-BANK-EXCEL-8 선택 검증·non-positive 거부(앱 only). `git diff --name-only 6f7f145..7d29a38 -- src/main/resources/db/migration/` = **`V165__user_account_requests_integrity.sql` 1파일**(round 179 documented, `05ca0a8` commit), `… -- '**/*Entity.java' '**/*Repository.java'` = **0파일**, `git diff 6f7f145..7d29a38 -- ':(exclude)src/main/resources/db/migration/' | grep -iE 'CREATE TABLE|ALTER TABLE|@Entity|@Column|@Query'` = **0건**(V165 commit 자체 제외 — 신규 테이블·컬럼·엔티티·레포지토리·쿼리 0건). **`UserAccountRequestSchemaReadinessProbe` 영향 분석**: 단 한 개의 `SELECT COUNT(*) FROM pg_constraint c JOIN pg_class t … WHERE t.relname='user_account_requests' AND c.conname='chk_user_account_requests_review_state_pair'` — V165 CHECK 존재만 boolean 으로 노출(`integrityCheckReady()`). `HealthController.isOperationReady`·`LiveE2eOperationReadinessSupport.requireOperationReady` 에 와이어되어 probe 실패 시 `/health 503 + operationReady=false` 응답 → V164 client_care_plan_forms / V158 cash_receipt_issuances 와 동일 readiness gate 패턴(라운드 173/176 동일). 신규 테이블·컬럼·인덱스·`@Query` 0건. **78/78 JPA `@Entity` ↔ Flyway `CREATE TABLE` 1:1 대조 carry** — 누락 0건(`audit_logs`·`billing_claim_status_history`는 raw SQL 전용, 설계 의도). **Must billing·attendance·NHIS 핵심 제약 7건** SQL 물리 재확인 — **전부 불변**(`uq_claim_branch_month` V1·`uq_billing_claim_items_claim_client` V26·`chk_billing_claims_amount_sum` V6·`trg_billing_claims_total_reconciliation` V11·`chk_attendance_presence_xor_absence` V11/V14·`uq_nhis_import_rows_org_id` V37·`chk_nhis_import_rows_match_requires_client` V19→V54). agents.yaml `core_entities` 11종 전수 충족(`users` 행 V165 적용 carry). `ls db/migration | wc -l` = **165** contiguous(V1–V165, 갭·중복 0). **결론**: **신규 V166 불필요** — V165 후속 라운드는 commit + live-e2e readiness probe 와이어(읽기 전용)·G-STAFF-NHIS-EXCEL-IMPORT·G-BANK-EXCEL-8 앱 단 선택 검증으로, 기존 V165 CHECK + Tenant FK 위 defense-in-depth 가 의도대로 활성됨을 confirmation. 본 라운드 산출: ERD §1 round 180 행·§7 본 검증 헤더(round 180)·메타 timestamp + DATA_RETENTION 메타 timestamp + PLAN_NOTES #168. **보류 carry**: V162 `branch_ids` 요소별 UUID CHECK(P3 — 앱 책임)·V162 이메일 lower-case CHECK(P3 — partial UK)·V164 `notified_at` 시간 정합(P3)·이동서비스일지④ 시간 tolerance(P2)·현금영수증 `amount==copay`·`issued_at` 시간성(P2 — 앱 책임)·PLAN↔BILLING reciprocal pair CHECK(P2)·`waypoint_label` 길이/공백(P3)·L02 M05 투약·`staff_attendance`/`staff_schedules`(Should)·`billing_payments`(Epic L). **coder**: 추가 DDL 0건 — 스키마 영향 없음. `mvn flyway:migrate`(V165 적재 0건 가정)·`mvn test` 재검증은 라운드 179 PASS 결과 carry(추가 변경분은 probe·앱 검증만 — 통합 테스트 외 영향 0).
>
...
| 대상 | 제약·인덱스·트리거 | 목적 |
|------|-------------------|------|
| `users` (V86/**V166**) | `lifecycle_status`(5값 incl. **`ON_LEAVE`**)/`hired_at`/`terminated_at`/…·`chk_users_lifecycle_status`·`chk_users_on_leave_*`(V166)·`idx_users_org_lifecycle_status` partial | `PATCH /users/{userId}` lifecycle·휴직(ON_LEAVE) 전이·`GET /api/v1/staff/lifecycle-summary`(US-R03/US-R03b) |
...
### 7-81. G-BILLING 대장 리포트 반월·기준 필터 (앱 only, round 182 @ `7b99313`)

| 대상 | 변경 | 목적 |
|------|------|------|
| `GET /billing/reports/{variant}` (입금·수납) | **신규 DDL 0건** — `period`(FULL/FIRST_HALF/SECOND_HALF)·`basis`(PAYMENT/CLAIM) 쿼리 파라미터는 **인메모리** 필터 | 케어포 PDF p.91 입금대장 반월·수납대장 청구/수납 기준 분기 — DB 술어 추가 없이 적재 후 분기 |
| 입금/수납 적재 쿼리 (불변) | `findByOrganizationIdAndBranchIdAndStatusOrderByPaidAtDesc(PAID)` | V153 `idx_billing_claims_org_branch_status_paid_at`(partial PAID·`paid_at DESC`) 1:1 backing — `matchesDepositPeriod`·`matchesReceiptLedgerMonth` 인메모리 분기 |
| 청구 적재 쿼리 (불변) | `findByOrganizationIdAndBranchIdAndYearMonthOrderByGeneratedAtDesc` | V149 `(org, branch, year_month DESC, status, generated_at DESC)` + V1 `idx_billing_claims_org_branch_month` backing |

> **검증**: `git diff --name-only 1d7cee2..7b99313 -- src/main/resources/db/migration/` = 0파일·`… -- '**/*Entity.java' '**/*Repository.java'` = 0파일·`CREATE TABLE|ALTER TABLE` grep = 0건. 반월(일자 1–15/16–말)·기준(수납월/청구년월) 분기는 전부 `BillingService.buildPaymentReportItems` 인메모리 필터이고, 기존 적재 쿼리(V153/V149/V1)가 정렬·범위를 모두 backing → **신규 인덱스·테이블·제약 0건**. Must billing·attendance·NHIS 핵심 제약 7건 SQL 물리 재확인 — 전부 불변. **신규 V167 불요**.
...
| `PATCH /users/{userId}` (lifecycle 전이) | `users` (입사~퇴사·**휴직(`ON_LEAVE`)** 전이 — `chk_users_lifecycle_status` V86/**V166** + `chk_users_terminated_after_hired`·`chk_users_terminated_requires_date` V87 + `chk_users_on_leave_requires_hired_at`·`chk_users_on_leave_no_termination_date` V166; `UserService.validateLifecycleState` 입사 계약/체크리스트·퇴사 증빙·ON_LEAVE 가드, US-R03/US-R03b) |
| `GET /api/v1/staff/lifecycle-summary` | `users`·`user_branches` (Tenant 직원 lifecycle 집계 — `onLeaveCount` 등 6종 카운트, V86 `idx_users_org_lifecycle_status` + 인메모리 필터, G-STAFF-LEAVE-STATUS `1d7cee2`) |
...
| `GET /billing/reports/deposits` · `GET /billing/reports/receipts` | `billing_claims` (`status=PAID` → `idx_billing_claims_org_branch_status_paid_at` V153; `findBy…OrderByPaidAtDesc` + 월별 `paidAt` 인메모리 필터; **round 182** `period`(반월 1–15/16–말)·`basis`(PAYMENT/CLAIM) 쿼리 파라미터도 인메모리 분기 — 신규 인덱스 0건, `7b99313`) |
| `GET/PUT /billing/reports/filters` | `billing_report_filters` (V170 → `findBy…OrgBranchUserYearMonthVariant` UK 단건 upsert·V171 `(org,branch)→branches`·`(org,user)→users` Tenant FK + temporal/search nonempty CHECK; charges/deposits/receipts/refunds 사용자별 마지막 필터 silent upsert — `BillingReportFilterService.recordFromReportQuery`, Q601, `479995e`) |
...
| `GET/POST /staff/attendance` | `staff_work_attendance` (V169 → `(org, branch, work_date DESC)`·`(org, user, work_date DESC)` 인덱스 + `(org, branch, user, work_date)` UK 일자 단건·V171 `check_out_at >= check_in_at` temporal CHECK; 직원 일일 출퇴근 roster — G-STAFF-WORK-ATTENDANCE 케어포 8-4, `560057f`) |
| `GET/PUT /staff/{userId}/annual-leave`·`GET /staff/annual-leave/roster` | `staff_annual_leave_yearly` (V172 → `(org, branch, leave_year)`·`(org, user, leave_year DESC)` 인덱스 + `(org, user, leave_year)` UK 직원×연도 1건·V173 `chk_*_month_max`/`chk_*_entitlement_max`/`chk_*_used_within_entitlement`/`chk_*_memo_nonempty` + `fk_*_user_branch_assignment` defense-in-depth; 직원 연차휴가 연간 현황 roster — G-STAFF-ANNUAL-LEAVE / US-R03e / ezCare worker-b100 tab01, BNK-516, `41fb84f`) |
