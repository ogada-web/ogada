<!-- doc:owner=UXD doc:audience=PLN,COD,TSR updated=2026-06-08T19:00:00+09:00 -->
# ogada 디자인 시스템 (product/DESIGN_SYSTEM.md)

> **작성**: ux_designer 에이전트 (`UXD`)
> **최초 작성일**: 2026-06-06
> **최종 갱신**: 2026-06-08 (50차 — **배차·이동경로 강제 색상 모드(forced-colors) 접근성 보강 + US-H02 BranchCompareChart 완료 확인** — USER_STORIES US-H02·FLOWCHART §8 HQ 노드 E2 `BranchCompareChart` 잔여 해소. 재이관 후 48차(`8a764df`) `DashboardPage` HQ variant에 `BranchCompareChart` 연동이 완료됐으나, `components.css` 배차 컴포넌트의 **`forced-colors` 지원이 없어** Windows 고대비 모드에서 지도 캔버스·정차 마커·강조 정차 시각 구분이 불가능하던 갭 해소. ① **`components.css` forced-colors 블록 확장** — `.ds-transport-map__canvas`·`.ds-transport-stop`·`.ds-transport-map-marker`에 `border: 1px solid CanvasText` 추가. `.ds-transport-map-marker--active`·`.ds-transport-stop__order`(순번 뱃지)를 `Highlight`/`HighlightText` 고대비 색으로 강제 적용(`forced-color-adjust: none`). `.ds-transport-stop--highlighted`는 기존 `box-shadow`가 forced-colors에서 소거되므로 `outline: 3px solid Highlight`로 대체. ② **BranchCompareChart(US-H02) 완료 확인** — `DashboardPage` HQ variant에 `BranchCompareChart` 이미 통합됨(48차 `8a764df`). `pilotPageFlows` transport US-T01~T03 E2E `637b9b3` HEAD PRESENT. `npm test` **189/60 PASS**·build PASS·audit 0.)
