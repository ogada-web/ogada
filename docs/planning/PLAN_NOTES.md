<!-- doc:owner=PLN doc:audience=COD,TSR,UXD,DBA,BNK,TWR updated=2026-06-08T15:00:00+00:00 -->
# 기획 메모 (planning/PLAN_NOTES.md)

> **작성**: planner 에이전트 (`PLN`)  
> **최종 갱신**: 2026-06-08 (43차 — workspace baseline 확정·SEC-D12/QA-B11 Fixed·BE-10 @ `3f9264f`·FE MVP @ `7c0ecdc`)  
> **상태**: 초안 (사용자 승인 전)

---

### [COD] 현재 baseline (planner 43차 확정 — 단일 진실 원천)

| stream | develop HEAD | test HEAD | WT | 비고 |
|--------|--------------|-----------|-----|------|
| backend | **`3f9264f`** | `2799e29` | CLEAN | BE-10 v1 E2E/routing·J01·v2/J03 NotificationService |
| frontend | **`e043eac`** | `e5fd48d` | CLEAN | 7c0ecdc MVP + vitest·ProtectedRoute/MaskedPhone tests |

> **폐기 SHA**: `d5654c0`(TSR57 frontend)·`e5fd48d`(스켈레톤)·`428ba7d`(TSR56 backend) — remote 미반영·submodule 재init 시 **유실**. ROADMAP/QA의 해당 참조는 **과거 기록** — checkout 재현 **금지**.  
> **확정 파일**: `.agents/workspace_baseline.yaml` · `run_agent.py` build 시 **실측 git HEAD 자동 주입**.  
> **Fixed @ 43차**: SEC-D12(route 가드)·QA-B11(baseline 단절)·SEC-D11(submodule drift) — 위 HEAD 기준. QA-B10(BE-10) @ `3f9264f` 유지.
...
42. **v1 backend develop baseline (2026-06-08, 42차, QA-B10·BE-10)** — develop `@f47ffa1`(J01·89 tests) vs ROADMAP merged `@e8750d2`(213 tests·v1 E2E). **선택지**: ① test를 `e8750d2` lineage로 replay 후 J01 cherry-pick ② v1 E2E/routing을 `f47ffa1`에 replay ③ ROADMAP v1 baseline을 `f47ffa1`로 갱신(완료 기준 조정). **미결정** — operation sign-off BLOCK.
43. **카카오 Directions API 비용 (2026-06-08, BNK-8)** — v1.3-B 설계 전 Directions 쿼터·단가 산정 필요. Geocoding·JS SDK는 파일럿 무료 쿼터 내(BNK-8 #2).
