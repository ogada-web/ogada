<!-- doc:owner=PLN doc:audience=COD,TSR,UXD,DBA,BNK,TWR updated=2026-06-20T18:00:00+09:00 -->
...
> **작성**: planner 에이전트 (`PLN`)  
> **최종 갱신**: 2026-06-20 (177차 — **자동 기획 동기화** BNK-424~429·TSR 1137~1143차·QA-B169/B170 Open→Planned·**★ G15-KAKAO-INTEGRATION 6-layer 우위**·merge gate 555·QA Open 0)  
...
#### [PLN] G15-KAKAO-QUOTA-DASH — Kakao REST API quota/usage 대시보드 위젯 (2026-06-20, 177차 — BNK-429 · **신규 candidate · 「가정」 P3 · BE WIP commit 후 P2 승격 검토**)

- **근거**: BNK-429 — ogada BE `@e2b764b` WT 4U untracked(`KakaoRestApiUsageTracker`·`TransportKakaoQuotaUsageResponse`·`KakaoRestApiKind`) = Kakao REST API **quota/usage tracking 신규 layer** · 케어포는 postcode 1-layer only(quota/status **부재**).
- **현황**: BE tracker WIP only · FE `TransportKakaoApiStatusPanel` @ `ba74bb5` = API status/route preview 가시화 ✅ · quota dashboard widget ❌.
- **검토 질문**: ① BE quota tracker commit 후 FE wire를 v1.3-A Must로 승격할지 ② HQ admin 대시보드 vs transport settings 전용 패널 중 IA — **planner 권고(추측·미확정)**: **P3 backlog·BE commit(QA-B169) 선행 후 P2 승격 검토**.
