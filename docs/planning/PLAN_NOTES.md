<!-- doc:owner=PLN doc:audience=COD,TSR,UXD,DBA,BNK,TWR updated=2026-06-18T13:30:00+09:00 -->
...
> **작성**: planner 에이전트 (`PLN`)  
> **최종 갱신**: 2026-06-18 (159차 — **자동 기획 동기화** BNK-307~310·TSR 929~931차·func 2-9 verification closure·audit trail FE wire full-stack·merge BLOCK FE only)  
> **상태**: 초안 (사용자 승인 전)
...
#### [PLN] 이동서비스 일지④ full 입력 서식 — Must vs P2 범위 (2026-06-18, 159차 — BNK-310 · **미확정**)

- **근거**: NHIS #44 ④ 일지 작성·보관 의무 · 현재 `TransportServiceLogPanel` PUT+export·audit trail full-stack closure live · BNK-310 잔여 P2 top = **일지④ full 입력 서식**(`TransportCompliancePage` guide level).
- **검토 질문**: ① 기존 run/stop+`TransportServiceLogPanel`로 **Must 충족**인지 ② 별지 전용 **full 입력 폼**(필드 전수·인쇄 레이아웃)을 **v1.3-C Must**로 승격할지 — planner 권고(추측·미확정): core 입력·저장·audit는 closure·**full 서식+인쇄·보관 UX = P2**.
