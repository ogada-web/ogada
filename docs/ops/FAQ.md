...
### 구현 상태 안내 (2026-06-15 develop HEAD `a4352a8` / frontend `3845f0c` 기준)
...
| 프론트엔드 | **77 라우트 · v1.3-B transport FE + L03_M11~M14·M13·M04 + G-NURSING + G17b + G21 + G2/7-5 + G41 + G30 + G34-QUAL + J03 + 8-12 + G42 · Vitest 1274/1274** | **`TransportSuggestPanel`**·**`BranchTransportSettingsPanel`** @ `2ffe59f` · **SideNav** @ `edfba7f` |
| 본 FAQ | **Q331·Q347 갱신·Q348 신규** | **v1.3-B FE Fixed** · **L03_M01 BE-only** · **Q340~Q346** L03 · **P2**: L03_M01 FE wire · 나머지 L03 leaf · **7-5 live PG** |
...
| **페이지 새로고침** | JWT가 **메모리**에만 있으므로 **즉시 로그아웃** (Q82) |
...
**A.** **예.** 로그인 화면(`/login`)에서 `POST /api/v1/auth/login`으로 access·refresh 토큰을 발급받습니다.

| 항목 | 상태 (FE `42f48e1`) |
|------|---------------------|
| JWT 로그인·역할 가드 | **구현됨** — `AuthContext` + `ProtectedRoute` + `roleNav.allowedRolesForPath` |
| 토큰 저장 | **메모리 전용** (`src/api/session.js`, SEC-005) — **localStorage/sessionStorage 미사용** |
| 세션 지속 | **새로고침 시 로그아웃** — 토큰이 메모리에서 사라짐. 탭 닫기와 동일 |
...
| refresh 자동 갱신 | **후속** — access 만료(30분) 시 API 401 → 재로그인 |
...
**A.** **Partial Fixed (FE `bf3d40d`·`b046444`)** — **`GuardianCheckinPage`** 체크인 대상·스캔 API가 **정합**되었습니다. **`QrGeneratePage`** 는 **지점 스코프 경로**(`POST /branches/{branchId}/qr`)로 연동되었으나, **요청 본문·QR 이미지 응답 갭**이 잔존합니다.

**체크인 (Fixed, `bf3d40d`)**

| 항목 | 내용 |
|------|------|
| 대상 목록 | `GET /guardian/checkin-targets` → `clientId`·`clientName` 매핑 |
| 스캔 | `POST /attendance/qr/scan` — `{ qrToken, clientId, transportType? }` (귀가 시 교통편 필수) |
| UI | 입소/귀가 `FilterChips` · 연결 이용자 1명이면 자동 선택 · 카메라 스캔은 **준비 중** — 토큰 직접 입력 |
...
| 지도 | **`KAKAO_REST_KEY`** 설정 시 `POST /transport/geocode` Kakao REST proxy 동작 — 미설정 시 지도·좌표 **미표시** |
...
**A.** **UI 안내 Fixed (FE `4c7c994`, G19)** — 급여종 **`INTEGRATED_HOME`(통합재가)** 선택 시 등록 모달에 **warning `Alert`** 가 표시됩니다.

| 항목 | 내용 |
|------|------|
| 문구 | 「통합재가 주·야간보호형 가산(월 100,000원/인, 고시 제55조의2)은 v1에서 자동 청구·정산하지 않습니다.」 |
| 범위 | **안내 라벨만** — v1 **Won't**: 가산 자동 청구·정산·명세서 라인 **미구현** |
| 근거 | MOHW 고시 **제2025-247호 제55조의2** (BNK-44 P1) |
| 운영 | 통합재가 지점은 **수동**으로 가산 여부·금액을 확인·기록해야 합니다 |

> 관련: Q180 · USER_MANUAL §5-3 · REQUIREMENTS G19
...
**A.** **Fixed (`3c8f9fe`·`9e3cab5`·`7389884`·`9dfef92`, G15 v1.3-C partial + G16 partial)** — 2026 평가 지표 **41·42** 대응 **운영 체크리스트**입니다. **수칙·서명은 서버에 저장**됩니다 (Q231). **별지 제22호 일지 출력·시간 준수 기록 UI Fixed**(Q236) · **G16 이동서비스비 청구 기록 생성 UI Fixed**(Q239) — **공단 포털 자동 제출**은 **미지원**(Q237).

| 항목 | 내용 |
|------|------|
| UI | **`/transport`** **`TransportCompliancePanel`** — **이동서비스 수칙·계약 (G15)** 카드 |
| 워크플로 | ① **수칙 5필수** 마련 → ② **계약서에 수칙 포함** → ③ **이동서비스일지(제22호)** 작성·출력 (Q236) → ④ **별지 제18호** 공단 신청 (Q237) |
| 체크리스트 5항목 | 운전자 자격 · 동승 직원 · 차량 안전 · 사고 조치 · 차량 운행표(별지 제22호) |
| Modal | **「계약서 템플릿 보기」** — 인쇄·별도 문서화 후 현장 보관 |
| 일지 | **`TransportRunDetailPage`** **`TransportServiceLogPanel`** — 확정 배차 기반 **인쇄·다운로드** (Q236) |
...
**A.** **Fixed (`7389884`·`eef07e5`, G15 v1.3-C, US-T05)** — **확정 배차** 루트 상세 화면에서 법정 형식 일지를 **인쇄·텍스트 다운로드**할 수 있습니다.

| 항목 | 내용 |
|------|------|
| UI | **`/transport/runs/:runId`** **`TransportServiceLogPanel`** — `run.status=CONFIRMED` 권장 |
| 입력 | **차량번호** · **운전자** · **동승 직원** · 정차별 **실제 픽업 시각**(`type=time`) · **동승 여부** |
| 시간 준수 | **`transportTimeCompliance.js`** — 계획 픽업 대비 **±15분** 이내 **「준수」** · 초과 **「지연」** · 미입력 **「미기록」** (평가 지표 42) |
| 출력 | **「인쇄」** — `body.ds-transport-log-printing` 인쇄 전용 스타일 · **「다운로드」** — `.txt` 파일 |
| DRAFT | 미확정 루트는 **미리보기**만 — **「배차 확정 후 제출용 일지로 출력」** 경고 |
| 저장 | **클라이언트 메모리** — 새로고침 시 실제 픽업·동승 입력 **초기화** (서버 영속화 **미구현**) |
| 연동 | 상단 안내에서 **`/transport`** **별지 제18호** 패널(Q237) 교차 참조 |

> 관련: Q230 · Q233 · Q237 · USER_MANUAL §5-8 · ADMIN_GUIDE §10-10
...
| **화면** | **`NHISImportPage`** — **`fetchFeeScheduleYearCoverageApi`** 병렬 로드 · **`FeeScheduleYearGuardBanner`** — **표준 25칸** 미완비 시 **warning** + **「수가표 등록」** 링크 · 인지지원 진행률 **별도 표시** |
...
**A.** **Fixed (BE `f082933`·FE `1c99bcd`·`8e66ae8`, G39 / US-T08 / BNK-107·109)** — MOHW **평가 지표 44** 대응 **연간 급여제공결과 평가** 화면과 대시보드 compliance 위젯이 연동되었습니다.
...
| API | `GET/POST/PATCH /api/v1/provision-result-evaluations*` · `GET …/compliance?year=` |
...
| 테스트 | **`ProvisionResultEvaluationServiceTest`** · **`ProvisionResultEvaluationPage.test.jsx`** · **`provisionResultCompliance.test.js`** · **`DashboardPage.test.jsx`** · **`programComplianceServices.test.js`**

> 관련: Q271 · USER_MANUAL §5-9 · ADMIN_GUIDE §10-11-3 · DATA_RETENTION_POLICY §2-1 G39
...
**A.** **Partial Fixed (BE `6f3315a`·FE `2642838`·`479e064`)** — **이용자 상세 「기초평가」 탭에서 회계연도별 욕구사정을 작성·저장할 수 있습니다.** 이지케어 FAQ **21800 [연간] 욕구사정**에 대응하는 **8항목 가정방문 평가**는 `ClientNeedsAssessmentForm`에서 **`PUT /clients/{clientId}/needs-assessments`** 로 저장됩니다. **연간 리포트 전수·지표15 자동화**는 P2입니다.

**화면 경로**: 이용자 목록 → 상세(`/clients/:id`) → **「기초평가」** 탭.

**조작 요약**:
1. **가정방문 일자**·**8항목**(신체·인지·가족·경제·사회·서비스 필요·방문 기록·만족도)을 입력합니다.
2. **저장** — 동일 회계연도 기록이 있으면 **갱신(upsert)** 됩니다.
3. **이전 회계연도 비교** — 전년도 기록이 있으면 항목별 변화가 표시됩니다 (`ClientNeedsAssessmentCompare`).
4. **회계연도 선택** — FE **`4e4bdf6`·`479e064`** 에서 fiscal-year 파싱이 강화되어, 연도·가정방문 일자·비교 연도가 **일관되게** 매핑됩니다.
5. **가정방문 일자**는 선택한 **회계연도(1/1~12/31) 범위** 밖이면 `422 BUSINESS_RULE` 오류가 반환됩니다 (BE `b238779`).

| 구현됨 (Fixed) | 미구현 (P2) |
|----------------|------------|
| **GET/PUT needs-assessments** API (G24, US-T09) | FAQ21800 **연간 리포트** 일괄 출력 |
| **기초평가 탭** 폼 작성·저장·이력 조회 | **지표15** 대시보드 자동 집계 |
| **`LifecycleWorkflowPanel`** 단계 안내 (Q287) | 전 이용자 **회계연도별 리포트 전수** (P3) |
...
| 불러오는 항목 | 욕구사정 **8항목**(신체·인지·가족·경제·사회·서비스 욕구·가정방문 의견·만족도) · **가정방문 일자** · 기능회복훈련 **최신 계획 본문** |
...
**A.** **Fixed (BE `b1dfd34` / FE `574bd08`, G30·BNK-181)** — **`/compliance/monitoring`** 화면 **상단「통합 모니터링 checklist」** 패널에서 **선택 연·월** 기준 **8문항 자동 집계**를 확인합니다.
...
| 12 | FAQ21841 | **월 5명** 유선상담 |
| 13 | FAQ21842 | **직전 6개월** 자가진단 rolling |

| 항목 | 내용 |
|------|------|
| API | **`GET /api/v1/compliance/monitoring/checklist?branchId=&referenceYear=&referenceMonth=`** |
| FE | **`MonitoringIntegratedChecklistPanel`** · **`fetchMonitoringIntegratedChecklistApi`** · **`monitoringCompliance.js`** |
...
**A.** **Fixed (BE `6191b91`·`0f11158`·`ee42e5d` / FE `f5658de`·`38d24b6`·`45a724a`, G41·G41b·BNK-184~188, US-S04)** — 케어포 **func.php `8-7.교육일지`** 를 **`/staff/training-logs`** 에서 등록·조회합니다. **보수교육(8-7-1)** 은 **`/staff/training`** (Q294).
...
**A.** **Fixed (BE `16a0734`·`82054f1` / FE `51f2505`·`745a2f6`·`7ec7cd4`, BNK-189~193 follow-up)** — 7-5 간편결제는 **API 입력 관용성**·**오입력 거부**·**WCAG 접근성**을 보강했습니다.

| 항목 | 내용 |
|------|------|
| **provider 정규화 (BE)** | **`EasyPayService.normalizeProvider`** — **`Locale.ROOT`** uppercase · `" card "`, `"kakao_pay"` 등 **대소문자·앞뒤 공백** 수용 → **`CARD`·`KAKAO_PAY`** canonical 저장 |
...
**A.** **Fixed (FE `1e111be`·`4ba7ea6`·`8a8b930`·`edfba7f`, US-UX-05, BNK-198)** — 좌측 **2단 SideNav** 5그룹(운영·이동·출석·기록·청구) 헤더를 클릭하면 하위 메뉴가 **토글**됩니다.
...
**A.** **BE-only Fixed (BNK-215, BE `9bd1660`, V123)** — 케어포 **3-1 간호급여 제공기록**(`view.nursing_service`) 대응 API가 구현되었으나 **프론트 화면은 아직 없습니다**. Swagger **`/api/v1/nursing/service-records`** 로만 등록·조회합니다 (**P2 FE wire**).
...
| FE | **미구현** — SideNav **기록 → 간호급여 제공기록** 메뉴 없음 |

> 관련: USER_MANUAL §5-20 · ADMIN_GUIDE §6-2-9f · BENCHMARK_REPORT L03_M01 · **P2**: `/nursing/service-records` FE wire
