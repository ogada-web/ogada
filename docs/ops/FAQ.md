...
### 구현 상태 안내 (2026-06-21 develop HEAD `3023c9e` / frontend `380be3c` 기준)

| 영역 | 상태 | FAQ에서의 의미 |
|------|------|----------------|
| 백엔드 API | **Must + … + V160 ogada_platform_admin ✅** @ `3023c9e` · **v1.3-A Kakao per-API quota tracker ✅** @ `138ac26` · **v1.3-A Kakao API status ✅** @ `e2b764b` · … | **`GET /transport/kakao-api-status`** **`quotaUsage[]`** 3종 API (Q554) · **`/platform/user-account-requests`** (Q555) |
| 데이터베이스 | Flyway **V1–V160** | **V160** `platform_admin`→**`ogada_platform_admin`** (Q556) · **V159** identifier CHECK |
| 프론트엔드 | **107 route · staff account request+platform approval ✅** @ `22718d0`/`380be3c` · **v1.3-A TransportKakaoApiStatusPanel per-API table ✅** @ `138ac26`/`ba74bb5` · **UXD-143 transport a11y ✅** @ `4d87e19` · … | **`/staff` 계정 생성 요청** · **`/platform` 승인** (Q555) · **hq_admin 발급 이름·비밀번호 필수** |
| 본 FAQ | **Q555~Q556 신규** (274차) · **Q554 갱신** (274차) · **Q553 신규** (272차) · **Q550~Q552** (271차) | **직원 계정 승인 workflow** · **역할 코드 rename** · **Kakao API 3종 사용량** |
...
**A.** **Fixed (BE `d7f1a9a`, QA-B135, Q448)** — bootstrap **시드가 아직 미완**이면 **`staff-bootstrap-not-ready`**(또는 guardian 쪽 **`guardian-bootstrap-not-ready`**)만 나옵니다. bootstrap **상태 조회 자체가 내부 오류**(`bootstrap=error`·`guardian-bootstrap=error`)이면 **`operationBlockers`** 에 **`-error`** 코드가 **추가**되어 credential 누락과 구분됩니다.
...
## 22. v2 로드맵 · P3 candidates (2026-06-20 177차 — BNK-424~429)

### Q431. 현금영수증 자동 발급(`G-CASH-RECEIPT-LOG`)이 완료됐나요?
...
### Q432. 출석 QR 체크인이 `CHECKED_IN`·`CHECKED_OUT` 상태로 완전히 고쳐졌나요?
...
### Q433. L03 간호급여(재가 인지·미용 등) 필드가 모두 구현됐나요?
