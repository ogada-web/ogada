...
**A.** 화면은 `/case-management/meetings`입니다. `GET /api/v1/case-management/compliance`의 `meetingsRecordedCount`, `evaluationsRecordedCount`, `reflectionGapCount`를 지점별로 비교하면 회의/평가/반영 누락을 빠르게 찾을 수 있습니다.

### Q569. 민원상담(G42) 사후관리 준수율은 어디서 보나요?

**A.** 화면은 `/staff/grievance-counselings`이며, 사후관리 집계는 `GET /api/v1/staff/grievance-counselings/follow-up/compliance`에서 제공합니다. `pendingFollowUpCount`가 0이 되도록 관리하고, 승인 후 60일 내 follow-up 완료 여부를 확인합니다.

### Q570. 방문요양(G21) 일괄확정 전에 공단 비교를 꼭 해야 하나요?

**A.** 예. `/visits`에서 먼저 `GET /api/v1/visits/confirm-readiness`와 필요 시 `GET /api/v1/visits/nhis-comparison`으로 비교한 뒤, 확인 체크(`nhisComparisonAcknowledged`, `changeHistoryChecked`)를 만족해야 `POST /api/v1/visits/batch-confirm`가 허용됩니다.

### Q571. 전월 미납이 있으면 신규 청구 생성이 왜 막히나요?

**A.** 청구 누락·중복을 방지하기 위한 가드입니다. `/dashboard`·`/dashboard/hq` 응답의 `claimGenerationGuardBlocked`와 `unpaidPriorMonthClaimCount`를 확인하고, 대시보드 **「청구 생성 제한 (7-1 선행입금 가드)」** StatCard가 표시되면 `/billing/payments`에서 전월 미입금을 정리한 뒤 생성 절차를 진행하세요. 상세 API는 `GET /api/v1/billing/claims/generation-guard`입니다 (Q310). StatCard·배너는 **UXD-145** (`a2f599c`)에서 `aria-label`·포커스 링이 보강되었습니다.

> **입금 처리 시에도 동일 규칙** — **더 이른 월 `CONFIRMED` 미납**이 남아 있으면 **신규 월 수납·은행 엑셀 import**가 **`422`「이전 미납 청구(YYYY-MM) 입금 선행이 필요합니다.」** 로 거부됩니다 (Q614, BE `a6eb8b7`).

### Q572. 은행 입금 엑셀 **미리보기**와 **지원 은행 목록** API는 무엇인가요? (G-BANK-EXCEL-8)

**A.** **✅ BE+FE Full-stack Fixed (`a18b30e`·`07a85a3`, G-BANK-EXCEL-8)** — 일괄 수납 전 형식·매칭을 검증합니다. **`/billing/payments` → `BankDepositImportPanel`** 에서 **「미리보기」→ 행 선택 → 「선택 행 등록」** 까지 한 화면에서 처리합니다.

| API | 용도 |
|-----|------|
| `GET /api/v1/billing/imports/bank-deposits/formats` | KB·우리·NH·신한·하나·부산·대구·광주 **8종 헤더 catalog** — 패널 기동 시 자동 조회 |
| `POST /api/v1/billing/imports/bank-deposits/preview` | `branchId` + `file` — **수납 없이** `detectedFormatId`·행별 `APPLIED`/`UNMATCHED`/`SKIPPED` |
| `POST /api/v1/billing/imports/bank-deposits` | 실제 반영 — **`rowNumbers`** 선택 전송 (Q576) |

| UI 단계 | 내용 |
|---------|------|
| 1 | **적용 월**·**xlsx** 선택 → **「미리보기」** |
| 2 | **`APPLIED`(자동 매칭 가능)** 행 **자동 체크** · Badge로 상태 확인 |
| 3 | **「선택 행 등록 (N건)」** → `importBankDepositsApi` (`services.js`) |

> 관련: Q227 · Q228 · Q576 · USER_MANUAL §4-6 · ADMIN_GUIDE §10-4 US-L01
...
| 단계 | 화면·API |
|------|----------|
| 1 | **`/staff` → 「공단 요양보호사 엑셀 업로드」** — 파일 선택 |
| 2 | **`POST /api/v1/staff/imports/nhis-caregivers/preview`** — 행별 상태 확인 · `APPLIED` 행 **자동 체크** |
| 3 | **`APPLIED`(등록 가능)** 행만 체크 → **`POST …/nhis-caregivers`** (`rowNumbers`) |
| 4 | **`user_account_requests` PENDING** 적재 → **`/platform` 승인** (Q555) · **`onImported`로 `/staff?tab=account-requests` 갱신** |

**행 상태**: `APPLIED` · `MATCHED`(기등록) · `PENDING`(승인 대기) · `SKIPPED` · `INVALID` · `DUPLICATE`

> **선택 0건**: FE **「등록할 행을 최소 1개 이상 선택해 주세요.」** · API 빈 `rowNumbers` → **`422`「선택된 행이 없습니다.」** (Q576)

> 관련: Q555 · Q576 · Q577 · USER_MANUAL §4-7-0 · ADMIN_GUIDE §3-2-1

### Q576. bulk import API에서 **`rowNumbers`를 빈 배열**로내면 어떻게 되나요?

**A.** **✅ BE Fixed (`2f6f3bc`·`e3b74a0`·`7d29a38`·`6ed7cd4`, G-STAFF-NHIS-EXCEL-IMPORT·G-BANK-EXCEL-8)** — `rowNumbers`를 **생략(`null`)** 하면 미리보기에서 **반영 가능한 모든 행**을 일괄 처리합니다. **`[]`·null만 포함·0·음수만 포함·유효 행 0건**이면 **`422 BUSINESS_RULE`** — **「선택된 행이 없습니다.」** **미리보기에 존재하지 않는 행 번호**만 보내면 **`422`「선택한 행 번호를 엑셀에서 찾을 수 없습니다: …」** (Q579, `6ed7cd4` — 은행 입금만).

| API | `rowNumbers` 생략 | 빈·무효 배열 | 미리보기에 없는 번호 | FE 가드 |
|-----|-------------------|--------------|---------------------|---------|
| `POST /staff/imports/nhis-caregivers` | 전체 `APPLIED` | `422` | *(현재 BE 미검증 — 미리보기 후 선택 권장)* | **`StaffNhisCaregiverImportPanel`** 선택 0건 차단 (`4315ee2`) |
| `POST /billing/imports/bank-deposits` | 전체 매칭 가능 행 (API) | `422` | **`422`** (`6ed7cd4`) | **`BankDepositImportPanel`** — **`rowNumbers` 선택 전송** · 0건 **「적용할 행을 최소 1건 이상 선택해 주세요.」** (`a18b30e`) |

> 관련: Q573 · Q572 · Q579 · USER_MANUAL §4-6 · §4-7-0

### Q577. live E2E에서 G21 seed가 지점 `serviceType` 때문에 실패할 수 있나요?

**A.** **✅ BE Fixed (`b11e29a`, QA-B95)** — `branches.service_type` 값의 **앞뒤 공백을 trim** 한 뒤 G21 applicable 여부를 판정합니다.

| `serviceType` (trim 후) | G21 seed |
|-------------------------|----------|
| `DAY_CARE` (legacy) | **applicable** |
| `HOME_VISIT` 등 home-visit-like | **applicable** |
| 공백만·미지원 enum | **not applicable** — `g21-seed=not-applicable` |

> 관련: Q541 · DEPLOYMENT_GUIDE §11-3 · `./scripts/run-live-e2e.sh`
...
> 관련: Q360 · Q542 · Q580 · Q583 · DEPLOYMENT_GUIDE §3-7 · `docs/qa/VITEST_CONCURRENCY.md`

### Q579. 은행 입금 일괄등록에서 **미리보기에 없는 행 번호**를 보내면 어떻게 되나요? (G-BANK-EXCEL-8)

**A.** **✅ BE Fixed (`6ed7cd4`, G-BANK-EXCEL-8)** — **`POST /api/v1/billing/imports/bank-deposits`** 에 **`rowNumbers`** 를 포함할 때, 업로드한 xlsx **파싱 결과에 없는 번호**가 있으면 **silent skip 없이** **`422 BUSINESS_RULE`** 로 거부됩니다.

| 상황 | 응답 |
|------|------|
| `rowNumbers: [99]` — 엑셀에 1행만 존재 | **`422`「선택한 행 번호를 엑셀에서 찾을 수 없습니다: [99]」** |
| 미리보기 후 **다른 파일**로 import 시도 | 동일 — **미리보기와 동일 파일**로 등록하세요 |
| `rowNumbers` 생략 | 미리보기·파싱 기준 **매칭 가능 전체 행** (Q576) |

**현장 조치**: **「미리보기」→「선택 행 등록」** 을 **같은 xlsx** 로 연속 실행하세요. Swagger/API 직접 호출 시 **미리보기 `rows[].rowNumber`** 와 일치하는 번호만 전송합니다.

> 관련: Q572 · Q576 · USER_MANUAL §4-6 · ADMIN_GUIDE §10-4 US-L01

### Q580. live E2E에서 **health probe blocker**가 있는데 suite가 실행될 수 있나요?

**A.** **✅ FE Fixed (`9105332`·`06c6bb5`·`5f1815f`·`33e9e1a`·`cb3fe3d`, QA-B95)** — **`liveConfig.js`** 가 **suite contract에 무관한 operation blocker** 를 **필터**합니다. auth blocker는 **`normalizeOperationBlocker`** 로 trim·소문자화한 뒤 **`-`·`_`·`:` → 공백**으로 정규화하고, **이미 인증에 성공한 staff/guardian** 에 대해 **recoverable** 로 처리합니다 (`33e9e1a`). **`getEffectiveOperationBlockers`** (`5f1815f`·`cb3fe3d`)는 **현재 suite가 요구하지 않는 blocker** 를 **제외**합니다.

| health `operationBlockers` | `liveDescribe` (staff, 기본) | `liveG21Describe` (`requireG21Ready`) | `liveCashReceiptDescribe` (`requireCashReceiptReady`) |
|---------------------------|------------------------------|---------------------------------------|------------------------------------------------------|
| `staff-*-not-ready` / **`staff_auth_not_ready: fallback`** | **`requireStaffAuth=false`** 또는 auth recovered → **제외** | auth recovered → **제외** | auth recovered → **제외** |
| **`staff-credentials-default`** / **`guardian-credentials-default`** 및 **wording variant** (`default-staff-credentials`·`staff default credentials`·`* placeholder credentials` 등) | **auth recovered** → **제외** (`6fcd750`/`a170f9c`, Q640) | staff-only suite에서 guardian default → **제외** | staff-only suite에서 default cred → **제외** |
| `guardian-*-not-ready` | **`requireGuardianAuth=false`** → **제외** | **제외** | **제외** |
| `visit-schedule-missing` · `nhis-import-missing` · `g21-*` | **`requireG21Ready=false`** → **제외** (`cb3fe3d`) | **유지** — G21 seed 필요 | **제외** |
| `g32-*` · `g32-v157-constraint-missing` | **`requireG32Ready=false`** → **제외** (`cb3fe3d`) | **제외** | **제외** |
| `cash-receipt-identifier-check-missing` | **`requireCashReceiptReady=false`** → **제외** (`cb3fe3d`) | **제외** | **유지** — V159 schema 필요 |
| `user-account-request-integrity-check-missing` | **`requireStaffNhisImportReady=false`** → **제외** | **제외** | **제외** |

> **placeholder 자격증명**은 여전히 **SKIP** (`fffc2c1`, Q578). **stale access token** 은 **credential login fallback** (`b60c622`, Q583). **`staff-credentials-default`/`guardian-credentials-default`** 및 **wording variant** 는 **auth recovered 시 staff-only suite에서 skip 제외** (`6fcd750`/`a170f9c`, Q640). auth recovery·feature-scoped filter·placeholder skip·stale token recovery·default-credential recovery는 **별개**입니다.

> 관련: Q578 · Q583 · Q640 · Q548 · DEPLOYMENT_GUIDE §3-7 · `./scripts/run-live-e2e.sh`

### Q581. 은행·NHIS **엑셀 미리보기** 표의 스크린리더·키보드 접근성은? (UXD-146)

**A.** **✅ FE Fixed (`a7d9a2f`, UXD-146)** — **`BankDepositImportPanel`**·**`StaffNhisCaregiverImportPanel`** 미리보기 영역에 접근성이 보강되었습니다.

| 요소 | 내용 |
|------|------|
| 미리보기 버튼 | **`aria-label="은행 입금 엑셀 미리보기"`** / **`…요양보호사 엑셀 미리보기"`** |
| 결과 영역 | **`aria-label="…미리보기 결과"`** · 요약 **`aria-live="polite"`** (등록 완료 후 polite 해제) |
| 행 체크박스 | **`aria-label="{예금주\|성명} {N}행 선택"`** — 행마다 고유 accessible name |
| 로딩 | **`aria-busy`** on 미리보기·등록 버튼 |
| 은행 형식 안내 | **`ds-bank-deposit-formats`** · 샘플 xlsx **`aria-label`** |

> 관련: Q572 · Q573 · USER_MANUAL §4-6 · §4-7-0

### Q582. NHIS import 엑셀 **「처리상태」** 헤더가 BOM·공백·영문 alias여도 파싱되나요? (G7)

**A.** **✅ BE Fixed (`2edbdc4`, G7)** — **`NhisExcelParser.normalizeHeader`** 가 export variant 헤더를 정규화합니다.

| 정규화 | 내용 |
|--------|------|
| BOM 제거 | UTF-8 **BOM(`U+FEFF`)** 선행 문자 제거 |
| 공백 | trim + **모든 공백·탭·개행 제거** 후 소문자 |
| alias | `처리상태`·`processing_status`·`processingstatus` · `장기요양인정번호`·`ltc_cert_no` · `급여일수`·`service_days` · `공단부담금`·`nhis_amount` |

**행 처리**: **「처리상태」** 값은 **capture** 됩니다 — `정상`/`대기`/`오류` 등. `대기`·`보류`·`pending`·`검토중` → **`PENDING_REVIEW`** (Q181). **「오류」** 포함 행은 **방문일정 import**에서 제외(Q3214) — **청구 import**는 행 유지 후 matcher가 분류합니다.

> 관련: Q111 · Q181 · USER_MANUAL §4-6-1 · ADMIN_GUIDE §10-5

### Q583. live E2E에서 **만료된 access token**만 env에 남아 있으면 suite가 skip 되나요?

**A.** **✅ FE Fixed (`b60c622`, QA-B95)** — **`liveGlobalSetup.probeStaffAuth`/`probeGuardianAuth`** 가 token probe **실패** 시 **credential login fallback** 을 시도합니다.

| env 상태 | 동작 |
|----------|------|
| `LIVE_E2E_ACCESS_TOKEN` invalid · `LIVE_E2E_EMAIL`/`PASSWORD` usable | **`POST /auth/login`** → token **env 교체** · **`staffAuthReady=true`** |
| `LIVE_E2E_GUARDIAN_ACCESS_TOKEN` invalid · guardian creds usable | 동일 — guardian token 교체 |
| token invalid · creds missing/placeholder | **SKIP** 유지 (Q578) |
| bootstrap refresh token | **authoritative replace** (Q427) — stale shell 잔여와 병행 |

**조치**: `./scripts/run-live-e2e.sh` 재실행 전 **`scripts/dev-live-e2e.env`** 에 **실제 스테이징 계정**이 있는지 확인하세요. token만 stale이면 **자동 복구**됩니다.

> 관련: Q580 · Q578 · Q427 · DEPLOYMENT_GUIDE §11-3 · `docs/qa/VITEST_CONCURRENCY.md`

### Q584. 직원 **휴직** 상태(`ON_LEAVE`)를 lifecycle API·화면에서 설정할 수 있나요? (G-STAFF-LEAVE-STATUS, FAQ21720)

**A.** **✅ BE+FE Full-stack Fixed (`2581347`·`1d7cee2`·`68d4457`·UXD-147 `1a614c9`, US-R03b·FAQ21720)** — 이지케어 FAQ21720 「입사·퇴사·**휴직** 등 상태변경」에 맞춰 **백엔드·DB·프론트**가 **`ON_LEAVE`** 를 지원합니다.

| 항목 | 내용 |
|------|------|
| 화면 | **`/staff`** — **`StaffLifecycleSummaryPanel`** 「입사·퇴사·휴직 현황」StatCard · **`/staff/{userId}` → 「입사~퇴사」** — **`StaffLifecyclePanel`** 「휴직」 선택 |
| 집계 API | **`GET /api/v1/staff/lifecycle-summary?branchId=`** — `onLeaveCount`·`activeCount` 등 6종 (`1d7cee2`) |
| 저장 API | **`PATCH /api/v1/users/{userId}`** — `{ "lifecycleStatus": "ON_LEAVE", "hiredAt": "YYYY-MM-DD" }` |
| DB | **V166** — `lifecycle_status` 5-state · **`chk_users_on_leave_requires_hired_at`** · **`chk_users_on_leave_no_termination_date`** |
| 휴직 규칙 | **`hiredAt` 필수** · **`terminatedAt` 설정 금지** (`422`) · **`active=false`** 자동 · **퇴사일 입력란 비활성** (UXD-147) |
| 복직 | **`lifecycleStatus=ACTIVE`** — **`active=true`** 복원 · **`StaffLifecyclePanel.test`**·**`StaffLifecyclePilotServiceFlowE2eTest`** regression |
| compliance | **`StaffTrainingLogService`** — **신규교육 대상** 집계에서 **ON_LEAVE·TERMINATED 제외** |
| 목록 | **`StaffPage`** lifecycle **필터·Badge** — **「휴직」** warning tone (`STAFF_LIFECYCLE_STATUS.ON_LEAVE`) |

> 관련: Q290 · USER_MANUAL §5-3 · ADMIN_GUIDE §6-2-6c · DEPLOYMENT_GUIDE §11-3

### Q585. **입금대장** 반월·**수납대장** 청구/수납 기준 필터는 어떻게 쓰나요? (G-BILLING-DEPOSIT-HALFMONTH-REPORT)

**A.** **✅ BE+FE Full-stack Fixed (`e38ccfd`·`375fb9d`·`b96d038`, G-BILLING-DEPOSIT-HALFMONTH-REPORT·G-BILLING-RECEIPT-DUAL-BASIS)** — 케어포 PDF p.91 ②·③ 패턴입니다. **`/billing/reports/deposits`**·**`/billing/reports/receipts`** 화면에서 **segmented control**로 선택하면 **`fetchBillingReportApi`** 가 **`period`**·**`basis`** 를 전송합니다.

| variant | 화면 필터 | API 파라미터 | 값 | 용도 |
|---------|-----------|--------------|-----|------|
| **입금대장** | **「입금 구간」** | **`period`** | **`FULL`**(월간, 기본) · **`FIRST_HALF`**(1~15일) · **`SECOND_HALF`**(16~말일) | 입금일(`paidAt`) 기준 **반월 split** |
| **수납대장** | **「집계 기준」** | **`basis`** | **`PAYMENT`**(수납기준, 기본) · **`CLAIM`**(청구기준) | 수납 ledger 월 집계 기준 |
| **`charges`/`refunds`** | — | `period`·`basis` | **거부** — 다른 variant에 지정 시 **`422`** (아래 표 참고) |

| 잘못된 필터 | 응답 |
|-------------|------|
| `GET …/charges?period=FIRST_HALF` | **`422`「period 필터는 입금대장(deposits)에서만 사용할 수 있습니다.」** |
| `GET …/refunds?basis=CLAIM` | **`422`「basis 필터는 수납대장(receipts)에서만 사용할 수 있습니다.」** |
| `GET …/deposits?period=MID_MONTH` | **`422`「period는 FULL, FIRST_HALF, SECOND_HALF만 사용할 수 있습니다.」** |
| `GET …/receipts?basis=invoice` | **`422`「basis는 PAYMENT 또는 CLAIM만 사용할 수 있습니다.」** |

| UI 단계 | 내용 |
|---------|------|
| 1 | SideNav **청구 → 입금대장** 또는 **수납대장** — **`BillingReportsContextNav`** 서브 탭 |
| 2 | **대상 월**·(선택) **이용자 검색** 입력 |
| 3 | **입금대장** — **「입금 구간」** **월간/1~15일/16~말일** 선택 · **수납대장** — **「집계 기준」** **수납기준/청구기준** 선택 |
| 4 | **조회** — **`billingReportFilters.js`** **`buildBillingReportQueryParams`** → **`GET /api/v1/billing/reports/{variant}`** — 응답 **`appliedFilters`** 로 서버 적용 필터 확인 (Q587) |

| FE 모듈 | 내용 |
|---------|------|
| **`billingReportFilters.js`** | **`BILLING_DEPOSIT_PERIOD_OPTIONS`** · **`BILLING_RECEIPT_BASIS_OPTIONS`** · **`depositPeriodLabel`** · **`receiptBasisLabel`** |
| **`BillingReportPage.jsx`** | variant별 segmented control · **「적용 조건:」** summary · **`resolveBillingReportScopeLabel`** — 서버 **`appliedFilters`** 라벨 우선 (`c6a412f`) · **`aria-busy`**·**`aria-live`** (`e2f1246`, Q588) |
| 테스트 | **`billingReportFilters.test`** · **`BillingReportPage.test`** · **`BillingServiceTest`** (`7b99313`, receipt CLAIM basis) |

> **잘못된 월 형식**: `month=2026-99` 등 regex만 통과하는 값은 **`422`「대상 월 형식이 올바르지 않습니다.」** (Q586, `375fb9d`).

> 관련: Q179 · Q586 · Q587 · Q588 · USER_MANUAL §5-10 · ADMIN_GUIDE §6-2-6e · DEPLOYMENT_GUIDE §11-3

### Q587. 본인부담 **대장 API** 응답의 **`appliedFilters`** 는 무엇인가요? (G-BILLING)

**A.** **✅ BE+FE Full-stack Fixed (`14935a3`/`c6a412f`/`e2f1246`, G-BILLING)** — **`GET /api/v1/billing/reports/{variant}`** 응답에 **`appliedFilters`** 객체가 포함되어, **서버가 실제 적용한 필터**를 클라이언트·인쇄 헤더·자동화 스크립트가 확인할 수 있습니다.

| 필드 | 입금대장 (`deposits`) | 수납대장 (`receipts`) | 청구·환불 (`charges`/`refunds`) |
|------|----------------------|----------------------|----------------------------------|
| `variant` | ✅ | ✅ | ✅ |
| `month` | ✅ `YYYY-MM` | ✅ | ✅ |
| `depositPeriod` | ✅ `FULL`/`FIRST_HALF`/`SECOND_HALF` | `null` | `null` |
| `depositPeriodLabel` | ✅ `월간`/`1~15일`/`16~말일` | `null` | `null` |
| `receiptBasis` | `null` | ✅ `PAYMENT`/`CLAIM` | `null` |
| `receiptBasisLabel` | `null` | ✅ `수납기준`/`청구기준` | `null` |

**예시** — `GET /api/v1/billing/reports/deposits?month=2026-06&period=SECOND_HALF`:

```json
"appliedFilters": {
  "variant": "deposits",
  "month": "2026-06",
  "depositPeriod": "SECOND_HALF",
  "depositPeriodLabel": "16~말일",
  "receiptBasis": null,
  "receiptBasisLabel": null
}
```

> **FE wire (`c6a412f`/`e2f1246`)**: **`BillingReportPage`** 가 조회 응답 **`appliedFilters`** 를 state에 저장하고, **`resolveBillingReportScopeLabel`** (`billingReportFilters.js`)로 **서버 라벨을 우선**하여 **「적용 조건:」** summary·인쇄 헤더에 표시합니다. segmented control과 불일치 시 **서버 응답을 우선**하세요.

> 관련: Q585 · Q586 · Q588 · USER_MANUAL §5-10 · **`BillingReportAppliedFiltersTest`** · **`BillingServiceTest`** (`7b99313`) · **`MustApiEndpointRoutingTest`**

### Q588. 입금·수납 **대장 필터** 화면의 스크린리더·키보드 접근성은? (UXD-148)

**A.** **✅ FE Fixed (`e2f1246`, UXD-148)** — **`BillingReportPage`** 입금·수납 대장 필터 영역 접근성이 보강되었습니다.

| 요소 | 내용 |
|------|------|
| **「조회」** 버튼 | **`aria-busy={loading}`** — API 조회 중 busy 상태 전달 |
| **「적용 조건:」** summary | **`ds-billing-report__applied-filters`** · **`role="status"`** · **`aria-live="polite"`** — 서버 **`appliedFilters`** echo 반영 |
| 대상 월 | **`<time dateTime={YYYY-MM}>`** — 월 값 semantics |
| 인쇄 전용 헤더 | **`ds-care-report-print-only`** · **`aria-hidden="true"`** — 화면 리더 중복 방지 |
| segmented control | **`role="tablist"`** · **`aria-labelledby`** · 로딩 중 **`disabled`** |

> 관련: Q585 · Q587 · USER_MANUAL §5-10 · **`BillingReportPage.test`**

### Q589. 직원 출석·출퇴근 명단(Roster)를 조회할 수 있나요?

**A.** **✅ G-STAFF-WORK-ATTENDANCE Full-stack Fixed (`a6eb8b7`·`5fd468b`, 케어포 8-4, Q612)** — **직원 일일 출퇴근**은 **`/staff/attendance`** · **이용자(수급자) 당일 출석**은 **`/attendance`** — API·화면이 **분리**되어 있습니다.

| 구분 | 화면 | API | 상태 |
|------|------|-----|------|
| **직원 출퇴근 (8-4)** | **`/staff/attendance`** | **`GET /api/v1/staff/work-attendance?date=&branchId=`** | **✅ Must** |
| 직원 수동 출근 | 동일 | **`POST /api/v1/staff/work-attendance/check-in`** — `{ userId, checkInMethod }` | **✅ Must** (당일·`MANUAL` 기본) |
| 직원 수동 퇴근 | 동일 | **`POST /api/v1/staff/work-attendance/check-out`** — `{ userId }` | **✅ Must** (당일) |
| **이용자 출석 roster** | **`/attendance`** | **`GET /api/v1/attendance?transportMode=`** | **✅ Must** (Q609) |
| 이용자 QR 체크인 | **`/attendance/qr`** 등 | **`POST /api/v1/attendance/qr-check-in`** | **△ Partial** — payload 검증만 · QR 이미지 생성 P3 (Q590) |
| 직원 출퇴근 CSV·월간 통계 | — | — | **❌ P3** — 지각·결근·휴직 집계·export 미구현 |

**직원 출퇴근 운영 방법 (8-4)**:
1. SideNav **운영 → 직원**(`/staff`) → **`StaffContextNav`「출퇴근 (8-4)」** 또는 **`/staff/attendance`**
2. **근무일** 선택 → **「조회」** — 지점 **활성 직원 전원** + **출근/퇴근/미출근** Badge
3. **「출근」** / **「퇴근」** — **`hq_admin`·`branch_admin`·`social_worker`** 만 (당일 Asia/Seoul 기준)
4. StatCard **출근·퇴근·미출근** 건수 확인

| `status` | 의미 | UI |
|----------|------|-----|
| `CHECKED_IN` | 출근만 완료 | **「퇴근」** 버튼 |
| `CHECKED_OUT` | 퇴근 완료 | **「완료」** |
| `null` | 미출근 | **「출근」** 버튼 |

| 오류 (422) | 원인 |
|------------|------|
| 「오늘 이미 출근 처리된 직원입니다.」 | 중복 출근 |
| 「출근 기록이 없어 퇴근 처리할 수 없습니다.」 | 미출근 퇴근 |
| 「이미 퇴근 처리된 직원입니다.」 | 중복 퇴근 |

> **P3 잔여**: **`checkInMethod` `MOBILE`/`NFC`** FE wire · 월간 통계·CSV export · QR/NFC 단말 연동  
> 관련: **Q612** · USER_MANUAL §5-3 · ADMIN_GUIDE §6-2-20 · **V169** `staff_work_attendance`

---

### Q590. QR 스캔 체크인은 어떤 형식인가요? (G-ATTENDANCE-QR)

**A.** (2026-06-21, **QR 출석 통합 기능**)

**Partial — 현 상태**:

**QR 체크인 워크플로**:
1. **QR 코드 생성 대상** — 이용자·보호자·직원(출퇴근)
2. **QR 페이로드** — JSON 인코딩:
   ```json
   {
     "entityType": "RESIDENT",  // RESIDENT | GUARDIAN | STAFF
     "entityId": "uuid-resident-123",
     "timestamp": "2026-06-21T08:30:00Z",
     "branchId": "uuid-branch-456"
   }
   ```
3. **스캔 프로세스**:
   - 모바일/태블릿에서 카메라 앱 또는 **`/checkin/qr-scan`** 페이지로 스캔
   - `POST /api/v1/attendance/qr-check-in` 전송
   - 응답: `{ checkInId, statusMessage, checkInTime }`

**미지원 (P3)**:
- **QR 이미지 생성** — `GET /api/v1/qr-code/generate` 엔드포인트 존재 but FE wire P3
- **대량 인쇄** — PDF/라벨 출력 템플릿 P3
- **오프라인 스캔** — 네트워크 없이 로컬 저장 후 동기화 P3

**사용 시나리오**:
- 이용자 **「도착」** 시 QR 스캔 → 자동 체크인 (보호자 동반 우대)
- 요양보호사 **「출근」** 시 QR 스캔 → 출석 기록
- 센터 장비 교체·방문자 영수증 용도로 활용 예정

> **REQUIREMENTS §3-1 · API_SPEC §G-ATTENDANCE-QR · USER_MANUAL §2 follow-up** 참고.

---

### Q591. Live E2E 테스트 환경에서 일부 기능만 활성화할 수 있나요?

**A.** (2026-06-21, **FE `cb3fe3d` · live E2E feature-scoped operation blockers**)

**Yes — Feature-scoped 차단 기능 (v1.2.1+)**:

**설정 방법** — `liveConfig.getEffectiveOperationBlockers()`:

| 설정 | 영향 범위 | 용도 |
|------|---------|------|
| `requireG21Ready` | 방문요양 출석·일지 | 방문요양 미검증 센터 블로킹 |
| `requireG32Ready` | 사례관리 회의·평가 | 사례관리 미구성 센터 블로킹 |
| `requireCashReceiptReady` | 현금영수증 발급 | 현금 수납 미연동 센터 블로킹 |
| `requireStaffNhisImportReady` | 직원 공단 import | 직원 등록 미완료 센터 블로킹 |

**사용 예**:
```javascript
// staff 전용 테스트 스위트
if (!liveConfig.requireStaffAuth) {
  // guardian blocker는 무시
  const staffBlocked = blockers.filter(b => !b.includes('guardian'));
}
```

**개선 사항** (290차):
- **`liveCashReceiptDescribe` suite guard** — cash-receipt live harness가 **`liveDescribe` gate** 뒤에서만 실행 (`cd6891f`, Q597)

**개선 사항** (288차):
- **snake_case 정규화** — `staff_auth_not_ready` 등 형식 자동 변환
- **관계없는 blocker 필터링** — G21 staff suite가 cash-receipt blocker만으로 SKIP 되지 않음
- **테스트 안정화** — 부분 E2E 커버 가능

> **liveE2eHarness.test.js · DEPLOYMENT_GUIDE §11-3** 참고.

---

### Q592. 청구 기준(basis)이 「수납기준」과 「청구기준」 어떻게 다른가요?

**A.** (2026-06-21, **BE `7b99313` · G-BILLING-RECEIPT-DUAL-BASIS**)

**Definition**:

| 기준 | 의미 | 시점 | 사용 예 |
|------|------|------|--------|
| **수납기준(Receipt Basis)** | 실제 입금된 날짜 | 현금 입금 | 월별 현금 수입 현황 분석 |
| **청구기준(Claim Basis)** | 청구서 발행 월 | 청구 생성 | 공단 청구 현황 → NHIS 연계 |
...
```json
{
  "variant": "DEPOSITS",
  "month": "2026-06",
  "depositPeriodLabel": "월간",
  "receipts": [
    { "receiptId": "...", "amount": 500000, "basis": "RECEIPT" },
    { "receiptId": "...", "amount": 300000, "basis": "CLAIM" }
  ],
  "appliedFilters": {
    "receiptBasisLabel": "수납기준"  // 또는 "청구기준"
  }
}
```

**선택 방법** — `/billing/reports/receipts` 페이지:
1. **「집계 기준」** segmented control 선택
2. **「조회」** → 조건에 맞는 수납 내역 표시
3. **필터 요약** 하단 **「적용 조건: 집계 기준=수납기준」** 확인

> **CHANGELOG 286차 · Q587 · API_SPEC G-BILLING · USER_MANUAL §5** 참고.

---

### Q593. 월별 입금 구간(반월별)을 나눠서 볼 수 있나요?

**A.** (2026-06-21, **FE `e38ccfd` · G-BILLING-DEPOSIT-HALFMONTH-REPORT**)

**Yes — 월간·반월별 구간 분석**:

**구간 선택** — `/billing/reports/deposits` 페이지:

| 구간 | 범위 | 용도 |
|------|------|------|
| **월간(Full)** | 1~31일 | 월별 전체 입금 현황 |
| **1~15일(1st Half)** | 1~15일 | 상순 입금 현황·선납 추적 |
| **16~말일(2nd Half)** | 16~말 | 하순 입금 현황·연체 추적 |

**선택 방법**:
1. `/billing/reports/deposits` 페이지 접속
2. **「입금 구간」** segmented control 선택:
   - 「월간」 (default)
   - 「1~15일」
   - 「16~말일」
3. **「조회」** → 구간별 입금 내역 필터링
4. **조건 요약**: 하단 **「적용 조건: 입금 구간=1~15일」** 확인
...
```json
{
  "variant": "DEPOSITS",
  "month": "2026-06",
  "depositPeriodLabel": "1~15일",  // 또는 "월간", "16~말일"
  "deposits": [...]
}
```

**사용 사례**:
- 급여일(15일) 전후로 입금 패턴 분석
- 월초 선납 현황·월말 연체 추적
- 지점별 입금 주기 최적화

> **CHANGELOG 286차 · Q585 · API_SPEC G-BILLING · USER_MANUAL §5** 참고.

---

### Q586. 입금·수납 대장에서 **잘못된 월**(예: `2026-99`)을 조회하면 어떻게 되나요?

**A.** **✅ BE Fixed (`375fb9d`, G-BILLING)** — **`BillingService.normalizeYearMonth`** 가 **`YYYY-MM` regex** 통과 후 **`YearMonth.parse`** 로 **실제 달력 월**을 검증합니다.

| 요청 | 응답 |
|------|------|
| `GET /api/v1/billing/reports/deposits?month=2026-99` | **`422 BUSINESS_RULE`「대상 월 형식이 올바르지 않습니다.」** |
| `GET …/receipts?month=2026-13` | 동일 |
| FE **`MonthInput`** | UI에서 **유효한 월**만 선택 — API 직접 호출·자동화 스크립트에서 invalid month 주의 |

> 관련: Q585 · USER_MANUAL §5-10 · **`BillingServiceTest.listBillingReportShouldRejectInvalidCalendarMonth`**

### Q594. 대시보드 **「공단 일정 불일치」** 위젯은 무엇인가요? (G21)

**A.** **✅ BE+FE Full-stack Fixed (`0796821`/`fe7df60`, G21, Q594)** — 방문요양·통합재가 지점의 **당월 PLAN 일정 vs NHIS import** 불일치 건수를 **`/dashboard`·`/dashboard/hq`** StatCard로 표시합니다.

| 항목 | 내용 |
|------|------|
| 라벨 | **「공단 일정 불일치」** (`NHIS_COMPARISON_GAP_WIDGET_LABEL`) |
| API | **`GET /api/v1/dashboard/branch`** · **`GET /api/v1/dashboard/hq`** — **`nhisComparisonGapCount`** |
| 집계 | **`discrepancyLineCount` + `missingNhisLineCount` + `extraNhisLineCount`** (`VisitService.countNhisComparisonGapLines`) |
| 표시 조건 | **home-visit-like** 지점(`HOME_VISIT` 등) 또는 **통합 대시보드** — **주야간-only** 지점 branch dashboard에서는 **숨김** (`ebc9f28`) |
| 폴백 | 구버전 API(필드 없음)만 **`GET /visits/confirm-readiness` `nhisComparisonSummary`** 병렬 조회 (`c01b880`) |
| 조치 | StatCard 클릭 → **`/visits`** — **`VisitNhisComparisonPanel`**·일괄확정 Modal 사전 비교 |

> 관련: Q570 · Q481 · Q484 · USER_MANUAL §4-2 · §5-8 · ADMIN_GUIDE §1-4

### Q595. 통합 대시보드 **「카카오 API 잔여」** 위젯은 어디서 확인하나요? (G15-KAKAO-QUOTA-DASH)

**A.** **✅ FE Fixed (`580a86b`, v1.3-A, Q595)** — **`hq_admin`·`sysadmin`** 의 **`/dashboard`·`/dashboard/hq`** 운영 위젯 그리드에 **「카카오 API 잔여」** StatCard가 추가되었습니다. 상세 테이블은 기존 **`/settings`「카카오 API」** 패널(Q554)과 동일 API를 사용합니다.

| StatCard 값 | 의미 |
|-------------|------|
| **미설정** (warning) | REST API 키 미구성 |
| **0건** (danger) | **`quotaExceeded=true`** |
| **N건** (default/warning) | **`quotaUsage[]` 최소 `remainingToday`** — **500건 이하 warning** |
| **정상** / **확인** | usage row 없을 때 Geocode/Directions probe 결과 |

| API | **`GET /api/v1/transport/kakao-api-status`** — **`fetchTransportKakaoApiStatusApi`** (`services.js`) |
| 링크 | StatCard → **`/settings`** (카카오 API 탭) |

> **주의**: ogada **in-memory 일일 카운트** — 재기동 시 리셋 · Kakao 콘솔 Quota와 다를 수 있음 (Q554). 관련: USER_MANUAL §5-2 · ADMIN_GUIDE §10-10 · DEPLOYMENT_GUIDE §3-7

### Q596. bootstrap **내부 오류**일 때 live E2E probe가 **너무 많은 blocker**를 보여줍니다. (QA-B95)

**A.** **✅ BE Fixed (`0c9518a`·`20485f1`·`018a781`·**`24d25f1`**, QA-B95, Q596·Q631)** — **`bootstrap=error`** 또는 **`guardian-bootstrap=error`** 이면 **`LiveE2eOperationReadinessSupport`** 가 **derived blocker**(visit-schedule·NHIS·G32·V159 등)를 **생략**합니다. **`20485f1`부터** detail에 **접미사(`: …`)** 가 붙어도 동일하게 error로 인식합니다.

**`24d25f1`부터 (Q631)**: **`operationBlockers`** 에 **`-error` 코드를 더 이상 추가하지 않습니다**. bootstrap 실패 시 **`staff-bootstrap-not-ready`·`guardian-bootstrap-not-ready`만** 노출하고, **내부 오류 여부는 `liveE2eStatusDetail`/`liveE2eGuardianStatusDetail`** (`bootstrap=error`·`guardian-bootstrap=error`)로 확인합니다. health·probe **동일 blocker 집합**입니다.

| bootstrap 실패 | `operationBlockers` | primary (`operationBlocker`) | root cause 확인 |
|----------------|---------------------|------------------------------|-----------------|
| staff·guardian 모두 error | `staff-bootstrap-not-ready`, `guardian-bootstrap-not-ready` | **`staff-bootstrap-not-ready`** | detail **`bootstrap=error`** · **`guardian-bootstrap=error`** |
| staff만 error | `staff-bootstrap-not-ready` (+ guardian not-ready 여부) | **`staff-bootstrap-not-ready`** | detail **`bootstrap=error`** |
| HealthController bootstrap exception | `staff-bootstrap-not-ready`, `guardian-bootstrap-not-ready` | **`staff-bootstrap-not-ready`** | *(구 `bootstrap-error` 단일 코드 **폐기**, Q631)* |

**조치**: env placeholder보다 **서버 로그·Flyway·live-e2e seed** 를 먼저 확인하세요. credential·G21 seed blocker는 **bootstrap 복구 후**에만 의미 있습니다.

> 관련: Q448 · Q631 · Q580 · DEPLOYMENT_GUIDE §11-3 · **`LiveE2eOperationReadinessSupportTest`** · **`LiveE2eControllerTest`**

### Q597. **`liveCashReceiptDescribe`** live suite는 언제 실행되나요? (QA-B195)

**A.** **✅ FE Fixed (`cd6891f`, QA-B195, Q597)** — **`liveE2eSuiteGuard.test`** 가 **`liveCashReceiptDescribe`** 를 **`liveDescribe` gate regex**에 포함합니다. cash-receipt routing harness는 **staff live E2E preflight 통과 후**에만 RUN 됩니다.

| suite | gate |
|-------|------|
| **`liveDescribe`** (staff 기본) | placeholder credential/token · auth blocker recovery · feature-scoped filter |
| **`liveCashReceiptDescribe`** | 위 gate **+ `requireCashReceiptReady`** (V159 schema 등) |

> 관련: Q580 · Q591 · Q596 · `./scripts/run-live-e2e.sh` · `docs/qa/VITEST_CONCURRENCY.md`

---

### Q574. `hq_admin`이 **통합 대시보드**와 **지점 대시보드**를 어떻게 오가나요? (US-H01)

**A.** **✅ FE Fixed (`c1ebaaf`·`a2f599c`, US-H01)** — **`DashboardContextNav`** 가 `/dashboard`·`/dashboard/hq` 상단에 표시됩니다.

| 링크 | API·동작 |
|------|----------|
| **통합 대시보드** | `/dashboard/hq` — `GET /api/v1/dashboard/hq` — 전 지점 집계 |
| **지점 대시보드** | `/dashboard` — `GET /api/v1/dashboard/branch` — **BranchSwitcher 활성 지점** |
| **지점 drill-down** | 통합 화면 **`HqBranchSummaryTable`** — 지점 행 클릭 시 활성 지점 전환 후 `/dashboard` |

> 관련: Q118 · Q177 · USER_MANUAL §5-2

### Q575. **V165** `user_account_requests` 마이그레이션은 왜 필요한가요?

**A.** **✅ BE Fixed (`05ca0a8`·`3bbfc00`, V165)** — G-STAFF-NHIS-EXCEL-IMPORT가 **한 엑셀에서 다수 PENDING 요청**을 적재하므로 DB 레벨 방어가 필요합니다.

| V165 내용 | 효과 |
|-----------|------|
| 8 CHECK | `email`·`display_name` nonempty · `role_code` enum · `branch_ids` JSONB array · status pair · `review_note` nonempty 등 |
| Tenant FK 3쌍 | `requested_by`·`created_user`·`active_branch_id` cross-tenant 차단 |
| live E2E probe | **`user-account-request-integrity-check-missing`** blocker — V165 미적용 시 operation ready **false** |

> 관련: Q555 · Q573 · `DATA_RETENTION_POLICY.md` · DEPLOYMENT_GUIDE §1-3 Flyway V165
...
**A.** **ogada 직원(`ogada_platform_admin`)**이 신규 고객을 등록합니다. 센터가 스스로 공개 가입(`/signup`)하는 방식은 MVP에 **포함하지 않습니다**.

| 순서 | 담당 | 작업 |
|------|------|------|
| 1 | ogada `ogada_platform_admin` | `/platform`에서 법인(Tenant) 등록 |
| 2 | ogada `ogada_platform_admin` | 첫 **통합 관리자(`hq_admin`)** 계정 발급 (이름·임시 비밀번호 필수, Q555) |
| 3 | 고객 `hq_admin` | 지점 등록, **직원 계정 생성 요청**·이용자 등록, 운영 시작 |

> 비유: 통신사 매장이 **새 회선을 개통**해 주는 것과 같습니다. (REQUIREMENTS §1-3)

### Q6. 센터장·요양보호사·보호자 계정은 누가 만드나요?

**A.** 고객 센터 **`hq_admin` 또는 `branch_admin`**이 **`/staff`에서 계정 생성 요청**을 등록합니다. ogada **`ogada_platform_admin`**이 **`/platform`에서 승인**하면 계정이 발급됩니다 (Q555). 보호자(`guardian`) 계정은 이용자 등록 시 **보호자 연결** 과정에서 발급·연결합니다.
...
| 역할 | 코드 | 주요 업무 |
|------|------|----------|
| 플랫폼 관리자 | `ogada_platform_admin` | 신규 Tenant 등록, 첫 `hq_admin` 발급, **직원 계정 요청 승인** (ogada 내부) |
...
### Q10. `ogada_platform_admin`과 `sysadmin`은 같은 관리자인가요?

**A.** **다릅니다.**

| 구분 | `ogada_platform_admin` | `sysadmin` |
|------|------------------------|------------|
...
| **출석** | **✅ Fixed (`d058e43`, Q628)** — **`fetchClientAttendanceHistoryApi`** · **`GET /clients/{id}/attendance?from=&to=`** · 출석 탭 테이블 (Q102 closure) |
...
| `/attendance`, `/attendance/checkin` | **당일 목록 + 입소·귀가·결석 버튼**(US-E01·E02) — **roster·`status` ✅** (Q609) | `GET /attendance` |
...
| 출석 목록 필드 | `name`, `status`, `checkInTime`, `checkOutTime`, `PENDING` | `clientId`, **`clientName`**, **`status`**, `checkInAt`, `checkOutAt`, pending **`id=null`** | **Fixed** (`0c69060`, Q609) — 시각 필드명 `checkInAt`/`checkOutAt` |
...
| ~~이용자 상세 출석 탭~~ | ~~`GET /attendance?clientId=&from=&to=`~~ | **`GET /clients/{id}/attendance?from=&to=`** | **Fixed (`d058e43`, Q628)** — **`fetchClientAttendanceHistoryApi`** · 출석 탭 테이블 |
...
|------|----------------|-----------|---------|
| 목록 | StatCard 요약 + 이용자별 **입소/귀가/결석** 버튼 | `GET /attendance` | **`clientName`·`status`·활성 이용자 전원** (`0c69060`, Q609) |
...
> **roster (Q609)**: **`GET /attendance`** 가 지점 **활성 이용자 전원**을 반환 — 미처리는 **`status=null`** · **입소·결석** 버튼 표시. 잔여: **교통편 필드명**(Q96)·**통계 API**(Q106).
...
**A.** **✅ BE+FE Full-stack Fixed (`0c69060`/`61e1970`/`8383f8d`, US-E01, Q609)** — **`GET /api/v1/attendance`** 가 활성 지점 **전체 이용자**를 **`clientName`·`status`·`usesTransport`** 와 함께 반환합니다. **`/attendance`** 화면은 **`fetchAttendanceApi()`** 만 사용해 **미처리 이용자도 목록에 표시**하고 **입소·결석 버튼**을 노출합니다.

| 항목 | 이전 (갭) | 현재 (`0c69060`+) |
|------|----------|-------------------|
| roster 범위 | 당일 **기록 있는 행만** | **활성 이용자 전원** (기록 없으면 pending row) |
| `clientName` | ❌ | ✅ 이용자 이름 |
| `status` | ❌ | ✅ `CHECKED_IN`/`CHECKED_OUT`/`ABSENT`/`null`(미처리) |
| pending row | 목록 누락 | **`id=null`**, **`status=null`** — 입소·결석 버튼 표시 |

**상태 파생 규칙** — **`AttendanceStatusSupport.deriveStatus()`** (보호자 포털과 공유):

| 조건 | `status` |
|------|----------|
| `absenceReason` 있음 | `ABSENT` |
| `checkOutAt` 있음 | `CHECKED_OUT` |
| `checkInAt`만 있음 | `CHECKED_IN` |
| 출석 기록 없음 | `null` (미처리) |

**RBAC** (`61e1970`): **`caregiver`·`branch_admin`·`social_worker`·`hq_admin`** — `GET /attendance` **200** · **`guardian`** — **403** (직원 수기 출석 전용).

**잔여 갭 (별도 FAQ)**:

| 항목 | 상태 | FAQ |
|------|------|-----|
| 귀가 교통편 필드명 | FE `transportMethod` vs BE `transportType` | Q96 |
| 출석 통계 쿼리·응답 | FE `yearMonth` vs BE `from`/`to` | Q106 |
| QR 이미지·payload | 생성 UI Partial | Q109·Q590 |

**현장 점검 (296차 이후)**

1. **BranchSwitcher**로 작업 지점을 확인합니다 (Q173).
2. **`/attendance`** — StatCard **「미처리」** 건수·이용자 **이름**·**상태 Badge** 확인.
3. 미처리 행에서 **입소**·**결석** 버튼이 보이면 정상 — roster **full-stack** 동작 중.
4. 여전히 UUID만 보이거나 목록이 비면 **FE HEAD `8383f8d`+** · **BE `0c69060`+** 확인 (`CHANGELOG` baseline).

> 상세 API: **Q609** · USER_MANUAL §4-4 · DEPLOYMENT_GUIDE §1-4 roster smoke
...
**A.** `ClientDetailPage`는 **5탭 UI**(기본·등급·건강·출석·청구)이며, 탭별 정합 수준은 다릅니다. **출석 탭**은 2026-06-22 **US-D03 FE wire closure** (`d058e43`, Q628)로 **정상 표시**됩니다.

| 탭 | UI (2026-06-22) | 백엔드 API | 증상 |
|----|----------------|-----------|------|
...
| 출석 | **✅ Fixed (`d058e43`, Q628)** — **`fetchClientAttendanceHistoryApi`** · 출석 탭 선택 시 lazy load · 테이블(날짜·상태·입소·귀가·결석 사유) | **`GET /clients/{id}/attendance?from=&to=`** | **정상** — 당일 체크인/아웃은 **`/attendance`** (Q609) |
...
**Swagger 우회 — 출석 탭** *(UI Fixed — 참고용)*

`GET /api/v1/clients/{id}/attendance?from=2026-05-07&to=2026-06-06`

> **307차 이후**: 화면 **`/clients/:id` → 「출석」** 탭에서 동일 API가 자동 호출됩니다 (Q628). **`from`/`to` 생략** 시 서버 기본 기간이 적용됩니다.
...
|------|---------|-----|-----------|
| `/attendance/qr/generate` | 출석 → QR 생성 | **`QrGeneratePage`** — **PNG 미리보기·저장·인쇄** (US-E03, Q624) | `POST /branches/{branchId}/qr` — **`qrToken` → PNG ✅** |
| `/attendance/stats` | 출석 → 출석 통계 | **`AttendanceStatsPage`** (US-E05) | `GET /attendance/stats/monthly?from=&to=` — **✅ FE wire** (Q615) |
| `/attendance/checkin/qr` | *(보호자 포털·직접 URL)* | **`GuardianCheckinPage`** — **Fixed** (Q109) | `POST /attendance/qr/scan` |
| `/attendance/checkin` | 출석 → 수기 체크인 | **`AttendancePage`** — 입소·귀가·결석 버튼 | `POST /attendance/check-in` — **roster ✅** (Q609) |
...
**A.** **✅ Fixed (US-E05, Q615, FE `dffd726`)** — **`fetchMonthlyAttendanceStatsApi`** 가 **`yearMonth` → `from`/`to`** 로 변환해 **`GET /api/v1/attendance/stats/monthly`** 를 호출하고, **`monthlyAttendanceStats.js`** 가 **`branches[].months[]`** 를 StatCard·차트·표에 매핑합니다.

| 항목 | FE (`dffd726`) | BE |
|------|----------------|-----|
| 쿼리 | **`from`·`to`·`branchId`** (`yearMonthToPeriod`·6개월 추이) | 동일 contract |
| StatCard | **활성 이용자·출석 일수·출석률(%)** | `activeClientCount`·`attendedDays`·`attendanceRate` |
| 차트 | **최근 6개월 월별 출석률 추이** (line) | `months[]` trend |
| 표 | **다지점 HQ** — 지점별 월 집계 | `branches[]` |

**P3 한계**: **일별·이용자별 breakdown** 은 BE 미제공 — 월·지점 집계만 표시합니다.

**여전히 빈 화면일 때**:
- 해당 월·지점 **활성 이용자 0명** 또는 **출석 기록 없음** — **`/attendance`**(Q609)에서 먼저 출석 입력
- **`hq_admin`** — **BranchSwitcher** 전환 후 **재조회** (Q173)

> **상세**: FAQ **Q613·Q615** · USER_MANUAL §4-4 · ADMIN_GUIDE §6-2-21
...
**A.** **Partial Fixed (FE `bf3d40d`·`b046444`·`99f2f3e`, US-E03·E04)** — **`GuardianCheckinPage`** 체크인 대상·스캔 API가 **정합**되었습니다. **`QrGeneratePage`** 는 **지점 스코프 경로**(`POST /branches/{branchId}/qr`)로 연동되었으나, **요청 본문·QR 이미지 응답 갭**이 잔존합니다.

**현장 체크리스트 (Must 갭 — 282차 TWR)**

| 구분 | 증상 | 원인 | 즉시 조치 |
|------|------|------|----------|
| **체크인** | 스캔 실패 | `clientId`·`qrToken` 불일치 | **`/guardian/checkin`** — 대상 선택 후 토큰 입력 (Q109 Fixed) |
| **체크인** | 대상 없음 | `guardian_link_status=PENDING` | 보호자 연결 후 재시도 (Q110) |
| **QR 생성** | **미리보기 빈 칸** | API가 **`qrDataUrl` 미반환** | Swagger로 `qrToken` 발급 → 토큰 수동 입력·인쇄 (아래) |
| **QR 생성** | **`400` 오류** | FE `{ type, validMinutes }` vs BE `{ direction, expiresInMinutes }` | Swagger **`direction: "in"\|"out"`** 사용 |
| **QR 생성** | 버튼 비활성 | **`activeBranchId` 없음** | **BranchSwitcher**로 지점 선택 (Q173) |
...
> **처리상태 열**: 엑셀 **첫 열 또는 선행 열**이 「처리상태」(`processing_status` 등 alias)이면 **`NhisExcelParser`** 가 값을 **capture** 합니다. BOM·탭·개행이 섞인 export variant 헤더도 **`normalizeHeader`** 로 매핑됩니다 (`2edbdc4`, Q582). `대기`·`보류` 등 → **`PENDING_REVIEW`** (Q181).
...
| **잔여 갭** | QR·통계 **API 정합**(Q109·Q106·Q151) · **건강 추이 `?days=`**(Q165) · **입금·보호자 목록 API** · **이용자 상세 건강 탭** · **출석 roster ✅** (Q609) |
...
| **지점 roster** | **`GET …/enrollments?branchId=&status=`** — **`clientId` 생략** 시 활성 지점 CMS 등록 목록 · **`branchId` 생략** 시 JWT **활성 지점(TenantContext) fallback** (`d0c0d12`, Q638) · **`clientName`** 포함 (`d361833`, Q637) |
...
### Q222. 이용자 상세 **「보호자 서류 발송」** 카드는 무엇인가요?

**A.** **Fixed (BE `0854fbd`·`f77a268` + FE `d1149a5`, G2, Q517)** — **`GuardianDocumentNotifyPanel`** 이 이용자 상세 **기본정보** 탭에서 **3종 보호자 서류** 이메일을 발송합니다.

| 문서 유형 | API | 템플릿 |
|----------|-----|--------|
| **노인학대예방 및 대응지침** | `POST …/notifications/elder-abuse-prevention-guideline` | `ELDER_ABUSE_PREVENTION_GUIDELINE` |
| **가정통신문** | `POST …/notifications/home-newsletter` | `HOME_NEWSLETTER` |
| **급여제공기록지** | `POST …/notifications/care-provision-record` | `CARE_PROVISION_RECORD` |

| 항목 | 내용 |
|------|------|
| 화면 | `/clients/:id` → **기본정보** 탭 — **「보호자 서류 발송 (G2)」** 카드 · **`GUARDIAN_DOCUMENT_NOTIFY_TYPES`** |
| 입력 | **문서 유형**(`Select`) · **대상 연월**(`MonthInput`, 필수) · **요약**(선택, 500자 이하) |
| 급여제공기록지 | 요약 미입력 시 서버가 **이동서비스 요약**을 본문에 자동 병합 |
| 권한 | `branch_admin`·`social_worker` |
...
> **급여제공기록지 조회** — **「급여제공」탭** (Q243). **납부확인서** — 청구 상세 **「납부확인서 발송」**(Q221).

> 관련: Q204 · Q216 · Q217 · Q243 · USER_MANUAL §4-7-3 · ADMIN_GUIDE §10-9
...
| **preview (Q572)** | **`GET …/bank-deposits/formats`** · **`POST …/bank-deposits/preview`** · **`POST …/bank-deposits`** — **FE full-stack ✅** (`a18b30e`) |
| UI | **`/billing/payments`** **`BankDepositImportPanel`** — **「미리보기」**·행별 Badge·**「선택 행 등록 (N건)」** · 성공 시 **`appliedCount`** 요약 |
...
| 항목 | 내용 |
|------|------|
| UI | **`/transport/compliance`** **`TransportCompliancePanel`**·**`TransportServiceLogRunsPanel`** — **이동서비스 수칙·계약 (G15)** (Q397·**Q426**) |
| 워크플로 | ① **수칙 5필수** 마련 → ② **계약서에 수칙 포함** → ③ **이동서비스일지(제22호)** 작성·**저장**·출력 (Q236·Q407·**Q426**) → ④ **별지 제18호** 공단 신청 (Q237) |
| 체크리스트 5항목 | 운전자 자격 · 동승 직원 · 차량 안전 · 사고 조치 · 차량 운행표(별지 제22호) |
| Modal | **「계약서 템플릿 보기」** — 인쇄·별도 문서화 후 현장 보관 |
| 일지 | **`TransportRunDetailPage`** **`TransportServiceLogPanel`** — **`GET/PUT /service-log`** · 인쇄·텍스트 저장 (Q407) · **`/transport/compliance`** **`TransportServiceLogRunsPanel`** 확정 배차→일지 링크 (Q426) |
...
**A.** **✅ full stack (`0cfa970`·`aaaeb10`·`aa42b9c`·`5994d15` / FE `088e906`·`dff2f32`·`3cc5a08`, G15 v1.3-C, US-T05, Q407·Q411)** — **확정 배차** 루트 상세에서 법정 형식 일지를 **서버 저장·감사 trail read·보관 안내·인쇄·텍스트 다운로드**할 수 있습니다.
...
| 감사 | 저장 성공 시 **`TRANSPORT_SERVICE_LOG_UPSERT`** — **`GET …/service-log/audit-trail`** 운행별 이력 (`5994d15`/`3cc5a08`) · **`/settings` → 감사 로그** (`aa42b9c`, Q411) |
| 보관 UX | **「일지 보관·감사 추적」** — 확정 시각·DB 저장 건수·마지막 저장·저장 상태·**일지 저장 이력** · **법정 보관 안내** (`088e906`/`3cc5a08`) |
...
**A.** **Fixed (`7dfcc9e`·`a0dcfc0`·`9641ab1`, G15 2-1-1·2-9, US-T05)** — **외출 예정·출발·복귀·취소** lifecycle과 **월간 리포트**를 지원합니다.

| 항목 | 내용 |
|------|------|
| UI | **`/transport/outings`** **`ClientOutingsPage`** · **`/reports/client-outings`** **`ClientOutingReportPage`** · 이용자 상세 **「외출」** 탭 **`ClientOutingPanel`** |
| 월간 리포트 a11y | **`9641ab1`** — 조회 form **`aria-label="외출 리포트 조회"`** · StatCard **`role="group" aria-label="월간 외출 리포트 요약"`** · **조회** 버튼 **`aria-busy`** |
| live E2E | **`3a0110f`** — **`clientOutingReportLiveApi.e2e.test.js`** — **`GET /reports/client-outings?branchId=&yearMonth=`** routing · **`pilotPageFlows.test`** G15 flow · **`ClientOutingReportPage.test`** empty-state |
...
### Q423. V152 업데이트 후 **운행 생성 시 «확인할 수 없는 이용자» 오류**가 나요.

**A.** **Fixed (BE `dd2fa2c`, V152, Q423)** — **`V152__transport_run_stops_guard_client_is_active_fix.sql`** — **DB 트리거** `trg_transport_run_stops_guard_client`가 **`clients.is_active`** 컬럼을 정합합니다 (V143 회귀 fix).

| 상황 | 원인 | 조치 |
|------|------|------|
| **운행 정차 추가 시 오류** | 이용자가 **퇴소됨** (`is_active=false`) 또는 **삭제됨** | 활성 이용자만 정차 대상으로 선택 (삭제 이용자는 선택 불가) |
| **기존 운행 수정 시 오류** | V143 이후 **퇴소 이용자**가 이미 정차로 존재 | BE `dd2fa2c` 배포 후 **운행 DRAFT 상태** 재확인·수정 |
| **V152 마이그레이션** | — | 마이그레이션 스크립트 자동 적용 (수동 조치 불요) |

| 테스트 | 내용 |
|------|------|
| `trg_transport_run_stops_guard_client` | 활성/퇴소 이용자 정차 생성 검증 |
| `TransportServiceTest.createRunShouldRejectInactiveClient` | 퇴소 이용자 정차 거부 |

> **운영**: V152 배포 후 기존 DRAFT 운행이 있다면, 저장 전 정차 이용자 상태를 재확인하세요.

---

### Q424. 배차 설정 **가중치 저장 시 «유효하지 않은 값»**이라고 하고, **「자동 제안」이 BRANCH 정차를 안 넣어줘요.**

**A.** **Fixed (BE `dd2fa2c`, US-T02, Q424) · FE `96db8bf` (Q424)**

#### **① 배차 설정 가중치 검증** (BE `dd2fa2c`)

**`BranchTransportSettingsService`** — 저장 시 가중치 **3개 조건** 검증:

| 조건 | 오류 | 메시지 |
|------|------|--------|
| **NULL 체크** | 일부 가중치 NULL | 「거리·시간·비용 가중치를 모두 입력해야 합니다.」 |
| **범위 검사 (0~1)** | 음수 또는 1 초과 | 「각 가중치는 0~1 사이의 값이어야 합니다.」 |
| **합 검사 (> 0)** | 모두 0 | 「최소 1개 이상의 가중치를 설정해야 합니다.」 |

**예시 (✅ 통과)**
```json
{ "distanceWeight": 0.3, "timeWeight": 0.4, "costWeight": 0.3 }
```

**예시 (❌ 실패)**
```json
{ "distanceWeight": 0.5, "timeWeight": null, "costWeight": 0.5 }  // NULL 미포함
{ "distanceWeight": 1.5, "timeWeight": 0.3, "costWeight": 0.2 }  // 범위 초과
{ "distanceWeight": 0, "timeWeight": 0, "costWeight": 0 }        // 합 = 0
```

**UI (FE `96db8bf`, Q424)** — **`BranchTransportSettingsPanel`** · **`transportSettingsForm.js`**

| 입력 필드 | 특징 |
|---------|------|
| **거리 가중치** | `type=number` · `step=0.1` · `min=0` `max=1` · **클라이언트 검증** |
| **시간 가중치** | 동일 |
| **비용 가중치** | 동일 |
| **저장 버튼** | **필드 비어있음 시 비활성** · **서버 검증 메시지 표시** |

#### **② 자동 제안 BRANCH 정차 포함** (BE `dd2fa2c`)

**`TransportSuggestService`** — DRAFT 운행 생성 시 **자동 제안 정차**에 **출발·복귀 BRANCH** 포함:

| 정차 종류 | 자동 포함 | 설명 |
|---------|---------|------|
| **BRANCH (지점)** | ✅ **신규** | 출발/복귀 기본 주소로 자동 추가 |
| **CLIENT (이용자)** | ✅ | 배차 설정 픽업 반경 내 활성 이용자 |
| **WAYPOINT** | — | 수동 추가 전용 |

**제안 순서**
1. 출발 BRANCH (지점)
2. 활성 이용자 (거리·시간·비용 최적화)
3. 복귀 BRANCH (지점)

**FE UI (FE `96db8bf`, Q424)** — **`TransportSuggestPanel`** (embedded in **`BranchTransportSettingsPanel`**)

| 컴포넌트 | 동작 |
|---------|------|
| **Compact dispatch grid** | **`ds-transport-dispatch-grid`** — 자동·수동 배차 **나란히 배치** |
| **자동 제안 목록** | BRANCH + CLIENT 정차 **순서대로 표시** |
| **수동 배차** | **「경유지 추가」** — `TransportAddWaypointModal` |
| **저장** | **`POST /transport/runs`** · **자동 제안 + 수동 정차** 통합 persist |

| 테스트 | 내용 |
|--------|------|
| `BranchTransportSettingsService.validateSettings*` | 가중치 3조건 검증 |
| `TransportSuggestService.suggestRunShouldIncludeBranchStops*` | BRANCH 정차 자동 포함 |
| `BranchTransportSettingsPanel.test` | 입력 검증·버튼 비활성 |
| `transportSettingsForm.test` | 빈 값 클라이언트 검증 |
| `TransportSuggestPanel.test` | 제안 정차 렌더링 |

> **운영**: 배차 설정 변경 후 **자동 제안이 BRANCH를 놓친다면**, 설정을 **저장 완료**했는지 확인 후 **새 운행 생성**을 다시 시도하세요. 가중치 저장 실패 시 **오류 메시지**를 센터 IT에 공유하세요.

> **관련**: Q347·Q400·Q402·Q418·Q421 · USER_MANUAL §5-8 · ADMIN_GUIDE §6-2-3
...
| 화면 | 동작 |
|------|------|
| **`CaseManagementPage`** | 지표43 StatCard · **「참석자별 의견 기록」StatCard** (Q516, `b272a7b`) · 연도·분기 필터 · **7필드+참석자별 의견 등록 폼** |
| **필수 입력 (FE)** | 이용자 · 회의일 · 선정사유 · 회의내용 · 회의결과 · **`caseManagementPlan`**(사례관리 계획) · **참석자(≥2인)** · **참석자별 의견**(FAQ21797, 참석자 2인 이상 시 각 `opinion` 필수) |
...
| **대시보드 위젯** | **`/dashboard`**·**`/dashboard/hq`** — **「사례관리 30일 미반영」** (`0821ce8`, Q263) · **「참석자별 의견 미기록」** (`b9e0947`·`e55ae96`, Q518) · **「사례관리 평가 미실시」**·**「사례관리 평가실시」** (`7f2289b`, Q266) — `hq_admin`·`branch_admin`·`social_worker` |
...
| DB | **V73** `case_management_meetings` · **V74** integrity · **V75** `case_management_plan` · **V156** `attendee_opinions` JSONB (FAQ21797, Q516) |
...
> 관련: Q516 · USER_MANUAL §4-2·§5-9 · ADMIN_GUIDE §10-11-2 · REQUIREMENTS G32 · DATA_RETENTION_POLICY §2
...
**A.** **Partial Fixed (BE `75440bc`·…·FE `10585b9`·`f31c346`·`f62402f`·`1b6d2b1`·`033b319` / US-R03·BNK-129·BNK-139·BNK-146, G-Staff-LC)** — **예.** 이지케어 FAQ21825 **4단계** 체크리스트·진행 상태를 **직원 목록·상세 화면**에서 관리할 수 있습니다. **입사/퇴사 서류 PDF·이미지 업로드(FAQ21806)** 는 **`/staff/{userId}` 「HR 파일함」** 탭에서 사용할 수 있습니다(Q298). **`/staff` 목록 상단**에서 **입사 처리 compliance 현황**(FAQ21806 7종 서류·신규교육 7일)을 **지점별 집계**로 확인할 수 있습니다(Q300). **FAQ21823 연간 임금협의·근로(재)계약**은 **`StaffEmploymentContractRenewalSummaryPanel`**(목록, Q540) · **대시보드「근로재계약 미충족」·due-date Alert**(Q540·Q547) · **`StaffEmploymentContractRenewalPanel`**(상세 checklist+서식 Modal+**재계약 기록**, Q540·Q546·Q547). **급여명세·G-Payroll 연동·전자서명 workflow**는 P2 잔여입니다.

| 항목 | 화면·API | 비고 |
|------|----------|------|
| 진입 | **`/staff`** 목록 → 이름·**「상세」** → **`/staff/{userId}`** | `hq_admin`·`branch_admin` (Q162) |
| lifecycle 필터 | **`StaffPage`** 상단 **lifecycle 상태 Select** | 입사 진행·재직·**휴직**·퇴사 진행·퇴사 완료·전체 (`2581347`) |
| lifecycle Badge | **`StaffPage`** 목록 · **`StaffDetailPage`** 기본정보 | **`StatusBadge`** + **`STAFF_LIFECYCLE_STATUS`** — **5-state incl. 휴직** (`2581347`) |
| lifecycle 집계 | **`StaffLifecycleSummaryPanel`** — **`/staff` 목록 상단** | **`GET /api/v1/staff/lifecycle-summary`** — **`onLeaveCount`** StatCard (`1d7cee2`/`2581347`, Q584) |
...
| FAQ21823 재계약 | **`StaffEmploymentContractRenewalSummaryPanel`** · **`DashboardPage`「근로재계약 미충족」** · **`EmploymentContractRenewalAlertsPanel`** · **`StaffEmploymentContractRenewalPanel`** | **목록·대시보드·상세 4계층** · **재계약 기록 Modal** (Q547) · checklist·서식 Modal (Q546) · 서명일+1년·3년 보관 (Q540) |
...
| 상태값 | `ONBOARDING` · `ACTIVE` · **`ON_LEAVE`** · `OFFBOARDING` · `TERMINATED` | **V86**·**V166** · **UI 5-state** — **`StaffLifecyclePanel`·필터·Badge** (`2581347`, Q584) |
...
**A.** **Fixed (BE `f1268c6`·`bad88f5`·FE `604787f`/`8e6310a` / US-R02, FAQ21799)** — 케어포 **8-10**·이지케어 FAQ21799 **지표9** 「연간 직원 건강검진」에 대응합니다. **일반 직원 1년** · **사무직 2년** 주기로 검진 여부를 집계합니다. **신규 직원 결과통보서 1년 이내 확인**은 FAQ **Q435** 참고.
...
| 신규 서류 (`8e6310a`/`e9d39a9`) | StatCard **「신규 서류 미확인」** · 목록 **「신규 서류」** — **1년 이내 확인**·**서류 필요** Badge · **해당 없음(NA)** 은 **「—」** (Q435·Q442) |
| 기록 | 목록 **「검진 기록」** Modal — **`POST /staff/health-checkups/users/{userId}`** — 첫 검진 **실시일**은 **근무 시작일 기준 1년 이내** (클라이언트 검증, Q435) |
...
| P2 잔여 | **대시보드 위젯** · 검진 기록 시 checklist **자동 반영** — 결과통보서 **HR 파일함 업로드**는 Q444에서 **부분 Fixed**
...
**A.** **Fixed (BE `b459f4c`·`c16f4fe`·`0db1e68`·`78cfb8a`, G21, US-V02, QA-B73)** — **예(요양보호사 한정).** 일정에 **`assignedUserId`(담당 직원)** 가 배정되어 있으면 **`caregiver`** 는 **본인이 배정된 일정만** 체크인·체크아웃할 수 있습니다.

| 상황 | 동작 |
|------|------|
| **`caregiver`** + 배정 직원 ≠ 로그인 사용자 | **`422 BUSINESS_RULE`** 「배정된 요양보호사만 체크인·체크아웃할 수 있습니다.」 |
| **배정 직원이 퇴사·비활성** (`0db1e68`, Q453) | **`422`** 「퇴사 또는 비활성화된 직원은 배정할 수 없습니다.」 — **감독 역할도 동일** |
| **배정 직원이 해당 지점 미소속** (`0db1e68`, Q453) | **`422`** 「해당 지점 직원만 방문일정에 배정할 수 있습니다.」 |
| **`hq_admin`·`branch_admin`·`social_worker`** | **감독 역할** — 배정 직원 **활성·지점 소속**이면 배정과 무관하게 체크인·체크아웃 가능 · JWT **`roleCode` 대소문자·공백 차이 허용** (`78cfb8a`, Q455) |
| **`assignedUserId` null** | 기존과 동일 — 배정 없으면 가드 **생략** |
| 검증 시점 | **`POST /visits/{id}/check-in`** · **`POST /visits/{id}/check-out`** — confirm 시 **재검증** |

> 관련: Q304(배정 검증) · **Q453(배정 직원 활성·지점)** · **Q455(감독 roleCode 정규화)** · Q238(페어 동기화) · USER_MANUAL §5-11 · ADMIN_GUIDE §10-12
...
| 항목 | 내용 |
|------|------|
| **공단 평가지표 cross-walk** | **G30-LEGEND** — **`MonitoringOfficialIndicatorLegendPanel`** (`fdc135b`, Q629) — ogada 15문항 ↔ 공단 1~15 매핑 · **#6↔15** · **#15↔12~14** |
...
> 관련: Q320 · Q629 · Q306(G34b) · DATA_RETENTION §3(5년 보관) · USER_MANUAL §4-3 · ADMIN_GUIDE §6-2-11

### [TWR] Q315. **8-12 직원현황** PDF·엑셀 출력은 되나요?

**A.** **Partial+ Fixed (BE `bc927f7`·`c4dbe43` / FE `488f547`·`5bba7a2`·`ff173af`·`a4ea2d5` / US-R02·BNK-175)** — **케어포 PDF p.106 7종**에 대응하는 **브라우저 인쇄 6종 + 서버 CSV(엑셀 호환) 1종**이 **`/staff/reports/status`** 에 있습니다. **엑셀**은 FE 클라이언트 생성이 아닌 **BE CSV API**를 사용합니다. **공식 PDF 서식 파일**은 아직 없습니다. **`a4ea2d5`부터** 인쇄 종료 시 **`afterprint`+`setTimeout` fallback** 으로 **body print class·blob URL** 이 정리되고 **다중 페이지 표 레이아웃**이 개선됩니다.
...
| 항목 | 내용 |
|------|------|
| **BE 동작** | **`NotificationService.isQuietHoursAt`** — **22:00 ≤ 시각 < 08:00** KST · **자동(non-emergency) dispatch skip** · **수동 발송 `422 BUSINESS_RULE`** · **`EMERGENCY` bypass** · 경계 **22:00·07:59** 단위 테스트 (`328874d`·`9a4ab8e`) · **보호자 서류 수동 발송** **`dispatchManualClientEvent`** (Q539, `71b2d32`) |
| **readiness UI** | **`NotificationChannelReadinessPanel`** — **`quietHoursActive`** Badge (Q318) |
| **청구 UI** | **`BillingDetailPage`** — **「보호자 발송」·「납부확인서 발송」** 버튼 **비활성** + 상단 Alert · **`OverduePage`** — 행별 **「안내 발송」** **비활성** · **`notificationQuietHoursBlockedMessage()`** |
| **보호자 서류 UI (신규)** | **`GuardianDocumentNotifyPanel`** — **22:00~08:00** API 호출 시 **danger Alert** — **「현재 22:00~08:00 (Asia/Seoul) 조용한 시간대입니다…」** (Q539, BE `71b2d32`) — 청구 UI와 달리 **버튼 비활성 없음** · 서버 거부 메시지 표시 |
...
| 항목 | 내용 |
|------|------|
| 사전 점검 | **`GET /api/v1/visits/confirm-readiness`** — `draftCount`·`confirmedCount`·**PLAN/BILLING split** (Q474) · **`readyPlan`·`readyBilling`**·per-kind paired/unassigned counts (Q477) · `ready`·`blockers[]` · Modal **StatCard 그리드** (Q477, `f9ed97d`) |
| **기간 필수 (BNK-213)** | **`POST /api/v1/visits/batch-confirm`** — **`fromDate`·`toDate` 필수** — 누락 시 **「일괄확정 기간(시작일/종료일)을 입력해주세요.」** (`422 BUSINESS_RULE`, `230659a`) · 종료일이 시작일보다 이르면 **「종료일은 시작일 이후여야 합니다.」** · 응답 **`confirmedPlanCount`·`confirmedBillingCount`** (Q477) |
| 차단 조건 | **DRAFT 0건** · **페어 PLAN/BILLING 확정 상태 불일치** · **담당자 미배정 DRAFT** — `ready=false` · **track별** `readyPlan`/`readyBilling` (Q477, `5f710e3`) · batch-confirm **`422`** 「담당자가 미배정된 방문일정이 있어 일괄확정할 수 없습니다.」 |
...
**A.** **BE·FE Fixed (BNK-212~214, BE `db94a65`·FE `2ffe59f`·**deepen FE `96db8bf`·BE `dd2fa2c`**, 결정 75)** — **`POST /api/v1/transport/runs/suggest`** 로 당일 **픽업(PICKUP) 배차 명단**을 **OR-Tools VRP** 기반으로 **DRAFT 운행 루트**에 자동 배치합니다. **`/transport`** 화면 **`TransportSuggestPanel`** 에서 **`hq_admin`** 이 실행·설정합니다 — **최적화 설정은 제안 카드 내부에 embedded** (`96db8bf`, Q424).
...
| DRAFT 정차 | **출발·복귀 `stopKind=BRANCH`** 지점 정차 **자동 포함** (`dd2fa2c`, Q424) — 수동 배차와 동일 패턴 |
| 설정 | **`GET/PUT /api/v1/transport/settings`** — 픽업 허용 ±분(기본 15) · OR-Tools 가중치 **w1 안정성 0.5 · w2 공정성 0.3 · w3 거리 0.2** · **0~1·합>0** 서버 검증 (`dd2fa2c`, Q424) |
| UI | **`TransportPage`** — **자동·수동 배차 나란히** (`ds-transport-dispatch-grid`) · **「자동 배차 제안」** → DRAFT 생성 → **DRAFT 검토** · **기존 DRAFT는 새 제안 시 대체** |
| DB | **V120** `branch_transport_settings`·`transport_suggest_events` · **V122** integrity · **`clients.transport_notes`** |

> 관련: Q159 · Q424 · Q241 · USER_MANUAL §5-8 · ADMIN_GUIDE §10-10 · DEPLOYMENT §3-6 · DATA_RETENTION §2 transport_suggest_events 90일
...
**A.** **✅ full Fixed (BE `81a2223` / FE `9ad8346`/`38642e2`, L02 v3.1 Must / BNK-248)** — SideNav **기록 → 통합식사도움기록 (L02_M13)** 또는 **`/care/meal-assistance-records`** 에서 목록·등록·수정합니다.
...
| **malformed 응답 (Q442, FE `38642e2`)** | API가 배열·`items[]`가 아닌 형식이면 **빈 목록 대신** 「통합식사도움기록 조회 응답 형식이 올바르지 않습니다.」 **Alert** |
...
**A.** **Fixed (BE `de25b3e` / QA-B125 `704478f`, J03 / BNK-248)** — **`GET /api/v1/notifications/channel-status`** 응답에 **`readinessBlockers[]`** 가 추가되어, Solapi·SMTP live 연동 전 **누락 원인 코드**를 확인할 수 있습니다. 비밀값(API 키 등)은 노출하지 않습니다.
...
| `MISSING_SMTP_CONFIG` | SMTP 호스트·필수 mail 설정 누락 | `missingEmailConfigKeys[]` — **`SMTP_HOST`** 등 (`704478f`: **`SMTP_HOST`만 누락**해도 이 코드·**알림톡 readiness는 유지**) |
...
**A.** **✅ full Fixed (BE `ea6092a` / FE `14a2bb9`/`4a47675`, L02 v3.1 Must / BNK-241)** — SideNav **기록 → 신체제재 기록 (L02_M07)** 또는 **`/care/body-restraint`** 에서 목록·등록·수정합니다. 이용자·기록 API **snake_case·`items[]` 래핑** 수용(Q454).
...
> 관련: **Q475**(인쇄 산출물 분리) · USER_MANUAL §4-6-0 · ADMIN_GUIDE §6-2-17 · REQUIREMENTS 7-1
...
| UI | **`CareNursingServiceReportPage`** 공통 패널 · **`CareNursingServiceReportNav`** 3탭 · **`CareNursingParityPanel`** L02↔L03 연계 링크 (`140bf92`, Q412) |
| 입력 vs 리포트 | **제공기록 입력** = `/nursing/service` · **집계 리포트** = `/care/reports/*` |
| 테스트 | **`CareReportServiceTest`** · **`CareReportLiveApiRoutingE2eTest`** · FE **`CareNursingServiceReportPage.test`** · **`careNursingReportsLiveApi.e2e.test.js`** (`5533ef5`) |
...
**A.** **Fixed (FE `d499130`/`7d2cb4a`, BNK-273, Q391)** — G30 FAQ21841 StatCard 패턴에 맞춰 **G24b·G26·G21·G30** 네 화면 모두 **`MonitoringEvidenceContextPanel`** 로 **FAQ 근거·증빙 기간·상호 링크**가 일관되게 표시됩니다.

| 화면 | 변경 |
|------|------|
| **`/compliance/monitoring`** (G30) | **`MonitoringEvidenceContextPanel` `variant="g30"`** — FAQ21838~21842 문항 6~15 증빙 기간·G24b/G21/G26 cross-link (`7d2cb4a`) |
| **`/clients/needs-assessments`** (G24b) | StatCard — **「연간 완료 (FAQ 21800)」** · **「연간 미완료 (FAQ 21800)」** · **「등급변경 재사정 필요 (FAQ 21810)」** · **`variant="g24b"`** evidence panel |
| **`/billing/reports/statistics`** (G26) | ② 본인부담금 월별 — **G30 FAQ21836/21842** cross-ref lede · ③ 이동서비스비 — **G24b FAQ 21800/21810** cross-ref lede · **`variant="g26"`** evidence panel |
| **`/visits`** (G21) | **`variant="g21"`** evidence panel — 계획·청구 split-view ↔ G30 문항10(RFID) 수동 점검 근거 |
| 참조 패턴 | G30 **`/compliance/monitoring`** — FAQ21841 StatCard suffix (기존) |

| 테스트 | **`NeedsAssessmentStatusPage.test`** · **`BillingStatisticsReportPage.test`** · **`MonitoringSelfDiagnosisPage.test`** · **`MonitoringEvidenceContextPanel.test`** |

> 관련: Q355·Q357·Q379·Q365 · USER_MANUAL §4-2·§4-3·§5-10 · ADMIN_GUIDE §6-2-6b·§6-2-11 · PLAN_NOTES BNK-273 P1 closure
...
**A.** **Fixed (BE `f5205e3`·`ec142db`·`7ac0a46`·`22396e0`·`b1a6aff`·`440ed84`·`d8d51a7`·`8a1f342`·`2926287`·`73cffc5` / FE `b3bd0cc`·`4e99ae1`·`ddd4489`·`6f2a4eb`·`825c6b0`·`af4d7f8`, BNK-283·BNK-285, QA-B95 deepen, Q393·Q394)** — dev/staging에서 bootstrap이 활성(`ogada.live-e2e.bootstrap-enabled=true`)이면, **`LIVE_E2E_GUARDIAN_EMAIL`/`PASSWORD` 미설정** 시에도 **`liveGlobalSetup`** 이 **`POST /api/v1/system/live-e2e/bootstrap-guardian`** 을 호출해 보호자 JWT·연결 **`clientId`** 를 자동 발급합니다. **`73cffc5`** — staff bootstrap 응답에 **기존 guardian-client seed가 ready** 이면 **`guardianAccessToken`/`guardianRefreshToken`/`guardianEmail`/`guardianUserId`** 를 **best-effort** 로 포함 — guardian token 발급 실패 시에도 staff bootstrap은 **성공** (`LiveE2eBootstrapServiceTest`). **`af4d7f8`** — FE가 embedded token을 persist하면 **`bootstrap-guardian` 2차 호출 생략** (FAQ **Q409**). **`8a1f342`** — probe가 guardian bootstrap **전** seeded client를 노출합니다. **`2926287`** — **`GET /api/v1/health`** 에 **`liveE2eClientReady`·`liveE2eSeedClientId`** 가 추가되어 health만으로 시드 이용자를 확인할 수 있습니다. **`825c6b0`** — staff bootstrap **`clientId`** 를 guardian auth **전**에 persist합니다. **`7ac0a46`** — staff/guardian bootstrap email fallback 시 **동일 tenant·동일 이메일·다른 역할** 계정 재사용을 **차단**합니다. **`22396e0`** — **V147** guardian link trigger가 bootstrap 트랜잭션에서 **`clients.updated_at`** 제약을 위반하지 않도록 **`GREATEST(created_at, NOW())`** 로 갱신합니다. **`b1a6aff`** — staff bootstrap 시 **legacy onboarding support** row가 있으면 **`organizationId` backfill** 후 재사용합니다. **`440ed84`/`d8d51a7`** — staff bootstrap이 **`ensureClient`** 로 live-e2e 이용자를 **항상 시드**하고 **`LiveE2eBootstrapResponse.clientId`** 를 반환합니다. probe **`clientReady`·`seedClientId`** 로 harness preflight가 이용자 시드 여부를 확인합니다. **`4e99ae1`** — **`liveConfig.resolveGuardianClientId`** 가 보호자 포털 **`/auth/me` targets** 와 env **`LIVE_E2E_CLIENT_ID`** 를 대조해 **연결 이용자**를 우선 선택합니다. **`ddd4489`** — bootstrap endpoint 실패 시 env creds로 **`login-fallback-ok`** 재로그인을 시도합니다.

| 항목 | 내용 |
|------|------|
| BE endpoint | **`POST /api/v1/system/live-e2e/bootstrap`** — **`LiveE2eBootstrapResponse`**: `accessToken`·`refreshToken`·`email`·`organizationId`·`branchId`·`userId`·**`clientId`** (`440ed84`) · optional **`guardianAccessToken`/`guardianRefreshToken`/`guardianEmail`/`guardianUserId`** (`73cffc5`) |
...
**희망 탑승 시각** 저장 시 레거시 **`defaultPickupTime`** 도 함께 갱신됩니다. `/transport` 명단 표에는 두 시각 열이 모두 표시됩니다. 확정 루트 배정 후 **계획 픽업/하차**·**지연** 비교는 FAQ **Q433** 참고.

> 관련: Q166 · Q433 · USER_MANUAL §4-3 · §5-8
...
**A.** **✅ full stack (BE `0cfa970`·`aaaeb10`·`aa42b9c`·`5994d15`·`52e3340`·`ac1d43f`·`bc3a35c` / FE `088e906`/`dff2f32`/`3cc5a08`/`b4644e8`/`f51e365`, G15 v1.3-C, US-T05, Q407·Q411·Q439·Q443·Q440·Q445)** — **확정(CONFIRMED)** 배차 루트에서만 **일지 준수 기록**을 서버에 저장합니다.

| 항목 | 내용 |
|------|------|
| 조회 | **`GET /api/v1/transport/runs/{runId}/service-log`** — **`TransportServiceLogResponse`** — header·**`driverSignatoryName`·`driverSignedOn`·`driverSignatureComplete`** · rows·summary |
| 저장 | **`PUT /api/v1/transport/runs/{runId}/service-log`** — **`UpsertTransportServiceLogRequest`** — `companionName` · **`driverSignatoryName`·`driverSignedOn`** (Q445) + `stops[]` (`clientId`·`actualPickupTime`·`companionAccompanied`·`dropoffTime`) |
| **법정 필드 필수 (Q439 FE `b4644e8` · Q443 BE `ac1d43f`)** | 정차별 **실제 픽업 시각**·**동승 여부(예/아니오)**·**하차 시각** — FE: **저장·인쇄·텍스트 다운로드 차단** · BE: 미입력·**하차 < 픽업** 시 **`422`** |
| **운전자 서명 (Q445·Q450, FE `f51e365`/`bfe0283` · BE `bc3a35c`)** | **「운전자 서명」** `fieldset` — **「서명 성명」·「서명일」** — FE: 저장·인쇄·다운로드 **필수** · BE: 쌍 입력·**서명일 ≥ 운행일**·**≤ 오늘** · **`driverSignatureComplete`** |
| **중복 이용자 거부 (Q440, BE `52e3340`)** | 동일 `clientId`가 `stops[]`에 **2회 이상**이면 **`422`** — 「일지 대상 이용자가 중복되었습니다.」 |
| 감사 | **`TRANSPORT_SERVICE_LOG_UPSERT`** — **`GET …/service-log/audit-trail`** (`5994d15`) · **`/settings` 감사 로그** (`aa42b9c`) |
| 제한 | **DRAFT** 루트 저장 시 **`422`** — `확정된 운행만 일지를 기록할 수 있습니다.` |
| DB | **V148** — 정차별 compliance columns · **V154** — `service_log_driver_signatory_name`·`service_log_driver_signed_on` |
| UI | **`TransportServiceLogPanel`** — **운전자 서명**·**일지 보관·감사 추적**·**일지 저장 이력** · **`/transport/compliance`** — **`TransportServiceLogLegalGuide`** (Q446) |
| 권한 | **`hq_admin`·`branch_admin`·`social_worker`·`caregiver`** |
| 검증 | **`TransportServiceTest`** · **`TransportTimeComplianceTest`** · **`TransportServiceLogPanel.test`** |

> 관련: Q236 · Q411 · Q439 · Q443 · Q440 · Q445 · Q446 · USER_MANUAL §5-8-0-1 · DEPLOYMENT §11-3
...
### [TWR] Q409. live E2E bootstrap 시 **이용자 중복**·**다른 tenant 시드 충돌**·**guardian bootstrap 2회 호출**이 발생해요.

**A.** **Fixed (BE `c13800c`/`2d6c063`/`87f901d`/`73cffc5` / FE `af4d7f8`/`fc916db`, QA-B95)** — 재사용 dev/staging DB에서 bootstrap이 실패하거나 guardian creds가 중복 발급·**다른 tenant 데이터가 덮어써지는** 경우를 방지합니다.
...
| **다른 tenant 이용자·계정 덮어쓰기** | 설정된 시드 ID·email이 **다른 Organization** row와 충돌 | BE **`fc280cf`/`2d6c063`** — foreign tenant row **변경 금지** · 안전한 대체 ID 생성 · **`LiveE2eBootstrapServiceTest`** |
| **`LIVE_E2E_SEED_CLIENT_CONFLICT`** | configured seed UUID가 **다른 tenant** 소유·LTC cert fallback 없음 | BE **`87f901d`** — **generated scoped client id** 로 신규 생성 · probe·guardian은 **configured id 또는 LTC cert** 로 해석 · **`LiveE2eBootstrapServiceTest`** |
| **`bootstrap-guardian` 불필요 호출** | staff bootstrap 응답에 guardian token이 이미 포함 | BE **`73cffc5`** — **`POST /api/v1/system/live-e2e/bootstrap`** 응답에 **`guardianAccessToken`/`guardianRefreshToken`/`guardianEmail`/`guardianUserId`** optional 포함(기존 guardian-client seed ready 시) · FE **`applyBootstrapTokens`** — embedded token persist · **`bootstrap-guardian` 미호출** · **`LiveE2eBootstrapServiceTest`** · **`liveE2eHarness.test`** |
| **bootstrap 200인데 suite SKIP** (토큰 비어 있음) | 백엔드가 **nested `staff`/`guardian`** 또는 **snake_case** 필드명으로 JWT·`clientId` 반환 | FE **`fc916db`** — **`pickBootstrapAccessToken`** 등이 flat·nested·snake_case 모두 수용 · **`liveE2eHarness.test`** (FAQ **Q425**) |
| **stale refresh token·placeholder guardian email** | 이전 shell run의 **`LIVE_E2E_REFRESH_TOKEN`**·**`guardian@ogada.test`** 잔여 | FE **`b2c09e1`** — bootstrap refresh **항상 교체** · placeholder email **authoritative 값으로 hydration** · **`liveE2eHarness.test`** (FAQ **Q427**) |
| **probe credential check throw** | staff/guardian credential 조회 중 예외 | BE **`844227a`** — **`safeBoolean`** per-field · probe **HTTP 200** 유지·해당 필드만 **false** (FAQ **Q427**) |
| **bootstrap branch inactive** | live E2E seed 지점이 **비활성** | BE **`d68c4bf`** — bootstrap 시 branch **`setActive(true)`** · **`LiveE2eBootstrapServiceTest`**

**운영 절차**

1. **`GET /api/v1/system/live-e2e/probe`** — **`clientReady`·`seedClientId`** 확인 (Q360).
2. bootstrap 실패 시 **`GET /api/v1/health`** — **`liveE2eOperationReady`·`liveE2eOperationBlocker`** 단일 확인 (Q419, `3908044`) · 또는 **`liveE2eClientReady`·`liveE2eSeedClientId`** (`2926287`).
3. harness 재실행 전 stale shell env 제거 — FAQ **Q408** (`b69c8ae`).
4. authenticated suite는 **`./scripts/run-live-e2e.sh`** 1회 RUN 확인 (QA-B95).

> 관련: Q360 · Q393 · Q408 · **Q419** · **Q425** · **Q427** · DEPLOYMENT §3-6 · ADMIN_GUIDE §1-4
...
**A.** **✅ full stack (BE `5994d15`/`aa42b9c` / FE `3cc5a08`/`088e906`/`dff2f32`, G15 v1.3-C, Q411)** — 일지 저장은 **감사 로그**에 남고, **운행 상세 화면**과 **설정 감사 로그** 양쪽에서 확인할 수 있습니다.

| 항목 | 내용 |
|------|------|
| **운행별 이력 API** | **`GET /api/v1/transport/runs/{runId}/service-log/audit-trail`** — **`TRANSPORT_SERVICE_LOG_UPSERT`** 최근 **50건** · `runDate`·`stopUpdateCount`·`recorded`/`onTime`/`total` (`5994d15`) |
| **운행 상세 패널** | **`TransportServiceLogPanel`** **「일지 보관·감사 추적」** — 저장 상태·**「일지 저장 이력」** 표(일시·작업·내용) · 저장 후 자동 갱신 (`3cc5a08`) |
| **테넌트 감사 로그** | **`sysadmin`·`hq_admin`** — **`/settings` → 감사 로그** 탭 · **`GET /api/v1/settings/audit-logs`** (`aa42b9c`) |
...
| 권한 | **`hq_admin`·`branch_admin`·`social_worker`** — **`caregiver` 403** (audit trail read) |

> 관련: Q407 · Q236 · USER_MANUAL §5-8-0-1 · ADMIN_GUIDE §1-4

### [TWR] Q412. **L02 care-scoped 간호 리포트**와 **L03 간호 리포트**는 어떻게 연결되나요?

**A.** **✅ full stack (FE `140bf92`/`5533ef5`, BNK-308, Q412)** — care-scoped **`/care/reports/nursing-*`** 와 L03 **`/nursing/service/reports/*`** 화면 하단 **`CareNursingParityPanel`** 에서 **상호 링크**로 이동합니다.

| 항목 | 내용 |
|------|------|
| 패널 | **`CareNursingParityPanel`** — **`aria-label="L02·L03 간호 리포트 연계"`** · care view에서는 **L03 리포트**·**「간호급여 제공기록 (L03_M01)」** 링크 |
| L03 view | **`NursingServiceReportsPage`** — 대응 **L02 care-scoped 리포트** 링크 |
| 매핑 | **L02_M14** ↔ **L03_M07** · **L03_M09** 병의원 · **L03_M10** 투약 (`careLeafParity.js` **`CARE_NURSING_PARITY_PAIRS`**) |
| L02_M09 | 케어포 **건강상태 평가 리포트(삭제예정)** — ogada **`/health` 건강 기록** 안내 (`CARE_DEPRECATED_LEAVES`) |
| live E2E | **`careNursingReportsLiveApi.e2e.test.js`** — L02_M14/M09/M10 **`GET /care/reports/*`** routing (`5533ef5`) |
| 테스트 | **`CareNursingParityPanel.test`** · **`careLeafParity.test`** · **`CareNursingServiceReportPage.test`** |

> 관련: Q386 · Q378 · USER_MANUAL §5-36 · ADMIN_GUIDE §6-2-26

### [TWR] Q413. **`GET /actuator/health`** 가 500을 반환하거나 liveness/readiness가 같게 보여요.

**A.** **Fixed (BE `5d7be9f`/`3f32ae5`/`911a1b9`/`c19206a`/`2157df5`, QA-B95, Q413)** — Spring Boot **actuator starter 미포함** 환경에서 `SecurityConfig`는 `/actuator/health` 를 permit 했으나 **핸들러 부재**로 500이 발생했습니다. **`ActuatorHealthController`** 가 **`LiveReadinessProbe`** 기반 최소 응답을 제공합니다. **`3f32ae5`** — probe runtime exception 시에도 **500 대신 503 + `DOWN`** 으로 degrade 합니다. **`911a1b9`** — **liveness**와 **readiness** 의미를 **분리**합니다 — liveness는 **프로세스 도달성(항상 `UP`)** · readiness는 **DB probe** 입니다. **`c19206a`** — k8s·로드밸런서 관례 **`/actuator/livez`**·**`/actuator/readyz`** alias를 추가합니다. **`2157df5`** — 일부 LB·모니터링 도구가 기대하는 **`/actuator/healthz`** readiness alias를 추가합니다.

| 항목 | 내용 |
|------|------|
| **Liveness URL** | **`GET /actuator/health/liveness`** · **`GET /actuator/health/live`** · **`GET /actuator/livez`** — **DB probe 없음** · HTTP **200** · `{"status":"UP"}` |
| **Readiness URL** | **`GET /actuator/health`** · **`GET /actuator/healthz`** · **`GET /actuator/health/readiness`** · **`GET /actuator/health/ready`** · **`GET /actuator/readyz`** — **DB probe** · 정상 **200** `UP` · degraded **503** `DOWN` |
| probe exception | **`GET /api/v1/health`** — `databaseStatusDetail: SELECT_1_FAILED_PROBE_EXCEPTION` · `ready: false` (500 아님) |
| 용도 | 로드밸런서·k8s liveness/readiness·**DEPLOYMENT_GUIDE §3-4·§9-1** 스모크 |
| 테스트 | **`ActuatorHealthControllerTest`** · **`HealthControllerTest`** · **`OgadaBackendApplicationTests`** (alias MVC) |

> 관련: DEPLOYMENT §3-4·§9-1·§11-3 · ADMIN_GUIDE §1-4 · Q409 · Q360

### [TWR] Q414. `/reports/transport-monthly` 집계 수치가 내부 시트와 다르게 보여요.

**A.** 먼저 집계 기준이 같은지 확인하세요. `2-7/2-8`은 배차·계약 원천 데이터에서 재계산됩니다.

| 점검 항목 | 확인 방법 |
|------|-----------|
| 대상 월 | 화면 `대상 월`과 내부 시트 월이 같은지 확인 (`YYYY-MM`) |
| 지점 범위 | 상단 `BranchScopeNotice` 지점과 비교 |
| 확정 상태 | `2-8 확정 배차`는 **CONFIRMED 운행만** 반영 (DRAFT 제외) |
| 계약 서명 | `2-8 계약 서명`은 G15 계약 저장 완료 건만 집계 |
| 변동 유형 | `2-7`은 `NEW_TRANSPORT`·`DISCHARGED`·`CONTRACT_CREATED`·`CONTRACT_UPDATED`만 포함 |

> 점검 API: `GET /api/v1/transport/reports/monthly-service-variation` · `GET /api/v1/transport/reports/monthly-resident-status`

### [TWR] Q415. `service-log/audit-trail`이 비어 있는데 일지는 저장했다고 들었어요.

**A.** `audit-trail`은 **일지 저장(UPSERT) 이벤트**만 기록합니다. 조회만 한 경우에는 항목이 생기지 않습니다.

| 확인 항목 | 정상 기준 |
|------|-----------|
| 저장 API 호출 | `PUT /api/v1/transport/runs/{runId}/service-log` 성공(2xx) |
| 운행 상태 | `CONFIRMED` 운행만 저장 가능 (`DRAFT`는 422) |
| 감사 이벤트 | `/settings` 감사 로그에서 `TRANSPORT_SERVICE_LOG_UPSERT` 확인 |
| 이력 상한 | 최근 50건만 반환 |

> 운영 팁: 월마감 전 run 샘플 3건을 지정해 `summary.recorded == summary.total`과 `audit-trail` 존재를 함께 확인하면 누락을 빠르게 찾을 수 있습니다.

### [TWR] Q416. `caregiver` 계정에서는 왜 `service-log/audit-trail`이 403인가요?

**A.** 설계된 권한 정책입니다. `caregiver`는 일지 조회·저장은 가능하지만, 감사 이력 조회는 관리자 역할로 제한됩니다.

| API | `hq_admin` | `branch_admin` | `social_worker` | `caregiver` |
|-----|:----------:|:--------------:|:---------------:|:-----------:|
| `GET /api/v1/transport/runs/{runId}/service-log` | ✅ | ✅ | ✅ | ✅ |
| `PUT /api/v1/transport/runs/{runId}/service-log` | ✅ | ✅ | ✅ | ✅ |
| `GET /api/v1/transport/runs/{runId}/service-log/audit-trail` | ✅ | ✅ | ✅ | ❌ (403) |

> 운영 권장: 현장(`caregiver`)은 일지 입력, 관리자(`branch_admin` 이상)는 월마감 감사 이력 검증으로 역할을 분리하세요.

### [TWR] Q417. 이용자 목록에 **연락처**가 안 보이거나 검색이 안 돼요.

**A.** **Fixed (FE `0baabe9`, 결정 96, Q417)** — **`ClientListPage`** 목록에 **「연락처」** 열이 추가되었습니다. API 응답 **`phoneMasked`** 를 **`MaskedPhone`** 으로 표시합니다 — **전체 번호·통화 링크는 노출하지 않습니다**.

| 항목 | 내용 |
|------|------|
| 목록 열 | 이름·나이·성별·지역·등급·배차 이용·인정번호·**연락처**·보호자·**보호자 연락처** (10열) |
| API 필드 | **`phoneMasked`** · **`primaryGuardianPhoneMasked`** |
| 검색 | 상단 검색창 — 이름·지역·보호자명·**연락처 마스킹 문자열**·인정번호 |
| 상세 | **`ClientDetailPage`** 기본 정보 **연락처** — 등록·수정 시 **`ClientFormPage`** **전화번호** 필드로 입력 |
| 미표시 | 등록 시 연락처를 **입력하지 않았거나** API가 `null` — 열에 **「—」** 표시 |

> 관련: USER_MANUAL §4-3 · ADMIN_GUIDE §1-4 · FAQ Q191

### [TWR] Q418. 수동 배차에서 **출발 시각**과 정차 **예상 도착**은 어떻게 보나요?

**A.** **Fixed (BE `0e46b37` / FE `bf73c4c`/`ea5d896`, US-T02, Q418)** — **`hq_admin`** 이 **`/transport/runs/new`** 에서 **「출발 시각」**(**`TimeInput`**, 기본 **08:00**)을 지정하면 **`POST /api/v1/transport/runs`** 요청에 **`plannedDepartureTime`**(HH:mm:ss)이 포함되어 **V150** 컬럼에 저장됩니다. 루트 상세(`/transport/runs/:runId`)에도 **출발 시각**이 표시됩니다.

| 항목 | 내용 |
|------|------|
| 입력 화면 | **`TransportRunNewPage`** — 운행일 **`DateInput`** · **「출발 시각」** **`TimeInput`** (`ea5d896`, Q422) |
| API 저장 | **`CreateTransportRunRequest.plannedDepartureTime`** · **`TransportRunResponse.plannedDepartureTime`** |
| 예상 도착 | **`TransportRouteSplitView`** — **`POST /api/v1/transport/route-preview`** **`legDurationsSeconds[]`** + 출발 시각 → **`computeStopEtas`** → **`TransportStopList`** **time chip** |
| chip 종류 | **희망 탑승/하차** · **예상 도착** · **희망 반영**(이전 정차 희망 시각 반영) — 희망보다 늦으면 **`--eta-late`** 강조·`aria-label` **「희망 시각보다 지연」** |
| 제한 | **운영 편의용 ETA** — 실제 픽업·일지 준수 기록은 확정 후 **`TransportServiceLogPanel`** (Q407) |

> 관련: USER_MANUAL §5-8 · FAQ Q422 · DEPLOYMENT_GUIDE §11-3

### [TWR] Q419. `GET /api/v1/health` 의 **`liveE2eOperationReady`** 는 무엇인가요?

**A.** **Fixed (BE `3908044`, QA-B95, Q419)** — staging·dev에서 **live E2E harness** 가 즉시 실행 가능한지 **단일 boolean**으로 알려 줍니다. 여러 bootstrap 필드를 일일이 해석하지 않아도 됩니다.

| 필드 | 의미 |
|------|------|
| `liveE2eStaffBootstrapReady` | staff bootstrap **ready** + **credentials configured** |
| `liveE2eGuardianBootstrapReady` | guardian bootstrap **ready** + guardian credentials + **seed client 존재** |
| `liveE2eOperationReady` | 위 두 값이 **모두 true** 일 때만 `true` |
| `liveE2eOperationBlocker` | 차단 사유 — `none` · `staff-bootstrap-not-ready` · `staff-credentials-missing` · `guardian-credentials-missing` · `seed-client-missing` · `guardian-bootstrap-not-ready` · **`bootstrap-disabled`** (Q448·Q472·**Q631**, `d7f1a9a`/`8cf09d8`/`24d25f1`) — *(구 `-error`/`bootstrap-error` 코드 **310차 폐기**)*

| 상황 | 조치 |
|------|------|
| `liveE2eOperationReady=false` + `staff-credentials-missing` | **`LIVE_E2E_EMAIL`/`LIVE_E2E_PASSWORD`** 환경변수 설정 (DEPLOYMENT §4) |
| `seed-client-missing` | **`POST /api/v1/system/live-e2e/bootstrap`** 실행 · FAQ **Q409** |
| **`bootstrap-disabled`** | **`ogada.live-e2e.bootstrap.enabled=true`** (non-prod only) — probe·health **단일 blocker** (Q472, `8cf09d8`) |

> 관련: DEPLOYMENT_GUIDE §9-1 · FAQ Q360·Q409

### [TWR] Q420. 배차 지도·정차 목록 레이아웃이 바뀌었거나, **`/transport`** 로딩이 멈춘 것 같아요.

**A.** **Fixed (FE `fde098f`/`05535a4`, US-T02·a11y, Q420)** — 두 가지가 **214차**에서 정리되었습니다.

| 증상 | 원인 | 조치·동작 |
|------|------|----------|
| 지도·목록이 **위아래**로만 보임 | **`TransportRouteSplitView`** 레이아웃 변경 | **`fde098f`** — **경로 지도 상단** · **정차 목록 하단** 세로 stack(모바일·PC 공통) — 가로 2열은 폐기 |
| **`/transport`** 가 **로딩 중**에서 멈춤 | React StrictMode 이중 마운트 시 이전 fetch 미취소 | **`TransportPage`** **`AbortController`** — 새 요청 시 이전 roster fetch **abort** (`TransportPage.test`) |
| 스크린리더가 연락처를 **두 번** 읽음 | **`MaskedPhone`** 비링크 `span`에 중복 `aria-label` | **`05535a4`** — 목록·상세 마스킹 `span`은 **화면 텍스트만** 접근 이름으로 사용 |

> 관련: USER_MANUAL §5-8 · FAQ Q395·Q417

### [TWR] Q421. 수동 배차에서 **주유소·편의점** 같은 **이용자가 아닌 경유지**를 넣을 수 있나요?

**A.** **BE·FE Fixed (`de3474d`/`bf73c4c`, US-T02, Q421)** — 이용자(**CLIENT**)·센터(**BRANCH**) 외에 **임의 주소 경유지(**`WAYPOINT`**)를 운행 루트에 넣을 수 있습니다. 주유·휴게·약국 등 **이용자와 무관한 정차**를 지도·경로에 반영할 때 사용합니다.

| 항목 | 내용 |
|------|------|
| 화면 | **`/transport/runs/new`** 또는 DRAFT **`/transport/runs/:runId`** — **`TransportRouteSplitView`** **「경유지 추가」** → **`TransportAddWaypointModal`** |
| 입력 | **주소**(필수) · **표시 이름**(선택 — 비우면 목록에 **「경유지」**) |
| API | **`POST/PATCH /api/v1/transport/runs`** — `stops[]` 에 **`stopKind: "WAYPOINT"`** · **`waypointAddress`** · optional **`waypointLabel`** · **`clientId` null** |
| DB | **V151** — `transport_run_stops.waypoint_address`·`waypoint_label` |
| geocode | 저장 시 **Kakao geocode** — 성공 시 좌표·`geocodeStatus=OK` · 실패 시 **지도 제외**·저장·확정 **차단**(이용자 정차와 동일, Q233) |
| 상한 | 이용자 **최대 15명** · **지점+경유지+이용자 합산 최대 17** 정차 |
| 검증 | 주소 없음 → **「경유지 주소를 입력하세요」** · 이용자 ID 지정 시 **422** |
| 테스트 | **`TransportAddWaypointModal.test`** · **`transportUtils.test`** · **`TransportRouteSplitView.test`** |

> 관련: USER_MANUAL §5-8 · DEPLOYMENT_GUIDE §11-3 · FAQ Q401·Q418

### [TWR] Q422. 날짜·시간 입력란이 **달력·시·분 선택**으로 바뀌었어요. 무엇이 달라졌나요?

**A.** **Fixed (FE `ea5d896`/`188ce71`/`ab4de83`/`7b8c7b9`, UXD / DESIGN_SYSTEM, Q422·Q430)** — 브라우저 기본 **`type="date"`**·**`type="time"`** 대신 ogada **디자인시스템 피커**로 통일되었습니다. 배차 **운행일**·**출발 시각**부터 적용되며, 기존 **`DateInput`** 사용 화면(G11·G15·수납일·건강기록·방문일정 등)도 **달력 팝오버**로 갱신됩니다.

| 컴포넌트 | 동작 | 주요 사용 화면 |
|----------|------|----------------|
| **`DateInput`** + **`DatePickerCalendar`** | 트리거 클릭 → **달력 팝오버** · `min`/`max` · **`viewAnchor`**(값 없을 때 달력 시작 월) · **`monthOnly`**(월 선택) · 바깥 클릭·**Esc** 닫기 · 닫을 때 **트리거로 포커스 복귀** | **`TransportRunNewPage`** 운행일 · **`ClientFormPage`** 생년월일(`viewAnchor="1945-01-01"`) · G11·G15 서명일 · 수납·건강기록 |
| **`TimeInput`** | **시·분** `Select` 드롭다운 · 5분 단위(`minuteStep`) · `HH:mm` 출력 | **`TransportRunNewPage`** **출발 시각** (Q418) · 일지 **실제 픽업/하차 시각** |
| **`pickerDate.js`** | `parseIsoDate` · `formatTimePickerValue` · `todayIsoDate` · **`addDaysToIso`/`addMonthsToIso`/`startOfWeekIso`/`endOfWeekIso`** (Q430) | 피커·ETA 계산 공통 |

| 이용 팁 | 내용 |
|---------|------|
| **생년월일** | 값이 비어 있어도 달력이 **1940~50년대 근처**에서 열려 **과거 날짜 선택**이 빠릅니다 |
| **월 선택** | **`MonthInput`**/`monthOnly` 필드는 **연·월만** 선택 — 일(day)은 저장 시 **1일**로 정규화 |
| **키보드 (Q430)** | 달력 열림 시 **roving tabindex** — **←→**(±1일) · **↑↓**(±1주) · **Home/End**(주 시작/끝) · **PageUp/PageDown**(±1월, `min`/`max` 범위 내) · **Enter/Space** 선택 · **Esc** 닫기 |

| 접근성 | **`Field` + id 연결** · 달력 **`aria-expanded`** · popover **`aria-labelledby`** · **roving tabindex** · time chip **`role="group"`** · 지연 ETA **`aria-label`** (Q418·Q430) |
| 테스트 | **`DateInput.test`** (+4 keyboard regression, Q430) · **`pickerDate.test`** · **`pickerTestUtils.js`** — QA-B127 Vitest 전역 migrate (`ab4de83`) |
| 금지 | raw **`<input type="date">`** — FE-16 규칙 유지 (Q281) |

> 관련: USER_MANUAL §3-2 · FAQ Q281·Q418 · DESIGN_SYSTEM §7-7o

### [TWR] Q423. 배차에 **퇴소·비활성 이용자**를 넣으려 하면 저장이 거부돼요.

**A.** **Fixed (BE `dd2fa2c`, V152, Q423)** — **V143** 이후 **`trg_transport_run_stops_guard_client`** 가 잘못된 컬럼(`active`)을 참조해 **퇴소·비활성 가드가 무력화**되었습니다. **V152** 가 **`clients.is_active`** 로 정합을 복구합니다 — **Flyway V152 committed** @ `dd2fa2c`.

| 항목 | 내용 |
|------|------|
| 대상 | **`stopKind=CLIENT`** 정차 — **이용자 배차** |
| 거부 조건 | **`is_active=false`** · **`discharged_at` 존재** · **`uses_transport=false`** · **다른 지점 이용자** |
| DB | **V152** — `CREATE OR REPLACE FUNCTION trg_transport_run_stops_guard_client()` — `clients.is_active` |
| API | **`POST/PATCH /api/v1/transport/runs`** — DB trigger 예외 → **`422`** 또는 **`500`** (운영 메시지: inactive or discharged) |
| UI | roster·수동 배차 명단은 **활성·이동서비스 이용자**만 표시 — 퇴소 직후 **기존 DRAFT** 에 잔존 정차가 있으면 **저장·확정 실패** |
| 관련 가드 | **G15 계약** — 퇴소·비활성 저장 거부 (V65, FAQ **Q235**) · **간편결제** — V110 |

| 조치 |
|------|
| 퇴소 이용자는 roster에서 제외 — **DRAFT 루트**에서 해당 정차 **삭제** 후 재저장 |
| staging·prod 배포 후 **Flyway V152** 적용 확인 (DEPLOYMENT §11-3) |

> 관련: USER_MANUAL §5-8 · FAQ Q235 · DEPLOYMENT_GUIDE §11-3

### [TWR] Q424. 배차 **최적화 설정 저장**이 실패하거나 화면 배치가 바뀌었어요.

**A.** **Fixed (BE `dd2fa2c`·FE `96db8bf`, US-T02, Q424)** — **배차 설정 검증 강화**와 **`/transport` compact dispatch layout** 이 적용되었습니다.

| 변경 | 내용 |
|------|------|
| UI 배치 | **`TransportPage`** 하단 **「배차 생성」** — **승차** 시 **자동 제안(왼쪽)·수동 배차(오른쪽)** 나란히 · **하차** 시 **수동 배차만** (`ds-transport-dispatch-grid`) |
| embedded 설정 | **`BranchTransportSettingsPanel embedded`** — **「자동 배차 제안」** 카드 **내부**에 픽업 허용분·가중치 필드 · 별도 설정 카드 **없음** |
| 필드 tooltip | **`Field` tooltip** — 픽업 허용 ±분 · 안정성·공정성·거리 가중치 의미 (`TRANSPORT_SETTINGS_FIELD_TOOLTIPS`) |
| 입력 step | 가중치 **0.1 단위** 스냅(`snapWeightInput`) · 픽업 허용 **정수 분** |
| FE 검증 | **`transportSettingsForm.js`** — 빈 가중치·픽업 허용분 → **필드 오류** · DB integrity 실패 **사전 차단** |
| BE 검증 | **`BranchTransportSettingsService`** — 가중치 **null·0~1·합>0** · 픽업 **0~120분** · **`BusinessRuleException`** 한글 메시지 |
| 자동 제안 | **`TransportSuggestService`** — 제안 DRAFT에 **출발·복귀 BRANCH 정차** 포함 — 수동 배차와 동일 |

| 증상 | 조치 |
|------|------|
| 「가중치를 입력하세요」 | 세 가중치 **모두 0~1** 입력 · **합이 0보다 커야** 저장 가능 |
| 「픽업 허용 시간을 입력하세요」 | **0~120** 정수 분 입력 |
| 설정은 보이는데 제안 버튼 없음 | **승차(PICKUP)** 방향인지 확인 — **하차**에서는 자동 제안 **숨김** |

> 관련: USER_MANUAL §5-8 · FAQ Q347 · ADMIN_GUIDE §10-10 · DEPLOYMENT §11-3

### [TWR] Q425. `npm run test:live-e2e` 실행 시 bootstrap은 **200**인데 **authenticated suite가 SKIP** 되거나 토큰이 비어 있어요.

**A.** **Fixed (FE `fc916db`, QA-B95, Q425)** — 백엔드 bootstrap 응답이 **flat camelCase**(`accessToken`·`clientId`)뿐 아니라 **nested 객체**(`staff.access_token`·`guardian.refresh_token`) 또는 **snake_case** 필드명으로 내려와도 harness가 토큰을 읽도록 강화되었습니다.

| 항목 | 내용 |
|------|------|
| 파서 | **`liveGlobalSetup.js`** — **`pickBootstrapAccessToken`** · **`pickBootstrapRefreshToken`** · **`pickBootstrapEmail`** · **`pickBootstrapClientId`** |
| 수용 형식 | 최상위 **`accessToken`/`access_token`/`token`** · **`staff`/`guardian` nested** · embedded **`guardian_access_token`**·**`guardian_client_id`** 등 |
| env 주입 | 성공 시 **`LIVE_E2E_ACCESS_TOKEN`**·**`LIVE_E2E_REFRESH_TOKEN`**·**`LIVE_E2E_EMAIL`**·**`LIVE_E2E_CLIENT_ID`** (guardian variant 동일) |
| embedded guardian | staff bootstrap payload에 guardian token이 **flat 또는 nested** 로 포함되면 **`bootstrap-guardian` 2차 호출 생략** (Q409 연계) |
| 테스트 | **`liveE2eHarness.test`** — nested·snake_case·embedded guardian **regression** (+74 lines) |

| 증상 | 조치 |
|------|------|
| bootstrap **200** · suite **SKIP** | FE **`fc916db`** 이상인지 확인 · **`liveGlobalSetup`** 로그에서 token persist 여부 확인 |
| **`LIVE_E2E_CLIENT_ID` 비어 있음** | bootstrap 응답에 **`clientId`/`client_id`** 또는 nested **`staff.client_id`** 존재 여부 확인 |
| 여전히 SKIP | FAQ **Q360** — health readiness·placeholder creds · **`./scripts/run-live-e2e.sh`** 재실행 · stale refresh token은 FAQ **Q427** (`b2c09e1`) |

> 관련: Q409 · Q360 · **Q427** · DEPLOYMENT §3-6 · ADMIN_GUIDE §1-4

### [TWR] Q426. **수칙·계약 화면**에서 **이동서비스일지(별지 제22호)** 를 바로 작성할 수 있나요?

**A.** **Fixed (FE `b93e098`, G15 v1.3-C, Q426)** — **`/transport/compliance`** 에 **「이동서비스일지 작성 (별지 제22호)」** 카드가 추가되어, 당일 **확정(`CONFIRMED`)** 승차·하차 배차 목록에서 **일지④** 화면으로 바로 이동할 수 있습니다.

| 항목 | 내용 |
|------|------|
| UI | **`TransportServiceLogRunsPanel`** — 방향·차량·정차 수·확정 시각 표 · **「일지 작성·보관」** 링크 |
| 이동 경로 | **`/transport/compliance`** → **`/transport/runs/:runId`** — 운행 상세 **`TransportServiceLogPanel`** (배차 화면 경유 불필요) |
| 데이터 | **`fetchTransportRunsApi`** — 당일 **PICKUP·DROPOFF** 병렬 조회 · **CONFIRMED** 만 표시 |
| 빈 상태 | 확정 배차 없음 → **「배차·이동경로로 이동」** 링크 (`/transport`) |
| 저장 | 운행 상세와 **동일 API** — **`GET/PUT /transport/runs/{runId}/service-log`** (Q407) |
| 권한 | 수칙·계약 화면과 동일 — **`hq_admin`·`branch_admin`·`social_worker`** 저장 · **`caregiver`** 조회 |
| 테스트 | **`TransportServiceLogRunsPanel.test`** · **`TransportCompliancePage.test`**

| 증상 | 조치 |
|------|------|
| 일지 카드가 비어 있음 | **`/transport`** 에서 당일 운행 **확정** 후 새로고침 |
| 링크 클릭 후 일지 폼 없음 | 운행 상태가 **CONFIRMED** 인지 확인 · DRAFT는 일지 저장 불가 (Q407) |

> 관련: Q230 · Q407 · Q411 · USER_MANUAL §5-8-0·§5-8-0-1 · ADMIN_GUIDE §1-4

### [TWR] Q427. live E2E에서 **이전 run의 refresh token**·**probe 500**·**비활성 지점** 때문에 harness가 실패해요.

**A.** **Fixed (BE `d68c4bf`/`844227a` / FE `b2c09e1`, QA-B95, Q427)** — bootstrap·probe·harness가 **재사용 dev DB·stale shell env** 에서도 안정적으로 동작하도록 보강되었습니다.

| 증상 | 원인 | 수정 |
|------|------|------|
| bootstrap **200** · authenticated suite **401/403** | shell에 남은 **stale `LIVE_E2E_REFRESH_TOKEN`** | FE **`b2c09e1`** — bootstrap이 refresh를 반환하면 **항상 교체** (authoritative) · **`liveE2eHarness.test`** |
| **`LIVE_E2E_GUARDIAN_EMAIL=guardian@ogada.test`** 고정 | example placeholder가 bootstrap authoritative email을 덮음 | FE **`b2c09e1`** — placeholder email이면 bootstrap **`guardianEmail`** 로 **hydration** |
| **`GET /probe` HTTP 500** | staff/guardian credential 조회 중 **한 필드** 예외 | BE **`844227a`** — **`safeBoolean`** per-field · probe **200** 유지·해당 flag만 **false** · **`LiveE2eControllerTest`** |
| bootstrap 후 **branch inactive** | seed 지점 **`active=false`** | BE **`d68c4bf`** — **`LiveE2eBootstrapService`** seed branch **`setActive(true)`** · **`LiveE2eBootstrapServiceTest`**

**운영 절차**

1. harness 재실행 전 FAQ **Q408** — stale shell **`LIVE_E2E_*`** 정리.
2. **`./scripts/run-live-e2e.sh`** 1회 RUN — bootstrap 후 token·email env 확인 (Q425·Q427).
3. probe 실패 시 **`GET /api/v1/system/live-e2e/probe`** — **`credentialsConfigured`·`clientReady`** per-field 확인 (Q409).

> 관련: Q409 · Q425 · Q408 · DEPLOYMENT §3-6 · ADMIN_GUIDE §1-4

### [TWR] Q428. live E2E에서 **`LIVE_E2E_GUARDIAN_EMAIL`/`PASSWORD` 미설정** 시 guardian bootstrap이 막혀요.

**A.** **Fixed (BE `92be918`/`09df8c7`, QA-B95, Q428)** — bootstrap enabled 환경에서 guardian env creds가 **비어 있으면** 시드 **기본값**으로 안전하게 fallback 합니다. probe·health·staff bootstrap 응답이 **동일한 resolved email**을 사용합니다.

| 항목 | 내용 |
|------|------|
| 기본 guardian email | **`live-e2e-guardian@ogada.test`** (`LiveE2eBootstrapService.DEFAULT_GUARDIAN_EMAIL`) |
| 기본 guardian password | **`ogada-guardian-e2e`** (`DEFAULT_GUARDIAN_PASSWORD`) |
| BE fallback | **`92be918`** — env blank 시 default creds로 bootstrap 진행 · explicit creds readiness signal 유지 |
| status·enrichment align | **`09df8c7`** — **`guardianStatus`**·staff bootstrap **`guardianEmail`** enrichment가 resolved default 사용 |
| probe | **`GET /api/v1/system/live-e2e/probe`** — **`defaultGuardianCredentials: true`** when env blank · **`245차 (`f932fd3`, Q490)`** — default staff/guardian creds 사용 시 **`operationReady=false`** · **`guardian-credentials-default`** blocker — **preflight는 env 명시 creds 필요** |
| health | **`GET /api/v1/health`** — **246차(`8fe1ccd`, Q491)부터 probe와 동일** — default creds 시 **`liveE2eOperationReady=false`** · **`staff-credentials-default`/`guardian-credentials-default` blockers** — **`liveE2eOperationBlockers[]`** 동기화 |
| health | **`GET /api/v1/health`** — **`liveE2eDefaultGuardianCredentials`** 필드 추가 |
| 테스트 | **`LiveE2eBootstrapServiceTest`** — `guardianStatusShouldReportReadyWhenDefaultGuardianCredentialsAreUsed` · staff bootstrap guardian email enrichment |

| 증상 | 조치 |
|------|------|
| **`guardian-credentials-missing` blocker** (env blank) | **`09df8c7`** 적용 후 health·probe 재확인 — default creds 사용 시 **ready** |
| probe **`defaultGuardianCredentials=false`** 인데 env blank | BE 재기동·bootstrap enabled 확인 · FAQ **Q393** |
| 여전히 SKIP | FAQ **Q360** · **`./scripts/run-live-e2e.sh`** 재실행 |

> 관련: Q393 · Q409 · Q427 · DEPLOYMENT §3-6·§11-3 · ADMIN_GUIDE §1-4

### [TWR] Q429. `src/frontend`에서 **`npm run test:live-e2e`** 가 **script not found** 또는 **127** 로 실패해요.

**A.** **Fixed (FE `8882d9f`, QA-B131, Q429)** — monorepo 루트에 **`scripts/`** 가 별도로 있어 **`frontend-test`** checkout에서 상대 경로가 어긋나던 문제를 해결했습니다. **`package.json` `test:live-e2e`** 가 repo root **`scripts/run-frontend-live-e2e.sh`** 를 **`bash`** 로 위임합니다.

| 항목 | 내용 |
|------|------|
| 변경 | **`"test:live-e2e": "bash ../../scripts/run-frontend-live-e2e.sh"`** — git root 기준 **`scripts/`** 탐색 |
| 권장 실행 | 루트 **`./scripts/run-live-e2e.sh`** (백엔드 기동·env source 포함) |
| 대안 | **`cd src/frontend && npm run test:live-e2e`** — **`8882d9f`** 이상에서 script path 정상 |
| 테스트 | **`liveE2eHarness.test`** — script delegation regression |

| 증상 | 조치 |
|------|------|
| **`ENOENT … run-frontend-live-e2e.sh`** | FE **`8882d9f`** 적용 확인 · repo root에서 실행 |
| bootstrap OK · suite SKIP | FAQ **Q360·Q425·Q427·Q428** — creds·readiness |
| Vitest hang | **`docs/qa/VITEST_CONCURRENCY.md`** — **`./scripts/vitest-stop.sh`** 후 재시도 |

> 관련: Q360 · Q425 · DEPLOYMENT §3-6 · ADMIN_GUIDE §1-4

### [TWR] Q430. **`DateInput` 달력**에서 키보드로 날짜를 빠르게 고를 수 있나요?

**A.** **Fixed (FE `7b8c7b9`, UXD, Q430)** — WAI-ARIA **date grid** 패턴으로 **화살표 키 탐색**이 추가되었습니다. 이전에는 Tab으로 최대 **42개** 일 버튼을 순회해야 했습니다.

| 키 | 동작 |
|----|------|
| **← →** | ±1일 (`min`/`max` 범위 내) |
| **↑ ↓** | ±1주 |
| **Home / End** | 해당 주 **첫/마지막** 활성일 |
| **PageUp / PageDown** | ±1월 (표시 월·`focusDate` 동기) |
| **Enter / Space** | 포커스된 날짜 **선택** |
| **Esc** | 팝오버 닫기 · 트리거로 포커스 복귀 |

| 구현 | **`DatePickerCalendar`** roving tabindex · **`DateInput`** `focusDate` state · **`pickerDate.js`** pure helpers |
| 테스트 | **`DateInput.test`** — roving tabindex·arrow move·PageUp month jump·max-bound block (+4) |

> 관련: Q422 · USER_MANUAL §3-2 · DESIGN_SYSTEM §52

### [TWR] Q431. 배포 후 **입금·수납 대장**·**G26 의료비공제** 조회가 느려졌나요? (V153)

**A.** **Fixed (BE `c8ee85c`, G26, Q431)** — **Flyway V153** 이 **PAID** 청구의 **`paid_at DESC`** 조회용 partial index를 추가했습니다. **성능 개선**이 목적이며, 화면·API 동작은 **변경 없습니다**.

| 항목 | 내용 |
|------|------|
| 마이그레이션 | **V153** `idx_billing_claims_org_branch_status_paid_at` — `(organization_id, branch_id, status, paid_at DESC) WHERE status='PAID'` |
| 대칭 | **V71** `idx_billing_claims_org_branch_status_refunded_at` (REFUNDED) 패턴 mirror |
| 대상 쿼리 | **`BillingClaimRepository.findByOrganizationIdAndBranchIdAndStatusOrderByPaidAtDesc`** — 입금·수납 대장 · G26 ① **의료비공제** · 이용자 상세 **청구 탭 PAID 집계** |
| 전제 | **V50** — PAID 행에 **`paid_at` NOT NULL** CHECK |

| 운영 | staging·prod 배포 후 Flyway **V153** `Successfully applied` 확인 (DEPLOYMENT §11-3) |

> 관련: Q174 · Q252 · USER_MANUAL §5-5 · DEPLOYMENT_GUIDE §3-6 · DATA_RETENTION_POLICY §2

### [TWR] Q432. **G15 이동서비스 일지(별지 제22호)** API가 FE·BE 계약과 맞는지 어떻게 검증하나요?

**A.** **Fixed (BE `4c5d3bc`, G15 v1.3-C, Q432)** — 백엔드 **`TransportServiceLogLiveApiRoutingE2eTest`** 가 프론트 **`transportServiceLogServices`** fetcher와 **동일 경로·메서드·응답 shape**를 검증합니다.

| API | harness 검증 |
|-----|-------------|
| **`GET /api/v1/transport/runs/{runId}/service-log`** | **`TransportServiceLogResponse`** — rows·`summary`·vehicle/driver read-only |
| **`PUT /api/v1/transport/runs/{runId}/service-log`** | **CONFIRMED only** · 정차별 실제 픽업·동승·하차 · **`TransportServiceLogUpsert`** |
| **`GET …/service-log/audit-trail`** | 최근 **50건** · `runDate`·`stopUpdateCount`·준수 요약 |

| 현장 확인 | **`/transport/runs/:runId`** — 확정 운행 **「일지 기록 저장」** · **`/transport/compliance`** — **「일지 작성·보관」** (Q426) |
| CI | **`mvn test`** — `TransportServiceLogLiveApiRoutingE2eTest` · FE **`transportServiceLogServices.test`** 병행 |

> 관련: Q407 · Q411 · Q426 · USER_MANUAL §5-8-0-1 · DEPLOYMENT §11-3

### [TWR] Q433. 배차 명단에서 **희망 시각과 계획 픽업**을 한눈에 비교하려면?

**A.** **Fixed (FE `e35efb2`, US-T02, Q433)** — **`/transport`** **「배차 신청 인원 명단」** 표에 **확정 루트 배정 정보**가 추가되었습니다. 운행 상세를 일일이 열지 않아도 **정차 순서·계획 시각·지연 여부**를 확인할 수 있습니다.

| 열 | 승차(PICKUP) | 하차(DROPOFF) | 내용 |
|----|-------------|---------------|------|
| **배차 루트** | 동일 | 동일 | 확정 루트에 포함된 이용자 — **「N번 정차」** 링크 → **`/transport/runs/:runId`** · **`aria-label`** `{이름} N번 정차` (UXD-129, `e9d39a9`) |
| **계획 픽업/하차** | **계획 픽업** | **계획 하차** | 확정 루트 정차의 **`pickupTime`** (HH:mm) |
| **지연** | 희망 탑승 시각 대비 | 희망 하차 시각 대비 | 계획 시각이 희망보다 늦으면 **「지연」** Badge · **`aria-label`** 로 지연 안내 (title-only 힌트 제거, `e9d39a9`) |

| 동작 | **`loadConfirmedRunDispatchIndex`** — 당일 **CONFIRMED** 루트를 **`GET /transport/runs/{runId}`** 로 병렬 조회 · 이용자별 **`stopOrder`·`plannedPickupTime`** 인덱스 구축 |
| 미배정 | 확정 루트에 없는 이용자 — **배차 루트**·**계획 픽업/하차** 열 **「—」** |
| 테스트 | **`transportRosterDispatch.test`** · **`TransportPage.test`** — link·planned time·late badge |

> 관련: Q400 · Q418 · USER_MANUAL §5-8 · DEPLOYMENT §11-3

### [TWR] Q434. **StaffLifecyclePanel** Vitest가 날짜가 지나면 실패하는 이유는?

**A.** **Fixed (FE `101aaee`, QA-B132, Q434)** — FAQ21806 **신규직원교육 7일** deadline은 **`입사일 + NEW_HIRE_TRAINING_DEADLINE_DAYS(7)`** 로 계산됩니다. 테스트 시드의 **`hiredAt`** 이 오래되면 wall-clock 기준으로 **기한 초과**가 되어 assertion이 깨질 수 있었습니다.

| 조치 | **`StaffLifecyclePanel.test`** — deadline helper **mock** — 날짜 경과와 무관하게 안정 실행 |
| 범위 | **테스트 안정화만** — 운영 UI·API 동작 변경 없음 |
| CI | **`npm test`** — `StaffLifecyclePanel.test` 회귀 포함 |

> 관련: Q290 · Q298 · USER_MANUAL §5-3 · DEPLOYMENT §11-3

### [TWR] Q435. 신규 직원 **건강검진 결과통보서 1년 이내** 확인은 어디서 보나요?

**A.** **Fixed (FE `8e6310a`, US-R02, FAQ21799, Q435)** — 이지케어 FAQ21799 **지표9** — 신규 채용 직원은 **근무 시작일(입사일) 기준 1년 이내** 건강검진 **실시일**이 있어야 합니다. ogada는 compliance 화면에서 **입사일·검진 실시일**을 대조해 **서류 미확인** 인원을 표시합니다.

| 항목 | 내용 |
|------|------|
| 화면 | **`/staff/health-checkups`** — StatCard **「신규 서류 미확인」** · warning **`Alert`** (미확인 인원 있을 때) |
| 목록 열 | **「입사일」** · **「신규 서류」** Badge — **1년 이내 확인**(`success`) · **서류 필요**(`warning`) · **입사일 미등록**(`neutral`) · **—**(갱신 검진만 해당) |
| 판정 규칙 | **`isWithinNewHireDocumentWindow`** — 검진일이 **입사일 이전(당일 포함)** 이고 **입사일 − 365일 이후**이면 **1년 이내 확인** |
| 저장 검증 | **「검진 기록」** Modal — **첫 검진**(`isFirstCheckup`) 시 실시일이 창 밖이면 필드 오류 — 「신규 직원은 근무 시작일 기준 1년 이내 건강검진 실시일이어야 합니다 (FAQ 21799).」 |
| 데이터 | compliance API + **`GET /users`** **`hiredAt`** 병합 — **`enrichStaffHealthCheckupItems`** |
| P2 잔여 | checklist `health-check` **자동 반영** — **HR 파일함 업로드**는 Q444에서 **부분 Fixed** (`b6ce301`) |

| 테스트 | **`staffHealthCheckupCompliance.test`** · **`StaffHealthCheckupsPage.test`** |

> 관련: Q296 · Q290 · Q444 · USER_MANUAL §5-3 · ADMIN_GUIDE §6-2-4

### [TWR] Q436. **DateInput** 달력 **「오늘」** 버튼 Vitest가 날짜마다 실패해요.

**A.** **Fixed (FE `40d4284`, QA-B133, Q436)** — 달력에서 **오늘** 날짜 버튼의 accessible name은 locale·날짜에 따라 **`2026-06-18`** 또는 **`2026-06-18 오늘`** 형태입니다. 테스트 selector가 고정 문자열이면 **wall-clock 날짜 변경** 시 flake가 발생할 수 있었습니다.

| 조치 | **`DateInput.test`** — **`getByRole("button", { name: /2026-06-18( 오늘)?$/ })`** — **「오늘」 접미사 optional** |
| 범위 | **테스트 안정화만** — 운영 UI·달력 키보드 동작(Q430) 변경 없음 |
| CI | **`npm test`** — `DateInput.test` 회귀 포함 |

> 관련: Q422 · Q430 · USER_MANUAL §3-2 · DEPLOYMENT §11-3

### [TWR] Q437. guardian env가 비어 있는데 **`liveE2eOperationReady`가 false**예요.

**A.** **Fixed (BE `2e6c35f`, QA-B95, Q437)** — Q428에서 guardian **기본 creds fallback**을 추가했으나, health readiness는 **`hasConfiguredGuardianCredentials`만** 보면 env blank 시 **`liveE2eGuardianBootstrapReady=false`**·**`liveE2eOperationBlocker=guardian-credentials-missing`** 가 남을 수 있었습니다.

| 조치 (`2e6c35f`) | **`HealthController`** — **`guardianCredentialsUsable = guardianCredentialsConfigured \|\| defaultGuardianCredentials`** — **의도적 default 사용** 시 operation readiness **통과** |
| **246차 갱신 (`8fe1ccd`, Q491)** | staging preflight 일관성을 위해 **health도 probe와 동일**하게 default creds를 **blocker**로 처리 — Q437 health **`operationReady=true` 예외 해소** |

| 여전히 false일 때 | **`liveE2eBootstrapEnabled=false`** · seed client 미시드 · guardian bootstrap not-ready — FAQ **Q409·Q428·Q491** |

> 관련: Q428 · Q419 · Q409 · Q491 · DEPLOYMENT §3-6 · ADMIN_GUIDE §1-4

### [TWR] Q438. live E2E bootstrap이 **stale seed**에서 토큰 오류·500이 나요.

**A.** **Fixed (BE `a6dfaad`/`d02f78a`, QA-B95, Q438)** — 재사용 dev DB에서 시드 계정 scope·refresh token이 어긋나면 staff bootstrap **토큰 발급**이 실패할 수 있습니다.

| 조치 (`a6dfaad`) | **`LiveE2eBootstrapService.issueSeedTokensWithRetry`** — 첫 **`issueTokensForUser`** 실패 시 **`organizationId`·`activeBranchId`·`active`·`onboardingCompleted`·`reportingCompleted` 재설정** 후 **1회 재시도** |
| harness (`d02f78a`) | **`LiveE2eBootstrapLiveApiRoutingE2eTest`** — staff bootstrap 응답 **`guardianAccessToken`/`guardianRefreshToken`/`guardianEmail`** embedded 필드 HTTP 계약 검증 — FE **`liveGlobalSetup.js`** mirror |
| 테스트 | **`LiveE2eBootstrapServiceTest`** — stale fixture retry regression |

| 운영 확인 | **`GET /api/v1/health`** — **`liveE2eOperationReady: true`** · **`liveE2eOperationBlocker: none`** · staging **`./scripts/run-live-e2e.sh`** |

> 관련: Q409 · Q427 · Q428 · Q437 · DEPLOYMENT §3-6·§11-3

### [TWR] Q439. 이동서비스일지에서 **일부 정차만 입력**하고 저장·인쇄할 수 있나요?

**A.** **Fixed (FE `b4644e8` / BE `ac1d43f`, G15 v1.3-C, Q439·Q443)** — **아니요.** 별지 제22호 **법정 기록**은 정차(이용자)별 **실제 픽업·동승 여부·하차 시각**이 **모두** 있어야 합니다. **브라우저 UI**와 **API 직접 호출** 모두 동일 규칙입니다.

| 동작 | 검증 |
|------|------|
| **「일지 기록 저장」** | **`validateServiceLogRecords`** — 미입력 필드 **`Field` 오류** · 상단 **「법정 보관 전 정차별…」** 메시지 · 서버 **`422`** (Q443) |
| **인쇄·텍스트 다운로드** | 동일 검증 — **미입력 시 차단** (기존 **미저장 변경** 가드와 병행) |
| **하차 < 픽업** | 하차 시각 필드 오류 — 「하차 시각은 실제 픽업 시각 이후여야 합니다.」 |
| **15분 준수** | 입력·저장은 가능 — **`StatusBadge`** 로 준수/지연 **표시만** (Q407) |

| 테스트 | **`TransportServiceLogPanel.test`** — **`validates required legal fields before save`**

> 관련: Q407 · Q440 · USER_MANUAL §5-8-0-1 · ADMIN_GUIDE §1-4

### [TWR] Q440. 일지 저장 시 **「일지 대상 이용자가 중복되었습니다」** 오류가 나요.

**A.** **Fixed (BE `52e3340`, G15 v1.3-C, Q440)** — **`PUT /service-log`** 본문 `stops[]`에 **동일 `clientId`가 2회 이상** 포함되면 서버가 **`422 BusinessRuleException`** 을 반환합니다.

| 원인 | 한 운행에 같은 이용자가 **두 정차**로 들어가거나, 클라이언트 payload 조립 오류 |
| 조치 | **`TransportServiceLogPanel`** 에서 정차별 **1행 = 1이용자** 확인 · 중복 제거 후 재저장 |
| 서버 | **`validateServiceLogStopRecords`** — **`Set<UUID>`** 로 중복 검출 |
| 테스트 | **`TransportServiceTest.upsertServiceLogShouldRejectDuplicateClientRows`**

> 관련: Q407 · Q443 · Q440 · USER_MANUAL §5-8-0-1

### [TWR] Q441. **`GET /system/live-e2e/probe`** 에 **`operationBlocker`** 가 무엇인가요?

**A.** **Fixed (BE `40ef105`/`c5dd4f2`/`d7f1a9a`, QA-B95, Q441·Q447·Q448)** — health의 **`liveE2eOperationBlocker`** 와 별도로, **anonymous probe**가 **operation-level readiness** 진단 필드를 노출합니다. **`c5dd4f2`** 부터 **복수 blocker**를 **`operationBlockers[]`** 로 동시에 볼 수 있고, **`d7f1a9a`** 부터 bootstrap **내부 오류**는 **`-error`** 코드로 구분됩니다.

| probe 필드 | 의미 |
|------------|------|
| **`staffBootstrapReady`** | staff 시드(org·branch·user·mapping) + **credentials configured** |
| **`guardianBootstrapReady`** | guardian 시드 ready + **guardian creds usable**(configured 또는 default) + **`clientReady`** |
| **`operationReady`** | 위 두 조건 **모두 true** |
| **`operationBlocker`** | **`operationBlockers[]`의 첫 실패 원인** (하위 호환) |
| **`operationBlockers`** | **미충족 prerequisite 전체 목록** (Q447, `c5dd4f2`) — bootstrap 실패 시 **`-not-ready`만** (Q631, `24d25f1`) · root cause는 **status detail** |

| **`operationBlocker` / `operationBlockers` 값** | 의미 |
|--------------------------------------------------|------|
| **`none`** / **`[]`** | operation ready |
| **`staff-bootstrap-not-ready`** | org·branch·user·mapping 미완 **또는 bootstrap 내부 오류** — detail **`bootstrap=error`** 확인 (Q631) |
| **`staff-credentials-missing`** | staff env creds 미설정·default 미사용 |
| **`guardian-credentials-missing`** | guardian creds 미사용 가능 |
| **`seed-client-missing`** | 시드 이용자 미확보 |
| **`guardian-bootstrap-not-ready`** | guardian mapping·user 미완 **또는 bootstrap 내부 오류** — detail **`guardian-bootstrap=error`** 확인 (Q631) |
| *(폐기, 310차)* **`staff-bootstrap-error`** · **`guardian-bootstrap-error`** · **`bootstrap-error`** | **Q631** — blocker에서 제거 · detail로만 구분 |

| health 동기 필드 | **`liveE2eOperationBlocker`** · **`liveE2eOperationBlockers`** (`c5dd4f2`) |
| 운영 | **`curl /api/v1/system/live-e2e/probe | jq '.operationBlockers'`** — health **`liveE2eOperationBlockers`** 와 **대조** (Q419·Q437) |
| 테스트 | **`LiveE2eControllerTest`** · **`HealthControllerTest`** · **`LiveE2eBootstrapLiveApiRoutingE2eTest`**

> 관련: Q409 · Q419 · Q437 · Q447 · Q448 · DEPLOYMENT §3-6 · ADMIN_GUIDE §1-4

### [TWR] Q442. **통합식사도움기록** 목록이 비어 있는데 API는 200이에요. (L02_M13)

**A.** **Fixed (FE `38642e2`/`e9d39a9`/`1c8f236`, L02_M13, Q442·Q449)** — API가 **200**이어도 body가 **`items[]` 배열이 아니면** 이전에는 **빈 목록**으로 보였습니다. 이제 **명확한 오류 Alert**가 표시됩니다. **`1c8f236`** 부터는 이용자 목록이 **snake_case**(`client_id`·`client_name`)이거나 **`{ items: [...] }`** 형태여도 **등록 폼 이용자 선택**이 동작합니다.

| 증상 | 조치 |
|------|------|
| 「통합식사도움기록 조회 응답 형식이 올바르지 않습니다.」 | 백엔드·프록시·버전 불일치 — **`GET /care/meal-assistance-records`** 응답 shape 확인 |
| 「이용자 목록 응답 형식이 올바르지 않습니다.」 | **`GET /clients`** 목록 API 점검 — **배열·`items[]` 둘 다 아닐 때** |
| 이용자 드롭다운 비어 있음·등록 실패 (Q449) | **`normalizeClient`** — `client_id`/`client_name` snake_case 수용 · **`MealAssistanceRecordPage.test`** regression |
| UXD-129 (`e9d39a9`) | 배차 루트 **`aria-label`** · 신규 서류 **NA → 「—」** · meal record CSS class 정합 |

| 테스트 | **`MealAssistanceRecordPage.test`** — **`shows a clear error when API payload is malformed`** · **`creates a record when client payload uses snake_case fields`** (Q449)

> 관련: Q366 · Q449 · USER_MANUAL §5-27 · ADMIN_GUIDE §6-2-18

### [TWR] Q443. API로 일지를 저장할 때 **「실제 픽업 시각을 입력하세요」** 등 422 오류가 나요.

**A.** **Fixed (BE `ac1d43f`, G15 v1.3-C, Q443)** — FE `b4644e8`(Q439)와 동일하게, **`PUT /api/v1/transport/runs/{runId}/service-log`** 는 정차별 **법정 필드 완전 입력**을 서버에서도 강제합니다.

| 조건 | 서버 응답 (`422 BusinessRuleException`) |
|------|----------------------------------------|
| `actualPickupTime` blank | 「실제 픽업 시각을 입력하세요.」 |
| `companionAccompanied` null | 「동승 여부를 선택하세요.」 |
| `dropoffTime` blank | 「하차 시각을 입력하세요.」 |
| 하차 < 픽업 | 「하차 시각은 실제 픽업 시각 이후여야 합니다.」 |

| 구현 | **`TransportService.validateServiceLogStopRecords`** — duplicate 검사(Q440)와 **동일 루프** |
| 테스트 | **`TransportServiceTest.upsertServiceLogShouldRejectIncompleteLegalFields`**

> 관련: Q407 · Q439 · Q440 · USER_MANUAL §5-8-0-1 · DEPLOYMENT §11-3

### [TWR] Q444. 신규 직원 **건강검진 결과통보서**는 어디서 업로드하나요?

**A.** **Fixed (FE `b6ce301`, US-R02, FAQ21799, Q444)** — **`/staff/health-checkups`** 에서 **스캔→파일함** 워크플로를 **8-10 roster 안에서** 완료할 수 있습니다. 별도 화면 이동 없이 HR 파일함 **`health-check`** 탭으로 deep link됩니다.

| 경로 | 동작 |
|------|------|
| 목록 **「서류 업로드」** | **신규 서류 필요** Badge 행 — **`/staff/{userId}?tab=files&doc=health-check`** |
| **「이력」** Modal | **`StaffHealthCheckupRecordsPanel`** — **「건강검진 결과통보서 (파일함 · US-R02)」** 등록 여부 · **「파일함에서 업로드/관리」** 링크 |
| 파일함 | **`StaffHrFilePanel`** — **`documentType=health-check`** — PDF·이미지 스캔본 (US-R03, Q298) |
| checklist | 업로드 시 lifecycle **`health-check`** checklist 연동 — **자동 반영은 P2** |

| 테스트 | **`StaffHealthCheckupsPage.test`** — **`links new-hire document gaps to HR file hub upload`** · history modal **`fetchStaffHrFilesApi`**

> 관련: Q296 · Q298 · Q435 · USER_MANUAL §5-3 · ADMIN_GUIDE §6-2-4

### [TWR] Q445. 이동서비스일지 **운전자 서명**은 어디에 입력하나요?

**A.** **Fixed (FE `f51e365`/`bfe0283` / BE `bc3a35c`, G15 v1.3-C, Q445·Q450)** — **`/transport/runs/:runId`** **`TransportServiceLogPanel`** 상단 **「운전자 서명」** `fieldset` 안 **「서명 성명」**·**「서명일」** 에 입력합니다. 별지 제22호 **운전자 서명란**에 대응하며 **V154**로 서버에 저장됩니다.

| 항목 | 내용 |
|------|------|
| UI | **`fieldset`/`legend`「운전자 서명」** + **`Field`「서명 성명」/「서명일」** (Q450, `bfe0283`) — **저장·인쇄·텍스트 다운로드 전 필수** (정차별 법정 필드와 병행, Q439) |
| API | **`PUT /api/v1/transport/runs/{runId}/service-log`** — **`driverSignatoryName`·`driverSignedOn`** · 응답 **`driverSignatureComplete`** |
| 서버 검증 | 성명·서명일 **쌍 입력** · **서명일 ≤ 오늘** · **서명일 ≥ 운행일** · 불완전 쌍 **`422`** 「운전자 서명자명과 서명일을 함께 입력하세요.」 |
| 감사·출력 | **「일지 보관·감사 추적」** — **운전자 서명: 완료/미완료** · 인쇄·`.txt`에 **운전자 서명 성명/일** 포함 |

| 테스트 | **`TransportServiceLogPanel.test`** · **`TransportServiceTest.upsertServiceLogShouldPersistDriverSignature`**

> 관련: Q407 · Q439 · Q443 · USER_MANUAL §5-8-0-1 · DEPLOYMENT §11-3

### [TWR] Q446. **수칙·계약** 화면의 **「법정 이동서비스일지 입력 서식」** 카드는 무엇인가요?

**A.** **Fixed (FE `0df6902`, G15 v1.3-C, Q446)** — **`/transport/compliance`** 상단 **`TransportServiceLogLegalGuide`** 가 NHIS **별지 제22호** **필수 입력 항목**과 **작성 위치**를 안내합니다.

| 안내 내용 | 동작 |
|----------|------|
| 필수 항목 | 정차별 **실제 픽업·동승 여부·하차 시각** · 차량·운전자·동승 직원 · **저장 후 감사 이력** |
| **「확정 배차 선택」** | 페이지 내 **`#transport-service-log-runs`** — **`TransportServiceLogRunsPanel`** 로 스크롤 |
| **「배차 화면으로 이동」** | **`/transport`** — 확정 전 DRAFT 배차 확인 |

| 테스트 | **`TransportServiceLogLegalGuide.test`** · **`TransportCompliancePage.test`**

> 관련: Q407 · Q426 · Q445 · USER_MANUAL §5-8-0 · ADMIN_GUIDE §1-4

### [TWR] Q447. live E2E **`operationBlockers`** 배열은 **`operationBlocker`** 와 어떻게 다른가요?

**A.** **Fixed (BE `c5dd4f2`, QA-B95, Q447)** — **`operationBlocker`** 는 **첫 번째** 미충족 코드만, **`operationBlockers`** 는 **동시에 걸린 모든** prerequisite를 나열합니다.

| 예시 | `operationBlocker` | `operationBlockers` |
|------|-------------------|---------------------|
| staff creds + seed client 누락 | `staff-credentials-missing` | `["staff-credentials-missing","seed-client-missing"]` |
| 모두 충족 | `none` | `[]` |

| 노출 위치 | **`GET /api/v1/system/live-e2e/probe`** · **`GET /api/v1/health`** — **`liveE2eOperationBlockers`** |
| 용도 | QA-B95 triage — **한 번에 전체 blocker** 확인 후 env·seed 순서대로 해소 |

| 테스트 | **`LiveE2eControllerTest`** · **`HealthControllerTest`**

> 관련: Q441 · Q409 · Q437 · Q448 · DEPLOYMENT §3-6 · ADMIN_GUIDE §1-4

### [TWR] Q448. live E2E **`staff-bootstrap-error`** 와 **`staff-bootstrap-not-ready`** 는 무엇이 다른가요?

**A.** **Fixed (BE `d7f1a9a`·`0c9518a`·`20485f1`·**`24d25f1`**, QA-B135, Q448·Q596·Q631)** — bootstrap **시드가 아직 미완**이면 **`staff-bootstrap-not-ready`**(또는 guardian 쪽 **`guardian-bootstrap-not-ready`**)만 나옵니다.

**`24d25f1`부터 (Q631)**: **`staff-bootstrap-error`/`guardian-bootstrap-error`/`bootstrap-error` blocker 코드는 폐기**되었습니다. bootstrap **상태 조회 자체가 내부 오류**(`bootstrap=error`·`guardian-bootstrap=error`·**`: …` 접미사 포함**)여도 **`operationBlockers`에는 `-not-ready`만** 남고, **오류 구분은 status detail**로 합니다. **`0c9518a`부터** bootstrap error 시 **derived seed/schema blocker는 생략** · **`20485f1`부터** detail suffix도 error로 인식 (Q596).

| `liveE2eStatusDetail` / guardian status | `operationBlockers` (310차+) | root cause |
|----------------------------------------|------------------------------|------------|
| **`bootstrap=error`** | **`staff-bootstrap-not-ready`** (+ guardian not-ready) | detail에 **error** — blocker에는 **not-ready만** |
| **`guardian-bootstrap=error`** | **`guardian-bootstrap-not-ready`** (+ staff not-ready) | detail에 **error** — blocker에는 **not-ready만** |
| *(미완·credential 미설정)* | **`staff-bootstrap-not-ready`** 등 | detail **error 아님** |

| 예시 `operationBlockers` | 의미 |
|---------------------------|------|
| `["staff-bootstrap-not-ready","guardian-bootstrap-not-ready"]` | staff·guardian bootstrap **미준비 또는 내부 오류** — **detail**·**서버 로그·DB seed** 우선 점검 (Q631) |
| `["staff-credentials-missing","seed-client-missing"]` | credential·시드 이용자 **미설정** — **`scripts/dev-live-e2e.env`** 확인 |

| 노출 | **`GET /api/v1/system/live-e2e/probe`** · **`GET /api/v1/health`** — **`liveE2eOperationBlockers`** · **`liveE2eStatusDetail`**
| 테스트 | **`HealthControllerTest.healthShouldSurfaceLiveE2eBootstrapErrorsWhenStatusProbesFail`** · **`LiveE2eControllerTest.probeShouldSurfaceBootstrapErrorsInOperationBlockers`** · **`LiveE2eOperationReadinessSupportTest`** (Q631)

> 관련: Q441 · Q447 · Q631 · DEPLOYMENT §3-6 · ADMIN_GUIDE §1-4

### [TWR] Q449. **통합식사도움기록** 등록 시 이용자를 고를 수 없어요. (L02_M13)

**A.** **Fixed (FE `1c8f236`, L02_M13, Q449)** — 이용자 목록 API가 **camelCase 대신 snake_case**(`client_id`·`client_name`)를 쓰거나 **`{ items: [...] }`** 로 감싸져 있어도 **`MealAssistanceRecordPage`** 가 **`normalizeClient`** 로 **드롭다운·등록 `clientId`** 를 정규화합니다.

| 증상 | 조치 |
|------|------|
| 이용자 **select** 옵션 없음 | 브라우저 **강력 새로고침** 후 재시도 — FE **`1c8f236`** 이상 배포 확인 |
| 등록 클릭 후 **`clientId` 누락** | **`GET /api/v1/clients`** 응답에 **식별자 필드**(`id`·`clientId`·`client_id`) 존재 여부 확인 |
| 「이용자 목록 응답 형식이 올바르지 않습니다.」 | 목록이 **배열·`items[]` 둘 다 아님** — Q442 참고 |

| 구현 | **`normalizeClientList`** — raw 배열 또는 **`items[]`** · **`normalizeClient`** — `id` ← `clientId` ← `client_id` |
| 테스트 | **`MealAssistanceRecordPage.test`** — **`creates a record when client payload uses snake_case fields`**

> 관련: Q366 · Q442 · USER_MANUAL §5-27 · ADMIN_GUIDE §6-2-18

### [TWR] Q450. 이동서비스일지 **「운전자 서명 성명」** 라벨이 **「서명 성명」** 으로 바뀌었어요. (UXD-130)

**A.** **Fixed (FE `bfe0283`, UXD-130, Q450)** — WCAG **1.3.1 paired-field grouping** 을 위해 **「운전자 서명」** `fieldset`/`legend` 로 **성명·서명일** 을 묶었습니다. 개별 `Field` 라벨은 짧게 **「서명 성명」·「서명일」** 입니다. **저장·검증·API 필드명**(`driverSignatoryName`·`driverSignedOn`)은 **변경 없습니다** (Q445).

| 항목 | 내용 |
|------|------|
| 구조 | **`fieldset.ds-transport-log__signature-group`** + **`legend`「운전자 서명」** |
| CSS | **8종 `ds-transport-log__*`** — `document`·`heading`·`summary`·`footnote`·`no-print`·`actions`·`meta`·`signature-group` (FE-16) |
| 인쇄 | **`ds-transport-log__document`** forced-colors border · **`no-print`** 영역 인쇄 시 숨김 |
| 테스트 | **`TransportServiceLogPanel.test`** — **`getByLabelText("서명 성명")`·`pickDateByLabel(user, "서명일", …)`** |

> 관련: Q445 · Q439 · USER_MANUAL §5-8-0-1 · DEPLOYMENT §11-3

### [TWR] Q451. live E2E에서 **staff 로그인이 실패**해도 **guardian suite** 가 실행되나요?

**A.** **Fixed (FE `7424c30`, QA-B95, Q451)** — **`liveGlobalSetup`** 의 global **`skipped`** 는 **백엔드 `reachable`·`ready`만** 봅니다. **`staffAuthReady=false`** 여도 **`guardianAuthReady=true`** 이면 **guardian-only live suite** 가 **skip되지 않습니다**. staff 전용 suite는 **suite-level gate** 가 별도로 staff auth를 확인합니다.

| 이전 (`staffAuthReady` 포함) | 현재 (`7424c30`) |
|------------------------------|------------------|
| staff bootstrap/login 실패 → **전체 live E2E skip** | backend ready → **guardian suite 실행 가능** |
| guardian creds만 유효한 staging | **보호자 QR·포털 harness** 단독 검증 가능 |

| 상태 파일 | **`skipped: false`** when `reachable && ready` — **`staffAuthReady`**·**`guardianAuthReady`** 는 **별도 플래그** |
| 테스트 | **`liveE2eHarness.test`** — **`keeps backend gate open for guardian-only suites when staff auth is not ready`** |

> 관련: Q409 · Q428 · Q448 · DEPLOYMENT §3-6 · ADMIN_GUIDE §1-4

### [TWR] Q452. 방문요양 **RFID 전송 엑셀**과 **공단 계획 엑셀**을 비교하는 API가 있나요? (G21)

**A.** **Fixed (BE `eeac205` / FE `27c9de3`/`4a112fe`, G21 P1, Q452·Q456)** — **`POST /api/v1/visits/imports/rfid/compare`** 가 NHIS **급여계획(`planFile`)**·**RFID 전송(`rfidFile`)** xlsx를 업로드받아 **이지케어 schedule-rfid 매트릭스(COMP_01~09)** 차이를 집계합니다. **`/visits`** 하단 **`VisitRfidDiffComparePanel`** 에서 **화면 업로드·7-code 집계·행별 diff 표**로 확인할 수 있습니다. **`diffCodes`** 가 배열·쉼표 문자열·**표준 외 코드**여도 **chip·표에 누락 없이 표시**됩니다 (`4a112fe`, Q456).

| UI 절차 | **`/visits`** → **「RFID 계획·태그 비교 (7-code)」** → 급여계획·RFID 엑셀 선택 → **엑셀 비교** |
| 집계 | **COMP_01~09** `StatusBadge` chip + **인정번호·방문일·계획·태그·차이 코드** 표 |
| 테스트 | **`VisitRfidDiffComparePanel.test`** · **`VisitServiceTest.compareRfidTransmissionShouldReturnSevenCodeDiffMatrix`** · **`VisitLiveApiRoutingE2eTest`** (`9e050b1`, Q457)
...
| API | multipart — `branchId`·`planFile`·`rfidFile` — 응답 **`diffCodeCounts`**·**`rows[]`** |
| 권한 | **`branch_admin`·`social_worker`** |
| 테스트 | **`VisitServiceTest.compareRfidTransmissionShouldReturnSevenCodeDiffMatrix`** · **`VisitRfidDiffMatcherTest`** · **`MustApiEndpointRoutingTest`** · **`VisitRfidDiffComparePanel.test`** · **`VisitLiveApiRoutingE2eTest`** (`9e050b1`, Q457)

> 관련: Q377 · Q189 · **Q456(diff code rendering)** · USER_MANUAL §5-11 · ADMIN_GUIDE §1-4 · DEPLOYMENT §11-3

### [TWR] Q453. 방문일정 체크인 시 **퇴사한 담당 직원**이 배정되어 있으면 어떻게 되나요?

**A.** **Fixed (BE `0db1e68`, G21, Q453)** — **`assignedUserId`** 가 있는 일정에서 check-in/out 시 **배정 직원의 활성·지점 소속**을 **일정 배정 규칙과 동일하게** 재검증합니다.

| 상황 | 오류 메시지 |
|------|------------|
| 배정 직원 **비활성** 또는 **`terminatedAt` 존재** | 「퇴사 또는 비활성화된 직원은 배정할 수 없습니다.」 |
| 배정 직원이 **해당 `branchId` 미소속** | 「해당 지점 직원만 방문일정에 배정할 수 있습니다.」 |
| 조치 | **담당 직원 재배정** 또는 **퇴사 처리 전 일정 취소·재등록** 후 체크인 |

| 테스트 | **`VisitServiceTest.checkInShouldRejectWhenAssignedUserIsInactive`** · **`checkInShouldRejectWhenAssignedUserIsNotInBranch`** |

> 관련: Q307 · Q304 · USER_MANUAL §5-11 · ADMIN_GUIDE §10-12

### [TWR] Q454. **신체제재 기록** 화면에서 이용자 목록이 비거나 오류만 보여요. (L02_M07)

**A.** **Fixed (FE `4a47675`, L02_M07, Q454)** — **`BodyRestraintRecordPage`** 가 이용자·기록 API 응답의 **`client_id`/`client_name` snake_case**·**`items[]` 페이지 래핑**을 수용합니다. 응답 형식이 깨진 경우 **빈 목록 대신** 「이용자 목록 응답 형식이 올바르지 않습니다…」·「신체제재 기록 조회 응답 형식이 올바르지 않습니다…」 **Alert**를 표시합니다.

| 화면 | **`/care/body-restraint`** — **「새 제재 기록」** 이용자 선택·목록 조회 |
| 패턴 | L02_M13 **통합식사도움** snake_case 수용(Q449)과 동일 **`normalize*List`** 패턴 |
| 테스트 | **`BodyRestraintRecordPage.test`** — snake_case client payload · malformed response |

> 관련: Q361 · Q449 · USER_MANUAL §5-24 · ADMIN_GUIDE §6-2-14

### [TWR] Q455. **사회복지사**인데 방문일정 **체크인·체크아웃**이 「배정된 요양보호사만…」 **422** 로 거절돼요.

**A.** **Fixed (BE `78cfb8a`, G21, Q455)** — 감독 역할(`hq_admin`·`branch_admin`·`social_worker`) 판별 시 JWT **`roleCode`를 trim+소문자 정규화**합니다. **`SOCIAL_WORKER`**·**` social_worker `** 등 **대소문자·공백 차이**로 감독 권한이 빠지던 회귀를 수정했습니다.

| 감독 역할 | 체크인·체크아웃 |
|----------|----------------|
| **`hq_admin`·`branch_admin`·`social_worker`** | 배정 직원 **활성·지점 소속**이면 **담당과 무관**하게 가능 (Q307) |
| **`caregiver`** | **본인 배정 일정만** — 배정 직원 ≠ 로그인 사용자 시 **`422`** |

| 테스트 | **`VisitServiceTest.checkInShouldAllowSocialWorkerForDifferentAssignedUser`** · **`checkInShouldAllowSocialWorkerWithUppercaseRoleCode`**

> 관련: Q307 · Q453 · USER_MANUAL §6-3-1 · ADMIN_GUIDE §10-12

### [TWR] Q456. **RFID 7-code 비교**에서 일부 **차이 코드 chip**이나 **행별 badge**가 보이지 않아요.

**A.** **Fixed (FE `4a112fe` → deepen `570912e`, G21, Q456)** — **`VisitRfidDiffComparePanel`** · **`visits.js` `normalizeVisitRfidDiffCode`** 가 **`diffCodes[]` 배열**·**쉼표 구분 문자열**·**소문자·공백·`COMP_4`/`comp-4` 변형**을 **표준 `COMP_04` 등**으로 정규화해 badge·집계 chip에 표시합니다. 행별 집계와 서버 **`diffCodeCounts`** 를 **병합**해 집계 chip 누락을 방지합니다.

| 증상 | 조치 |
|------|------|
| API는 diff가 있는데 UI chip 0건 | **232차 FE** — unknown code·comma payload **정규화 후 표시** |
| 집계 chip과 행별 badge 불일치 | **`normalizedDiffCodeCounts`** — 행 집계 + 서버 count **max 병합** |

| 테스트 | **`VisitRfidDiffComparePanel.test`** — comma·unknown diff code rendering |

> 관련: Q452 · USER_MANUAL §5-11 · ADMIN_GUIDE §1-4

### [TWR] Q457. **live E2E**·스테이징에서 **방문일정(G21) API**가 「방문요양 지점에서만…」 **422** 로 실패해요.

**A.** **Fixed (BE `9e050b1`/`cc295ec`, QA-B95, Q457·Q501)** — 레거시 live E2E seed가 **`DAY_CARE` 지점**이면 bootstrap 시 **`HOME_VISIT`으로 승격**합니다 (Q457). **248차**부터 legacy **`DAY_CARE`** 는 bootstrap 승격·paired visit seed 완료 전까지 **G21 seed applicable** 로 **`operationReady=false`** 를 유지합니다 — false-green gate 방지 (Q501).

| 항목 | 내용 |
|------|------|
| bootstrap | **`LiveE2eBootstrapService.activateBranch`** — `serviceType=DAY_CARE` → **`HOME_VISIT`** |
| G21 seed gate | **`isG21SeedApplicableBranch`** — legacy **`DAY_CARE`** applicable until upgrade (Q501) |
| paired seed | **`ensureSampleVisitSchedules`** — PLAN+BILLING upsert (Q500) |
| 기본 seed | 신규 branch **`DEFAULT_BRANCH_SERVICE_TYPE=HOME_VISIT`** |
| 테스트 | **`LiveE2eBootstrapServiceTest`** · **`VisitLiveApiRoutingE2eTest`** |

| 확인 | **`./scripts/run-live-e2e.sh`** 또는 **`npm run test:live-e2e`** — visit·RFID compare routing PASS |

> 관련: Q180 · Q248 · Q409 · Q500~Q501 · DEPLOYMENT §3-6 · ADMIN_GUIDE §1-4

### [TWR] Q458. 배차 **경유지(WAYPOINT)** 에 **공백만** 주소를 넣으면 저장되나요? (V155)

**A.** **Fixed (BE `64c4c80`, US-T02, Q458)** — **아니요.** V151은 `waypoint_address IS NOT NULL`만 강제해 **공백 문자열**을 허용했으나, **V155** `chk_transport_run_stops_waypoint_address_nonempty` 가 **`btrim(waypoint_address) <> ''`** 를 추가합니다. **지오코딩 실패·이동서비스일지(별지 제22호) 공란**을 DB 레벨에서 차단하는 **defense-in-depth** 입니다. **CLIENT/BRANCH** 정차에는 영향 없습니다.

| 계층 | 동작 |
|------|------|
| FE | **`TransportAddWaypointModal`** — 주소 미입력·공백만 입력 시 클라이언트 검증 |
| BE | **`TransportService`** — whitespace-only 주소 **`422`** 「경유지 주소를 입력하세요.」 — **createRun·updateRun** 모두 적용 (Q467, `a179256`) |
| DB | **V155** CHECK — 공백-only WAYPOINT insert/update 거부 |

> 관련: Q421 · USER_MANUAL §5-8 · DEPLOYMENT §1-3

### [TWR] Q459. 교육일지(8-7)에서 **「PDF 8-7 필수 교육이 미작성입니다」** Alert가 뭐예요? (G41)

**A.** **Fixed (FE `caa215f`, G41, Q459)** — 케어포 PDF p.104 기준 **작성 필수**인 **재난상황대응**·**소화·경보설비** 교육이 **해당 연도 미충족**이면 **`/staff/training-logs`** 상단에 warning Alert가 표시됩니다. **`buildMandatoryTrainingUnwrittenAlerts`** 가 compliance API의 **`disasterResponseAnnualMet`·`fireSafetyEquipmentAnnualMet`** 를 검사합니다. StatCard에는 미충족 시 **「미작성」** unit이 붙습니다.

| 대상 | 주기 | Alert 대상 |
|------|------|-----------|
| `DISASTER_RESPONSE` | 연 1회 | ✅ |
| `FIRE_SAFETY_EQUIPMENT` | 연 1회 | ✅ |
| 노인인권·운영규정·직원권익 | 각각 | StatCard만 (Alert 제외) |

> 관련: Q321 · Q325 · USER_MANUAL §5-14 · ADMIN_GUIDE §1-4

### [TWR] Q460. **보수교육(8-7-1)** 화면에서 **엑셀 다운로드**가 가능한가요? (US-S02)

**A.** **Fixed (FE `caa215f`, US-S02, Q460)** — **`/staff/training`** **`StaffRefresherTrainingPage`** 우측 **「엑셀 다운로드 (8-7-1)」** 버튼으로 **케어포 8-7-1 리포트** CSV를 내려받습니다. **`staffRefresherTrainingReport.js`** 가 compliance 목록을 **직원별 이수 현황** 형태로 직렬화합니다. 목록이 비어 있으면 버튼이 비활성화됩니다.

| 항목 | 내용 |
|------|------|
| 형식 | CSV (브라우저 다운로드) |
| 데이터 | `GET /staff/refresher-training/compliance` summary **`items[]`** |
| 테스트 | **`StaffRefresherTrainingPage.test`** — export from compliance summary |

> 관련: Q294 · Q295 · USER_MANUAL §5-3

### [TWR] Q461. 대시보드 **「직원교육 미충족」** 위젯은 무엇을 세나요? (G41)

**A.** **Fixed (FE `9e91e6a`, G41, Q461)** — **`/dashboard`**·**`/dashboard/hq`** 에 **`hq_admin`·`branch_admin`·`social_worker`** 전용 **「직원교육 미충족」** StatCard가 표시됩니다. **`countStaffTrainingComplianceGaps`** 가 **노인인권 상·하반기**·**운영규정 연간**·**G41b 3종 연간**·**신규 7일 오리엔테이션**·**FAQ21808 주제** 중 미충족 **항목 수**를 집계합니다. 1건 이상이면 **danger** 톤 · 클릭 **`/staff/training-logs`**.

| 폴백 | 대시보드 스냅샷에 **`staffTrainingComplianceGapCount`** 없으면 **`GET /staff/training-logs/compliance`** 병렬 조회 |
| 실패 | **「일부 준수지표를 불러오지 못했습니다: 직원교육 준수」** warning |

> 관련: Q321 · Q459 · USER_MANUAL §4-2

### [TWR] Q462. 교육일지 Alert·RFID compare 패널 **접근성(a11y)** 이 개선되었나요? (UXD-131)

**A.** **Fixed (FE `7f94654`, UXD-131, Q462)** — WCAG **1.3.1·4.1.2** 정합을 위해 라벨·연결이 보강되었습니다.

| 화면 | 변경 |
|------|------|
| **`/staff/training-logs`** | mandatory Alert **`id="staff-training-mandatory-alert"`** · StatCard **`aria-describedby`** 연결 |
| **`/visits` RFID compare** | 패널 **`aria-label="RFID 계획·태그 엑셀 비교"`** · diff code **`Field` label** |

> 관련: Q459 · Q452 · Q456 · USER_MANUAL §5-11·§5-14

### [TWR] Q463. 이동서비스일지 **인쇄**에 **픽업 주소** 열이 추가되었나요? (G15)

**A.** **Fixed (FE `a1d6e32`/`7de5a6f`, G15 v1.3-C, Q463·Q466)** — **`/transport/runs/:runId`** **`TransportServiceLogPanel`** 의 **인쇄**·**텍스트 다운로드**에 정차별 **픽업 주소** 열이 포함됩니다. **`7de5a6f`** 부터 **입력 화면** 정차별 **read-only「탑승 장소」** 도 동일 값을 표시합니다. **`transportServiceLog.js`** 가 API note의 **`픽업:`** prefix·stop payload **`pickupAddress`** 를 merge합니다. 별지 제22호 **주소란** 대응입니다.

| 출력 | 픽업 주소 |
|------|----------|
| 인쇄 (`ds-transport-log-printing`) | 표 **픽업 주소** 열 |
| 텍스트 다운로드 | 파이프 구분 행에 주소 포함 |

> 관련: Q463 · Q407 · Q445 · USER_MANUAL §5-8-0-1

### [TWR] Q464. **live E2E** bootstrap 후 **방문일정 목록이 비어** 있어요. (G21)

**A.** **Fixed (BE `dac19d3`, QA-B95, Q464)** — Q457 **`HOME_VISIT` 승격** 이후에도 visit harness용 seed가 없으면 list/detail live test가 skip될 수 있습니다. **`LiveE2eBootstrapService`** 가 **당월 DRAFT PLAN visit** 을 upsert합니다.

| 항목 | 내용 |
|------|------|
| seed | **DRAFT** · **PLAN** 상태 visit · 현재 월 |
| 테스트 | **`LiveE2eBootstrapServiceTest`** |

> 관련: Q457 · Q180 · DEPLOYMENT §11-3

### [TWR] Q465. **live E2E** 에서 **배차 명단·이동서비스** seed가 없어요. (US-T01/T02)

**A.** **Fixed (BE `2d98040`, QA-B95, Q465)** — bootstrap seed 이용자에 **`usesTransport=true`** · **pickup address/coordinates** · **default pickup time** 을 설정합니다. **`dac19d3` visit seed** 와 함께 US-T01/T02 transport roster·write live harness가 seed tenant에서 동작합니다.

| 필드 | 용도 |
|------|------|
| `usesTransport` | 배차 명단 대상 |
| pickup address/coords | geocode·roster 표시 |
| default pickup time | 계획 픽업 시각 seed |

| 테스트 | **`LiveE2eBootstrapServiceTest`** |

> 관련: Q433 · Q457 · Q464 · DEPLOYMENT §11-3 · ADMIN_GUIDE §1-4

### [TWR] Q466. 이동서비스일지 **입력 화면**에도 **탑승 장소·시간 준수**가 보이나요? (G15)

**A.** **Fixed (FE `7de5a6f`/`07be394`, G15 v1.3-C, Q466·Q468)** — Q463에서 **인쇄·텍스트 다운로드**에 추가된 **픽업 주소**·**시간 준수** 열이, **정차별 입력 `fieldset`** 에도 **read-only `Field`** 로 표시됩니다. **`07be394`** 부터 **운행 방향**에 따라 라벨이 전환됩니다 — **승차(PICKUP)** 는 **탑승 장소·계획/실제 픽업**, **하차(DROPOFF)** 는 **하차 장소·계획/실제 하차**. **「비고」** read-only Field도 추가되었습니다 (Q468). 화면에서 보는 항목과 출력물 열 구성이 **1:1** 로 맞습니다.

| 정차별 Field (PICKUP) | 편집 | 내용 |
|-------------|:----:|------|
| **「{이용자명} 탑승 장소」** | read-only | 배차·이용자 **픽업 주소** — 별지 제22호 주소란 (Q470 API `pickupAddress`) |
| **「{이용자명} 계획 픽업」** | read-only | 배차 계획 시각 |
| **「{이용자명} 실제 픽업」** | 입력 | 실제 탑승 시각 — 입력 시 **시간 준수** 즉시 갱신 |
| **「{이용자명} 시간 준수」** | read-only | 계획 대비 **±15분** 기준 **「준수」/「지연」** |
| **「{이용자명} 동승 여부」** | 입력 | 예/아니오 |
| **「{이용자명} 하차 시각」** | 입력 | 실제 하차 시각 |
| **「{이용자명} 비고」** | read-only | 특이사항 — 픽업 주소와 중복 `픽업:` prefix 자동 제거 (Q468) |

| DROPOFF 루트 | **탑승 장소 → 하차 장소** · **계획/실제 픽업 → 계획/실제 하차** 로 라벨 전환 (`transportServiceLogFieldLabels`) |
| 픽업 주소 없음 | help **「픽업 주소가 없습니다. 이용자 프로필·배차 정차를 확인하세요.」** |
| 테스트 | **`TransportServiceLogPanel.test`** · **`transportServiceLog.test`** |

> 관련: Q463 · Q468 · Q470 · Q407 · Q445 · USER_MANUAL §5-8-0-1

### [TWR] Q467. **V155 경유지 주소 검증**이 **배차 수정(updateRun)** 에도 적용되나요?

**A.** **Fixed (BE `a179256`, US-T02, Q467)** — Q458 **V155**·서비스 레이어 검증은 **신규 배차(createRun)** 뿐 아니라 **기존 DRAFT 수정(updateRun)** 에도 동일합니다. **`a179256`** 에 regression test가 추가되었습니다.

| 테스트 | 검증 내용 |
|--------|----------|
| **`TransportServiceTest.updateRunShouldRejectBlankWaypointAddress`** | 공백-only waypoint → **`422`** · stop delete/save 미실행 |
| **`TransportServiceTest.createRunShouldTrimWaypointAddressBeforePersist`** | 앞뒤 공백 trim 후 persist |
| **`TransportPilotServiceFlowE2eTest.usT02UpdateRunShouldRejectBlankWaypointAddress`** | US-T02 pilot flow update 경로 |
| **`LiveE2eBootstrapServiceTest`** | bootstrap client pickup address **PII encrypt** assert |

| 메시지 | 「경유지 주소를 입력하세요.」 |
| DB | **V155** `btrim(waypoint_address) <> ''` — defense-in-depth |

> 관련: Q458 · Q421 · USER_MANUAL §5-8 · DEPLOYMENT §11-3 · ADMIN_GUIDE §1-4

### [TWR] Q468. **하차(DROPOFF)** 배차 일지에서 Field 라벨이 **「하차 장소」** 로 바뀌나요? (G15)

**A.** **Fixed (FE `07be394`, G15 v1.3-C, Q468)** — **`transportServiceLogFieldLabels(run.direction)`** 가 운행 방향에 따라 정차별 Field·인쇄 표 헤더 라벨을 전환합니다.

| 방향 | 장소 | 계획 | 실제 |
|------|------|------|------|
| **PICKUP** (승차) | 탑승 장소 | 계획 픽업 | 실제 픽업 |
| **DROPOFF** (하차) | 하차 장소 | 계획 하차 | 실제 하차 |

| **「비고」** | read-only — **`formatServiceLogRemark(note, pickupAddress)`** 가 note의 **`픽업:`** prefix가 탑승/하차 장소와 동일하면 **빈 값**으로 표시해 중복 방지 |
| 테스트 | **`TransportServiceLogPanel.test`** — dropoff direction labels·remark in print table · **`transportServiceLog.test`** — remark dedup·label helpers |

> 관련: Q466 · Q463 · USER_MANUAL §5-8-0-1

### [TWR] Q469. 이동서비스일지 **인쇄·다운로드**에 **기관(지점) 연락처**가 나오나요? (G15)

**A.** **Fixed (BE `a8e2bb2` + FE `b1a16ff`, G15 v1.3-C, Q469)** — **`GET /api/v1/transport/runs/{runId}/service-log`** 응답에 지점 연락처가 포함되고, 프론트 **인쇄·텍스트 다운로드** 헤더에 그대로 노출됩니다.

| API 필드 | 출처 | 인쇄·txt 표시 |
|----------|------|--------------|
| **`branchName`** | 지점명 | 기관명 |
| **`branchAddress`** | 지점 **`addressLine1`** | 주소 |
| **`branchRegionPath`** | **`regionDongCode`** → 행정동 경로 | 지역 |
| **`branchPhone`** | **`phoneEncrypted`** 복호화 | 대표 연락처 (`tel:` 링크) |

| FE | **`normalizeServiceLogBranchContact`** · **`transportServiceLog.js`** build printable text |
| 빈 값 | 해당 항목은 인쇄 헤더에서 **생략** |
| 테스트 | **`TransportServiceTest`** · **`TransportControllerRoutingTest`** · **`TransportServiceLogPanel.test`** |

> **운영 팁**: 지점 연락처가 비어 있으면 **`/branches`** 에서 지점 **주소·행정동·대표 전화**를 먼저 등록하세요.

> 관련: Q407 · USER_MANUAL §5-8-0-1 · ADMIN_GUIDE §1-4

### [TWR] Q470. 일지 API의 **픽업 주소**는 note 파싱만 쓰나요? (G15)

**A.** **Fixed (BE `e358f2d`, G15 v1.3-C, Q470)** — 정차별 **`TransportServiceLogRowResponse.pickupAddress`** 전용 필드가 추가되었습니다. FE는 **API `pickupAddress`를 우선**하고, legacy **`note`의 `픽업:`/`하차:` prefix** 는 하위 호환용으로만 유지합니다 (Q471 — **DROPOFF** 루트는 **`하차:`** prefix).
...
> 관련: Q463 · Q466 · Q471 · USER_MANUAL §5-8-0-1 · API_SPEC §12 transport service-log

### [TWR] Q471. 일지 API에 **운행 방향(`direction`)** 이 포함되나요? (G15)

**A.** **Fixed (BE `72124f7`, G15 v1.3-C, Q471)** — **`GET/PUT /api/v1/transport/runs/{runId}/service-log`** 응답 **`TransportServiceLogResponse`** 에 **`direction`** 필드(`PICKUP` 또는 `DROPOFF`)가 추가되었습니다. 별도 run 조회 없이 **별지 제22호 export·note prefix** 가 방향과 일치합니다.

| 필드 | 값 | 의미 |
|------|-----|------|
| **`direction`** | `PICKUP` | 승차(픽업) 루트 — 기본값(미설정 run도 `PICKUP`) |
| **`direction`** | `DROPOFF` | 하차 루트 |

| legacy **`note` prefix** | 방향 |
|--------------------------|------|
| **`픽업: {주소}`** | `PICKUP` |
| **`하차: {주소}`** | `DROPOFF` |

| FE | **`TransportServiceLogPanel`** — Field 라벨은 **`run.direction`** 기준 · API **`direction`** 은 export·integration용 |
| 테스트 | **`TransportServiceTest`** dropoff note prefix · **`TransportServiceLogLiveApiRoutingE2eTest`** contract |

> 관련: Q468 · Q470 · USER_MANUAL §5-8-0-1 · ADMIN_GUIDE §1-4

### [TWR] Q472. live E2E **bootstrap이 꺼져 있을 때** probe가 credential 누락을 여러 개 보여줘요. (QA-B95)

**A.** **Fixed (BE `8cf09d8`, QA-B95, Q472)** — **`ogada.live-e2e.bootstrap.enabled=false`** (또는 bootstrap service 미구성)일 때 **`GET /api/v1/system/live-e2e/probe`** 가 **`staff-credentials-missing`·`guardian-credentials-missing` 등 복수 blocker** 를 반환하던 동작을 수정했습니다. 이제 **`operationBlocker`·`operationBlockers` = `bootstrap-disabled` 단일**만 반환합니다 — **`GET /api/v1/health`** 의 operation gating과 **동일 의미**입니다.

| bootstrap 상태 | probe `operationBlocker` | probe `operationBlockers` |
|----------------|--------------------------|---------------------------|
| **disabled** | `bootstrap-disabled` | **`["bootstrap-disabled"]`만** (`8cf09d8`) |
| **enabled** | readiness·credential 조합 | 기존과 동일 (Q447·Q448) |

| 테스트 | **`LiveE2eControllerTest.probeShouldReturnBootstrapDisabledWhenBootstrapIsDisabled`** |

> 관련: Q419 · Q409 · DEPLOYMENT_GUIDE §9-1 · ADMIN_GUIDE §1-4

### [TWR] Q473. live E2E **guardian 로그인 fallback** 이메일이 bootstrap과 달라요. (QA-B138)

**A.** **Fixed (FE `94c65e2`, QA-B138, Q473)** — **`liveGlobalSetup.js`** 의 default guardian login fallback이 **`guardian@ogada.test`/`ogada1234`** 에서 **`live-e2e-guardian@ogada.test`/`ogada-guardian-e2e`** 로 변경되었습니다. **`LiveE2eBootstrapService`**·**`dev-live-e2e.env.example`**·**`GET /api/v1/health` `liveE2eDefaultGuardianCredentials`** 와 **일치**합니다.

| 항목 | 값 |
|------|-----|
| default guardian email | **`live-e2e-guardian@ogada.test`** |
| default guardian password | **`ogada-guardian-e2e`** |
| env override | **`LIVE_E2E_GUARDIAN_EMAIL`/`LIVE_E2E_GUARDIAN_PASSWORD`** — 설정 시 fallback **대체** |

| 증상 (수정 전) | env·bootstrap 없이 guardian suite login **401** · staff bootstrap **`guardianEmail`** 과 FE fallback **불일치** |
| 테스트 | **`liveE2eHarness.test`** — fallback creds로 login probe |

> 관련: Q428 · Q427 · DEPLOYMENT_GUIDE §4 · ADMIN_GUIDE §1-4

### [TWR] Q474. 방문일정 **일괄확정 사전 점검**에서 PLAN/BILLING 건수를 따로 볼 수 있나요? (G21)

**A.** **Fixed (BE `6aeafe7` → deepen `28860ae`/`f26abb0`, FE wire `f9ed97d`, G21, Q474·Q477)** — **`GET /api/v1/visits/confirm-readiness`** 응답 **`VisitConfirmReadinessResponse`** 가 **2-track 집계**를 제공합니다. 이지케어 **계획일정 vs 청구일정** split-view·**일괄확정 Modal**이 **클라이언트에서 재계산하지 않고** API만으로 준비도를 확인합니다.

| 필드 | 의미 |
|------|------|
| **`draftPlanCount`·`draftBillingCount`** | 기간 내 **PLAN/BILLING** `DRAFT` 건수 |
| **`confirmedPlanCount`·`confirmedBillingCount`** | **PLAN/BILLING** `CONFIRMED`/`IN_PROGRESS`/`COMPLETED` 건수 |
| **`pairedDivergedPlanCount`·`pairedDivergedBillingCount`** | track별 **페어 확정 상태 불일치** (Q477, `28860ae`) |
| **`unassignedDraftPlanCount`·`unassignedDraftBillingCount`** | track별 **담당자 미배정 DRAFT** (Q477, `28860ae`) |
| **`readyPlan`·`readyBilling`** | track별 **일괄확정 가능** — `draft*Count > 0` 이고 해당 track **페어 불일치 0** · **담당 미배정 0** (Q477, `f26abb0`/`5f710e3`) |

| UI | **`VisitBatchConfirmPanel`** Modal — **StatCard 그리드**(`buildKindReadinessStats`, `f9ed97d`) — DRAFT·확정·**확정 가능**·페어 불일치·담당 미배정 · **`aria-label`「계획·청구 유형별 일괄확정 현황」** |
| 테스트 | **`VisitServiceTest`** · **`VisitControllerRoutingTest`** · **`VisitBatchConfirmPanel.test`** |

> 관련: Q330 · Q376 · Q477 · USER_MANUAL §5-11 · ADMIN_GUIDE §1-4

### [TWR] Q475. 청구 상세 **「명세 4채널 발송」** 과 **「명세 인쇄 산출물」** 은 무엇이 다른가요? (G-7-1)

**A.** **Fixed (FE `50d330d`, G-7-1, Q475)** — 케어포 PDF **p.87 7-1 본인부담금 청구관리**를 **발송(①)** 과 **인쇄(②)** 로 분리했습니다.

| 패널 | 역할 | 화면 |
|------|------|------|
| **`BillingStatementDispatchPanel`** | **우편·문자·이메일·직접수령** 4채널 **발송 기록** | **「명세 4채널 발송」** (Q364) |
| **`BillingStatementPrintPanel`** | **주소라벨·급여비용명세서·영수증·청구리스트** **인쇄·PDF** | **「명세 인쇄 산출물 (7-1)」** (`50d330d`) |

| 인쇄 종류 | 비고 |
|----------|------|
| **주소라벨** | 보호자·우편 주소 |
| **급여비용명세서** | 이용자별 본인부담·등급·이용시간대 |
| **영수증** | **`PAID`+`paidAt`** 청구만 |
| **청구리스트** | 당월 청구 이용자 목록 |
| **전체 일괄 인쇄** | 4종 한 번에 Print Dialog — **`PAID`+`paidAt`** 아니면 **「전체 일괄 인쇄 (영수증 제외)」** (Q478, `f5639df`) |
| **명세서 PDF 다운로드** | **`downloadBillingClaimStatementPdfApi`** |

| 표시 조건 | **`CONFIRMED` 또는 `PAID`** + 급여 항목 1건 이상 |
| 테스트 | **`BillingStatementPrintPanel.test`** · **`billingStatementPrint.test`** · **`BillingDetailPage.test`** |

> 관련: Q364 · USER_MANUAL §4-6-0-1

### [TWR] Q476. 이동서비스 일지 **「시간 준수」** 표시와 SideNav **그룹 라벨**이 바뀌었어요. (UXD-132)

**A.** **Fixed (FE `f8321c7`, UXD-132, Q476)** — 접근성·시각 일관성 polish입니다.

| 화면 | 변경 |
|------|------|
| **`TransportServiceLogPanel`** | 정차별 입력 폼 **「시간 준수」** — 텍스트만 → **`StatusBadge`「준수」/「지연」** — **인쇄·txt 표와 동일 패턴** |
| **`TransportServiceLogPanel`** | **감사 경고 Alert** · **지점 대표 연락처 `tel:` 링크** a11y 보강 |
| **`SideNav`** | 그룹 토글 **`aria-label`「{그룹명} 메뉴 그룹, N개 항목」** · 현재 route 그룹 **`ds-sidenav__group--active` accent** · 항목 수 **시각 badge**(`aria-hidden`) |

| 테스트 | **`TransportServiceLogPanel.test`** · **`SideNav.test`** |

> 관련: Q466 · Q469 · Q331 · USER_MANUAL §3-1·§5-8-0-1

### [TWR] Q477. 방문일정 일괄확정 Modal에서 **계획·청구 track별「확정 가능」** 과 **차단 메시지**는 어떻게 표시되나요? (G21)

**A.** **Fixed (BE `28860ae`·`f26abb0`·`5f710e3`, FE `f9ed97d`, G21, Q477)** — 238차 **2-track 집계**(Q474)를 **운영 UI·track별 gate**까지 확장했습니다. **240차(`5f710e3`)** 에 **담당 미배정 DRAFT** 가 **`ready`/`readyPlan`/`readyBilling` 및 batch-confirm** 을 **차단**합니다.

| BE 응답 | 의미 |
|---------|------|
| **`readyPlan`** | PLAN track에 **확정할 DRAFT**가 있고 **PLAN 페어 불일치 0** · **PLAN 담당 미배정 0** (`5f710e3`) |
| **`readyBilling`** | BILLING track에 **확정할 DRAFT**가 있고 **BILLING 페어 불일치 0** · **BILLING 담당 미배정 0** (`5f710e3`) |
| **`blockers[]`** | 전체 **「DRAFT 없음」**·**「연결된 계획/청구 일정 상태 불일치」** + **track별** — 예: 「계획(PLAN) 일정: 담당자 미배정 3건」·「청구(BILLING) 일정: 연결된 계획 일정 상태 불일치 1건」 |

| FE Modal | **`VisitBatchConfirmPanel`** — **`buildKindReadinessStats`** — **계획(PLAN) DRAFT/확정/확정 가능** · **청구(BILLING) DRAFT/확정/확정 가능** StatCard · 페어 불일치·담당 미배정 **해당 track만** 추가 StatCard |
| batch-confirm 응답 | **`BatchConfirmVisitSchedulesResponse`** — **`confirmedPlanCount`·`confirmedBillingCount`** (`28860ae`) |
| batch-confirm 거부 | **담당 미배정 DRAFT** — **`422`** 「담당자가 미배정된 방문일정이 있어 일괄확정할 수 없습니다.」 (`5f710e3`) |

| 운영 팁 | **담당 미배정**은 `blockers[]` **경고**와 **`ready*` false**·**batch-confirm 거부**가 **동시** 적용됩니다 — 편집 화면에서 **담당 직원 배정** 후 재시도하세요. |
| 테스트 | **`VisitServiceTest.getConfirmReadinessShould*`** · **`batchConfirmShouldRejectWhenDraftAssignedUserMissing`** · **`VisitBatchConfirmPanel.test`** |

> 관련: Q474 · Q479 · Q330 · USER_MANUAL §5-11 · ADMIN_GUIDE §10-12

### [TWR] Q478. 청구 상세 **「전체 일괄 인쇄」** 버튼이 **「영수증 제외」** 로 보여요. (G-7-1)

**A.** **Fixed (FE `f5639df`, G-7-1, Q478)** — **영수증**은 **`PAID`+`paidAt`** 청구만 인쇄 가능합니다. **미수납(`CONFIRMED`)** 청구에서 **「전체 일괄 인쇄」** 를 누르면 **영수증 없이** 주소라벨·명세서·청구리스트만 출력됩니다 — 버튼·**`aria-label`** 이 **「전체 일괄 인쇄 (영수증 제외)」** 로 표시되어 **오해를 방지**합니다.

| 수납 상태 | 버튼 라벨 | 일괄 인쇄 포함 |
|----------|----------|---------------|
| **`PAID`+`paidAt`** | **「전체 일괄 인쇄」** (기본 라벨) | 주소라벨·명세서·**영수증**·청구리스트 |
| **`CONFIRMED`(미수납)** | **「전체 일괄 인쇄 (영수증 제외)」** | 주소라벨·명세서·청구리스트 |

| 테스트 | **`BillingStatementPrintPanel.test`** |
| 개별 버튼 | **「영수증 인쇄」** 는 미수납 시 **비활성** — 기존과 동일 (Q475) |

> 관련: Q475 · USER_MANUAL §4-6-0-1

### [TWR] Q479. 방문일정 **일괄확정 전 공단 명세 대조 API**는 어디서 호출하나요? (G21)

**A.** **Fixed (BE `03a052a`, G21, Q479)** — **`GET /api/v1/visits/nhis-comparison`** 가 **이용자별 방문일 수**와 **최신 NHIS import 배치의 `serviceDays`** 를 대조합니다. 이지케어 **schedule-fix `chk-ltm-fix`** parity — **일괄확정 Checkbox** 「공단 청구명세서와 비교했습니다.」 확인 **전** IT·운영 점검용입니다.

| 쿼리 | 설명 |
|------|------|
| **`from`·`to`** | **필수** — **동일 월**만 허용 · 월이 다르면 **`422`** 「공단 명세서 비교는 동일 월 범위에서만 가능합니다.」 |
| **`scheduleKind`** | 선택 — 기본 **`BILLING`** · `PLAN`/`BILLING` 정규화 |
| **`branchId`** | 선택 — JWT 스코프 내 **`HOME_VISIT` 지점** |

| 응답 요약 | **`matchedLineCount`·`discrepancyLineCount`·`missingNhisLineCount`·`extraNhisLineCount`** · **`nhisBatchPresent`·`nhisBatchId`·`yearMonth`** |
| **`items[]`** | **`clientId`·`clientName`·`visitDayCount`·`nhisServiceDays`·`serviceDaysMatch`·`overallMatch`·`nhisMatchStatus`** |
| 집계 규칙 | **`CANCELLED`** 일정은 **방문일 수에서 제외** · **`overallMatch`** = 방문일 > 0 **且** NHIS 행 존재 **且** 일수 일치 |

| UI | **243차(`797c529`/`ad18606`)**: **`VisitNhisComparisonPanel`** — **`/visits`** StatCard 4종·**수급자별 상세** (Q486) · **`VisitBatchConfirmPanel`** Modal — embed StatCard (Q484) · 불일치 시 **drill-down** (Q487) · **`GET /visits/nhis-comparison`** — 이용자별 상세 |
| 권한 | **`branch_admin`·`social_worker`** |
| 테스트 | **`VisitServiceTest.getNhisComparisonShould*`** · **`MustApiEndpointRoutingTest`** · **`RoleBasedControllerAccessTest$VisitAccess`** |

> 관련: Q477 · Q330 · Q264(청구 상세 `GET /billing/claims/{id}/nhis-comparison`) · USER_MANUAL §5-11 · ADMIN_GUIDE §10-12

### [TWR] Q480. **`/visits`** 에 **계획·청구·분할 비교** 링크가 생겼어요. 북마크할 수 있나요? (G21)

**A.** **Fixed (FE `3a27303`, G21, Q480)** — **`VisitsContextNav`** 가 **이지케어 이중 일정 nav** 와 동일하게 **3개 cross-link** 를 제공합니다.

| 링크 | URL | 동작 |
|------|-----|------|
| **계획 일정** | `/visits?kind=PLAN` | `PLAN` 탭·달력 |
| **청구 일정** | `/visits?kind=BILLING` | `BILLING` 탭 · **청구반영** 열 |
| **계획·청구 분할 비교** | `/visits?split=1` | RFID split-view · 기존 **`Switch`** 와 **URL 동기화** |

| 구현 | **`visitScheduleNav.js`** — **`parseVisitScheduleSearchParams`** · **`buildVisitScheduleSearch`** · **`VisitsPage`** — **`useSearchParams`** |
| 테스트 | **`VisitsContextNav.test`** · **`visitScheduleNav.test`** · **`VisitsPage.test`** |

> 관련: Q377 · Q480 · USER_MANUAL §5-11 · ADMIN_GUIDE §10-12

### [TWR] Q481. 일괄확정 **사전 점검 API**에 **공단 명세 집계**가 포함되나요? (G21)

**A.** **Fixed (BE `8a8c5b3`/`4046046`, G21, Q481·Q483)** — **`GET /api/v1/visits/confirm-readiness`** 응답에 **`nhisComparisonSummary`** (`VisitNhisComparisonSummary`) 가 **동일 월** `from`/`to` 범위일 때 **자동 포함**됩니다. **`GET /visits/nhis-comparison` 별도 호출 없이** 이지케어 **schedule-fix `chk-ltm-fix`** parity 집계를 확인할 수 있습니다.

| 필드 | 의미 |
|------|------|
| **`yearMonth`·`overallMatch`** | 대상 월 · **전 라인 일치 여부** (`4046046`, Q483) |
| **`nhisBatchPresent`·`nhisBatchId`** | 최신 NHIS import 배치 존재 여부·ID |
| **`matchedLineCount`·`discrepancyLineCount`** | 이용자별 방문일수 vs import **일치·불일치** 건수 |
| **`missingNhisLineCount`·`extraNhisLineCount`** | ogada만·NHIS만 존재하는 이용자 건수 |

| 조건 | 동작 |
|------|------|
| **동일 월** `from`/`to` | **`nhisComparisonSummary` 객체** — `GET /visits/nhis-comparison` 과 **동일 집계** |
| **월 교차** 범위 | **`nhisComparisonSummary` = `null`** — 별도 API도 **`422`** (Q479) |

| **`blockers[]` (Q483)** | import 배치 없음·불일치 건수 등 **정보성 NHIS 메시지** — **`ready=true`와 공존** 가능 · **확정 차단**(담당 미배정·페어 불일치)과 **별도**로 읽으세요 |
| UI | **Fixed (FE `68a4e35`/`ad18606`, Q484·Q487)** — **`VisitBatchConfirmPanel`** Modal **StatCard 4종** · **수급자별 drill-down** · **`VisitNhisComparisonPanel`** 사전 비교 (`797c529`, Q486) · **`overallMatch` success/warning Alert** |
| 테스트 | **`VisitServiceTest.getConfirmReadinessShouldExposeNhisComparisonSummary*`** · **`VisitControllerRoutingTest.confirmReadinessRouteShouldSerializeNhisComparisonSummary`** · **`VisitBatchConfirmPanel.test`**

> 관련: Q479 · Q483 · Q484 · Q477 · Q330 · USER_MANUAL §5-11 · ADMIN_GUIDE §10-12

### [TWR] Q482. RFID 엑셀 비교 후 **「차이가 없습니다」** 초록 안내가 나와요. (G21)

**A.** **Fixed (FE `f232285`, G21, Q482)** — **`VisitRfidDiffComparePanel`** 에서 **7-code diff 0건**이면 **success Alert** 가 표시됩니다 — 「업로드한 급여계획과 RFID 전송 내역 사이에 차이가 없습니다.」

| 상황 | 표시 |
|------|------|
| 비교 완료·**차이 0건** | **success Alert** (Q482) + 상단 **info Alert** (비교 행 수 요약) |
| 비교 완료·**차이 1건 이상** | **7-code 집계 chip** · **행별 diff 표** (Q452·Q456) |
| 업로드·API 오류 | **danger Alert** — 오류 메시지 |

| 테스트 | **`VisitRfidDiffComparePanel.test`** — no-diff success alert regression |

> 관련: Q452 · Q456 · USER_MANUAL §5-11 · ADMIN_GUIDE §10-12

### [TWR] Q483. 일괄확정 **blockers**에 **공단 import 없음** 안내가 뜨는데 **확정 가능**으로 보여요. (G21)

**A.** **Fixed (BE `4046046`, G21, Q483)** — **`appendNhisComparisonBlockers`** 가 **정보성 NHIS 메시지**를 **`blockers[]`** 에 추가합니다. **담당 미배정·페어 불일치**처럼 **`ready=false`를 강제하지 않습니다**.

| 메시지 예 | 의미 |
|----------|------|
| 「공단 명세서 import 배치가 없습니다. NHIS 엑셀을 import한 뒤 비교해주세요.」 | **`nhisBatchPresent=false`** |
| 「공단 명세서 비교: 불일치 N건, NHIS 누락 N건, …」 | **`overallMatch=false`** — 건수 요약 |

| **`nhisComparisonSummary`** | **`yearMonth`·`overallMatch`** — **`overallMatch=true`** = 배치 존재 + 불일치·누락·extra **0건** + **matched > 0** |
| UI | Modal **StatCard**·warning/success Alert (Q484) · **수급자별 상세** drill-down (Q487) · **`/visits` 사전 비교 패널** (Q486) · **Checkbox ack** 전 **내용 확인** 권장 |
| 테스트 | **`VisitServiceTest`** · **`VisitPilotServiceFlowE2eTest`** (`39fa41a`) |

> 관련: Q481 · Q484 · Q479 · USER_MANUAL §5-11

### [TWR] Q484. 일괄확정 Modal에서 **공단 명세 StatCard**를 볼 수 있나요? (G21)

**A.** **Fixed (FE `68a4e35`/`ad18606`, G21, Q484·Q487)** — **`VisitBatchConfirmPanel`** 이 **`readiness.nhisComparisonSummary`** embed로 **일치·불일치·NHIS 누락·일정 외 NHIS** StatCard를 표시합니다 — **집계는 별도 API 호출 없음**. 불일치 시 **「수급자별 상세 보기」** 로 **`VisitNhisComparisonDetail`** 표를 펼칩니다.

| 상황 | Modal 표시 |
|------|-----------|
| import 배치 **없음** | info Alert — 「NHIS import 배치가 없어…」 |
| 배치 **있음·전 라인 일치** | StatCard 4종 + **success Alert** |
| 배치 **있음·불일치/누락** | StatCard 4종 + **warning Alert** + **수급자별 상세** expand (`ad18606`, Q487) |

| a11y | **`aria-label`「공단 명세 비교 요약」** · **「수급자별 상세 보기」** `aria-expanded`/`aria-controls`** · 확정 버튼 **`aria-describedby`** 에 NHIS 섹션 id 연결 |
| 테스트 | **`VisitBatchConfirmPanel.test`** · **`VisitNhisComparisonDetail.test`**

> 관련: Q481 · Q483 · Q479 · Q486 · USER_MANUAL §5-11

### [TWR] Q485. 미수납 청구 **영수증 인쇄** 버튼이 비활성인데 스크린 리더 안내는 어디서 보나요? (G-7-1, UXD-133)

**A.** **Fixed (FE `265fc42`, UXD-133, Q485)** — **`BillingStatementPrintPanel`** — **`CONFIRMED`(미수납)** 청구에서 **「영수증 인쇄」**·**「전체 일괄 인쇄 (영수증 제외)」** 버튼이 **`aria-describedby="billing-statement-print-receipt-unavailable"`** 로 **info Alert** 와 연결됩니다.

| 항목 | 내용 |
|------|------|
| Alert | 「수납 완료된 청구서만 영수증을 인쇄할 수 있습니다.」 |
| 인쇄 CSS | **`BILLING_CLAIM_PRINT_BODY_CLASS`** 토큰 — `@media print` body class 일관 |
| 기록 화면 | L02/L03 다수 페이지 **`ds-visually-hidden`** 정의 누락 수정 — 스크린 리더 전용 텍스트 정상 노출 |
| 테스트 | **`BillingStatementPrintPanel.test`** — receipt-unavailable **`aria-describedby`** regression |

> 관련: Q475 · Q478 · USER_MANUAL §4-6-0-1

### [TWR] Q486. **`/visits`** 에 **「공단 명세 사전 비교」** 패널이 생겼어요. (G21)

**A.** **Fixed (FE `797c529`, G21, Q486)** — **`VisitNhisComparisonPanel`** 이 **일괄확정 Modal 밖**에서도 **동일 월·`scheduleKind`** 기준으로 **`GET /api/v1/visits/nhis-comparison`** 결과를 표시합니다. 이지케어 **schedule-fix `chk-ltm-fix`** parity — 월말 **일괄확정 전** 선행 점검용입니다.

| UI | **StatCard 4종**(일치·불일치·NHIS 누락·일정 외 NHIS) · **「비교 새로고침」** · 불일치 시 **「수급자별 상세 보기」** (`VisitNhisComparisonDetail`, Q487) |
| 공유 로직 | **`visitNhisComparison.js` `buildNhisComparisonView`** — Modal embed·패널 **동일 StatCard 뷰** |
| 위치 | **`VisitsPage`** — **`VisitBatchConfirmPanel`** 위 · **`NhisScheduleConfirmLockGuide`** 아래 |
| 테스트 | **`VisitNhisComparisonPanel.test`** · **`visitNhisComparison.test`** · **`VisitsPage.test`**

> 관련: Q479 · Q484 · Q487 · USER_MANUAL §5-11 · ADMIN_GUIDE §10-12

### [TWR] Q487. 일괄확정·사전 비교에서 **수급자별 공단 명세 상세**를 볼 수 있나요? (G21)

**A.** **Fixed (FE `ad18606`/`797c529`, G21, Q487)** — **`VisitNhisComparisonDetail`** 이 **`GET /api/v1/visits/nhis-comparison`** 응답 **`items[]`** 를 **이용자별 표**로 표시합니다.

| 위치 | **일괄확정 Modal** — 불일치 시 **「수급자별 상세 보기」** · **`/visits` 사전 비교 패널** — 동일 (Q486) |
| 표 열 | **수급자** · **일정 일수**(`visitDayCount`) · **공단 일수**(`nhisServiceDays`) · **상태**(`StatusBadge`, `MATCH_STATUS`) |
| 불일치 행 | **`ds-row--warning`** · 스크린 리더 **「공단 일수와 불일치」** 보조 텍스트 |
| import 없음 | info Alert + **`/billing/imports/nhis`** 링크 |
| 테스트 | **`VisitNhisComparisonDetail.test`** · **`VisitBatchConfirmPanel.test`**

> 관련: Q479 · Q484 · Q486 · USER_MANUAL §5-11

### [TWR] Q488. live E2E bootstrap에 **NHIS import 배치**가 시드되나요? (G21, QA-B95)

**A.** **Fixed (BE `b73e5f4`, G21, Q488)** — **`LiveE2eBootstrapService.ensureSampleNhisImportBatch`** 가 **당월 NHIS import batch/row** 를 upsert합니다 — 샘플 G21 방문일정(**`serviceDays=1`**)과 정합.

| 시드 | **`DEFAULT_NHIS_IMPORT_BATCH_ID`** · **`yearMonth`=당월** · **`serviceDays=1`** · **`matchStatus=MATCHED`** |
| 대상 지점 | **`HOME_VISIT` 계열** 지점만 |
| 효과 | **`GET /visits/confirm-readiness` `nhisComparisonSummary`** · **`GET /visits/nhis-comparison`** — **`nhisBatchPresent=true`** · live E2E G21 routing harness |
| 테스트 | **`LiveE2eBootstrapServiceTest`** · **`VisitLiveApiRoutingE2eTest`** — per-client comparison endpoint |

> 관련: Q479 · Q481 · Q457 · Q464 · DEPLOYMENT §11-3 · `./scripts/run-live-e2e.sh`

### [TWR] Q489. 교육일지 **기준 연도**를 잘못 입력하면 어떻게 되나요? (G41, US-S04)

**A.** **Fixed (FE `f26e075`·`28e5525`, G41, Q489)** — **`StaffTrainingLogPage`** 가 **목록 필터·등록/수정 Modal** 모두에서 **`parseStaffTrainingReferenceYear`**(`staffTrainingLogs.js`) 로 **4자리·2000~2100** 범위만 허용합니다.

| 상황 | 동작 |
|------|------|
| **기준 연도 필터** | `abcd`·범위 밖(예: `1999`) → **`Field error`** 「기준 연도는 2000~2100 사이 4자리 숫자여야 합니다.」 · API **`referenceYear` 쿼리 생략** · **compliance 요약 StatCard 숨김** (`28e5525`) — **잘못된 연도로 준수 현황이 표시되지 않음** |
| **등록/수정 Modal** | 유효하지 않은 **기준 연도** → **동일 필드 오류** · **POST/PATCH 차단** |
| 공유 상수 | **`STAFF_TRAINING_REFERENCE_YEAR_MIN/MAX`** · **`STAFF_TRAINING_REFERENCE_YEAR_ERROR`** — filter·Modal·**`staffTrainingLogs.test`** 공유 |
| BE | 서버 **`trainedAt.year = referenceYear`** 가드는 **기존과 동일** — FE 선검증으로 **422 오류 빈도 감소** |
| 테스트 | **`StaffTrainingLogPage.test`** · **`staffTrainingLogs.test`** — invalid filter error · invalid submit blocked |

> 관련: Q321 · Q325 · USER_MANUAL §5-3·§5-14 · ADMIN_GUIDE §6-2-12

### [TWR] Q490. live E2E **probe**에서 **`staff-credentials-default`**·**`guardian-credentials-default`** blocker가 나와요. (QA-B95)

**A.** **Fixed (BE `f932fd3`, QA-B95, Q490)** — Q428·Q437에서 **기본 creds fallback**을 허용했으나, **267차(`beef81e`, Q542) 이전** staging preflight는 env 시크릿 미설정 시 **`operationReady=false`** 를 강제했습니다. **`ogada.live-e2e.allow-default-credentials=false`** 일 때만 아래 blocker가 적용됩니다. **기본값 `true`** — 로컬 quick start는 Q542 참고.

| blocker | 조건 | 조치 |
|---------|------|------|
| **`staff-credentials-default`** | **`LIVE_E2E_EMAIL`/`PASSWORD`** 미설정 → 시드 기본 staff creds 사용 **且 `allow-default-credentials=false`** | **`scripts/dev-live-e2e.env`** 에 staff creds **명시** · 또는 **`allow-default-credentials=true`**(기본) |
| **`guardian-credentials-default`** | **`LIVE_E2E_GUARDIAN_EMAIL`/`PASSWORD`** 미설정 → **`live-e2e-guardian@ogada.test`** fallback **且 `allow-default-credentials=false`** | guardian creds **명시** · 또는 Q542 기본 허용 |
| **health vs probe** | **`GET /api/v1/health`** — **`liveE2eAllowDefaultCredentials`** 노출 · **`allow-default-credentials=false`** 시 default creds → **`liveE2eOperationReady=false`** | **`curl …/health`** 와 **`curl …/live-e2e/probe`** **동일 blocker 기대** |

| preflight 통과 조건 (probe, `allow-default-credentials=false`) | **`defaultCredentials=false`** · **`defaultGuardianCredentials=false`** · staff/guardian bootstrap ready · seed client present |
| preflight 통과 조건 (probe, **`allow-default-credentials=true`**, 기본) | default creds 허용 · **G21 seed·client 등 나머지 blocker**만 충족 |
| 테스트 | **`LiveE2eControllerTest.probeShouldBlockOperationWhenDefaultStaffCredentialsAreUsed`** · **`probeShouldAllowDefaultCredentialsWhenFlagEnabled`** (`beef81e`, Q542) |

> 관련: Q428 · Q437 · Q491 · Q409 · DEPLOYMENT §3-6·§11-3 · `./scripts/run-live-e2e.sh`

### [TWR] Q491. health와 probe의 **`liveE2eOperationReady`** 가 달라요. (QA-B95)

**A.** **Fixed (BE `8fe1ccd`, QA-B95, Q491)** — Q437에서 health만 default guardian creds 시 **`operationReady=true`** 를 허용했으나, Q490 probe blocker와 **불일치**했습니다. **`HealthController`** 가 probe와 **동일 규칙**을 적용합니다.

| 변경 | **`staffBootstrapReady`** — **`credentialsConfigured && !defaultCredentials`** · **`guardianCredentialsUsable`** — **`guardianCredentialsConfigured && !defaultGuardianCredentials`** |
| blockers | **`staff-credentials-default`** · **`guardian-credentials-default`** — health **`liveE2eOperationBlockers[]`** 에도 노출 |
| 조치 | **`scripts/dev-live-e2e.env`** 에 staff·guardian creds **명시** (Q490과 동일) |
| 테스트 | **`HealthControllerTest.healthShouldBlockOperationReadinessWhenStaffCredentialsAreDefault`** · **`healthShouldBlockOperationReadinessWhenGuardianCredentialsAreDefault`** |

> 관련: Q437 · Q490 · DEPLOYMENT §3-6·§11-3 · ADMIN_GUIDE §1-4

### [TWR] Q492. live E2E bootstrap이 **다른 Tenant ID**와 충돌해요. (QA-B95)

**A.** **Fixed (BE `54d7f36`, QA-B95, Q492)** — 재사용 dev DB에서 **configured visit schedule·NHIS import UUID**가 **다른 `organization_id`** 에 이미 존재하면 bootstrap이 **잘못된 tenant 데이터**를 upsert할 수 있었습니다.

| resolver | **`resolveVisitScheduleId`** · **`resolveNhisImportBatchId`** · **`resolveNhisImportRowId`** |
| 동작 | configured ID 조회 시 **org 불일치** → **새 seed ID** 생성 · 동일 org면 **기존 ID 재사용** |
| **247차 갱신 (`c651b30`, Q498)** | foreign-tenant fallback ID는 **`UUID.randomUUID()`** 대신 **`scopedFallbackId(kind, orgId)`** — **tenant별 결정적 UUID** — 재부트스트랩 안정 |
| 영향 | **`ensureSampleDraftPlanVisit`** · **`ensureSampleNhisImportBatch`** — G21 visit·NHIS comparison live seed **cross-tenant 안전** |
| 테스트 | **`LiveE2eBootstrapServiceTest.bootstrapShouldFallbackWhenVisitScheduleIdBelongsToForeignTenant`** · **`bootstrapShouldGenerateNewNhisSeedIdsWhenConfiguredIdsExistForForeignTenant`** · **`g21SeedStatusShouldStayReadyAfterBootstrapWhenDefaultSeedIdsAreForeignTenantOwned`** (Q498) |

> 관련: Q488 · Q457 · Q498 · Q409 · DEPLOYMENT §11-3

### [TWR] Q493. **공단 명세 사전 비교**·**교육일지 기준 연도** 화면 접근성이 개선됐나요? (UXD-134)

**A.** **Fixed (FE `5c4e241`, UXD-134, Q493)** — G21 standalone NHIS comparison panel과 G41 filter-year 필드의 **스크린리더·키보드** 지원을 보강했습니다.

| 화면 | 개선 |
|------|------|
| **`VisitNhisComparisonPanel`** | **「수급자별 상세 보기」** — **`aria-describedby`** → gap Alert · **단일 로딩 Spinner** (중복 제거) · **`aria-busy`** on refresh |
| **`VisitNhisComparisonDetail`** | **`role="region"`** + **`h4`「수급자별 공단 명세 비교 상세」** — **`aria-labelledby`** |
| **`StaffTrainingLogPage`** | filter **기준 연도 `Field help`** — **「2000~2100…준수 현황이 숨겨집니다」** → **`aria-describedby`** 연결 |
| CSS | **`components.css`** — 누락 **`ds-*` utilities** 정의 |

| 테스트 | **`VisitNhisComparisonPanel.test`** · **`VisitNhisComparisonDetail.test`** |

> 관련: Q486 · Q487 · Q489 · USER_MANUAL §5-11·§5-14

### [TWR] Q494. **`LIVE_E2E_CLIENT_ID`에 공백만** 넣었는데 live E2E가 통과해요.

**A.** **Fixed (FE `cf85003`, QA-B95, Q494)** — env에 **`LIVE_E2E_CLIENT_ID="   "`** 처럼 **공백-only** 값이 있으면 **`hasLiveClientId()`** 가 false로 처리하고, **`assertLiveE2eConfig({ requireClientId: true })`** 가 **`LIVE_E2E_CLIENT_ID` missing** 오류를 던집니다.

| 조치 | **`scripts/dev-live-e2e.env`** 에 **유효 UUID** 설정 · bootstrap 응답 **`clientId`** 자동 주입 확인 |
| 테스트 | **`liveE2eHarness.test`** — **`treats whitespace LIVE_E2E_CLIENT_ID as missing in assertLiveE2eConfig`** |

> 관련: Q360 · Q393 · Q409 · DEPLOYMENT §3-6·§11-3

### [TWR] Q495. health/probe에서 **`visit-schedule-missing`**·**`billing-visit-schedule-missing`**·**`nhis-import-missing`** blocker가 나와요. (QA-B95, G21)

**A.** **Fixed (BE `14582bf`/`429661e`, QA-B95, G21, Q495·Q500)** — 방문요양(`HOME_VISIT`)·legacy **`DAY_CARE`**(Q501) 지점에서 live E2E **운영 가능(`operationReady`)** 은 staff·guardian bootstrap뿐 아니라 **G21 시드 데이터 3종**도 필요합니다.

| health/probe 필드 | 의미 |
|-------------------|------|
| `liveE2eG21SeedApplicable` | 시드 지점이 **방문요양 계열**(`HOME_VISIT`) 또는 **legacy `DAY_CARE`** 이면 `true` — bootstrap **`HOME_VISIT` 승격 전**에도 G21 seed 검사 (Q501) |
| `liveE2eVisitScheduleReady` | configured/resolved **PLAN 방문일정 UUID**가 해당 org에 존재 |
| `liveE2eBillingVisitScheduleReady` | **paired BILLING** 일정 존재 · PLAN과 **`pairedScheduleId` 양방향 연결** (Q500, `429661e`) |
| `liveE2eNhisImportReady` | **NHIS import batch + row** 존재 · **row `batchId` = batch `id`** (Q499) |
| `liveE2eVisitScheduleId`·`liveE2eBillingVisitScheduleId`·`liveE2eNhisImportBatchId`·`liveE2eNhisImportRowId` | bootstrap이 resolve한 시드 ID — **`POST /system/live-e2e/bootstrap`** 응답에도 동일 (`fd275f4`/`429661e`) |

| blocker | 조치 |
|---------|------|
| `visit-schedule-missing` | bootstrap 재실행 · HOME_VISIT 지점 **당월 DRAFT PLAN visit** seed 확인 (Q464) |
| **`billing-visit-schedule-missing`** | bootstrap 재실행 · **paired PLAN+BILLING** DRAFT seed 확인 (Q500, `fd275f4`) |
| `nhis-import-missing` | bootstrap 재실행 · **NHIS import batch/row** seed 확인 (Q488·Q499) |

> 관련: Q457 · Q464 · Q488 · Q499 · Q500~Q502 · DEPLOYMENT §9-1·§11-3 · ADMIN_GUIDE §1-4

### [TWR] Q496. live E2E가 **`LIVE_E2E_CLIENT_ID` 없이** health probe만으로 clientId를 채우나요? (QA-B95)

**A.** **Fixed (FE `689f377`, QA-B95, Q496)** — globalSetup이 **`GET /api/v1/health`** 를 probe할 때 **`liveE2eSeedClientId`** 를 파싱합니다. env에 **`LIVE_E2E_CLIENT_ID`가 없으면** probe 값으로 **자동 주입**합니다.

| 함수 | 용도 |
|------|------|
| `extractLiveE2eHealthFields` | health 응답에서 **`liveE2eOperationReady`·G21 seed flags·`liveE2eSeedClientId`** 추출 |
| `isLiveOperationReady` | **`liveE2eOperationReady === true`** (fresh probe state) |
| `isLiveG21SeedReady` | G21 미적용이거나 **visit schedule + NHIS import** 모두 ready |

| 주의 | **공백-only `LIVE_E2E_CLIENT_ID`** 는 여전히 거부 (Q494) · **명시 UUID** 설정이 staging 표준 |

> 관련: Q360 · Q393 · Q494 · DEPLOYMENT §3-7

### [TWR] Q497. G21 live E2E suite(`liveG21Describe`)가 **operation 미준비**인데 실행돼요. (QA-B95)

**A.** **Fixed (FE `d61ab5e`/`0915f80`, QA-B95, Q497)** — G21 NHIS comparison·batch-confirm live suite는 **`liveG21Describe`** 로 게이트됩니다.

| 조건 | `liveG21Describe` 실행 |
|------|------------------------|
| `LIVE_E2E=1` + staff creds + staff auth ready | 필수 |
| **`isLiveG21ReadyForRun()`** = `isLiveOperationReady() && isLiveG21SeedReady()` | 필수 |
| **`isLiveG21SeedReady()`** | **251차 (`c3b6a5c`, Q512)** — PLAN + **paired BILLING** + NHIS 모두 ready |

| 구현 | **`liveE2eSuiteGuard.test`** — `liveG21Describe`를 live guard 허용 목록에 추가 (`0915f80`) · **`liveE2eHarness.test`** — operation false·**BILLING seed missing** 시 **`isLiveG21ReadyForRun` false** (`d61ab5e`/`c3b6a5c`) |

| 기대 | `npm run test:live-e2e` — G21 suite **SKIP** when health reports **`liveE2eOperationReady=false`** or G21 seed missing — **123 PASS / N SKIP** 패턴 |

> 관련: Q495 · Q496 · Q500 · Q512 · Q488 · DEPLOYMENT §3-7·§11-3

### [TWR] Q498. live E2E bootstrap이 **foreign tenant UUID**를 매번 새로 만들어 충돌해요. (QA-B95)

**A.** **Fixed (BE `c651b30`, QA-B95, Q498)** — Q492에서 foreign-tenant configured ID 시 **랜덤 UUID**를 쓰면 재부트스트랩마다 ID가 바뀌었습니다. **`scopedFallbackId(kind, organizationId)`** 로 **tenant별 결정적 UUID**를 생성합니다.

| kind | 예시 |
|------|------|
| `visit-schedule` | foreign-tenant default visit schedule ID 대체 |
| `nhis-import-batch` | foreign-tenant default NHIS batch ID 대체 |
| `nhis-import-row` | foreign-tenant default NHIS row ID 대체 |

| 효과 | 동일 org에서 bootstrap **반복 실행해도 동일 fallback ID** — upsert 안정 · Q492 cross-tenant 격리 **유지**

> 관련: Q492 · Q495 · DEPLOYMENT §11-3

### [TWR] Q499. NHIS import **batch·row는 있는데** `nhis-import-missing` blocker가 나와요. (G21, QA-B95)

**A.** **Fixed (BE `c0403b0`, G21, Q499)** — batch·row가 **각각 존재**해도 row의 **`batchId`** 가 시드 batch와 **다르면** `nhisImportReady=false` 입니다. orphan row만 남은 **false-positive operationReady** 를 방지합니다.

| 검증 | `nhisImportReady` = batch exists **AND** row exists **AND** `row.batchId == batch.id` |
| 조치 | bootstrap 재실행 — **`ensureSampleNhisImportBatch`** 가 batch·row를 **동일 linkage**로 upsert (Q488) |
| 테스트 | **`LiveE2eBootstrapServiceTest.g21SeedStatusShouldReportNhisImportNotReadyWhenRowBatchMappingMismatches`** |

> 관련: Q488 · Q495 · DEPLOYMENT §11-3 · ADMIN_GUIDE §1-4

### [TWR] Q500. **`billing-visit-schedule-missing`** blocker가 나와요. PLAN visit은 있는데요. (QA-B95, G21)

**A.** **Fixed (BE `429661e`/`fd275f4`, QA-B95, G21, Q500)** — G21 live E2E는 **PLAN 일정만**으로는 부족합니다. **paired BILLING** 일정(`scheduleKind=BILLING`)이 **동일 org**에 존재하고 PLAN과 **`pairedScheduleId`로 양방향 연결**되어야 **`billingVisitScheduleReady=true`** 입니다.

| 항목 | 내용 |
|------|------|
| bootstrap | **`ensureSampleVisitSchedules`** — 당월 **linked DRAFT PLAN+BILLING** upsert · **`LiveE2eBootstrapResponse.billingVisitScheduleId`** 반환 (`fd275f4`) |
| health/probe | **`liveE2eBillingVisitScheduleReady`·`liveE2eBillingVisitScheduleId`** (`429661e`) |
| blocker | **`billing-visit-schedule-missing`** — PLAN만 있거나 페어 linkage 불완전 |
| 조치 | **`POST /api/v1/system/live-e2e/bootstrap`** 재실행 · health에서 resolved ID 확인 |

| 테스트 | **`LiveE2eBootstrapServiceTest`** · **`HealthControllerTest`** · **`LiveE2eControllerTest`**

> 관련: Q495 · Q477 · USER_MANUAL §5-11 · DEPLOYMENT §9-1

### [TWR] Q501. legacy **`DAY_CARE`** live E2E 지점인데 G21 seed blocker가 나와요. (QA-B95)

**A.** **Fixed (BE `cc295ec`, QA-B95, Q501)** — 예전 live E2E seed는 **`DAY_CARE`**(주야간) 지점을 썼지만, G21 visit API는 **`HOME_VISIT`**(방문요양)을 요구합니다. **`isG21SeedApplicableBranch`** 가 legacy **`DAY_CARE`** 도 **G21 seed applicable** 로 처리해, bootstrap이 **`HOME_VISIT` 승격**·paired visit seed를 완료하기 전까지 **`operationReady=false`** 를 유지합니다 — **false-green gate 방지**.

| 조치 | **`POST /api/v1/system/live-e2e/bootstrap`** — **`activateBranch`** 가 **`DAY_CARE` → `HOME_VISIT`** 승격 (Q457) · paired PLAN/BILLING seed upsert (Q500) |
| 기대 | bootstrap 후 **`liveE2eG21SeedApplicable=true`** · visit·billing·NHIS ready flags **true** |

> 관련: Q457 · Q495 · Q500 · ADMIN_GUIDE §1-4

### [TWR] Q502. live E2E seed 지점이 **없거나 비활성**인데 operationReady가 true였어요. (QA-B95)

**A.** **Fixed (BE `191703f`/`7898aa5`/`bc754a0`, QA-B95, Q502·Q505·Q511)** — configured **`branchId`** 가 **존재하지 않거나 비활성(`!active`)** 이거나 **다른 org 소속**이면 **`g21SeedStatus`** 가 **`missingBranchOrInactive()`** 를 반환합니다. **`applicable=false`** · **`operationReady=false`** — tenant 격리 위반·orphan seed ID로 인한 **false-green** 을 차단합니다. **249차**부터 **`GET /api/v1/system/live-e2e/probe`** · **251차**부터 **`GET /api/v1/health`** 도 **`g21-branch-missing-or-inactive`** blocker를 명시 노출합니다 — **`operationReady=false`인데 `blocker=none`** 인 preflight 오탐 방지 (Q505·Q511).

| 조건 | `g21SeedStatus` |
|------|-----------------|
| branch `null` | not applicable · all ready flags **false** |
| branch `!active` | 동일 |
| branch `organizationId` ≠ configured org | 동일 |

| probe (`7898aa5`) | **`operationBlockers[]`** = **`["g21-branch-missing-or-inactive"]`** — health는 **`liveE2eOperationReady=false`** 만 반환(blocker 코드 미노출) |

| 조치 | bootstrap env **`LIVE_E2E_BRANCH_ID`**·org 일치 확인 · 지점 **활성화** · bootstrap 재실행 |

| 테스트 | **`LiveE2eBootstrapServiceTest.g21SeedStatusShouldReportNotReadyWhenBranchIsMissingOrInactive`** · **`LiveE2eControllerTest.probeShouldExposeG21BranchBlockerWhenSeedBranchIsMissingOrInactive`**

> 관련: Q495 · Q492 · Q505 · DEPLOYMENT §11-3

### [TWR] Q503. 교육일지 **기준 연도 필터**가 잘못됐는데 compliance StatCard가 잠깐 보였어요. (QA-B146, G41)

**A.** **Fixed (FE `cefb7c7`, QA-B146, G41, Q503)** — Q489에서 invalid filter 시 **compliance 요약 숨김**·**`referenceYear` omit** 은 적용됐지만, **`load()`** 가 여전히 API를 호출해 **stale compliance** 가 잠깐 표시될 수 있었습니다. **`filterYearError`가 설정된 동안 `load()`를 skip** 하고 compliance state를 **clear** 합니다.

| 동작 | invalid filter (`abcd`·범위 밖) |
|------|--------------------------------|
| 목록 API | **`referenceYear` 없이** 조회 (Q489) |
| compliance API | **호출하지 않음** · StatCard **숨김 유지** (Q503) |
| 수정 | 유효한 4자리 연도 입력 → error 해제 → **load() 재실행** |

> 관련: Q489 · USER_MANUAL §5-3 · ADMIN_GUIDE §6-2-12

### [TWR] Q504. 교육일지 **등록 Modal**에서 help·오류가 스크린리더에 안 읽혀요. (UXD-135, G41)

**A.** **Fixed (FE `40d0ca3`/`974b018`, UXD-135, Q504)** — Q493에서 **필터 기준 연도**에 **`Field controlProps`**·**`aria-describedby`** 를 적용했고, **248차**에서 **등록/수정 Modal** 전 필드에도 동일 패턴을 확장했습니다.

| 화면 | a11y |
|------|------|
| **필터 기준 연도** | help·error → input **`aria-describedby`** (`974b018`) |
| **등록/수정 Modal** | **기준 연도·교육 유형·교육일·…** 모든 **`Field`** — **`controlProps` spread** (`40d0ca3`) |
| **RFID diff compare** | 차이 0건 **단일 success Alert** consolidate (`974b018`, Q482 deepen) |

| 테스트 | **`StaffTrainingLogPage.test`** — Modal field **`aria-describedby`** · **`BillingStatementPrintPanel.test`** layout |

> 관련: Q489 · Q493 · USER_MANUAL §5-3 · DESIGN_SYSTEM Field render-prop

### [TWR] Q505. live E2E probe에서 **`operationReady=false`인데 blocker가 `none`** 이에요. (QA-B95, G21)

**A.** **Fixed (BE `7898aa5`, QA-B95, G21, Q505)** — Q502에서 **`g21SeedStatus.missingBranchOrInactive()`** 로 **`operationReady=false`** 는 차단했지만, **`GET /api/v1/system/live-e2e/probe`** 가 **`operationBlockers[]`를 비워** preflight가 **`blocker=none`** 으로 통과하는 **false-green** 이 남아 있었습니다.

| 조건 | probe 동작 (`7898aa5`) |
|------|------------------------|
| **`!g21SeedApplicable && !g21SeedOperationReady`** | **`operationBlockers[]`** = **`["g21-branch-missing-or-inactive"]`** |
| **`operationBlocker`** | 첫 항목 = **`g21-branch-missing-or-inactive`** |
| health (`GET /api/v1/health`) | **251차 (`bc754a0`, Q511)** — probe와 **동일 blocker 코드** 노출 — 이전에는 **`liveE2eOperationReady=false`** 만 반환 |

| 조치 | Q502와 동일 — **`LIVE_E2E_BRANCH_ID`**·org 일치 · 지점 활성화 · bootstrap 재실행 |

| 테스트 | **`LiveE2eControllerTest.probeShouldExposeG21BranchBlockerWhenSeedBranchIsMissingOrInactive`**

> 관련: Q502 · Q495 · Q511 · DEPLOYMENT §9-1·§11-3 · ADMIN_GUIDE §1-4

### [TWR] Q506. 방문일정 **일괄확정 Modal**에서 「공단 청구명세서와 비교했습니다.」 Checkbox가 StatCard 내용을 스크린리더가 안 읽어요. (QA-B147, UXD-147, G21)

**A.** **Fixed (FE `0002943`, QA-B147, UXD-147, G21, Q506)** — Q484에서 Modal **「공단 명세 비교」** StatCard·region heading은 추가됐지만, **확인 Checkbox**의 **`aria-describedby`** 가 help·blocker warning만 연결되어 **집계 StatCard 맥락**이 누락됐습니다.

| UI | a11y (`0002943`) |
|----|------------------|
| **「공단 청구명세서와 비교했습니다.」** Checkbox | **`aria-describedby`** 에 **`visit-batch-confirm-nhis-comparison`** region id 포함 |
| NHIS comparison region | **`id="visit-batch-confirm-nhis-comparison"`** — StatCard·Alert·**수급자별 상세** expand와 동일 region |
| 스크린리더 | Checkbox 포커스 시 **help + 비교 집계(일치·불일치·누락 등)** 함께 안내 |

| 테스트 | **`VisitBatchConfirmPanel.test`** — **`aria-describedby`** contains **`visit-batch-confirm-nhis-comparison`**

> 관련: Q484 · Q487 · USER_MANUAL §5-11 · DESIGN_SYSTEM Field·Checkbox `aria-describedby`

### [TWR] Q507. live E2E G21 seed가 **다른 지점(branch) 소속**인데 operationReady가 true였어요. (QA-B148, G21)

**A.** **Fixed (BE `02cf036`, QA-B148, G21, Q507)** — Q502·Q492에서 org·branch **존재/활성**·foreign-tenant ID는 차단했지만, **같은 org 내 다른 지점**에 PLAN/BILLING visit schedule·NHIS import batch가 있으면 **`operationReady=true`** 로 오탐될 수 있었습니다.

| 검증 | configured **`LIVE_E2E_BRANCH_ID`** 와 seed **`branchId` 일치** 필수 |
|------|---------------------------------------------------------------------|
| PLAN visit schedule | **`isScheduleInBranch(schedule, branchId)`** — 불일치 → **`visitScheduleReady=false`** |
| BILLING visit schedule | 동일 + paired PLAN/BILLING 검증 |
| NHIS import batch | **`batch.getBranchId().equals(branchId)`** — 불일치 → **`nhisImportReady=false`** · resolved batch/row ID **null** |
| **`operationReady()`** | PLAN + paired BILLING + NHIS **모두 configured branch 소속**일 때만 true |

| 조치 | bootstrap env **`LIVE_E2E_BRANCH_ID`** 와 시드 데이터 **지점 일치** 확인 · **`POST /system/live-e2e/bootstrap`** 재실행 |

| 테스트 | **`LiveE2eBootstrapServiceTest.g21SeedStatusShouldRejectVisitSchedulesFromAnotherBranch`** · **`g21SeedStatusShouldRejectNhisImportBatchFromAnotherBranch`**

> 관련: Q502 · Q495 · Q500 · DEPLOYMENT §11-3 · ADMIN_GUIDE §1-4

### [TWR] Q508. 방문일정 **일괄확정 Modal**이 **「사전 점검 중」** 에서 멈춰요. (QA-B147, G21)

**A.** **Fixed (FE `5743333`, QA-B147, G21, Q508)** — **`hq_admin`** 등 **활성 지점(`branchId`)이 없는** 상태에서 **「일괄확정 시작」** 을 누르면 오류 메시지는 표시됐지만 **`loading=true`** 가 유지되어 Modal이 **「사전 점검 중」** Spinner에 **갇히는** 회귀가 있었습니다.

| 동작 (`5743333`) | **`branchId` 없음** → **`setLoading(false)`** 즉시 호출 |
|------------------|--------------------------------------------------------|
| 표시 | **「활성 지점이 없습니다. 지점을 선택한 뒤 다시 시도해 주세요.」** |
| API | **`fetchVisitConfirmReadinessApi` 호출하지 않음** |
| 버튼 | **「일괄확정 실행」** **disabled** 유지 |

| 조치 | AppShell **지점 선택기(Branch Switcher)** 로 작업 지점 선택 후 재시도 (FAQ Q89·Q11)

| 테스트 | **`VisitBatchConfirmPanel.test`** — **`surfaces branch selection error without leaving the modal in a loading state`**

> 관련: Q506 · Q477 · USER_MANUAL §5-11 · USER_MANUAL §2-4

### [TWR] Q509. **L03 간호기록** 표의 **수정** 버튼 포커스·스크린리더가 이상해요. (UXD-136·UXD-137, G-NURSING)

**A.** **Fixed (FE `6f07803`/`f86c76c`, UXD-136·UXD-137, G-NURSING, Q509)** — L03 간호급여 **5종 기록 화면**·**통합 바이탈(L03_M11)** 의 행 **수정** 버튼이 raw **`<button class="ds-table__action-btn">`** 또는 **`variant=secondary`** 를 사용해 **`ds-btn`** 포커스 링·forced-colors outline·스크린리더 **`aria-label`** 이 **DESIGN_SYSTEM §35(110차) tertiary 패턴**과 불일치했습니다.

| 화면 | 수정 (`6f07803`/`f86c76c`) |
|------|---------------------------|
| **간호급여 제공기록** · **응급상황** · **유치도뇨관** · **구강상태** · **체중 기록** | **`Button variant=tertiary size=sm`** · **행별 `aria-label`「{이용자} {일자} … 수정」** · **`ds-btn` 클래스** |
| **통합 바이탈 (L03_M11)** | 동일 — **`aria-label`「{이용자} {점검일} 통합 바이탈 수정」** (`f86c76c`, UXD-137) |

| 사용자 영향 | **동작·권한 불변** — 키보드 Tab 포커스·고대비 모드·스크린리더 **맥락 안내** 개선 |

| 테스트 | 각 **`Nursing*Page.test`** · **`NursingVitalCheckPage.test`** — edit 버튼 **`ds-btn`** · **`aria-label`** query |

> 관련: Q340 · USER_MANUAL §5-16 · DESIGN_SYSTEM Button tertiary

### [TWR] Q510. live E2E staff bootstrap에 **guardian token**이 붙는데 probe는 **guardian not-ready**예요. (QA-B95)

**A.** **Fixed (BE `02a2eb8`, QA-B95, Q510)** — Q409·Q428에서 **default guardian creds**(`live-e2e-guardian@ogada.test`) 사용 시 probe·health는 **`guardian-credentials-default`** 로 **`operationReady=false`** 인데, staff bootstrap 응답에 **guardian token을 best-effort 첨부**해 **payload·readiness 불일치**가 남아 있었습니다.

| 변경 (`02a2eb8`) | **`isUsableGuardianCredentialsConfigured()`** = **`hasConfiguredGuardianCredentials()` only** — env에 **명시 guardian creds** 없으면 **token enrichment skip** |
| staff bootstrap 응답 | **`guardianAccessToken`/`guardianRefreshToken`/`guardianEmail`/`guardianUserId` = null** |
| probe·health | **기존과 동일** — default creds → **`guardian-credentials-default`** blocker |

| preflight 통과 | **`LIVE_E2E_GUARDIAN_EMAIL`/`PASSWORD` env 명시** (Q490·Q491) · **`POST /system/live-e2e/bootstrap-guardian`** 또는 configured creds로 guardian suite 별도 준비 |

| 테스트 | **`LiveE2eBootstrapServiceTest.bootstrapShouldSkipGuardianTokensWhenDefaultGuardianCredentialsAreUsed`**

> 관련: Q409 · Q428 · Q490 · Q491 · DEPLOYMENT §11-3 · FAQ Q473

### [TWR] Q511. **`GET /api/v1/health`** 에서도 **`g21-branch-missing-or-inactive`** blocker가 나와야 하나요? (QA-B95, G21)

**A.** **Fixed (BE `bc754a0`, QA-B95, G21, Q511)** — Q505에서 **`GET /api/v1/system/live-e2e/probe`** 만 **`g21-branch-missing-or-inactive`** 를 노출하고, **`GET /api/v1/health`** 는 **`liveE2eOperationReady=false`** 만 반환해 **health·probe 진단 불일치**가 남아 있었습니다.

| 조건 | health 동작 (`bc754a0`) |
|------|-------------------------|
| **`!g21SeedApplicable && !g21SeedOperationReady`** | **`liveE2eOperationBlockers[]`** = **`["g21-branch-missing-or-inactive"]`** |
| **`liveE2eOperationBlocker`** | 첫 항목 = **`g21-branch-missing-or-inactive`** |
| probe | Q505와 **동일 코드** — health·probe **대칭** |

| 조치 | Q502·Q505와 동일 — **`LIVE_E2E_BRANCH_ID`**·org 일치 · 지점 활성화 · bootstrap 재실행 |

| 테스트 | **`HealthControllerTest.healthShouldExposeG21BranchBlockerWhenSeedBranchIsMissingOrInactive`**

> 관련: Q505 · Q502 · DEPLOYMENT §9-1·§11-3 · ADMIN_GUIDE §1-4

### [TWR] Q512. G21 live E2E가 **PLAN 일정만** 있을 때 실행돼요. (QA-B95, G21)

**A.** **Fixed (FE `c3b6a5c`, QA-B95, G21, Q512)** — Q500에서 BE는 **paired BILLING** readiness를 **`liveE2eBillingVisitScheduleReady`** 로 노출했지만, FE **`isLiveG21SeedReady`** 는 **PLAN + NHIS만** 검사해 **BILLING 미준비 시에도 `liveG21Describe` 실행**이 가능했습니다.

| 변경 (`c3b6a5c`) | **`liveBackendProbe.extractLiveE2eHealthFields`** — **`liveE2eBillingVisitScheduleReady`** 파싱 |
|------------------|--------------------------------------------------------------------------------------------------|
| **`isLiveG21SeedReady()`** | **`visitScheduleReady && billingVisitScheduleReady && nhisImportReady`** — field 미존재 시 PLAN으로 폴백 |
| **`liveGlobalSetup`** | warn — **「PLAN/BILLING schedule or NHIS import missing」** |

| 기대 | **`isLiveG21ReadyForRun()`** false when **BILLING seed missing** — G21 suite **SKIP**

| 테스트 | **`liveE2eHarness.test`** — billing readiness false → **`isLiveG21SeedReady` false**

> 관련: Q497 · Q500 · DEPLOYMENT §3-7·§11-3

### [TWR] Q513. **요양급여 주간 제공기록**에서 **상태변화 기록 7일 SLA** 경고가 뭐예요? (FAQ21817, G39, L02_M01)

**A.** **Fixed (FE `b881883`, G39, FAQ21817, Q513)** — 이지케어 [FAQ 21817](https://ezcare.easyms.co.kr/help/faq.ez?rowid=21817&type=web) 에 따르면 **요양보호사 상태변화 기록**은 **주 1회 이상·7일 이내** 작성해야 합니다. **`/care/weekly-service-records`** 목록 상단에 **warning Alert** 로 미작성·기한 초과 이용자를 안내합니다.

| 상태 | 의미 |
|------|------|
| **`missing`** | **`stateChangeNotes` 기록 없음 |
| **`due`** | 이번 주 미작성 · 마지막 기록 후 **7일 이내** |
| **`overdue`** | 마지막 주 종료 후 **7일 초과** — **기한 초과** |

| 계산 | **`computeStateChangeDueAlerts`** — **`STATE_CHANGE_DUE_MAX_DAYS=7`** · 이번 주(월요일 기준) **`stateChangeNotes` 있으면 Alert 제외** |
| 폼 help | **`stateChangeNotes` Field** — 「주 1회 이상, 7일 이내 작성 (시행규칙 별지·FAQ21817)」 |

| 조치 | Alert에 표시된 이용자 — **「새 주간 기록」** 또는 **수정**으로 **`stateChangeNotes`** 입력 |

| 테스트 | **`CareServiceWeeklyRecordPage.test`** · **`careServiceStateChangeDue.test`**

> 관련: Q362 · Q276(지표44 위젯) · USER_MANUAL §5-25 · ADMIN_GUIDE §6-2-15

### [TWR] Q514. **RFID 특이사항**이 **별지 서식**과 같다는 안내는 어디에 있나요? (FAQ21817, G39)

**A.** **Fixed (FE `b881883`, G39, FAQ21817, Q514)** — 이지케어 FAQ21817: **RFID 전송 엑셀 특이사항란**에 충실히 기록하면 **시행규칙 별지 서식 작성과 동등**하게 인정됩니다. ogada는 아래 3곳에 **동치 안내**를 표시합니다.

| 화면 | 안내 |
|------|------|
| **`/care/weekly-service-records`** | **`specialNotes`·`stateChangeNotes` Field `help`** |
| **`/care/service-special-notes`** | **`specialNotes` Field `help`** |
| **`/visits` RFID compare** | **`VisitRfidDiffComparePanel`** info Alert — 불일치 시 **`/care/weekly-service-records`·`/care/service-special-notes`** 에서 보정 권고 |

| 상수 | **`SPECIAL_NOTES_RFID_EQUIVALENCE_HELP`** · **`RFID_SPECIAL_NOTES_GUIDE`**

| 테스트 | **`VisitRfidDiffComparePanel.test`** — equivalence guide text

> 관련: Q452 · Q513 · USER_MANUAL §5-11·§5-25·§5-28

### [TWR] Q515. **`GET /api/v1/health`** 의 **`liveE2eG21SeedStatusDetail`** 은 무엇인가요? (QA-B95, live E2E)

**A.** **Fixed (BE `12d1a7b`, QA-B95, Q515)** — Q511에서 health·probe **blocker 코드**는 정렬되었지만, **guardian bootstrap·G21 seed** 실패 원인을 한눈에 보기 어려웠습니다. **`GET /api/v1/health`** 가 아래 **진단 문자열**을 추가로 반환합니다.

| 필드 | 예시 값 | 의미 |
|------|---------|------|
| **`liveE2eGuardianStatusDetail`** | `guardian-bootstrap=ready` · `guardian-bootstrap=disabled` · `guardian-bootstrap=error` | guardian bootstrap 상태 |
| **`liveE2eG21SeedStatusDetail`** | `g21-seed=applicable visit-schedule=present billing-visit-schedule=present nhis-import=present` | G21 seed 3축 준비도 |
| | `g21-seed=branch-missing-or-inactive` | configured branch 없음·비활성 |
| | `g21-seed=not-applicable` | G21 미해당 지점(주야간-only) |
| | `g21-seed=disabled` · `g21-seed=error` | bootstrap disabled·오류 |

| 용도 | staging **`./scripts/run-live-e2e.sh`** preflight — **`liveE2eOperationBlockers[]`** 와 **함께** 확인하면 **PLAN/BILLING/NHIS 중 어느 축이 missing인지** 빠르게 파악 |

| 테스트 | **`HealthControllerTest`** — guardian·G21 seed detail exposure

> 관련: Q495 · Q500 · Q511 · DEPLOYMENT §9-1·§11-3 · ADMIN_GUIDE §1-4

### [TWR] Q516. 사례관리 회의록 **참석자별 의견**은 어디서 입력하나요? (FAQ21797, G32)

**A.** **Fixed (BE `5222a8f` + FE `b272a7b`, G32, FAQ21797, Q516)** — 이지케어 [FAQ 21797](https://ezcare.easyms.co.kr/help/faq.ez?rowid=21797&type=web) 에 따르면 **회의 내용 = 참가 직원별 의견** 이 필수입니다. ogada는 **`/case-management/meetings`** 에서 **참석자 2인 이상**일 때 **참석자별 의견** 입력을 강제합니다.

| 항목 | 내용 |
|------|------|
| BE | **`attendeeOpinions[]`** — `{ name, jobRole?, opinion }` — **V156** `case_management_meetings.attendee_opinions` JSONB |
| compliance | **`attendeeOpinionsMet`** · **`attendeeOpinionsMetCount`** — 참석자 수와 의견 수 일치 여부 |
| FE 폼 | 참석자 입력 후 **`fieldset`「참석자별 의견 (FAQ21797)」** — 이름·직종(선택)·의견 `Field` |
| FE 검증 | 참석자별 **`opinion` 미입력** → **「{이름}님의 의견을 입력하세요 (FAQ21797).」** · **참석자 이름 중복**(대소문자 무시) → 즉시 오류 (Q520, `c7fb69a`) |
| StatCard | **「참석자별 의견 기록」** — 미충족 시 warning Alert |
| 하위 호환 | 기존 회의록(**`attendeeOpinions` 없음**) — API optional · 수정 시 의견 입력 권장 |

| API | `POST`·`PATCH /api/v1/case-management/meetings` — body에 **`attendeeOpinions`** 배열 포함 |

| 테스트 | **`CaseManagementServiceTest`** · **`CaseManagementPage.test`** · **`caseManagementCompliance.test`** · **`pilotPageFlows`** · **`programComplianceLiveApi.e2e.test.js`** (Q522, `3f871d7`)

> 관련: Q518 · Q519 · Q520 · USER_MANUAL §4-2·§5-9 · ADMIN_GUIDE §10-11-2

### [TWR] Q517. 이용자 상세에서 **가정통신문·급여제공기록지**도 보낼 수 있나요? (G2, 케어포 10-2)

**A.** **Fixed (FE `d1149a5`, G2, Q517)** — Q222에서 **노인학대예방 지침만** UI가 있었고, **가정통신문·급여제공기록지**는 Swagger만 가능했습니다. **`GuardianDocumentNotifyPanel`** 이 **문서 유형 선택**으로 3종을 통합합니다.

| 문서 유형 | API | 발송 버튼 라벨 |
|----------|-----|---------------|
| 노인학대예방 및 대응지침 | `POST …/elder-abuse-prevention-guideline` | **지침 이메일 발송** |
| **가정통신문** | `POST …/home-newsletter` | **가정통신문 이메일 발송** |
| **급여제공기록지** | `POST …/care-provision-record` | **급여제공기록지 이메일 발송** |

| 화면 | `/clients/:id` → **기본정보** — **「보호자 서류 발송 (G2)」** — **`Select`·`MonthInput`·요약 `Textarea`** |
| 급여제공기록지 | 요약 미입력 시 서버가 **이동서비스 요약** 자동 병합 (Q216) |
| 전제 | Q204 — **`channelEmail=true`** · 보호자 등록 이메일 |

| 테스트 | **`GuardianDocumentNotifyPanel.test`** · **`billingGuardianPlatformServices.test`**

> 관련: Q216 · Q217 · Q222 · Q243 · USER_MANUAL §4-7-3 · ADMIN_GUIDE §10-9

### [TWR] Q518. 대시보드 **「참석자별 의견 미기록」** 위젯은 무엇을 집계하나요? (FAQ21797, G32)

**A.** **Fixed (BE `b9e0947` + FE `e55ae96`, G32, FAQ21797, Q518)** — Q516에서 **회의록 화면**에 참석자별 의견 입력을 추가했습니다. **지점·통합 대시보드**에서 **현재 분기** 기준 **회의록은 있으나 참석자별 의견이 미충족**인 이용자 수를 한눈에 확인합니다.

| 항목 | 내용 |
|------|------|
| BE 필드 | **`caseManagementAttendeeOpinionGapCount`** — **`BranchDashboardResponse`·`HqDashboardResponse`** |
| 집계 | **`meetingsRecorded − attendeeOpinionsMetCount`** (음수 방지 `max(…, 0)`) — HQ는 지점별 **`attendeeOpinionsMetCount` 합산 정정** (`b9e0947`) |
| FE 위젯 | **`DashboardPage`** — **「참석자별 의견 미기록」** — 단위 **명** · 1명 이상 **warning** 톤 |
| 역할 | **`hq_admin`·`branch_admin`·`social_worker`** — 기존 사례관리 준수 위젯과 동일 |
| 폴백 | 스냅샷 필드 누락 시 **`GET /case-management/compliance`** → **`countCaseManagementAttendeeOpinionGaps`** |
| 링크 | 클릭 **`/case-management/meetings`** — §5-9에서 참석자별 의견 보완 |

| API | `GET /api/v1/dashboard/branch` · `GET /api/v1/dashboard/hq` |

| 테스트 | **`DashboardServiceTest`** · **`DashboardPage.test`** · **`dashboardSummary.test`** · **`caseManagementCompliance.test`**

> 관련: Q263 · Q516 · USER_MANUAL §4-2·§5-9 · ADMIN_GUIDE §1-4·§10-11-2

### [TWR] Q519. 사례관리 회의록 **`attendee_opinions` DB 무결성**(V157)은 무엇을 보장하나요? (G32)

**A.** **Fixed (BE `8835aa2` + `9ecd019`, G32, Q519)** — V156에서 추가된 **`case_management_meetings.attendee_opinions`** JSONB 컬럼에 **DB 레벨 배열 형태 검증**을 추가했습니다. 앱·코덱 외에 **raw SQL로 객체·스칼라가 들어가는 것**을 차단합니다.

| 항목 | 내용 |
|------|------|
| 마이그레이션 | **V157** — **`chk_case_management_meetings_attendee_opinions_array`** |
| CHECK | **`attendee_opinions IS NULL OR jsonb_typeof(attendee_opinions) = 'array'`** |
| 배경 | **`AttendeeOpinionsCodec`** 은 malformed JSON을 **빈 목록**으로 처리 — DB에 잘못된 형태가 쌓이면 **조용한 데이터 손상** 가능 |
| 패턴 | V155 waypoint `btrim` non-empty · V154 운전자 서명 pair-CHECK와 동일 **defense-in-depth** |
| 테스트 | **`AttendeeOpinionsCodecTest`** — null/blank/malformed → empty · round-trip preserve (`9ecd019`) |

> 관련: Q516 · Q520 · ADMIN_GUIDE §10-11-2 · DEPLOYMENT_GUIDE §1-3

### [TWR] Q520. 사례관리 회의록에서 **참석자 이름을 중복** 입력하면 어떻게 되나요? (FAQ21797, G32)

**A.** **Fixed (BE `eed39ab` + FE `c7fb69a`, G32, FAQ21797, Q520)** — 참석자 명단과 참석자별 의견 작성자 모두 **대소문자 무시 중복**을 허용하지 않습니다.

| 상황 | FE 동작 | BE 응답 |
|------|---------|---------|
| 참석자 **「김사복, 김사복」** | **`buildDuplicateAttendeeNamesError`** — 참석자 필드 오류 | **422** — 「사례관리 회의 참석자 이름은 중복될 수 없습니다.」 |
| 의견 작성자 **동일 이름 2건** | API 오류 시 **`mapCaseManagementMeetingApiErrorField`** → 참석자 필드 | **422** — 「참석자별 의견 작성자는 중복될 수 없습니다.」 |
| 의견 수 ≠ 참석자 수 | 의견 필드별 오류 | **422** — 「참석자별 의견은 참석자별로 1건씩 입력해야 합니다.」 |

| 구현 | **`caseManagementCompliance.js`** · **`CaseManagementPage`** · **`CaseManagementService.validateAttendeeCount`/`validateAttendeeOpinions`** |

| 테스트 | **`caseManagementCompliance.test`** · **`CaseManagementPage.test`** · **`CaseManagementServiceTest`** · **`pilotPageFlows`** (BNK-394 duplicate block) · **`ProgramCompliancePilotServiceFlowE2eTest`** (Q522, `510d2f3`)

> 관련: Q516 · Q518 · USER_MANUAL §5-9 · ADMIN_GUIDE §10-11-2

### [TWR] Q521. **live E2E** 테스트가 skip될 때 **원인**을 어디서 보나요? (QA-B95)

**A.** **Fixed (FE `9969746`, QA-B95, Q521)** — gated live suite(`liveG21Describe` 등)가 skip되면 Vitest 출력에 **구체적 skip 사유**가 표시됩니다.

| 함수 | 역할 |
|------|------|
| **`getLiveE2eSkipReasons(options)`** | operation blocker·G21 seed·staff/guardian auth·`LIVE_E2E_CLIENT_ID`·`LIVE_E2E_WRITE` 등 **정규화된 사유 배열** 반환 |
| 예시 사유 | `operation readiness blocked: staff-credentials-default` · `G21 seed not ready: BILLING schedule seed missing` · `LIVE_E2E_CLIENT_ID missing` |

| 선행 확인 | **`GET /api/v1/health`** — **`liveE2eOperationBlockers[]`** · **`liveE2eG21SeedStatusDetail`** (Q515) · **`GET /api/v1/system/live-e2e/probe`** |

| 실행 | **`./scripts/run-live-e2e.sh`** · **`npm run test:live-e2e`** (flock 경유, `docs/qa/VITEST_CONCURRENCY.md`) |

| 테스트 | **`liveE2eHarness.test`** — **`describe("liveConfig skip diagnostics")`**

> 관련: Q490~Q512 · Q515 · Q522 · DEPLOYMENT_GUIDE §9-1 · ADMIN_GUIDE §6-2-12

### [TWR] Q522. **live E2E**에서 G32 **참석자별 의견**이 create/update·compliance·대시보드까지 검증되나요? (FAQ21797, G32)

**A.** **Fixed (FE `3f871d7` + BE `510d2f3`, G32, FAQ21797, Q522)** — 254차에서 UI·API·대시보드 위젯을 연동한 뒤, **staging live E2E harness**가 end-to-end로 **`attendeeOpinions[]`** 를 검증합니다.

| 계층 | 검증 내용 |
|------|----------|
| FE live E2E | **`programComplianceLiveApi.e2e.test.js`** — create/update body에 **`attendeeOpinions[]`** · compliance **`attendeeOpinionsMetCount`** · branch dashboard **`caseManagementAttendeeOpinionGapCount`** |
| FE unit | **`programComplianceServices.test`** — **`createCaseManagementMeetingApi`** 가 **`attendeeOpinions`** 를 **`apiFetch` body**에 전달 |
| FE pilot | **`pilotPageFlows`** — **「홍길동, 김영희, 홍길동」** 중복 참석자 시 **POST 미호출**·필드 오류 (BNK-394) |
| BE pilot | **`ProgramCompliancePilotServiceFlowE2eTest`** — G32 flow에 per-attendee opinions wire |
| BE service | **`CaseManagementServiceTest`** — **`updateMeetingShouldPersistAttendeeOpinionsWhenProvided`** · **`updateMeetingShouldRejectDuplicateAttendeeOpinionAuthor`** |

| 전제 | **`LIVE_E2E_WRITE=1`** · **`./scripts/run-live-e2e.sh`** · develop HEAD 백엔드 기동 (Q523) |

> 관련: Q516 · Q518 · Q520 · Q521 · USER_MANUAL §5-9 · DEPLOYMENT_GUIDE §3-7

### [TWR] Q523. **live E2E**에서 `attendeeOpinionsMetCount`·`caseManagementAttendeeOpinionGapCount`가 **undefined**로 실패해요. (QA-B152)

**A.** **운영 이슈 (QA-20260619-B152, Q523)** — FE·BE **소스는 develop 최신**이어도 **`localhost:8080`에 오래된 JAR/프로세스**가 떠 있으면 G32 필드가 응답에 없을 수 있습니다. **256차**부터 probe·live harness가 **사전 진단·graceful skip**을 제공합니다 (Q525).

| 증상 | `programComplianceLiveApi.e2e.test.js` **FAIL** — compliance **`attendeeOpinionsMetCount`** · dashboard **`caseManagementAttendeeOpinionGapCount`** **undefined** |
|------|------|
| 원인 | 런타임 백엔드가 develop **`@45d95ea` 이전** 빌드 — compliance·dashboard에 G32 필드 미포함 · Flyway **V157 미적용** 가능 |
| **사전 확인 (Q525)** | **`GET /api/v1/system/live-e2e/probe`** — **`operationBlockers[]`** 에 **`g32-compliance-field-missing`·`g32-dashboard-field-missing`·`g32-v157-constraint-missing`** 포함 여부 |
| 확인 | **`GET /api/v1/health`** 200이어도 **코드 버전 불일치** 가능 — git HEAD와 **실행 중 프로세스**를 대조 |
| 조치 | 백엔드 **재빌드·재기동** — `cd src/backend && mvn spring-boot:run`(또는 배포 파이프라인 최신 이미지) · Flyway **V157** 적용 확인 후 **`./scripts/run-live-e2e.sh`** 재실행 |
| **256차 FE 동작** | probe에 G32 blocker가 있거나 필드가 **undefined**이면 해당 assertion **skip** — **false FAIL 방지** (`09912ba`, Q525) |

| 예방 | merge·배포 후 **스테이징 백엔드 재시작**을 live E2E 전 필수 절차로 포함 (DEPLOYMENT_GUIDE §3-7) |

> 관련: Q522 · Q521 · Q525 · DEPLOYMENT_GUIDE §3-7·§9-1 · `docs/qa/QA_FEEDBACK.md` QA-B152

### [TWR] Q524. **건강 기록** 화면에서 **급여제공 세분화 메뉴**는 무엇인가요? (케어포 3-1, v1.2.1 P1)

**A.** **Fixed (FE `1d5747d`, v1.2.1 P1, Q524)** — **`/health`** 상단에 **「급여제공 기록 (케어포 3-1)」** 세분화 네비가 표시됩니다. 일일 건강·투약 입력과 **별도 화면**의 급여제공 기록으로 바로 이동합니다.

| 링크 | 경로 | 용도 |
|------|------|------|
| 식사 관리 | `/meals` | 일자별 식사 등록 |
| 식사 도움 | `/care/meal-assistance-records` | L02_M13 통합식사도움 |
| 간호 제공 | `/nursing/service` | L03 간호급여 제공기록 |
| 목욕 일정 | `/care/bathing-schedules` | L02_M03 목욕 일정·제공 |
| 프로그램 | `/programs` | 프로그램 참여 기록 |
| 특이사항 | `/care/service-special-notes` | L02_M15 요양급여 특이사항 |

| UI | **`CareProvisionSegmentNav`** — **`RecordsContextNav`** 아래 · **`aria-label`「급여제공 기록 세분화」** · **`careProvisionSegments.js`** 단일 원천 |
| 안내 문구 | 카드 lede — 「급여제공 기록은 상단 세분화 메뉴에서 입력합니다」 |
| 테스트 | **`CareProvisionSegmentNav.test`** · **`HealthPage.test`** |

> 관련: USER_MANUAL §4-5·§6-4 · Q243(G15 이용자 상세 급여제공) · ROADMAP v1.2.1 P1

### [TWR] Q525. **live E2E probe**에서 G32 **schema readiness** blocker는 무엇인가요? (QA-B95·QA-B152)

**A.** **Fixed (BE `c0a59aa`·`45d95ea`·`caeac0d` + FE `09912ba`, G32, Q525·Q528)** — **`GET /api/v1/system/live-e2e/probe`** 및 **`GET /api/v1/health`** 가 G32 관련 **3가지 readiness**를 검사하고, 미충족 시 **`operationBlockers[]`** 에 추가합니다. live E2E 실행 **전** stale runtime을 감지합니다.

| Probe 필드 | 검사 내용 | blocker 코드 |
|------------|----------|--------------|
| `g32ComplianceAttendeeOpinionsFieldReady` | **`CaseManagementComplianceResponse`** 에 **`attendeeOpinionsMetCount`** record component 존재 | `g32-compliance-field-missing` |
| `g32DashboardAttendeeOpinionGapFieldReady` | **`BranchDashboardResponse`** 에 **`caseManagementAttendeeOpinionGapCount`** 존재 | `g32-dashboard-field-missing` |
| `g32AttendeeOpinionsArrayCheckReady` | **`G32SchemaReadinessProbe`** — **`pg_catalog`** 로 V157 **`chk_case_management_meetings_attendee_opinions_array`** 존재 | `g32-v157-constraint-missing` |

| `operationReady` | 위 3축 + staff/guardian bootstrap + G21 seed **모두 true**일 때만 true (`c0a59aa`·`45d95ea`) |
| FE harness | **`getLiveE2eOperationBlockers()`** — probe state에서 blockers 파싱 · **`hasG32SchemaBlocker()`** 시 G32 live assertion **skip** · **`shouldSkipForMissingG32Field()`** — legacy 응답 **undefined** 시 skip (`09912ba`) |
| 조치 | blocker별 — **JAR 재빌드·재기동**(필드 missing) · **Flyway V157 적용**(constraint missing) — Q523과 동일 |

| 테스트 | **`LiveE2eControllerTest`** · **`G32SchemaReadinessProbeTest`** · **`LiveE2eBootstrapLiveApiRoutingE2eTest`** · **`liveE2eHarness.test`** · **`programComplianceLiveApi.e2e.test.js`**

> 관련: Q521 · Q522 · Q523 · Q528 · DEPLOYMENT_GUIDE §3-7·§9-1 · ADMIN_GUIDE §1-4

### [TWR] Q526. **RFID split-view**에서 **계획·청구 NHIS 비교**가 각각 보이나요? (G21)

**A.** **Fixed (FE `9b80505`, G21, Q526)** — **분할 비교 모드**(`?split=1`)일 때 **「공단 명세 사전 비교」** 패널이 **계획(PLAN)·청구(BILLING) 각 1개**로 나뉘어 표시됩니다. 탭을 바꿀 필요 없이 **양 track**을 동시에 점검할 수 있습니다.

| 항목 | 내용 |
|------|------|
| 화면 | **`/visits?split=1`** — **`role="group" aria-label="계획·청구 공단 명세 분할 비교"`** 래퍼 안에 패널 2개 |
| 제목 | **「계획 공단 명세 사전 비교」** · **「청구 공단 명세 사전 비교」** (`SCHEDULE_KIND_LABELS`, Q527) |
| API | 각 패널이 **`GET /api/v1/visits/nhis-comparison`** — **`scheduleKind=PLAN|BILLING`** 별도 호출 |
| 단일 모드 | split-view OFF 시 **활성 탭**(`PLAN`/`BILLING`) 기준 **패널 1개** (Q486 carry) |
| 월말 루틴 | 계획·청구 **페어 점검(Q377)** → **양쪽 NHIS 대조(Q526)** → **일괄확정(Q330)** |

| 테스트 | **`VisitNhisComparisonPanel.test`** · **`VisitsPage.test`**

> 관련: Q377 · Q486 · Q480 · USER_MANUAL §5-11

### [TWR] Q527. **방문일정·사례관리·보호자 문서** 화면 **접근성(UXD-138)** 이 강화됐나요?

**A.** **Fixed (FE `d354a0e`, UXD-138, Q527)** — 최근 G21·G32·G2 커밋의 a11y·CSS 갭을 정리했습니다.

| 화면 | 개선 |
|------|------|
| **`/visits` split-view NHIS** | 패널 제목 **「{계획\|청구} 공단 명세 사전 비교」** — 동일 문구 중복 제거 · **group landmark** (Q526) |
| **`/case-management/meetings`** | 행 **수정** 버튼 **`aria-label`「{회의일} 사례관리 회의록 수정」** |
| **`/clients/:id` 보호자 문서** | **`GuardianDocumentNotifyPanel`** 발송 **`aria-describedby`** — 문서 유형·수신 대상 help 연결 |
| **공통 CSS** | **`ds-form-grid__full`·`ds-field__hint`** 정의 추가 — FE-16 폼 레이아웃 누락 수정 |

| 테스트 | **`CaseManagementPage.test`** · **`GuardianDocumentNotifyPanel.test`** · **`VisitNhisComparisonPanel.test`**

> 관련: Q516 · Q517 · Q526 · USER_MANUAL §2-2

### [TWR] Q528. **`GET /api/v1/health`** 에도 G32 **schema readiness** 가 있나요? (QA-B95·QA-B152)

**A.** **Fixed (BE `caeac0d`, G32, Q528)** — **256차**부터 probe만 있던 G32 3축 검사가 **`GET /api/v1/health`** 에도 노출됩니다. live E2E **`globalSetup`** 이 health 1회 호출로 **stale backend**를 더 빨리 감지합니다.

| Health 필드 | 의미 | blocker |
|-------------|------|---------|
| `liveE2eG32ComplianceAttendeeOpinionsFieldReady` | compliance **`attendeeOpinionsMetCount`** | `g32-compliance-field-missing` |
| `liveE2eG32DashboardAttendeeOpinionGapFieldReady` | dashboard **`caseManagementAttendeeOpinionGapCount`** | `g32-dashboard-field-missing` |
| `liveE2eG32AttendeeOpinionsArrayCheckReady` | V157 JSONB array CHECK | `g32-v157-constraint-missing` |

| `liveE2eOperationBlockers[]` | 위 3축 미충족 시 probe와 **동일 코드** 추가 |
| 조치 | Q525·Q523과 동일 — **JAR 재빌드·재기동** · **Flyway V157** |

| 테스트 | **`HealthControllerTest`** — G32 readiness true/false · blockers mirror probe

> 관련: Q525 · Q523 · DEPLOYMENT_GUIDE §3-7 · ADMIN_GUIDE §1-4 item 10

### [TWR] Q529. live E2E **자격증명 env**에 **앞뒤 공백**이 있으면 어떻게 되나요?

**A.** **Fixed (BE `7848b0f`, QA-B95, Q529)** — **`LiveE2eBootstrapService`** 가 staff/guardian email·password를 **`trim()`** 한 뒤 readiness·seed-user 대조를 수행합니다.

| 상황 | 동작 |
|------|------|
| **`LIVE_E2E_EMAIL=" user@x.com "`** | trim 후 정상 인식 |
| **공백만** (`"   "`) | **미설정**과 동일 — bootstrap 실패·**`operationReady=false`** |
| **default seed + trim** | **`defaultCredentials`** blocker 유지 (Q490·Q491) |

| 구현 | **`normalizeCredential(String)`** — null → `""` · **`LiveE2eBootstrapServiceTest`** whitespace regression |
| 운영 | `.env`·CI secret에 **의도치 않은 trailing space** 제거 — Q494 **`LIVE_E2E_CLIENT_ID` trim** 과 동일 패턴 |

> 관련: Q490 · Q491 · Q494 · DEPLOYMENT_GUIDE §3-6 · `scripts/dev-live-e2e.env.example`

### [TWR] Q530. **현금영수증 발급목록**은 ogada에서 어디서 관리하나요? (이지케어 FAQ21700·21701)

**A.** **✅ full stack (BE `4432558`/`fe54af8`/`ab5708b`/`58ff35e`·FE `cfc4b04`/`a17f148`/`221458e`/`8aebe55`, G-CASH-RECEIPT-LOG, BNK-398/400/404/405, Q530·Q532·Q533)** — SideNav **청구 → 현금영수증 발급목록**(`/billing/cash-receipts`) 또는 **청구 리포트 서브 탭**에서 **국세청(NTS) 발급 이력**을 등록·조회합니다. **수납대장**(`/billing/reports/receipts`)과 **별도**입니다. **현금 수납**(`POST /claims/{id}/payments`) 직후 **`/billing/payments`** 에서 **`CashReceiptRegisterModal`** 이 **자동으로 열립니다** (Q531, FE `a17f148`). **대시보드**(`/dashboard`·`/dashboard/hq`) **「현금영수증 미발급」**·**「발급 지연」** 위젯으로 미처리 건을 먼저 확인할 수 있습니다 (Q532, FE `221458e`). **작년분 청구** 발급 시 **연말정산 불가** 안내 Alert가 표시됩니다 (Q533, FE `8aebe55`).

| 이지케어 | ogada |
|----------|-------|
| **2.10 현금영수증 발급목록** | **`/billing/cash-receipts`** — **「발급 목록」** 탭 |
| **현금영수증 발급정보 관리** (수급자별) | **「수급자별 발급정보」** 탭 — 이용자 검색·최근 20건 |
| 수납 **동시** 발급(체크박스) | **△** — 수납 Modal **체크박스 없음** · **수납 직후 발급 Modal 자동 표시** (Q531) |
| NTS **자동** 발급·취소 API | **❌** — **국세청 발급번호 수동 입력** (Q531) |

| 화면 | **`CashReceiptIssuancePage`** — **`BillingReportsContextNav`** 7탭 중 하나 |
| API | **`GET /api/v1/billing/cash-receipt-issuances?month=&q=&page=&size=`** · **`GET …/cash-receipt-issuances/pending?branchId=`** — **`branchId` 생략 시 다지점 `hq_admin` 전 지점 집계** (Q533, `58ff35e`) · **`POST …/cash-receipt-issuances`** · **`GET …/clients/{clientId}/cash-receipt-profile`** · **`GET /dashboard/branch`**·**`/dashboard/hq`** — **`cashReceiptPendingCount`·`cashReceiptOverdueCount`** |
| 권한 | **`hq_admin`·`branch_admin`** — `caregiver` 등 **거부** (`RoleBasedControllerAccessTest`) |
| 등록 조건 | 청구 **`PAID`** · 결제수단 **`CASH`** · 청구당 **1건** · 발급금액 = **본인부담금** · 발급일 ≥ 수납일 · 발급일 ≤ 오늘 |
| **즉시발급** | **`immediateIssuanceMet`** — 발급일이 수납일 **+7일 이내**면 「충족」, 초과 시 「지연」(FAQ 21716) |
| 미발급 알림 | 목록 상단 **「미발급 현금수납」** StatCard · **대시보드** **「현금영수증 미발급」**·**「발급 지연」** 위젯 (Q532) — `PAID`+`CASH` 중 발급 기록 없는 건수·7일 SLA 초과 건수 |
| 법정 안내 | **`CashReceiptLegalAlerts`** — FAQ **21716**(납입 즉시 발급)·**21717**(작년분 연말정산) · **`CashReceiptRegisterModal`** — **`priorYearIssuanceEligible`** 시 **warning Alert** (Q533) |
| DB | **V158** `cash_receipt_issuances` — `nts_receipt_no`·`identifier_type`(PHONE/BIZ) unique per org |

**발급 등록 절차**

1. **현금**으로 **수납 완료(`PAID`)** 한 청구를 준비합니다 (§5-10).
2. **(권장)** **청구 → 입금 처리** — **현금** 수납 저장 시 **`CashReceiptRegisterModal`** 이 **자동 표시**됩니다 (Q531).
3. 또는 **청구 → 현금영수증 발급목록** → **「발급 등록」**.
4. Modal에서 **미발급 현금 수납 청구**를 선택합니다 — 이용자 **휴대폰**이 있으면 식별자가 **자동 제안**됩니다.
5. **국세청 발급번호**·**식별자**·**발급일**·**금액**을 입력하고 **발급 저장**합니다.

> 관련: Q531 · Q532 · Q533 · Q252 · Q379 · USER_MANUAL §5-10-1 · ADMIN_GUIDE §10-4

### [TWR] Q532. **현금영수증 미발급·발급 지연**을 대시보드에서 확인할 수 있나요? (FAQ21716)

**A.** **✅ full stack (BE `fe54af8`/`ab5708b`·FE `221458e`, G-CASH-RECEIPT-LOG, BNK-404, Q532)** — **`hq_admin`·`branch_admin`** 은 **`/dashboard`**(지점) 또는 **`/dashboard/hq`**(통합)에서 **「현금영수증 미발급」**·**「현금영수증 발급 지연」** StatCard 2종을 확인할 수 있습니다. 이지케어 FAQ **21716** 「본인부담금이 **납입되는대로 바로 발급**」 원칙에 맞춘 **7일 SLA due-gate** 입니다.

| 위젯 | 집계 | 톤 | 클릭 이동 |
|------|------|-----|-----------|
| **현금영수증 미발급** | `PAID`+`CASH` 중 NTS 발급 이력 **없음** (`cashReceiptPendingCount`) | 1건↑ **warning** | `/billing/cash-receipts` |
| **현금영수증 발급 지연** | 수납일 **+7일 초과** 미발급 (`cashReceiptOverdueCount`) | 1건↑ **danger** | `/billing/cash-receipts` |

| API | **`GET /api/v1/dashboard/branch`** · **`GET /api/v1/dashboard/hq`** — **`cashReceiptPendingCount`·`cashReceiptOverdueCount`** |
| pending 목록 | **`GET /api/v1/billing/cash-receipt-issuances/pending?branchId=`** — **`items[]`** — `billingClaimId`·**`branchId`**·`clientName`·`paidAt`·**`issuanceDueAt`**·**`overdue`**·**`priorYearIssuanceEligible`** · **`totalElements`·`overdueCount`** — **`branchId` 생략** 시 다지점 **`hq_admin` 전 지점 집계** (Q533) |
| 권한 | **`hq_admin`·`branch_admin`** — `caregiver` 등 **위젯 미표시** |
| HQ | **`/dashboard/hq`** — **전 지점 합산** |

**운영 절차**

1. 아침 **`/dashboard`** 에서 **「현금영수증 발급 지연」** 이 1건 이상이면 **우선 처리**합니다.
2. StatCard 클릭 → **`/billing/cash-receipts`** → **「발급 등록」** 또는 **입금 처리** 연동 Modal(Q531)로 NTS 발급번호를 등록합니다.
3. **`sysadmin`**·통합 API 점검 시 **`GET …/cash-receipt-issuances/pending`** 로 상세 목록을 확인합니다 (ADMIN_GUIDE §10-4).

> 관련: Q530 · Q531 · Q533 · FAQ21716 · USER_MANUAL §4-2·§5-10-1 · ADMIN_GUIDE §1-4·§10-4

### [TWR] Q533. **작년 본인부담금** 현금영수증을 발급하면 **올해 연말정산**에 반영되나요? (FAQ21717)

**A.** **✅ full stack (BE `58ff35e`·FE `8aebe55`, G-CASH-RECEIPT-LOG, BNK-405, Q533)** — **발급 자체는 가능**하지만, **작년 청구월(`yearMonth`) 본인부담금**은 **올해 연말정산(근로·연금 소득)** 에 **반영되지 않습니다**. 이지케어 FAQ **21717** 「작년분은 **종합소득세 신고**로 공제」 원칙을 ogada가 **발급 Modal**에서 **warning Alert**로 안내합니다.

| 조건 | `priorYearIssuanceEligible` | UI |
|------|----------------------------|-----|
| 청구월 연도 **< 올해** | `true` | **`CashReceiptRegisterModal`** — 「작년 본인부담금은 올해 연말정산에 반영되지 않습니다…」 |
| 올해 청구월 | `false` | Alert **미표시** |

| API | **`GET …/cash-receipt-issuances/pending`** — 항목별 **`priorYearIssuanceEligible`** (BE `58ff35e`) |
| 수납 연동 | **`PaymentPage`** — 현금 수납 후 Modal에 **`isPriorYearClaim(yearMonth)`** 로 동일 Alert (FE `8aebe55`) |
| HQ pending | **`branchId` 생략** — 다지점 **`hq_admin`** 이 **전 지점 미발급**을 한 API로 조회 (수납일 내림차순) |
| P3 carry | **G-CASH-RECEIPT-TAX-DEDUCTION-EXPORT** — 케어포 **7-2-1 연말정산 batch Excel** **연 1회 일괄 자동 export** (지점 **「국세청 CSV」** 는 Q534 ✅) |

**운영 절차**

1. **작년 미발급** 건을 **`/billing/cash-receipts`** → **「발급 등록」** 또는 **입금 처리** Modal에서 등록합니다.
2. **warning Alert**를 확인하고, 보호자·이용자에게 **종합소득세 신고** 안내를 별도로 합니다.
3. **통합 관리자**는 **`GET …/pending`** (`branchId` 생략)으로 **전 지점** 미발급·작년분 플래그를 점검합니다.

> 관련: Q530 · Q531 · Q532 · FAQ21717 · USER_MANUAL §5-10-1 · ADMIN_GUIDE §10-4

### [TWR] Q534. **본인부담 통계**에서 **수납년도**와 **청구년도**를 바꿔 조회할 수 있나요? **국세청 batch CSV**도 받을 수 있나요? (케어포 7-8, G26)

**A.** **✅ full stack (BE `ceeaeb9`/`43ef73b`·FE `19ed7f3`, G26, BNK-407/408, Q534)** — SideNav **청구 → 본인부담 통계**(`/billing/reports/statistics`) ① **국세청 의료비공제 통계** 섹션에서 **「수납년도」**(`PAID_YEAR`, 기본) · **「청구년도」**(`CLAIM_YEAR`) segmented control로 집계 기준을 전환할 수 있습니다. **「국세청 CSV」** 로 선택 기준·조회 연도·(선택) 검색어에 맞는 **지점 batch CSV**를 다운로드합니다.

| 항목 | 내용 |
|------|------|
| 화면 | **`BillingStatisticsReportPage`** — **`MEDICAL_EXPENSE_YEAR_BASIS_OPTIONS`** · **`role="tablist"`** a11y |
| 목록 API | **`GET /api/v1/billing/reports/medical-deduction?taxYear=&yearBasis=PAID_YEAR\|CLAIM_YEAR&…`** |
| export API | **`GET /api/v1/billing/reports/medical-deduction/export?taxYear=&yearBasis=&q=`** — UTF-8 CSV(BOM) |
| 집계 규칙 | Q252·Q254와 동일 — **`CASH`·`BANK_TRANSFER`만** · **CMS·간편결제 제외** |
...
**A.** **✅ full stack (BE `e454d3b`·FE `58d6694`, G-7-1, BNK-409, Q535)** — **`/billing/claims/:claimId`** **「명세 인쇄 산출물 (7-1)」** 카드 **`BillingStatementPrintPanel`** 에 **인쇄** 버튼 아래 **Excel(CSV) 다운로드** 행이 추가되었습니다. 케어포 PDF p.87 ② **엑셀다운로드** parity입니다.

| `kind` | 버튼 라벨 | 비고 |
|--------|----------|------|
| `address-label` | 주소라벨 Excel | |
| `statement` | 급여비용명세서 Excel | |
| `receipt` | 영수증 Excel | **`PAID`+`paidAt`만** — 미수납 시 비활성 (Q478·Q485) |
| `claim-list` | 청구리스트 Excel | 기본 kind |
| `all` | 전체 Excel | 미수납 시 **「(영수증 제외)」** |
...
**A.** **✅ full stack (BE `298bcdf`/`35d1560`/`4da0ca8`/`15061a9`·FE `76a462d`/`99b795a`/`501fedc`/`965e569`, G-CASH-RECEIPT-LOG, Q537)** — 발급 목록 검색과 발급 등록 식별자가 **숫자 기준으로 정규화**되며, 잘못된 자릿수·**숫자가 없는 입력**은 **저장 전에 거부**됩니다. FE **`CashReceiptRegisterModal`** 은 **API 호출 전** 필드 단위로 검증합니다. **V159** DB CHECK로 raw SQL 적재도 차단합니다 (Q543).

| 항목 | 동작 |
|------|------|
| 목록 검색 `q` | 이용자명·국세청 발급번호 외 **휴대폰·사업자번호 숫자**로도 검색 — `010-1234-5678`·`01012345678` **동일 매칭** (`GET …/cash-receipt-issuances?q=`) |
| 발급 등록 식별자 | 입력값에서 **하이픈·공백 제거 후 숫자만 저장** — 예: `010-1234-5678` → `01012345678` |
| **FE submit 전 검증** | **`CashReceiptRegisterModal`** — **`normalizeIdentifierValue`** · **PHONE 10~11·BIZ 10·빈 값 `fieldErrors`** · payload **digit-only** (FE `76a462d`) · **`CashReceiptRegisterModal.test`** |
| **FE a11y (Q544)** | 식별자 **`TextInput`** — 프로필 로드 중 **`aria-busy`** (UXD-141, `965e569`) |
| 검증 (BE) | **휴대폰(PHONE)**: 10~11자리 · **사업자(BIZ)**: 10자리 — 미달 시 **「휴대폰 번호는 10~11자리 숫자여야 합니다.」** / **「사업자등록번호는 10자리 숫자여야 합니다.」** |
| 검증 (FE) | **「휴대폰 번호는 숫자만 입력하고 10~11자리여야 합니다.」** · **「사업자등록번호는 숫자 10자리여야 합니다.」** |
| 숫자만 검증 | **문자만 입력**(예: `abcdefghij`) 시 **「발급 식별자(휴대폰/사업자번호)는 숫자만 입력해 주세요.」** (BE `4da0ca8`) |
| **DB CHECK (Q543)** | **V159** **`chk_cash_receipt_issuances_identifier_value_format`** — **`identifier_value ~ '^[0-9]+$'`** · PHONE 10~11·BIZ 10 — V158 nonempty CHECK **유지** (defense-in-depth) |
| pending API 실패 | **`/billing/cash-receipts`** — **warning Alert** 「미발급 현금수납 목록을 불러오지 못했습니다…」 — Alert **`id`**·**「발급 등록」`aria-describedby`** (Q538) — 발급 목록 조회는 계속 가능 |
| 작년분 표시 | 발급 등록 Modal 청구 선택 목록에 **`priorYearIssuanceEligible`** 건 **「· 작년분」** suffix |

> 관련: Q530 · Q533 · Q536 · Q538 · Q543 · Q544 · USER_MANUAL §5-10-1 · ADMIN_GUIDE §10-4

### [TWR] Q539. **22:00~08:00**에 **보호자 서류(가정통신문·급여제공기록지·노인학대예방)** 수동 발송이 실패하면? (J03)

**A.** **✅ BE Fixed (`71b2d32`/`7e4c07e`, J03, Q539)** — 이용자 상세 **`GuardianDocumentNotifyPanel`** 에서 **직원이 수동 발송**하는 보호자 서류 이메일은 **조용한 시간대(22:00~08:00 KST)** 에 **`422 BUSINESS_RULE`** 로 **거부**됩니다. 이전에는 **자동 dispatch skip** 과 동일하게 **성공처럼 보일 수 있었으나**, 수동 경로는 **명시적 오류**를 반환합니다.

| 항목 | 내용 |
|------|------|
| **대상 API** | **`POST /api/v1/clients/{clientId}/notifications/care-provision-record`** · **`…/home-newsletter`** · **`…/elder-abuse-prevention-guideline`** |
| **BE 동작** | **`GuardianDocumentNotificationService`** → **`NotificationService.dispatchManualClientEvent`** — quiet-hours 시 **`BusinessRuleException`** · 메시지 **`NotificationQuietHoursPolicy.manualDispatchBlockedMessage()`** |
| **오류 메시지** | **「현재 22:00~08:00 (Asia/Seoul) 조용한 시간대입니다. 긴급 알림을 제외한 보호자 알림 발송이 제한됩니다.」** |
| **자동 알림** | 출석·건강·수납 등 **`dispatchClientEvent`** — quiet-hours **skip(무음)** 동작 **불변** (Q147) |
| **UI** | **`GuardianDocumentNotifyPanel`** — API 오류를 **danger Alert**로 표시 — **08:00 이후 재시도** |
| **테스트** | **`J03GuardianDocumentManualNotifyQuietHoursE2eTest`** · **`NotificationServiceTest.dispatchManualClientEventShouldRejectDuringQuietHours`** |

> 관련: Q329 · Q517 · Q147 · USER_MANUAL §4-7-3 · ADMIN_GUIDE §10-8 · DEPLOYMENT §11-3
...
| 항목 | 내용 |
|------|------|
| **재계약 기한** | 서명일 **+1년** — **`computeEmploymentContractRenewalDueDate`** |
| **보관 기한** | 서명일 **+3년** — **`computeEmploymentContractRetentionUntil`** (근로계약서·임금협의 기록) |
| **기한 초과** | **`isEmploymentContractRenewalOverdue`** — **warning Alert**·**「기한 초과」** Badge |
| **서명일 미등록** | **`MISSING`** 상태 — lifecycle **「입사~퇴사」** **`DateInput`** 또는 **「재계약 완료 기록」** Modal (Q547) |
| **재계약 기록 (Q547)** | **`updateUserApi`** — **`contractSignedAt`** + **`lifecycleChecklist["employment-contract"]=true`** · **`branch_admin`·`social_worker`만** |
| **기한 임박 Alert** | **`EMPLOYMENT_CONTRACT_RENEWAL_DUE_SOON_DAYS=30`** — **`computeEmploymentContractRenewalDueAlerts`** |
| **최저임금 참고** | **2026년 10,320원/시간** — **`MINIMUM_WAGE_HOURLY_KRW_2026`** (정적 안내) |
| **필수 5항 (Q546)** | **임금·소정근로시간·휴일·연차유급휴가·근로조건** — **`EMPLOYMENT_CONTRACT_REQUIRED_CLAUSES`** ordered checklist |
| **연간 갱신 절차 (Q546)** | **① 임금협의 → ② 근로(재)계약서 작성 → ③ 보관·등록** — **`EMPLOYMENT_CONTRACT_RENEWAL_WORKFLOW`** |
| **서식 Modal (Q546)** | **「근로계약서 서식 보기」** — **`EMPLOYMENT_CONTRACT_RENEWAL_TEMPLATE`** — 출력·필기용 **`pre`** (BE PDF 생성 ❌) |
| **법적 안내** | 임금·소정근로·휴일·연차·근로조건 **5항** · 기관·근로자 **각 1부** 보관 |
...
> 관련: Q290 · Q298 · Q300 · Q546 · Q547 · USER_MANUAL §4-2·§5-3 · ADMIN_GUIDE §6-2-6b

### [TWR] Q541. live E2E에서 **G21 seed가 applicable=false** 이고 **`g21-seed=not-applicable`** 이면? (QA-B95)

**A.** **✅ BE Fixed (`7a9d7a5`/`091c372`, QA-B95, Q541)** — **`GET /api/v1/health`**·**`GET /api/v1/system/live-e2e/probe`** 의 G21 seed 상태는 **`LiveE2eBootstrapService.g21SeedStatus`** 결과입니다. **`LIVE_E2E_BRANCH_ID`** 지점의 **`serviceType`** 이 G21 seed **대상이 아니면** **`applicable=false`** 이지만, **`091c372` 이후** **`liveE2eOperationReady=true`** 를 **유지**합니다 — **일반 live E2E suite는 통과**하고 **`liveG21Describe`** suite만 **skip**됩니다.

| 원인 | 확인·조치 |
|------|----------|
| **지점 없음·비활성·org 불일치** | **`g21-branch-missing-or-inactive`** (Q505·Q502) — env **`LIVE_E2E_BRANCH_ID`**·지점 **`active=true`** 확인 · **`operationReady=false`** |
| **unsupported serviceType (갱신)** | **`HOME_VISIT`(home-visit-like)·legacy `DAY_CARE` 외** — 예: **`FACILITY_CARE` 전용** → **`notApplicable()`** (`091c372`) · health detail **`g21-seed=not-applicable`** · **`liveE2eOperationReady=true`** · **`liveG21Describe` skip** |
| **적용 가능하나 seed 미비** | **`visit-schedule-missing`·`billing-visit-schedule-missing`·`nhis-import-missing`** (Q495·Q500) — **`POST /api/v1/system/live-e2e/bootstrap`** 재실행 |
| **cross-branch seed** | PLAN/BILLING/NHIS가 **다른 branchId** — ready flags **false** (Q507) |

| G21 seed applicable serviceType | health·operation | live E2E |
|--------------------------------|------------------|----------|
| **`HOME_VISIT`** (및 home-visit-like) | **`applicable=true`** — seed readiness 검증 | **`liveG21Describe` 실행** |
| **`DAY_CARE`** (legacy) | **`applicable=true`** — bootstrap **`HOME_VISIT` 승격** 전까지 (Q501) | **`liveG21Describe` 실행** |
| **그 외** | **`notApplicable()`** — **`g21-seed=not-applicable`** · **`operationReady` 유지** (`091c372`) | **`liveG21Describe` skip** · **다른 suite는 통과 가능** |

> 관련: Q495 · Q500 · Q501 · Q502 · Q505 · ADMIN_GUIDE §1-4 · DEPLOYMENT §3-7

### [TWR] Q542. live E2E에서 **`staff-credentials-default`** blocker가 **사라졌어요**. **`ogada.live-e2e.allow-default-credentials`** 는? (QA-B95)

**A.** **✅ BE Fixed (`beef81e`, QA-B95, Q542)** — Q490·Q491에서 **default staff/guardian creds** 사용 시 **`operationReady=false`** 를 강제했으나, **로컬·staging quick start** 에서 `./scripts/run-live-e2e.sh` 가 **env 미설정으로도 preflight 통과**할 수 있도록 **`ogada.live-e2e.allow-default-credentials`** 를 추가했습니다. **기본값 `true`** 입니다.

| 설정 | 동작 |
|------|------|
| **`ogada.live-e2e.allow-default-credentials=true`** (기본) | **`LIVE_E2E_EMAIL`/`PASSWORD`**·guardian env **미설정** → default creds 사용해도 **`staff-credentials-default`/`guardian-credentials-default` blocker 없음** · **`liveE2eOperationReady=true`** (G21 seed·client 등 **다른 blocker**는 유지) |
| **`ogada.live-e2e.allow-default-credentials=false`** | Q490·Q491 **이전 동작** — default creds → **`operationReady=false`** · **`staff-credentials-default`/`guardian-credentials-default`** in **`liveE2eOperationBlockers[]`** |
| **health 필드** | **`liveE2eAllowDefaultCredentials`** — probe·health **동일** |
| **프로덕션 권장** | **명시 creds** + **`allow-default-credentials=false`** — false-green 방지 |
| **테스트** | **`LiveE2eControllerTest.probeShouldAllowDefaultCredentialsWhenFlagEnabled`** · **`HealthControllerTest`** |

> 관련: Q490 · Q491 · Q510 · DEPLOYMENT §3-7·§11-3 · `./scripts/dev-live-e2e.env.example`

### [TWR] Q543. **`cash-receipt-identifier-check-missing`** blocker와 **V159** 현금영수증 식별자 CHECK는? (G-CASH-RECEIPT-LOG)

**A.** **✅ BE Fixed (`15061a9`/`1139e79`, G-CASH-RECEIPT-LOG, Q543)** — Q537에서 앱 레벨(`CashReceiptIssuanceService`)로 강제하던 **숫자-only·자릿수** 규칙을 **V159 DB CHECK**로도 방어합니다. live E2E **health probe**는 G32 V157 패턴과 동일하게 **마이그레이션 누락 시 `operationReady=false`** 를 노출합니다.

| 항목 | 내용 |
|------|------|
| **V159 CHECK** | **`chk_cash_receipt_issuances_identifier_value_format`** — **`identifier_value ~ '^[0-9]+$'`** · **PHONE 10~11·BIZ 10** — V158 nonempty CHECK **유지** (defense-in-depth) |
| **health 필드** | **`cashReceiptIdentifierValueCheckReady`** — **`CashReceiptSchemaReadinessProbe.identifierValueFormatCheckReady()`** — `pg_catalog`로 CHECK 존재 검증 |
| **blocker** | **`cash-receipt-identifier-check-missing`** — **`liveE2eOperationBlockers[]`** · **`liveE2eOperationReady=false`** |
| **조치** | Flyway **V159** 적용 확인 → **`mvn flyway:migrate`** 또는 배포 파이프라인 재실행 → **`GET /api/v1/health`** 재확인 |
| **테스트** | **`CashReceiptSchemaReadinessProbeTest`** · **`HealthControllerTest`** |

> 관련: Q537 · Q528 · Q525 · DEPLOYMENT §11-3 · ADMIN_GUIDE §10-4 · DATA_RETENTION_POLICY §2

### [TWR] Q544. **FAQ21823 근로재계약**·**현금영수증 식별자** 접근성(a11y)은? (UXD-141)

**A.** **✅ FE Fixed (`965e569`/`202c1fe`/`debe6dd`, UXD-141, Q544)** — UXD-140 이후 **FAQ21823 근로재계약 UI**와 **현금영수증 발급 Modal 식별자 필드**를 재점검했습니다. **`debe6dd`** 에서 **직원 목록·현황 리포트** 링크 **`aria-label`** 에 **stable staff ID**를 append해 full-suite 중복 accessible name을 방지합니다.

| 화면 | 보강 내용 |
|------|----------|
| **`/staff/{userId}` → 「입사~퇴사」** | **`StaffEmploymentContractRenewalPanel`** — 기한 초과 **warning Alert `id`** · **「근로계약서 파일함」** 버튼 **`aria-label`「{직원명} 근로계약서 파일함」** · overdue 시 **`aria-describedby`** 로 Alert 연결 |
| **`/staff` 목록** | **`StaffEmploymentContractRenewalSummaryPanel`** — 확인 필요 표 이름 링크 **`aria-label`「{이름} 근로(재)계약 lifecycle 보기」** — 직원 테이블 **「직원 상세」** 라벨과 **분리** (QA-B161, `202c1fe`) |
| **`CashReceiptRegisterModal`** | 발급 식별자 **`TextInput`** — 수급자 프로필 로드 중 **`aria-busy`** |

| 테스트 | **`StaffEmploymentContractRenewalPanel.test`** · **`StaffEmploymentContractRenewalSummaryPanel.test`** · **`StaffPage.test`** · **`CashReceiptRegisterModal.test`** |

> 관련: Q540 · Q537 · Q538 · USER_MANUAL §4-2·§5-3·§5-10-1 · ADMIN_GUIDE §6-2-6b

### [TWR] Q545. live E2E가 **health 응답 형식이 달라** operation readiness를 못 읽으면? (QA-B95)

**A.** **✅ FE Fixed (`682d647`, QA-B95, Q545)** — staging·로컬에서 **구버전 백엔드** 또는 **중첩 JSON** health payload를 쓰는 경우, FE live E2E gate가 **false skip** 하던 문제를 **`liveBackendProbe.extractLiveE2eHealthFields`** 확장으로 해소했습니다.

| 항목 | 내용 |
|------|------|
| **지원 형식** | **flat camelCase** (`liveE2eOperationReady`) · **nested `liveE2e` object** · **snake_case** (`live_e2e_operation_ready`·`operation_blockers`·`seed_client_id` 등) |
| **파싱 대상** | **`liveE2eOperationReady`·`liveE2eG21SeedApplicable`·`liveE2eVisitScheduleReady`·`liveE2eBillingVisitScheduleReady`·`liveE2eNhisImportReady`·`liveE2eOperationBlockers[]`·`liveE2eSeedClientId`** |
| **효과** | stale/mixed runtime에서도 **blocker·seed readiness** 를 정확히 읽어 **`liveG21Describe`** skip reason 개선 |
| **테스트** | **`liveE2eHarness.test`** — nested·snake_case fixture regression |

> 관련: Q496 · Q521 · Q543 · DEPLOYMENT §3-7·§11-3 · `./scripts/run-live-e2e.sh`

### [TWR] Q546. FAQ21823 **근로계약서 필수 5항 checklist**·**서식 Modal**은? (US-R03)

**A.** **✅ FE Fixed (`1b6d2b1`, US-R03, Q546)** — Q540 상세 패널에 이지케어 FAQ21823 **필수 기재 5항**·**연간 갱신 절차**·**출력용 서식**을 추가했습니다. **BE PDF 생성·전자서명 API 없음** — 현장 **필기·스캔 업로드** workflow입니다.
...
> 관련: Q540 · Q544 · Q290 · USER_MANUAL §5-3 · ADMIN_GUIDE §6-2-6b
...
| **테스트** | **`StaffEmploymentContractRenewalPanel.test`** · **`EmploymentContractRenewalAlertsPanel.test`** · **`staffEmploymentContractCompliance.test`** · **`DashboardPage.test`** |
| **P2 잔여** | **전자서명 workflow** · **서식 PDF 자동 생성** · **G-Payroll 급여명세 연동** |

> 관련: Q540 · Q546 · Q290 · USER_MANUAL §4-2·§5-3 · ADMIN_GUIDE §6-2-6b

### [TWR] Q548. **`GET /health`** 와 **`GET /system/live-e2e/probe`** 의 **operation blocker** 가 다르면? (QA-B95)

**A.** **✅ BE Fixed (`bfad37d`/`4d4457f`, QA-B95, Q548)** — health·probe가 **별도 로직**으로 drift하던 문제를 **`LiveE2eOperationReadinessSupport`** 로 통합했습니다. **V159 cash-receipt schema readiness** 도 probe **`operationBlockers[]`** 에 동일하게 노출됩니다.

| 항목 | 내용 |
|------|------|
| **공유 클래스** | **`LiveE2eOperationReadinessSupport`** — **`operationReady`** · **`resolveOperationBlockers`** · **`cashReceiptIdentifierValueCheckReady`** · bootstrap·G32 gates |
| **적용 endpoint** | **`HealthController`** · **`LiveE2eController`** (`/api/v1/system/live-e2e/probe`) |
| **V159 probe alignment** | **`4d4457f`** — V159 미적용 시 probe도 **`cash-receipt-identifier-check-missing`** |
| **조치** | health·probe **`liveE2eOperationBlockers[]`** 비교 — 불일치 시 **백엔드 재기동**·**develop HEAD 확인** · Flyway **V159** 적용 |
| **테스트** | **`LiveE2eOperationReadinessSupportTest`** · **`LiveE2eControllerTest`** · **`HealthControllerTest`** |

> 관련: Q543 · Q525 · Q541 · DEPLOYMENT §3-7·§11-3 · `./scripts/run-live-e2e.sh`

### [TWR] Q549. live E2E가 **legacy health payload**에서 G21 seed applicable을 못 읽으면? (QA-B95)

**A.** **✅ FE Fixed (`16afd4c`, QA-B95, Q549)** — 구버전 백엔드 health 응답에 **`liveE2eG21SeedApplicable` 필드 없이** `liveE2eVisitScheduleReady` 등 **seed flags만** 있는 경우, FE gate가 G21을 **not applicable** 로 오판하던 문제를 **`liveConfig.js`** 에서 해소했습니다.

| 항목 | 내용 |
|------|------|
| **legacy 해석** | seed readiness flags 존재 + explicit **`applicable=false` 없음** → **G21 applicable** 로 treat |
| **효과** | mixed/stale runtime에서 **`liveG21Describe`** skip reason 정확도 개선 |
| **테스트** | **`liveE2eHarness.test`** — legacy payload regression |
| **권장** | staging·로컬은 **develop HEAD `bfad37d`/`16afd4c` 이상** 백엔드·프론트 사용 |

> 관련: Q541 · Q545 · Q548 · DEPLOYMENT §3-7 · `./scripts/run-live-e2e.sh`

### [TWR] Q554. 배차 **카카오 API 상태·사용량**은 어디서 확인하나요? (v1.3-A Kakao API status)

**A.** **✅ BE+FE Fixed (`138ac26`/`e2b764b`/`ba74bb5`, v1.3-A, Q554)** — 배차 경로·좌표에 쓰는 **Kakao REST API** 연결 상태와 **ogada 백엔드 기준 오늘 호출량(API별)** 을 화면에서 확인할 수 있습니다. **API 키 값은 노출되지 않습니다.**

| 항목 | 내용 |
|------|------|
| **API** | **`GET /api/v1/transport/kakao-api-status`** — **`hq_admin`·`sysadmin`** |
| **sysadmin UI** | **`/settings` → 「카카오 API」** 탭 — **`TransportKakaoApiStatusPanel`** |
| **hq_admin UI** | **`/organization/settings` → 「배차·카카오 API」** 카드 — 동일 패널 |
| **표시 항목** | REST 키 **설정됨/미설정** · **Geocode·Directions** 상태 Badge · **`quotaUsage[]` 테이블** — **좌표(Geocode)·경로(단일)·경로(다중경유)** 각 **오늘 사용·일일 한도·잔여 추정** · **한도 초과** warning |
| **suggest 연동** | **`POST /transport/runs/suggest`** 응답에 **`legDurationsSeconds`·`routePath`·거리·소요** 포함 — FE **「반영도착」**·명단 ETA · legs 없으면 **「경로 미확인」** (거짓 ETA 방지) |
| **a11y (UXD-143)** | Kakao Developers 콘솔 링크 **`(새 탭)` `ds-sr-only`** · suggest **DRAFT 검토** 링크 **차량번호 `aria-label`** (`4d87e19`) |
| **주의** | 사용량은 **서버 메모리·오늘 기준** — 재기동 시 리셋 · **Kakao 공식 Quota·브라우저 JS SDK와 다를 수 있음** · 일일 한도는 **`TransportProperties` 정책 기본값**(Geocode 100,000·Directions 10,000·Waypoints 10,000) |
| **조치** | `KAKAO_REST_KEY` 미설정 → DEPLOYMENT §4-7 env 설정 · `LIMIT_EXCEEDED` → 콘솔 Quota 확인·호출량 절감 · 지도만 실패 → **`VITE_KAKAO_MAP_JS_KEY`**·Web 도메인 등록 (Q394) |
| **테스트** | **`KakaoRestApiUsageTrackerTest`** · **`TransportKakaoApiStatusServiceTest`** · **`TransportKakaoApiStatusPanel.test`** |

> 관련: Q370 · Q394 · Q553 · USER_MANUAL §5-5·§5-8 · ADMIN_GUIDE §10-10 · DEPLOYMENT §4-7 · [DEV_ACCOUNTS.md](./DEV_ACCOUNTS.md)

### [TWR] Q555. 센터 **직원 계정**은 어떻게 만들어지나요? (staff account request workflow)

**A.** **✅ BE+FE Fixed (`22718d0`/`380be3c`, Must onboarding, Q555)** — MVP에서는 센터가 직원 계정을 **즉시 생성하지 않고**, **ogada 플랫폼 승인 후 발급**됩니다.

| 단계 | 담당 | 화면·API | 작업 |
|------|------|----------|------|
| 1 | `hq_admin`·`branch_admin` | **`/staff` → 「+ 계정 생성 요청」** | **이름·이메일·역할·소속 지점** 입력 → **`POST /api/v1/users/account-requests`** (`displayName`·`roleCode`·`branchIds`) |
| 2 | 동일 | **`/staff` → 「계정 생성 요청」** 카드 | **대기·승인·반려** 상태 확인 |
| 3 | `ogada_platform_admin` | **`/platform` → 「계정 생성 요청 (승인 대기)」** | 목록 검토 → **승인**(임시 비밀번호 입력) 또는 **반려**(사유) |
| 4 | — | — | 승인 시 **`UserAccountRequestService.approve`** 가 **`POST /users`와 동등**하게 계정 생성 |

**첫 `hq_admin` 발급 (Tenant 온보딩)** — **`/platform` → 고객사 「관리」** — **`PlatformOrgDetailModal`** — **이름·이메일·임시 비밀번호(8자+)** 필수 (`380be3c`) → **`POST /platform/organizations/{orgId}/admins`**

| 금지·주의 | 내용 |
|-----------|------|
| **`ogada_platform_admin` 직접 생성** | Tenant **`POST /users`** 로 **`ogada_platform_admin`** 역할 생성 **불가** |
| **이메일 중복** | Tenant 내 **대소문자 무시 유일** (V30) |
| **Swagger 대체** | UI 실패 시 **`POST /users/account-requests`** 또는 플랫폼 **`/platform/user-account-requests/{id}/approve`** |

> 관련: Q5 · Q6 · Q162 · USER_MANUAL §4-7·§5-3 · ADMIN_GUIDE §3 · [DEV_ACCOUNTS.md](./DEV_ACCOUNTS.md)

### [TWR] Q556. **`platform_admin`** 과 **`ogada_platform_admin`** 은 같은 역할인가요? (V160)

**A.** **✅ BE Fixed (`3023c9e`, V160, Q556)** — **같은 「플랫폼 관리자」** 역할이며, **2026-06-20**부터 JWT·DB **`role_code`가 `ogada_platform_admin`으로 통일**되었습니다. 구 **`platform_admin`** 은 **더 이상 신규 발급·로그인에 사용되지 않습니다.**

| 항목 | 내용 |
|------|------|
| **마이그레이션** | Flyway **V160** — 기존 `platform_admin` 행 **`UPDATE` → `ogada_platform_admin`** · **`chk_users_role_code`·UK 인덱스** 재생성 |
| **Spring Security** | **`@PreAuthorize("hasRole('OGADA_PLATFORM_ADMIN')")`** — JWT **`role` claim** 동기화 |
| **조직 소속** | **`organization_id IS NULL`** (전 Tenant 메타만) — V160 **`chk_users_ogada_platform_admin_org`** |
| **배포 주의** | **V160 미적용** DB에 구 코드로 로그인 시 **403/역할 불일치** — Flyway migrate 선행 (DEPLOYMENT §3-7) |
| **문서** | 구 FAQ·매뉴얼의 **`platform_admin`** 표기는 **`ogada_platform_admin`** 로 점진 갱신 중 |

> 관련: Q5 · Q8 · Q10 · ADMIN_GUIDE §1-3 · [DEV_ACCOUNTS.md](./DEV_ACCOUNTS.md)

### [TWR] Q557. NHIS **10항목 급여제공계획서**는 어디서 작성하나요? (G14-NHIS-PLAN-FORM)

**A.** **✅ full stack (BE `80b9619`/`34eb47a` · FE `ce422e3`/`d723d5a` · V164, US-T08b, FAQ21802)**

| 경로 | 설명 |
|------|------|
| **`/clients/:id` → 「급여계획서」** 탭 | **`ClientCarePlanForm`** — 10항목·연도별 load/save |
| **`/clients/:clientId/care-plan-form`** | 동일 폼 전용 페이지 |
| **`/clients/care-plan-notifications`** | G38 통보 compliance — 행 **「급여계획서」** 링크 |

**10항목**: 급여종류·수급자명(스냅샷)·작성일·작성자·세부목표·필요내용·세부제공내용·횟수·시간·종합의견·(옵션) 통보 일시.

**API**: `GET/PUT /api/v1/clients/{clientId}/care-plan-forms` · `GET …/{planYear}` — 이용자×**plan_year** UK 1건.

**구분**:
- **이용계획서 PDF** → **「등급 이력」** 탭 G37 첨부
- **구조화 10항목** → **본 탭** (케어포 별지 제21호 대응)

> 관련: Q277(G38) · USER_MANUAL §3-3 · ADMIN_GUIDE §6-2-2a · `DATA_RETENTION_POLICY` §2-1

### [TWR] Q553. **자동 배차 제안** 후 **명단·지도 ETA**가 바로 보이나요? (v1.3-A transport preview cache)

**A.** **✅ FE Fixed (`acc5933`, v1.3-A, Q553)** — 자동 배차 suggest·DRAFT 검토·수동 배차 화면 간 **경로 미리보기·geocode 결과**를 **탭 세션 in-memory 캐시**로 재사용하고, **제안 직후 명단**에 **반영도착 ETA**를 표시합니다.

| 항목 | 내용 |
|------|------|
| **route-preview 캐시** | **`transportRunPreviewCache.js`** — **`storeSuggestRunPreviews`** / **`readTransportRunPreview`** — **정차 시그니처** 변경 시 캐시 **자동 무효화** |
| **geocode 캐시** | **`kakaoMapGeocoder.js`** — **주소+지역경로** 키로 Kakao Geocoder 결과 **재사용** |
| **명단 ETA 병합** | **`buildSuggestRunDispatchIndex`** — suggest 응답 **`legDurationsSeconds`** → **`/transport` 명단「계획 픽업」** · 확정 루트와 **병합 표시** |
| **제안 요약 UI** | **`TransportSuggestPanel`** — **「반영도착」** 열(첫~마지막 이용자) · 버튼 **「자동 배차·경로 계산 중…」** · **「DRAFT 검토」** 링크가 **preview state** 전달 |
| **split-view hydrate** | **`TransportRouteSplitView`** — 캐시·링크 state로 **정차 time chip·지도 polyline** 즉시 채움 |
| **SideNav 라벨** | **`navConfig.js`** — **「모니터링 자가진단 (G30)」·「급여제공결과 평가 (G39)」** — 대시보드 위젯명과 **구분** |
| **테스트** | **`transportRunPreviewCache.test`** · **`kakaoMapGeocoder.test`** · **`transportRosterDispatch.test`** · **`TransportSuggestPanel.test`** · **`TransportRunNewPage.test`** |

> 관련: Q370 · Q418 · Q433 · Q550 · USER_MANUAL §3-1·§5-8 · ADMIN_GUIDE §10-10

### [TWR] Q550. 수동 배차 **이전 배차 불러오기**·**명단 추가**·**희망 시각 경고**는? (v1.3-A)

**A.** **✅ FE Fixed (`4681b5a`/`863b135`, v1.3-A, Q550)** — **`hq_admin`** 수동 배차 workflow에 **roster 도구**와 **확정 전 ETA guardrail**을 추가했습니다. **✅ BE Fixed (`48eea95`)** — DRAFT **`PATCH`** 에 **`plannedDepartureTime`** · **희망 시각 순서 서버 검증**을 추가했습니다.

| 항목 | 내용 |
|------|------|
| **이전 배차 불러오기** | **`/transport/runs/new`** — **「이전 배차 불러오기」** → **`TransportLoadPreviousRunModal`** — 과거 **운행일·차량** 선택 → **`buildStopsFromPreviousRun`** |
| **명단에서 추가** | **`/transport/runs/:runId`** DRAFT — **`TransportRouteSplitView`** **「명단에서 추가」** → **`TransportAddRosterModal`** — **미포함 이용자** 다중 선택 · **확정 배차 포함** 이용자 차단 |
| **확정 전 경고** | **`TransportConfirmWarningModal`** — **`findDesiredBoardingViolations`** — **예상 도착 지연** · **정차 순서·희망 시각 불일치** · **「그래도 확정」** |
| **허용 오차** | 지점 **`GET/PUT /transport/settings`** — **`pickupToleranceMinutes`** (기본 **15분**, Q424) |
| **서버 검증** | **`validateClientPickupTimeSequence`** — **`POST/PATCH /transport/runs*`** — 역전 시 **`422 BUSINESS_RULE`** |
| **출발 시각 PATCH** | **`UpdateTransportRunRequest.plannedDepartureTime`** — DRAFT **`PATCH /transport/runs/{id}`** |
| **테스트** | **`TransportRunNewPage.test`** · **`TransportRunDetailPage.test`** · **`transportMapEtas.test`** · **`TransportServiceTest`** |

> 관련: Q418 · Q424 · Q433 · USER_MANUAL §5-8 · ADMIN_GUIDE §10-10

### [TWR] Q551. live E2E **`guardian-credentials-missing`** 와 **`guardian-credentials-default`** 는 무엇이 다른가요? (QA-B95)

**A.** **✅ BE Fixed (`520f10a`, QA-B95, Q551)** — Q542에서 **default guardian creds 허용**을 추가한 뒤, health·probe가 **「미설정」**과 **「기본값 사용(정책상 거부)」** 을 **동일 blocker**로 표시하던 문제를 **`LiveE2eOperationReadinessSupport`** 에서 분리했습니다.

| blocker | 의미 | 조치 |
|---------|------|------|
| **`guardian-credentials-missing`** | **`LIVE_E2E_GUARDIAN_EMAIL`/`PASSWORD`** 미설정 **且** default fallback **미사용** | env에 guardian creds **명시** |
| **`guardian-credentials-default`** | env 미설정 → **`live-e2e-guardian@ogada.test`** fallback **且** **`ogada.live-e2e.allow-default-credentials=false`** | Q542 — **`allow-default-credentials=true`**(기본) 또는 env **명시** |
| **둘 다 없음** | **`allow-default-credentials=true`**(기본) + default creds 사용 | preflight **통과** (G21 seed 등 **다른 blocker**는 별도) |

**테스트**: **`LiveE2eOperationReadinessSupportTest`** · **`LiveE2eControllerTest`** · **`HealthControllerTest`**

> 관련: Q490 · Q542 · Q548 · DEPLOYMENT §3-7 · `./scripts/run-live-e2e.sh`

### [TWR] Q552. FAQ21823 **근로재계약 알림** 접근성(a11y)은 어떻게 보강되었나요? (UXD-142)

**A.** **✅ FE Fixed (`bd6e1c2`, UXD-142, Q552)** — Q547에서 추가된 **`EmploymentContractRenewalAlertsPanel`**·**「재계약 완료 기록」** Modal에 **스크린리더·시맨틱 HTML** 보강과 **누락 CSS 클래스 승격**을 적용했습니다.

| 화면 | 보강 내용 |
|------|----------|
| **`EmploymentContractRenewalAlertsPanel`** | **`<time dateTime>`** on 재계약 기한 · Alert **직원명 링크 `aria-label`** · region **`aria-busy`** on load |
| **`StaffEmploymentContractRenewalPanel`** | checklist·Modal **Field `aria-describedby`** · **「재계약 완료 기록」`aria-busy`** on submit |
| **CSS** | **`ds-section-title`** · **checklist row/hint** classes — **`components.css`** 에 정의 승격 (undefined class 방지) |
| **테스트** | **`EmploymentContractRenewalAlertsPanel.test`** · **`StaffEmploymentContractRenewalPanel.test`** |

> 관련: Q540 · Q544 · Q547 · USER_MANUAL §4-2 · ADMIN_GUIDE §6-2-6b

### [TWR] Q538. **본인부담 통계·명세 인쇄·현금영수증** 화면의 **접근성(a11y)** 은 어떻게 보강되었나요? (UXD-140)

**A.** **✅ FE Fixed (`501fedc`, UXD-140, Q538)** — UXD-139 이후 coder 커밋(`19ed7f3`·`58d6694`·`99b795a`)에서 누락됐던 **G26·G-7-1·현금영수증 pending guard** 접근성을 재점검했습니다.

| 화면 | 보강 내용 |
|------|----------|
| **`/billing/cash-receipts`** | pending API 실패 Alert **`CASH_RECEIPT_PENDING_ERROR_ID`** · **「발급 등록」** 버튼 **`aria-describedby`** · **「조회」·「검색」`aria-busy`** |
| **`/billing/reports/statistics`** (G26) | **「수납년도/청구년도」** segmented control — **Windows forced-colors** 테두리·선택 강조 (`ds-segmented` CSS) |
| **`/billing/claims/:id`** (G-7-1) | **「명세서 PDF 다운로드」** 버튼 **`aria-label`「급여비용명세서 PDF 다운로드」** |
| **이용자 상세 의료비공제** | **`MedicalExpenseDeductionPanel`** **「조회」`aria-busy`** |

> 관련: Q534 · Q535 · Q536 · Q537 · USER_MANUAL §4-6-0-1·§5-10-1·§5-10-2 · ADMIN_GUIDE §1-4

### [TWR] Q536. 현금영수증 **발급 등록 Modal**에서 **발급 대상이 없을 때** 어떻게 표시되나요? (UXD-139, G-CASH-RECEIPT-LOG)

**A.** **✅ full stack (FE `17374f1`/`a2ef127`, UXD-139, Q536)** — **`CashReceiptRegisterModal`** 이 **미발급 `PAID`+`CASH` 0건**이면 **warning Alert** 「발급 가능한 현금 수납 건이 없습니다…」를 표시하고 **「발급 저장」** 을 **비활성**합니다. UXD-139에서 **Field 단위 검증**·**즉시발급 Badge**·**작년분 Alert `aria-describedby`** 도 보강되었습니다.

| 개선 | 내용 |
|------|------|
| empty pending guard | **`a2ef127`** — submit disabled · **`CashReceiptRegisterModal.test`** |
| UXD-139 a11y | **`Field`·`aria-describedby`** · **`CASH_RECEIPT_PRIOR_YEAR_WARNING_ID`** · **`CASH_RECEIPT_IMMEDIATE_STATUS`** Badge on 목록 |
| 법정 Alert id | **`CashReceiptLegalAlerts`** — **`CASH_RECEIPT_LEGAL_IMMEDIATE_ID`·`CASH_RECEIPT_LEGAL_PRIOR_YEAR_ID`** |

> 관련: Q530 · Q531 · Q533 · USER_MANUAL §5-10-1 · ADMIN_GUIDE §10-4

### [TWR] Q531. 수납할 때 **현금영수증을 자동 발급**하거나 **국세청에 직접 전송**할 수 있나요? (FAQ21702)

**A.** **△ partial — 수납 후 발급 안내 + 사후 등록 (P3 candidate G-CASH-RECEIPT-NTS-API, Q531)** — ogada는 **발급 이력 ledger**와 **현금 수납 직후 발급 등록 Modal**을 제공하지만, **홈택스·NTS API 실연동**과 **수납 Modal 내 동시 발급 체크박스**(이지케어 FAQ21702)는 **아직 없습니다**.

| 기능 | 상태 | 비고 |
|------|------|------|
| 발급목록·수급자별 이력 | **✅** Q530 | `/billing/cash-receipts` |
| **현금 수납 후** 발급 Modal 자동 표시 | **✅** Q531 | **`PaymentPage`** — FE `a17f148` · **`PaymentRecordModal`** CASH 법정 Alert |
| 수납 Modal **「현금영수증 발급」** 체크 | **❌** | 이지케어 FAQ21702 **동시 발급** UI 미구현 |
| NTS **자동** 발급·**취소** API | **❌** | 발급번호 **수동 입력** — 외부 PG·홈택스 병행 |
| 발급 **삭제**·수납 연동 취소 | **❌** | DB unique·감사 보존 — 오류 시 IT 문의 |

| 당장 | **현금** 수납 저장 → **발급 Modal**에서 NTS 발급번호 등록. Modal을 닫았으면 **현금영수증 발급목록**에서 등록. NTS 실제 발급은 **홈택스·단말기** 등 외부 수단 사용. |
| 로드맵 | **G-CASH-RECEIPT-NTS-API** — NTS 연동·수납 동시 발급 체크박스 (COMPETITOR_MATRIX P3 carry) |

> 관련: Q530 · Q252 · FAQ21702 · USER_MANUAL §5-10-1
...
| 2026-06-23 | **333차** — **US-R01-c leave-ledger RBAC+pilot test**·**Q665 신규**·**Q663 deepen**·baseline **`62fce23`/`b7101d5`** |
| 2026-06-23 | **332차** — **US-R01-c leave-ledger BE API**·**Q663·Q664 신규**·**Q655·Q659 갱신**·baseline **`bb9df48`/`b7101d5`** |
| 2026-06-23 | **331차** — **G2 CMS roster status filter normalization**·**Q662 신규**·**QA-B266/B267 Fixed**·baseline **`f1225b0`/`949e9bf`** |
| 2026-06-23 | **330차** — **Vitest 동시 실행·full-suite pollution 운영 대응**·**Q661 신규**·**DEPLOYMENT_GUIDE §1-3 baseline sync**·baseline **`40ab9e7`/`949e9bf`** carry |
| 2026-06-23 | **329차** — **QA-B266 full-suite test pollution**·**US-R01-c leave-ledger P1**·**M6 v3.1 `/meals` LIVE**·**Q658·Q659·Q660 신규**·merge gate **713 carry**·baseline **`40ab9e7`/`949e9bf`** |
| 2026-06-23 | **328차** — **annual-leave multi-branch activeBranch fallback**·**RelatedSurfacesPanel UXD-156·HR BranchScopeNotice**·**Q656·Q657 신규**·baseline **`40ab9e7`/`949e9bf`** |
| 2026-06-23 | **327차** — **G-COMM-CALLER-AUTH P3**·**US-R01 leave-ledger PLANNED**·**Q654·Q655 신규**·baseline **`83a26e7`/`95f55aa`** carry |
| 2026-06-23 | **326차** — **work-attendance API cross-link metadata**·**Q653 신규**·**Q651·Q652 deepen**·baseline **`83a26e7`/`95f55aa`** |
| 2026-06-23 | **325차** — **HR nav 역할 분리 통합 참조표**·**Q652 신규**·**ADMIN_GUIDE §1-4 baseline 정정**·baseline **`6ab3760`/`2040571`** |
| 2026-06-23 | **324차** — **G-STAFF-WORK-ATTENDANCE reverse cross-links**·**Q651 신규**·**Q648·Q650 deepen**·baseline **`6ab3760`/`2040571`** |
| 2026-06-23 | **323차** — **yearly cross-link metadata·related surfaces FE panel**·**Q648 deepen·Q650 신규**·baseline **`6ab3760`/`0b0d7ba`** |
| 2026-06-23 | **322차** — **US-R01 roster cross-link metadata**·**live API harness·PILOT_CHECKLIST R03e/E05**·**Q648·Q649 신규**·**Q647 deepen**·baseline **`bbf333c`/`e296387`** |
| 2026-06-23 | **321차** — **V173 annual leave integrity probe**·**UXD-155 a11y·Modal field errors**·**Q645·Q646·Q647 신규**·baseline **`8c5dd65`/`31ab1aa`** |
| 2026-06-22 | **320차** — **J03 Solapi placeholder readiness guard**·**QA-B255 page title constant**·**Q643 정정·Q644 신규**·**Q641·Q642 carry**·baseline **`19ffa84`/`971c7f1`** |
| 2026-06-22 | **319차** — **G-STAFF-ANNUAL-LEAVE validation deepen**·**Q641·Q642·Q643 신규**·baseline **`6b35fb5`/`8434435`** |
| 2026-06-22 | **317차** — **G-STAFF-ANNUAL-LEAVE negative usage guard**·**live E2E credential variant recovery**·**Q639·Q640 deepen**·**Q580 갱신**·baseline **`a45745c`/`a170f9c`** |
| 2026-06-22 | **316차** — **G-STAFF-ANNUAL-LEAVE test deepen**·**live E2E default-credential blocker recovery**·**Q640 신규**·**Q639·Q580 갱신**·baseline **`6b84bcd`/`6fcd750`** |
| 2026-06-22 | **314차** — **G2-CMS-ENROLLMENT-ROSTER deepen**·**Q638 신규**·**Q637 deepen**·baseline **`d3d4d2d`/`3ece965`** |
| 2026-06-22 | **312차** — **G34-WORKFLOW-CATALOG**·**G2 CMS branch roster**·**Q635 closure**·**Q637 신규**·baseline **`d361833`/`9f110a5`** |
| 2026-06-22 | **310차** — **live E2E bootstrap blocker normalize**·**Q631 신규**·**Q596·Q448 갱신**·baseline **`24d25f1`/`fdc135b`** |
| 2026-06-21 | **301차** — **G-BILLING-DEPOSIT-ORDER-GUARD**·**G-STAFF 출근방식**·**Q612 갱신·Q614 신규**·FE test **434** KPI · `a6eb8b7`·`5fd468b` |
| 2026-06-21 | **299차** — **G-ATTENDANCE-STATS contract**·**Q106 deepen·Q613 신규**·FE test **433** KPI · baseline carry |
| 2026-06-21 | **298차** — **G-STAFF-WORK-ATTENDANCE full-stack**·**Q589 정정·Q612 신규**·`560057f`·`53d65a0` |
| 2026-06-21 | **297차** — **UXD-151 mobile capture CSS**·**Q608 UXD-151 deepen·Q611 신규**·`61e1970`·`9812ac4` |
| 2026-06-21 | **296차** — **G-ATTENDANCE-ROSTER-STATUS FE wire full-stack**·**Vitest serial pool**·**Q609 갱신·Q610 신규**·`61e1970`·`8383f8d` |
| 2026-06-21 | **295차** — **G-ATTENDANCE-ROSTER-STATUS roster API**·**Q609 신규**·**Q94 갱신(closure)**·`61e1970`·`3bffb17` |
| 2026-06-21 | **294차** — **mobile HR camera capture**·**duplicate SMS overdue guard**·**live E2E blocker priority**·**8-12 print cleanup**·**Q607–Q608 신규**·**Q596·Q604·Q315 갱신**·`56cb5d9`·`6bde24a` |
| 2026-06-21 | **290차** — **G21 dashboard NHIS gap**·**G15 Kakao quota widget**·**live E2E bootstrap·cash-receipt guard**·**Q594–Q597 신규**·**Q448·Q591 갱신**·`0c9518a`·`580a86b` |
| 2026-06-21 | **288차** — **G-BILLING appliedFilters FE wire**·**UXD-148 a11y**·**Q580 feature-scoped blockers**·**Q587·Q588**·`7b99313`·`e2f1246` |
| 2026-06-21 | **287차** — **G-BILLING appliedFilters echo**·**Q580 snake_case blocker**·**Q585 갱신·Q587 신규**·`14935a3`·`33e9e1a` |
| 2026-06-21 | **286차** — **G-BILLING report FE wire**·**invalid month guard**·**Q580·Q585 갱신·Q586 신규**·`375fb9d`·`e38ccfd` |
| 2026-06-21 | **285차** — **G-STAFF-LEAVE-STATUS full-stack**·**G-BILLING deposit half-month·receipt dual-basis**·**Q290·Q584 갱신·Q585 신규**·`b96d038`·`1a614c9` |
| 2026-06-21 | **284차** — **G-STAFF-LEAVE-STATUS ON_LEAVE**·**live E2E placeholder token bootstrap**·**Q290·Q578 갱신·Q584 신규**·`68d4457`·`82a542c` |
| 2026-06-21 | **281차** — **G-BANK-EXCEL-8 full-stack**·**Q572·Q576 갱신·Q578 신규(live E2E placeholder)**·`7d29a38`·`a18b30e` |
| 2026-06-21 | **274차** — **V160 ogada_platform_admin**·**Kakao per-API quota**·**staff account request workflow**·**platform hq_admin issuance validation**·**Q554 갱신·Q555~Q556 신규**·`3023c9e`·`380be3c` |
| 2026-06-21 | **273차** — **v1.3-A Kakao API status+usage probe**·**suggest route-preview embed**·**TransportKakaoApiStatusPanel**·**Q554 신규**·`e2b764b`·`ba74bb5` |
| 2026-06-21 | **272차** — **v1.3-A transport preview cache+roster ETA merge**·**G30/G39 nav label disambiguation**·**Q553 신규**·`48eea95`·`acc5933` |
| 2026-06-21 | **271차** — **v1.3-A transport roster tools+ETA guardrails**·**guardian credential blocker separation**·**UXD-142 renewal a11y**·**Q550~Q552 신규**·`48eea95`·`863b135` |
| 2026-06-21 | **270차** — **US-R03 FAQ21823 renewal record+due alerts**·**live E2E operation readiness centralization**·**Q540·Q544 갱신·Q547~Q549 신규**·`bfad37d`·`16afd4c` |
| 2026-06-21 | **269차** — **US-R03 FAQ21823 clauses checklist+template modal**·**G21 not-applicable live E2E**·**Q540·Q541 갱신·Q546 신규**·`091c372`·`1b6d2b1` |
| 2026-06-21 | **268차** — **V159 cash receipt identifier CHECK**·**health V159 probe**·**UXD-141 a11y**·**nested health readiness**·**Q537·Q540 갱신·Q543~Q545 신규**·`1139e79`·`202c1fe` |
| 2026-06-21 | **267차** — **US-R03 FAQ21823 list+dashboard renewal widgets**·**live E2E allow-default-credentials**·**Q540 갱신·Q542 신규**·`beef81e`·`f31c346` |
| 2026-06-20 | **266차** — **US-R03 FAQ21823 employment contract renewal panel**·**live E2E unsupported branch seed guard**·**Q540·Q541 신규**·`7a9d7a5`·`f62402f` |
| 2026-06-20 | **265차** — **J03 guardian document quiet-hours manual reject**·**G-CASH-RECEIPT-LOG FE identifier pre-submit validation**·**Q537 갱신·Q539 신규**·`7e4c07e`·`0038846` |
| 2026-06-20 | **264차** — **G-CASH-RECEIPT-LOG numeric-only identifier**·**UXD-140 a11y**·**Q537 갱신·Q538 신규**·`4da0ca8`·`501fedc` |
| 2026-06-20 | **263차** — **G-CASH-RECEIPT-LOG identifier normalize+validation**·**pending load error guard**·**Q537 신규**·`298bcdf`·`35d1560`·`99b795a` |
| 2026-06-20 | **262차** — **G26 yearBasis+NTS CSV**·**G-7-1 Excel export**·**UXD-139 a11y**·**Q534~Q536 신규**·`ceeaeb9`·`19ed7f3`·`e454d3b`·`58d6694` |
| 2026-06-20 | **261차** — **G-CASH-RECEIPT-LOG 4-계층 closure**·**HQ pending**·**prior-year advisory**·**Q530·Q532 갱신·Q533 신규**·`58ff35e`·`8aebe55` |
| 2026-06-20 | **260차** — **G-CASH-RECEIPT-LOG dashboard due-gate**·**pending issuance API**·**Q530 갱신·Q532 신규**·`ab5708b`·`221458e` |
| 2026-06-20 | **259차** — **G-CASH-RECEIPT-LOG payment bridge**·**live API harness**·**Q530·Q531 갱신**·`a17f148`·`8e6e0c6` |
| 2026-06-20 | **258차** — **G-CASH-RECEIPT-LOG full-stack**·**V158**·**Q530 갱신·Q531 NTS API gap**·`4432558`·`cfc4b04`·`f79a19e` |
| 2026-06-19 | **257차** — G21 **split-view dual NHIS**·**health G32 alignment**·**credential trim**·**UXD-138 a11y**·**G-CASH-RECEIPT gap FAQ**·**Q525 갱신·Q526~Q530**·`7848b0f`·`d354a0e` |
| 2026-06-19 | **256차** — G32 **probe schema readiness·stale runtime guard**·**케어포 3-1 health segment nav**·**Q523 갱신·Q524~Q525**·`45d95ea`·`1d5747d` |
| 2026-06-19 | **255차** — G32 **FAQ21797 live E2E harness·pilot E2E extend**·**Q516·Q520·Q521 갱신·Q522~Q523**·`510d2f3`·`3f871d7` |
| 2026-06-19 | **254차** — G32 **V157·중복 참석자 차단·live E2E skip diagnostics**·**Q516·Q518 갱신·Q519~Q521**·`eed39ab`·`c7fb69a` |
| 2026-06-19 | **253차** — G32 **dashboard attendee-opinion gap widget**·**Q263·Q516 갱신·Q518**·`b9e0947`·`e55ae96` |
| 2026-06-20 | **252차** — G32 **FAQ21797 attendee opinions**·G2 **guardian document 3-type panel**·live E2E **health diagnostics**·**Q515~Q517**·`5222a8f`·`b272a7b`·`d1149a5` |
| 2026-06-19 | **251차** — G39 **FAQ21817 7-day SLA**·**RFID equivalence**·health **G21 branch blocker**·FE **billing readiness gate**·**Q511~Q514**·`bc754a0`·`c3b6a5c`·`b881883` |
| 2026-06-19 | **250차** — G21 **cross-branch seed scope**·**guardian bootstrap enrichment guard**·QA-B147 **batch-confirm loading**·UXD-136/137 **L03 edit a11y**·**Q507~Q510**·`02a2eb8`·`f86c76c` |
| 2026-06-19 | **249차** — G21 **probe branch-missing blocker**·UXD-147 **batch-confirm NHIS ack a11y**·**Q502 갱신·Q505~Q506**·`7898aa5`·`0002943` |
| 2026-06-19 | **248차** — G21 **paired PLAN/BILLING seed**·**billing visit schedule readiness**·**legacy DAY_CARE guard**·**missing branch block**·G41 **filter-year load guard**·UXD-135 **modal a11y**·**Q495·Q457 갱신·Q500~Q504**·`191703f`·`cefb7c7` |
| 2026-06-19 | **247차** — G21 **seed readiness health/probe/bootstrap**·**per-tenant scoped fallback IDs**·**NHIS row-batch linkage**·**liveG21Describe operation+seed gate**·**Q492 갱신·Q495~Q499**·`c0403b0`·`d61ab5e` |
| 2026-06-19 | **246차** — live E2E **health/probe credential alignment**·**org-scoped bootstrap IDs**·UXD-134 **a11y**·**LIVE_E2E_CLIENT_ID whitespace guard**·**Q437·Q490 갱신·Q491~Q494**·`8fe1ccd`·`cf85003` |
| 2026-06-19 | **245차** — live E2E **probe default-credentials blocker**·G41 **filter-year inline error**·**Q489 갱신·Q490**·`f932fd3`·`28e5525` |
| 2026-06-19 | **244차** — G41 **reference-year input guard**·**Q489**·`b73e5f4`·`f26e075` |
| 2026-06-18 | **243차** — G21 **NHIS comparison panel+per-client drill-down FE closure**·live E2E **NHIS import seed**·**Q479·Q484 갱신·Q486~Q488**·`b73e5f4`·`797c529` |
| 2026-06-18 | **242차** — G21 **NHIS summary deepen+Modal StatCard FE wire**·UXD-133 **print a11y**·**Q479·Q481 갱신·Q483~Q485**·`39fa41a`·`68a4e35` |
| 2026-06-18 | **241차** — G21 **confirm-readiness NHIS summary embed**·**RFID no-diff success alert**·**Q479 갱신·Q481~Q482**·`8a8c5b3`·`f232285` |
| 2026-06-18 | **240차** — G21 **visit NHIS comparison API**·**unassigned batch-confirm gate**·**VisitsContextNav URL sync**·**RFID diff normalize deepen**·**Q477·Q456 갱신·Q479~Q480**·`03a052a`·`570912e`·`3a27303`·`5f710e3` |
| 2026-06-18 | **239차** — G21 **per-kind readiness deepen+FE StatCard**·G-7-1 **unpaid all-print label**·**Q474·Q475 갱신·Q477~Q478**·`f26abb0`·`f9ed97d`·`f5639df` |
| 2026-06-19 | **238차** — G21 **readiness split**·G-7-1 **print bundle**·UXD-132 **a11y**·**Q474~Q476**·`6aeafe7`·`50d330d`·`f8321c7` |
| 2026-06-18 | **237차** — G15 **service log direction API**·live E2E **probe bootstrap-disabled**·**guardian credential FE align**·**Q419·Q470 갱신·Q471~Q473**·`72124f7`·`8cf09d8`·`94c65e2` |
| 2026-06-18 | **236차** — G15 **별지 제22호 form completion·branch contact·pickupAddress API**·**Q466 갱신·Q468~Q470**·`07be394`·`b1a16ff`·`e358f2d` |
| 2026-06-18 | **235차** — G15 **per-stop form parity**·V155 **waypoint test deepen**·**Q458 갱신·Q466~Q467**·`7de5a6f`·`a179256` |
| 2026-06-18 | **234차** — V155 **waypoint**·G41 **PDF 8-7·dashboard widget·8-7-1 export**·G15 **print pickup**·live E2E **visit·transport seed**·**Q321·Q421·Q457 갱신·Q458~Q465**·`64c4c80`·`caa215f` |
| 2026-06-18 | **232차** — G21 **RFID diff code rendering**·visit check-in **supervisory role normalize**·live E2E **HOME_VISIT seed**·**Q307·Q452 갱신·Q455~Q457**·`4a112fe`·`78cfb8a`·`9e050b1` |
| 2026-06-18 | **231차** — G21 **RFID compare UI wire**·visit check-in **assigned-user guard**·L02_M07 **body restraint normalization**·**Q307·Q452 갱신·Q453·Q454**·`27c9de3`·`0db1e68`·`4a47675` |
| 2026-06-18 | **230차** — UXD-130 **driver signature fieldset**·live E2E **guardian suite gate**·G21 **RFID compare API**·**Q445 갱신·Q450~Q452**·`bfe0283`·`7424c30`·`eeac205` |
| 2026-06-18 | **229차** — live E2E **bootstrap error blockers**·L02_M13 **create client normalization**·**Q441·Q442 갱신·Q448·Q449**·`d7f1a9a`·`1c8f236` |
| 2026-06-18 | **228차** — G15 **driver signature**·**service-log legal guide**·live E2E **operationBlockers list**·**Q441 갱신·Q445~Q447**·`bc3a35c`·`f51e365` |
| 2026-06-18 | **227차** — G15 **server legal field guard**·staff **health checkup HR file hub wire**·**Q439·Q435 갱신·Q443·Q444**·`ac1d43f`·`b6ce301` |
| 2026-06-18 | **226차** — G15 **service log legal field guard**·**duplicate row rejection**·probe **operation readiness**·L02_M13 **malformed API**·UXD-129 **a11y**·**Q407·Q433·Q435·Q366 갱신·Q439~Q442**·`b4644e8`·`52e3340` |
| 2026-06-18 | **225차** — staff **new-hire health checkup document window**·live E2E **readiness/retry deepen**·QA-B133 **DateInput test**·**Q296·Q428 갱신·Q435~Q438**·`8e6310a`·`2e6c35f` |
| 2026-06-18 | **224차** — transport **roster planned pickup hub**·QA-B132 **StaffLifecyclePanel test**·**Q433·Q434**·`e35efb2`·`101aaee` |
| 2026-06-18 | **223차** — QA-B131 **`test:live-e2e` script path**·DatePicker **keyboard a11y**·**V153**·G15 service log harness·**Q422 갱신·Q429~Q432**·`8882d9f`·`4c5d3bc` |
| 2026-06-17 | **222차** — G30 **`MonitoringEvidenceContextPanel`**·live E2E **guardian default credentials**·**Q391 갱신·Q428**·`7d2cb4a`·`09df8c7` |
| 2026-06-17 | **221차** — G15 **compliance→일지 hub**·live E2E **bootstrap/probe harden**·**Q230·Q409 갱신·Q426·Q427**·`b93e098`·`b2c09e1`·`844227a` |
| 2026-06-17 | **220차** — FE `fc916db` live E2E **nested/snake_case bootstrap payload** parsing · **`liveE2eHarness.test`** · **Q409 갱신·Q425** |
| 2026-06-18 | **219차** — BE `2157df5` **`/actuator/healthz`** readiness alias · FE `ab4de83`/`188ce71` DateInput/TimeInput **QA-B127**·**`viewAnchor`**·**Q413·Q422 갱신** |
| 2026-06-18 | **218차** — BE `dd2fa2c`·FE `96db8bf` transport **settings validation**·**compact dispatch**·V152 **committed**·**Q347·Q423 갱신·Q424** |
| 2026-06-17 | **217차** — BE `73cffc5` staff bootstrap **embedded guardian tokens**·V152 transport inactive client guard △ · **Q409·Q393 갱신·Q423** |
| 2026-06-18 | **216차** — FE `bf73c4c`·`ea5d896` 배차 **경유지 FE closure**·**DatePicker+TimeInput**·ETA time chip·BE `704478f` SMTP readiness·**Q418·Q421 갱신·Q422·Q367** |
| 2026-06-18 | **214차** — BE `0e46b37`·FE `fde098f` 배차 **계획 출발·ETA**·**`liveE2eOperationReady`**·split-view 세로 stack·**Q418·Q419·Q420** |
| 2026-06-18 | **213차** — BE `f0e52b8`·FE `0baabe9` 이용자 연락처 열·actuator readyz/livez·liveness/readiness split·**Q413 갱신·Q417** |
| 2026-06-18 | **211차** — BE `30243f7`·FE `3a0110f` actuator liveness/readiness·health probe harden·G15 outing live E2E·**Q240·Q413 갱신** |
| 2026-06-18 | **210차** — BE `87f901d`·FE `140bf92` L02/L03 parity cross-links·actuator health·live E2E safe client id·**Q240·Q409 갱신·Q412·Q413** |
| 2026-06-18 | **209차** — BE `5994d15`·FE `3cc5a08` G15 audit trail read API full stack·live E2E cross-tenant bootstrap·**Q411·Q409 갱신** |
...
## 21. v1.3-A 배차·Kakao 카카오맵 (2026-06-21 274차 — BNK-424~429)

### Q425. 배차 화면에서 카카오맵 API 연결 상태를 확인할 수 있나요?

**A.** **FE `ba74bb5` 추가 · v1.3-A TransportKakaoApiStatusPanel** — **배차(`/transport`) 우상단** 또는 **조직 설정(`/organization/settings`)「배차·카카오 API」 탭**에서 **Kakao API 연결 상태 및 일일 사용량**을 모니터링할 수 있습니다.

| 탭 위치 | 내용 | 역할 |
|--------|------|------|
| `/transport` 「Kakao API」 | 연결 상태 probe · 경로 계산 응답 시간 | 배차 담당자 |
| `/organization/settings` 「배차」 | 조직별 **API 3종 일일 사용량** 테이블 · **Geocode·Directions** quota 추적 | `hq_admin`·`sysadmin` |

**노출 항목**:
- **Geocode** — 주소→좌표 변환 (검색, 이용자·직원 소재 주소록)
- **Directions(단일)** — 포인트-투-포인트 경로 (추천 배차 미리보기)
- **Directions(다중경유)** — 경유지 최적 경로 (배차 확정 전 ETA)

> **Q554 (Kakao per-API daily limit)** 참고.

### Q426. Kakao API 일일 limit이 얼마나 남았는지 확인하려면?

**A.** **BE `138ac26` + FE `ba74bb5` · v1.3-A Kakao per-API daily usage tracker · Q554**

`GET /transport/kakao-api-status` 응답 예시:
...
```json
{
  ...
  "quotaUsage": [
    { "kind": "GEOCODE", "used": 450, "limitPerDay": 5000, "quotaPercent": 9 },
    { "kind": "DIRECTIONS_SINGLE", "used": 280, "limitPerDay": 3000, "quotaPercent": 9 },
    { "kind": "DIRECTIONS_MULTI", "used": 120, "limitPerDay": 1000, "quotaPercent": 12 }
  ]
}
```

**설정 마법사** (`/organization/settings` → 「배차·카카오 API」):
- **조직 카카오 API 키** 등록
- **Directions 다중경유 활성화 여부** (비용 증가·정확도 향상 trade-off)
- **배차 자동 제안 활성화** (시뮬레이션 vs 실제 Kakao Directions 호출)

### Q427. 통합재가(장기요양+방문요양) 기관을 ogada로 관리할 수 있나요?

**A.** **FE `ba74bb5` + BE 274차 · BranchServiceType 5-type·IntegratedHomeProviderDiscoveryPanel · v1.3-C roadmap**

**현재 상태 (v1 Must)**:
- **5가지 서비스 유형** 지원 — `DAY_NIGHT`, `SHORT_TERM`, `INTEGRATED_HOME`, `HOME_CARE`, `NURSING_FACILITY`
- **주간보호** (`DAY_NIGHT`), **시설급여** (`NURSING_FACILITY`)가 주요 대상
- **통합재가** (`INTEGRATED_HOME`) — 공단 longterm 검색 API 연동·월 10만원 가산 안내 **P3 candidate G-INTEGRATED-HOME-MONITORING-DEEPEN**

**P3 carry (v1.3-C+)**:
- 통합재가용 **배차·회의록·특이사항** 필드 분화
- **공단 이동서비스비** 청구 자동화 (이동서비스일지→공단 format 변환)
- **보호자 포털 통합재가 구간별 명세** (방문 시작·종료 시간 구간 표기)

> **Q4·Q41·Q325** 참고.

### Q428. 배차 중 「반영도착」시간이 실제와 다르면 어떻게 하나요?

**A.** **FE `acc5933` · v1.3-A transport preview cache + roster ETA merge · Q553**

**「반영도착」시간** — 배차 확정 후 각 이용자별 **실제 픽업 예상 시간** (Kakao Directions 계산·교통 상황 반영)

| 상황 | 대응 |
|------|------|
| **배차 수정 전 확인 필요** | **배차 DRAFT**에서 **「자동 배차 제안」** → **경로 계획·ETA 재계산·순서 조정** |
| **배차 DRAFT에서 ETA 오류 · 경로 계산 불가** | **Kakao API 상태 확인**(Q425)·**주소 주민센터 등재 여부** 확인 · `Q99·Q100` 참고 |
| **배차 확정 후 실제 지연 발생** | 현장에서 **배차 상태 추적**(`/transport/runs/{id}`)·**출발 지연·신호 대기** 등 수동 기록 (향후 P3 자동 추적 후보) |

### Q429. ogada는 방문요양(홈케어) 기관을 지원하나요?

**A.** **Partial + v1.3-C roadmap · BranchServiceType `HOME_CARE` · US-INTEGRATED-HOME deepen**

**현재 상태**:
- **방문요양(`HOME_CARE`)** 서비스 유형 등록 가능
- **기본 배차·일지** 필드는 주간보호와 **대부분 동일** (출석·건강 기록·특이사항)
- **시설급여(`NURSING_FACILITY`)** 대비 **차량·배차** 필드가 선택적 (도보 이용자도 포함)

**미지원 (v2 이상)**:
- **공단 제공기관 번호** 구분 (day-care `0008`, nursing `0009`, home-care `0010` 등)
- **방문 일지 Form-21 호출 기록** 자동 생성 (지명·시간·거리·이용료·차량번호 9-항목)
- **회의록·평가 주기** 방문요양 표준 (3개월 1회 vs 시설급여 반기 1회)

> **Q41·Q327·Q427** 참고 · [케어포 module 2](https://www.carefor.co.kr/daycare/func.php) 참고.

### Q430. V160 마이그레이션 후 `role_code` 값이 `PLATFORM_ADMIN`에서 무엇으로 바뀌나요?

**A.** **V160 `rename_platform_admin_to_ogada_platform_admin` · BE `3023c9e` · Q556**

**V160 전후 비교**:

| 롤 코드 | 권한 | V160 전 | V160 후 |
|--------|------|--------|--------|
| 플랫폼 관리자 | 신규 Tenant·`hq_admin` 발급·계정 요청 승인 | `PLATFORM_ADMIN` | **`OGADA_PLATFORM_ADMIN`** |
| 통합 관리자 | 다지점 대시보드·지점·계정·청구 | `HQ_ADMIN` | `HQ_ADMIN` (변경 없음) |

**기존 시스템 연동 영향**:
- API 권한 검사: **`@PreAuthorize("hasRole('OGADA_PLATFORM_ADMIN')")`** 로 업데이트
- JWT token `roles` 배열: `["OGADA_PLATFORM_ADMIN"]`
- 데이터베이스 `user.role_code`: `"OGADA_PLATFORM_ADMIN"`

> 기존 `PLATFORM_ADMIN` 레코드는 자동 마이그레이션되며, 로그인 후 자동 갱신됩니다. **Q96(역할·권한)** 참고.

---

## 22. v2 로드맵 · P3 candidates (2026-06-21 277차 — BNK-424~429 · §22 Q558–Q566)

> **번호 안내**: Q430–Q439는 §20·§21 **기술·배차 FAQ**와 번호가 겹치지 않도록, 본 섹션 v2 roadmap 항목은 **Q558–Q566** 으로 부여합니다.

### Q558. 현금영수증 자동 발급(`G-CASH-RECEIPT-LOG`)이 완료됐나요?

**A.** **★ v1 Must 4-계층 closure ✅ @ `58ff35e` (PLN 173차 BNK-399~405) · P2 gap closure · Q210 refactor needed**

**구현 완료**:
- **식별자(`receiptNumber`)** — 숫자만 검증·사용 시스템별 제약(건강보험공단 13자 제약) 대응
- **일자별 현금수령 내역**(`/billing/cash-receipts`) — 조회·가격·상태 필터·정렬
- **회차 재발행** — 기존 현금영수증 수정·취소 후 재발행 workflow
- **공단 업로드** — Kakao B (간편결제) 연동 예정 (Q208, stub)

**남은 과제 (v2)**:
- **NTS API 직연동**(`G-CASH-RECEIPT-NTS-API`) — 홈택스 실시간 현금영수증 전송 (Q210)
- **PDF 자동 발행·이메일·SMS 발송** — 보호자 자동 통지
- **환급 현금영수증** — 취소분 별도 처리

> **Q205·Q210** 참고.

### Q559. 출석 QR 체크인이 `CHECKED_IN`·`CHECKED_OUT` 상태로 완전히 고쳐졌나요?

**A.** **Partial Fixed (`99f2f3e`) · US-E01~E05 · Must 출석 roster 정합 갭 잔존 (Q94 Extended · PLN 179차+)**

**현재 상태**:
- **보호자 QR 체크인**(`/guardian/checkin`) ✅ — 입소·귀가 대상 선택·토큰 스캔·상태 업데이트
- **배차 중 귀가 교통편** ✅ — `transportType` (SELF, RIDE_PROVIDED 등) 수집
- **QR 생성**(`/branches/{branchId}/qr`) △ — 토큰 발급 OK·**이미지 URL 미구현** (이미지 미리보기 빈 칸, Q109)

**FE 요청 본문 불일치 (API spec 정정 필요)**:
- FE 보내는 값: `{ type: "checkin", validMinutes: 480 }`
- BE 기대값: `{ direction: "in"|"out", expiresInMinutes?: 1–720 }`
- **Swagger 우회**: Q109 참고 (직접 `POST /api/v1/branches/{branchId}/qr`)

> **Q94·Q96·Q109** 참고.

### Q560. L03 간호급여(재가 인지·미용 등) 필드가 모두 구현됐나요?

**A.** **L03 13/14 ✅ effective 100% @ PLN 139차 (BNK-217~218) · L03_M01/M06/M11/M13/M14 완료 · L03_M15 △ partial+ (PLN 140차+)**

**구현 완료 (13개 항목)**:
- **L03_M01** — 인지 자극 (보조 활동, **ASSISTED**)
- **L03_M04** — 목욕·위생 (신체 수발, **FULL_ASSISTANCE**)
- **L03_M06** — 배변 관리 (배뇨 도움, **ASSISTANCE_PROVIDED**)
- **L03_M11** — 치매 증상 관리 (정신 건강 모니터링)
- **L03_M13** — 호흡기 관리 (산소 요법)
- **L03_M14** — 체중 기록 (월 1회 기준 추적)
- 기타 8개 (복약 지도·영양 관리·우울증 선별·욕창 예방 등)

**진행 중 (1개)**:
- **L03_M15** — **△ partial+** — 말기 돌봄 (호스피스) — v1.3-B 예정·form 입력·리뷰 필드 추가 필요

> **USER_MANUAL §2-1·§4-3** · **Q300~Q324** 참고.

### Q561. 출석 현황(Roster)에 정확히 무엇이 표시되나요?

**A.** **FE `ba74bb5` · v1.3-A transport roster 확장 · US-E01/E02·154차 BNK-340 · Q559 연계**

**당일 픽업 명단 (`GET /transport/roster`)**:

| 필드 | 설명 | 노출 대상 | 비고 |
|------|------|---------|------|
| `contact` | 이용자 본인 연락처 | 모든 역할 (마스킹·non-HQ) | `clients.phone` 복호화 |
| `guardianContact` | 대표 보호자 연락처 | 모든 역할 (마스킹·non-HQ) | `guardian.phone` 복호화 |
| `pickupContact` | 픽업 담당자 연락처 (미입력 시 이용자 연락처) | **HQ 역할만 전체 노출** | **`010-****-5678` 마스킹** · `social_worker`·`caregiver` 링크 불가 |
| `pickupAddress` | 픽업 주소 (단계별 수정) | 마스킹 정책 동일 | 중단계 이상 주소 생략 가능 |

**미충족 요구사항 (Q94·Q559 Extended)**:
- **QR 코드 이미지 미구현** — 토큰 발급 ✅이나 **URL 미리보기 빈 칸** (Q109·이미지 생성 service 필요)
- **배차 기반 체크인** — 실제 이용자 탑승 후 QR 스캔 시 `confirmCheckIn(qrToken)` 응답 404
- **PII 마스킹 규칙** — non-HQ 역할 전화 연락처 `maskPhone()` 자동 적용 확인 중

> **Q100·Q102·Q427·Q559** 참고.

### Q562. 간호급여 L03_M15 말기 돌봄(호스피스)은 언제 지원되나요?

**A.** **P2 Planned v1.3-B 이후 · L03_M15 △ partial+ · PLN 140차+ · Q560 follow-up**

**목표 기능**:
- **호스피스 상태 기록** — 주말기 환자 돌봄 여부·통증 관리·영양 상태
- **기록지 양식** — 주 1회 평가·특이사항·부모 동의 확인
- **규정 준수 지표** — 말기 돌봄 회차별 평가율 대시보드 반영

**현재 상태 (v1)**:
- `NursingProgressNote` 테이블 예약 (호스피스 데이터 저장 공간)
- **입력 Form·필드 정의 미완** — 다른 L03 12개 항목(L03_M01/M04/M06 등)이 v1 Must로 우선 구현됨

**다음 단계 (v1.3-B)**:
- hospice service type `HOSPICE_CARE` 추가 (L03_M15 전용)
- 말기 환자 신원 확인·진료기록 업로드 필드
- 다학제팀(의사·간호·사회복지사) 회의록 template

> **Q560·§3-8·API_SPEC §15** 참고.

### Q563. 데이터는 얼마나 오래 보관되나요? 삭제 정책이 있나요?

**A.** **자체 보안 정책 수립 중 · 법정 요구사항 = 장기요양보험법 시행규칙 **최소 3년** · PII 암호화 저장**

**현재 상태 (v1 Must)**:
- **모든 기록 무기한 보관** — 삭제 기능 없음 (감사 추적 필수)
- **청구·정산 기록** — 3년 이상 보관 권장 (건강보험공단·세무 조회 대비)
- **개인정보** — `residentRegistrationNo` (주민등록번호) **암호화 저장** @ V1
- **로그인 이력** — 90일 자동 삭제 (회원사별 정책 가능)

**추가 요구사항 (법정·자체)**:
- **GDPR 개인정보 우향 이행** — 보호자·직원 요청 시 개인 데이터 export
- **정보주체 삭제 권리** — 이용자·직원 퇴사 시 PII만 **비식별화**(UUID 치환)
- **백업·복구 정책** — 일일 자동 백업·7일 기간 보관

**귀책성**:
- ogada 클라우드 호스팅 환경에서 **자동 백업·암호화** 적용
- 센터 자체 data residency 요구(특정 지역 서버)는 **별도 계약** 필요

> **DEPLOYMENT_GUIDE §11 · [`DATA_RETENTION_POLICY.md`](ops/DATA_RETENTION_POLICY.md)** 참고.

### Q564. 이용자·직원·보호자 개인정보가 노출되지 않도록 어떤 보호 조치가 있나요?

**A.** **rules.md §3 security · PII masking @ Q94/Q102·SEC-D9 · FE `@1d910c2`/BE `@e7d4cf6`·154차 BNK-340**

**저장 시 (저장 단계)**:
- **주민등록번호** — 256-bit AES 암호화 + salt (복호화 필요 시에만)
- **전화번호·이메일** — 평문 저장 but **조회/목록 응답 시 마스킹**
- **로그인 암호** — bcrypt(해시만 저장, 복구 불가)
- **민감 문서** — PDF/이미지 암호화 저장소 (multipart S3 업로드)

**전송 시 (응답 단계)**:
- **HTTPS/TLS** — 모든 외부 통신 암호화
- **JWT 토큰** — httpOnly 쿠키 저장 (XSS 방지)
- **전화번호 마스킹** — 예: `010-****-5678` (non-HQ 역할에만 적용)
- **응답 필터** — `maskPii()` 자동 적용 (Q100·Q102)

**접근 제어**:
- **테넌트 격리** — 모든 쿼리에 `organization_id` 필수 검증
- **역할 기반 필터링** (RBAC) — `@PreAuthorize` + row-level access control
- **지점 스코프** — branch_admin은 소속 지점만·HQ는 전체 (승인됨 범위)
- **감사 로그** — 누가·언제·무엇을 조회했는지 기록

**제3자 서비스**:
- **Kakao Local API** (배차·지도) — 주소만 전달·위치정보 최소화
- **CMS/결제게이트웨이** — PCI DSS 준수 기관만 연동
- **이메일·SMS** — 보호자 동의 하의 채널만 사용 (Q15·Q331)

> **SECURITY_AUDIT.md·REQUIREMENTS §2-4·API_SPEC §0-3** 참고.

### Q565. 방문요양 기관에서는 ogada의 「형식」이 가정방문 요양보호사와 안 맞는데?

**A.** **Partial + v2 roadmap · BranchServiceType `HOME_CARE` · US-INTEGRATED-HOME deepen · Q429 follow-up**

**주간보호 vs 방문요양 필드 차이**:

| 기능 | 주간보호(`DAY_NIGHT`) | 방문요양(`HOME_CARE`) | 상태 |
|------|--------|----------|------|
| **배차·차량** | 필수 (센터 소유 버스) | 선택 (보호사 도보/자차) | **△ 조건부** — `transportMode` null 허용 |
| **일지(Form-22)** | 센터·기사·동승자 명시 | 보호사 1인 호출 기록 | **P2** — 제공기관번호 구분 필요 |
| **회의·평가** | 월 1회 필수 | 3개월 1회 + 월 1회 상담 | **P3** — 주기 자동 설정 |
| **청구 주기** | 월 1회 청구 | **일 단위 청구**(회마다) | **P2** — 일일 청구 처리 로직 추가 |
| **보호자 포털** | 월별 명세 | **방문 시간·위치 추적** | **P3** — GPS·실시간 추적 |

**현재 v1에서 방문요양 사용 시 제약**:
- 배차/차량 관련 필드 **무시 가능**
- 청구는 월 1회 고정 (일일 청구 워크플로 불가)
- 이동서비스비(`TRANSPORT_COST`) 별도 계산 필요

> **Q41·Q327·Q429·REQUIREMENTS §3-13·ROADMAP v2** 참고.

### Q566. 데이터 내보내기(Export)·이관은 가능한가요?

**A.** **Partial + v2 roadmap · CSV/Excel export ✅ · 타 시스템 데이터 import △**

**현재 지원**:
- **CSV Export** — 이용자 목록·청구 내역·출석 현황 (모두 UTF-8 BOM)
- **Excel Export** — 본인부담 통계·지급결과 평가·NHIS 수가표 (P1 complete)
- **PDF 출력** — 급여비용 명세서·증빙 서류·일지(Form 22호)
- **NHIS Import** — 공단 청구내역상세 엑셀 읽기 (reconciliation with matching)

**미지원 (v2+)**:
- **타 센터용 데이터 Export** — 보호자·직원 정보 이관 (테넌트 간 전송)
- **케어포·이지케어 Import** — 기존 시스템 마이그레이션 (수동 변환 필요)
- **API 대량 import** — Batch 용량 제한 (multipart file < 50MB)
- **실시간 sync** — 타 시스템과의 양방향 동기화 (향후 webhook)

**이관 프로세스 (현재 수동)**:
1. 기존 시스템 → CSV 추출 (이용자·출석·청구)
2. **데이터 정규화** — ogada 필드명·형식 맞춤 (수동 Excel 작업)
3. **CSV Import** → ogada (bulk insert 또는 form 반복 입력)
4. **검증·대조** — 이전 기관과 신규 기관 기록 비교 (Q94·Q322 참고)

> **DEPLOYMENT_GUIDE §6·FAQ Q150~Q160** 참고.

### Q598. 목욕서비스 일정을 매달 자동으로 복사할 수 있나요? (G-BATHING)

**A.** **✅ BE+FE Full-stack Fixed (`49a1721`·`9a957fb`, L02_M03, G-BATHING-SCHEDULE-PREV-MONTH-COPY)**

**배경**: 주야간 목욕 일정은 패턴이 반복되는 경우가 많아, 전월 **예정·완료** 일정을 당월로 복사하면 입력 시간을 줄일 수 있습니다 (케어포 3-3 전월 일정 불러오기). **방문목욕(재가) 중복배정**은 ezCare FAQ21191 범위로 ogada와 **별개**입니다.

**화면 절차** (`/care/bathing-schedules`):

| 단계 | 내용 |
|------|------|
| 1 | **「대상 월」** 을 복사 받을 월로 설정 → **조회** |
| 2 | **`hq_admin`·`branch_admin`·`social_worker`** — **「전월 일정 복사」** 클릭 |
| 3 | 확인 후 **생성·건너뜀 건수** toast 확인 |

**API**:

```bash
POST /api/v1/care/bathing-schedules/copy-from-previous-month
...
| 항목 | 동작 |
|------|------|
| 복사 대상(전월) | **`SCHEDULED`·`COMPLETED`만** |
| 제외 | **`CANCELLED`·`SKIPPED`** · 대상 월 **동일 이용자·동일 날짜** 기존 일정 |
| 생성 상태 | 복사 건은 **`SCHEDULED`** |
| 유지 필드 | 목욕 유형·시간·비고 — **담당 요양보호사 필드 없음**(L02_M03 스키마) |
| RBAC | **`caregiver`는 복사 불가** — 조회·개별 등록만 |

> 관련: USER_MANUAL §5-26·§A4 · ADMIN_GUIDE §6-2-16 · BNK-466

### Q599. 출석 QR 체크인 시 생성되는 QR 코드 형식이 무엇인가요?

**A.** **QR payload 갭 · Q109 carry · Must 기능 but FE image generation 미완료**

**현재 상태**:
- **QR 코드 데이터** (payload) — 보호자 셀프 체크인용 URL 인코딩
- **형식** — Base64 + URL safe encoding (RFC 4648)
- **내용 예시** — `ogada://visit/{visitId}/qr-token/{tokenUUID}`
- **유효 기간** — 24시간 (당일 체크인만)
- **생성 API** — `GET /api/v1/visits/{visitId}/qr-token` (BE ✅)

**갭 분석**:
| 항목 | 상태 | 비고 |
|------|------|------|
| **QR 생성 API** | ✅ | BE `@0c9518a` |
| **QR 코드 이미지** | ❌ | FE wire P3 · qrcode.js 또는 html5-qrcode |
| **모바일 브라우저 카메라 인식** | ❌ | P3 · navigator.mediaDevices.getUserMedia() |
| **토큰 검증 로직** | ✅ | `QrTokenValidationService` |

**운영상 우회 방법** (FE 개발 전):
- Swagger UI에서 `GET /api/v1/visits/{visitId}/qr-token` 실행 → JSON 응답의 `qrPayload` 복사
- 온라인 QR 코드 생성 서비스(예: goQR.me) 에 payload 입력 → 이미지 생성
- 센터 게시판 또는 보호자에게 인쇄 배포

> **API_SPEC §5-6 · REQUIREMENTS §2-9 · Q109 · CHANGELOG · 출석 Must 기능** 참고.

### Q600. 청구서 작성 시 공단 자동 제출(NTS 연동)은 언제 되나요?

**A.** **P3 candidate · 현재 수동 제출 · v2+ roadmap · `G-CASH-RECEIPT-NTS-API`**

**현재 워크플로우** (v1):
1. ogada에서 청구 생성 → PDF 또는 Excel 다운로드
2. 국세청 현금영수증 포털(https://www.nts.go.kr) 에 **수동 접속**
3. CSV 업로드 또는 JSON API 호출 → 영수증 자동 발급
4. 이용자/보호자 에게 번호 전달

**자동 제출 기능** (개발 예정, v2):
- ogada 설정 → 국세청 API 키 등록
- 청구 생성 시 → 자동으로 NTS에 제출
- 결과 응답 → ogada 대시보드에 상태 반영

**선행 요구사항**:
- 센터의 **국세청 사업자번호** 인증 (사업자 증명)
- **NTS 현금영수증 API** 신청 (세정 상담원)
- **ISMS 보안 감시** — 공공 API 연동 시 보안 인증 필수

**현재 대체 방안** (Q531 참고):
- 청구 클릭 → 현금영수증 PDF 다운로드
- 모니링 도구(Kakao i Connect) 등 중간 서비스 활용
- **월말 batch 수동 NTS 업로드** (가장 안전)

> **ROADMAP v2 · REQUIREMENTS §3-8 · Q531 · NTS 공식 문서** 참고.

### Q601. 청구 API에서 반환되는 `appliedFilters` 필드가 뭔가요?

**A.** **FE wire ✅ · 운영 투명성 · 적용된 청구 필터 목록 반환 · BNK-457 · Q587 참고**

**필드 설명**:
```json
{
  "appliedFilters": {
    "depositPeriodLabel": "1~15일",
    "receiptBasisLabel": "청구기준",
    "branchName": "본점"
  },
  "items": [...]
}
...
**사용 사례**:
- **화면 헤더 표시** — 「조회 조건: 1~15일 · 청구기준 · 본점」
- **인쇄 시 포함** — 청구 명세서 맨 위에 적용 조건 명시
- **모니터링 대시보드** — 월별 청구 패턴 분석

**설정 우선순위** (내림차순):
1. **서버 반환값 (`appliedFilters`)** — 항상 우선 (신뢰)
2. **클라이언트 캐시** — 마지막 조회 필터 (UI 편의)

> **API_SPEC §8-4 · BillingReportAppliedFilters · CHANGELOG Q587 · BNK-457** 참고.

### Q602. 미납 관리에서 **독려기록**과 **금액 조정**은 어떻게 하나요? (G-BILLING-OVERDUE-ADJUSTMENT)

**A.** **✅ BE+FE Full-stack Fixed (`4d92844`·`0420e6b`, G-BILLING, 케어포 PDF p.89 7-3 ②③④)**

**`/billing/overdue`** 목록에서 행별 **「관리」** → **`OverdueManagementModal`** (2탭).

| 탭 | API | 용도 |
|----|-----|------|
| **독려기록** | `GET/POST /api/v1/billing/overdue/claims/{claimId}/management-records` | 전화·문자·기타 연락 이력 · **`note`** 최대 2000자 |
| **조정처리** | `GET/POST /api/v1/billing/overdue/claims/{claimId}/adjustments` | **`PARTIAL`**(부분) · **`FULL_WRITE_OFF`**(전액 조정) · **`reason`** 필수 |

**조정 규칙**:

| 유형 | 동작 |
|------|------|
| `PARTIAL` | **`adjustedAmount`** — 조정 **후** 잔여 미납액(원) · 현재 미납액보다 **작아야** 함 |
| `FULL_WRITE_OFF` | 미납 **0원** 처리 |
| 거부 (`422`) | 조정할 미납 없음 · 조정 후 금액이 현재 미납 이상 |

**자동 독려기록**: **「안내 발송」** SMS 성공 시 **`autoGenerated=true`** 독려기록이 적재될 수 있습니다 (BE `OverdueManagementService.recordAutomaticSmsRemindersForClaim`, `f6266ec`).

**자동 기록 scope guard (`f6266ec`)** — 아래를 **모두** 만족할 때만 자동 등록:

| 조건 | 미충족 시 |
|------|-----------|
| 청구 **`status=CONFIRMED`** | skip (미확정·초안) |
| 청구 **`yearMonth` < 현재 월** (과거월) | skip (당월·미래월 premature 방지) |
| **`jwtScopeResolver.validateBranchWriteScope`** 통과 | skip (타 지점 cross-scope) |
| claim line **`copayAmount > 0`** | skip (미납 0) |
| **`dispatchedCount > 0`** | skip (발송 0건) |

**DB**: Flyway **V167** — `billing_overdue_management_records` · `billing_overdue_adjustments`

**권한**: **`hq_admin`·`branch_admin`** (지점 스코프)

> 관련: USER_MANUAL §4-2 미납 관리 · ADMIN_GUIDE §6-2-18 · Q196 · BNK-468

### Q603. 대시보드 StatCard가 고대비(강제 색상) 모드에서 구분이 안 됩니다. (UXD-149)

**A.** **✅ FE Fixed (`b969570`, UXD-149)** — Windows **고대비/강제 색상(forced-colors)** 모드에서 **`/dashboard`·`/dashboard/hq`** 위젯 그리드의 인접 StatCard 경계가 누락되던 문제를 **`components.css`** 로 보강했습니다.

| 항목 | 내용 |
|------|------|
| 대상 | **`.ds-dashboard-widgets__item .ds-stat`** — G21 **「공단 일정 불일치」**·G15 **「카카오 API 잔여」** 등 US-M02 위젯 |
| 조치 | forced-colors 시 **ButtonText** 색 경계선 — WCAG **1.4.11 Non-text Contrast** |
| 범위 | **CSS-only** — 일반 모드 시각·데이터·동작 **불변** |

> 관련: Q594 · Q595 · USER_MANUAL §4-2

### Q604. 직원 **서류관리 21-slot** 진행률은 어디서 보나요? (G-STAFF-DOCUMENT-REPOSITORY)

**A.** **✅ BE+FE Full-stack Fixed (`b583c11`·`fd15a2f`·`751c593`·`6bde24a`, US-R03 P2/P3, 케어포 PDF p.96 zone③)**

**`/staff/{userId}` → 「HR 파일함」** 탭 상단 **`StaffDocumentRepositoryPanel`**.

| 항목 | 내용 |
|------|------|
| API | **`GET /api/v1/staff/hr-files/users/{userId}/repository-progress`** — **`fd15a2f`부터 FE 단일 authoritative source** (client-side checklist+파일 merge 제거) |
| 슬롯 | FAQ21825 lifecycle **21종** — 입사(7)·신고(4)·근로(5)·퇴사(5) |
| 완료 | lifecycle **checklist 체크** 또는 **HR 파일 업로드**(8 upload types) |
| parity | **`careforParityLabel`** = 완료/ **20** (케어포 PDF 「서류 20개」) · **`progressLabel`** = 완료/ **21** |
| 업로드 8종 | `id-card`·`certificate`·`bank-account`·`health-check`·`dementia-training`·`employment-contract`·`criminal-record`·`resignation` |
| RBAC | 조회 **`hq_admin`·`branch_admin`·`social_worker`** · 업로드 **`branch_admin`·`social_worker`** |

**운영 절차**:

1. panel StatCard에서 **진행률·케어포 parity·미비 건수** 확인
2. phase별 **미비 Badge** — upload 지원 슬롯은 **「업로드 선택」** 또는 **「모바일 촬영」** (`6bde24a`, Q608) — 스마트폰 후면 카메라·**`accept="image/*"`** · **`uploadStaffHrFileApi`**
3. checklist만 필요한 항목(급여명세서·법정교육 등)은 **「lifecycle 탭 열기」** → **「입사~퇴사」** 탭에서 체크

| 모바일 촬영 | 내용 |
|-------------|------|
| 대상 | **`StaffDocumentRepositoryPanel`** 미비 upload 슬롯 · **`StaffHrFilePanel`** · **`StaffRefresherCertificatePanel`** (`FileUpload enableMobileCapture`) |
| 동작 | **`capture="environment"`** hidden input — iOS/Android **카메라 앱** 연동 (getUserMedia 스트림 ❌) |
| 검증 | **`validateStaffHrFile`** — PDF/PNG/JPEG·10MB (xlsx 업로드와 동일) |
| RBAC | **`branch_admin`·`social_worker`** (`canUpload=true`) |

> **P3 잔존**: 출석 QR **카메라 스캔 UI** (Q589) · **QR 코드 이미지** 생성 (Q590)

> 관련: USER_MANUAL §5-3 · ADMIN_GUIDE §6-2-19 · Q298 · Q300 · BNK-470 · **Q606**(UXD-150 a11y) · **Q608**(mobile capture) · **Q611**(UXD-151 mobile full-width CSS)

### Q605. 미납 **독려기록·조정**에 빈 메모만 넣으면 저장되나요? (G-BILLING-OVERDUE-ADJUSTMENT V168)

**A.** **✅ BE Fixed (`399c698`, G-BILLING, V168, Q605)** — **앱·DB 모두** 빈 본문을 거부합니다.

| 계층 | 규칙 |
|------|------|
| API | **`OverdueManagementService.normalizeNote`/`normalizeReason`** — 공백만 → **`422`** |
| DB **V168** | **`chk_billing_overdue_mgmt_note_nonempty`** · **`chk_billing_overdue_adj_reason_nonempty`** — raw SQL도 **REJECT** |
| DB **V168** | **`chk_*_recorded_at_after_created`** — **`recorded_at` < `created_at`** 거부 |
| DB **V168** | **Tenant FK 8쌍** — cross-tenant **branch/claim/client/recorded_by** 참조 차단 |
| 의도적 제외 | **퇴소 이용자** 미납 회수·조정 **허용** (V164 active-client guard 미적용) |

> 관련: Q602 · USER_MANUAL §4-2 · ADMIN_GUIDE §6-2-18 · `DATA_RETENTION_POLICY` · **`OverdueManagementServiceTest`**

### Q606. 미납 **관리**·서류 **repository**·목욕 **전월 복사** 화면 접근성은? (UXD-150)

**A.** **✅ FE Fixed (`751c593`, UXD-150, Q606)** — DESIGN_SYSTEM §73 기준 재점검.

| 화면 | 조치 |
|------|------|
| **`OverdueManagementModal`** | 독려·조정 **Field label** · 기록 시각 **`<time dateTime>`** · 탭 전환·저장 **`aria-busy`** |
| **`StaffDocumentRepositoryPanel`** | phase·슬롯 **`aria-label`** · forced-colors 경계 **`components.css`** |
| **`BathingSchedulePage`** | **「전월 일정 복사」** 실행 중 **`aria-busy="true"`** |

> 관련: Q602 · Q604 · Q598 · USER_MANUAL §4-2·§5-3·§5-26 · **UXD-149**(dashboard forced-colors, Q603)

### Q607. 미납 **「안내 발송」**을 여러 번 하면 SMS **자동 독려기록**이 중복되나요? (G-BILLING-OVERDUE-ADJUSTMENT)

**A.** **✅ BE Fixed (`a45c040`, G-BILLING, Q607)** — **claim·client·SMS·`autoGenerated=true` 조합당 1건만** 자동 등록됩니다.

| 상황 | 동작 |
|------|------|
| 동일 청구·이용자에 **이미 SMS 자동기록** 존재 | **`recordAutomaticSmsRemindersForClaim` skip** — 추가 insert 없음 |
| **수동** 독려기록 (`autoGenerated=false`) | **별도** — 전화·기타 연락은 중복 제한 없음 |
| scope guard (`f6266ec`) | **`CONFIRMED`+과거월+지점 스코프+copay>0** 만 자동 기록 대상 |

> 관련: Q602 · Q605 · USER_MANUAL §4-2 · **`OverdueManagementServiceTest`**

### Q608. 직원 서류를 **스마트폰 카메라**로 바로 올릴 수 있나요? (G-STAFF-DOCUMENT-REPOSITORY mobile capture)

**A.** **✅ FE Fixed (`6bde24a`/`9812ac4`, US-R03 P2, Q608)** — 케어포 PDF p.96 **모바일 서류 등록** 패턴입니다. **네이티브 앱 ❌** · **모바일 브라우저 `<input capture="environment">`** 로 촬영 후 업로드합니다. **`9812ac4`(UXD-151, Q611)** 에서 모바일 **「모바일 촬영」** 버튼이 **전폭(`width: 100%`)** 으로 표시되도록 CSS 셀렉터를 **`.ds-btn`** 으로 정합했습니다.

| 화면 | 조작 |
|------|------|
| **`/staff/{userId}?tab=files`** | **`StaffDocumentRepositoryPanel`** — 미비 슬롯 **「모바일 촬영」** |
| HR 파일함 | **`StaffHrFilePanel`** · **`StaffRefresherCertificatePanel`** — **`FileUpload`** **「카메라 촬영」** 버튼 |
| API | **`POST /api/v1/staff/hr-files/users/{userId}`** — 기존 HR 업로드와 동일 (`uploadStaffHrFileApi`) |

| 제한 | 내용 |
|------|------|
| 파일 형식 | **이미지(PNG/JPEG)** — xlsx·PDF는 **「업로드 선택」** 사용 |
| 권한 | **`branch_admin`·`social_worker`** |
| 출석 QR | **`/guardian/checkin` 카메라 스캔** 은 **별도 Must 갭** (Q589) |

> 관련: Q604 · Q611 · USER_MANUAL §5-3 · ADMIN_GUIDE §6-2-19 · **`StaffDocumentRepositoryPanel.test`**

### Q609. **`GET /attendance`** 가 **활성 이용자 전원·이름·상태**를 반환하나요? (G-ATTENDANCE-ROSTER-STATUS)

**A.** **✅ BE+FE Full-stack Fixed (`0c69060`/`61e1970`/`8383f8d`, US-E01, Q609)** — 케어포 **func 2-3 출석관리** roster 패턴입니다. **`AttendancePage`** 가 **`fetchAttendanceApi()`** 단일 호출로 **이름·상태·미처리 행**을 표시합니다(별도 **`fetchClientsApi()`** 병합 **없음**, `8383f8d`).

| API | 내용 |
|-----|------|
| `GET /api/v1/attendance?date=&branchId=&transportMode=` | 지점 **활성 이용자 전원** — 이름순 정렬 |
| 응답 `items[]` | **`clientName`·`status`·`usesTransport`·`checkInAt`·`checkOutAt`·`transportType`·`absenceReason`** |
| pending (미처리) | **`id=null`**, **`status=null`**, **`checkInAt`/`checkOutAt` null** |
| `transportMode` | `all`(기본) · `boarding` · `on_site` — **`usesTransport` 필터** (Q232) |

**`status` 값**:

| 값 | 의미 |
|----|------|
| `CHECKED_IN` | 입소 완료·귀가 전 |
| `CHECKED_OUT` | 귀가 완료 |
| `ABSENT` | 결석 (`absenceReason` 있음) |
| `null` | **미처리** — 입소·결석 버튼 대상 |

**쓰기 API 응답** — `POST /attendance/check-in`·`/check-out`·`/absence` 도 동일 **`clientName`·`status`** 를 반환합니다 (`61e1970` routing test).

**RBAC** — **`RoleBasedControllerAccessTest$AttendanceAccess`**:

| 역할 | `GET /attendance` | `POST /attendance/check-in` |
|------|-------------------|----------------------------|
| `caregiver` | ✅ 200 | ✅ 200 |
| `branch_admin` | ✅ 200 | ✅ 200 |
| `guardian` | ❌ 403 | ❌ 403 |

**FE wire (`8383f8d`)** — **`AttendancePage.jsx`**:

| 항목 | 내용 |
|------|------|
| 데이터 로드 | **`fetchAttendanceApi()`** only — redundant **`fetchClientsApi`** merge **제거** |
| 표시 | **`record.clientName`** 우선 · 없으면 **`clientId`** fallback |
| 테스트 | **`AttendancePage.test`** — pending `id=null`·`status=null`·입소/결석 버튼 · checked-in 귀가 버튼 |

> 관련: Q94 · Q232 · USER_MANUAL §4-4 · ADMIN_GUIDE §10-5 · DEPLOYMENT_GUIDE §1-4 · **`AttendanceRosterLiveApiRoutingE2eTest`**

### Q610. **`npm test`** 가 OOM·hang으로 실패하면 어떻게 하나요? (QA-B222)

**A.** **✅ FE Fixed (`8383f8d`, QA-B222, Q610)** — Vitest가 **fork pool을 직렬화**하고 **dot reporter·30s timeout**을 적용해 locked 실행 중 OOM/no-output hang을 줄입니다. **동시 `npm test` 2개 이상**은 여전히 금지입니다.

| 설정 (`vite.config.js`) | 값 | 목적 |
|-------------------------|-----|------|
| `pool` | `"forks"` | 프로세스 격리 |
| `maxWorkers` | `2` | 워커 상한 |
| `fileParallelism` | `false` | 파일 단위 직렬 |
| `reporters` | `["dot"]` | hang 시 무출력 완화 |
| `testTimeout` / `hookTimeout` | `30_000` | 장기 hang 차단 |

| 규칙 | 내용 |
|------|------|
| 실행 경로 | **`npm test`** → **`scripts/npm-test-locked.sh`** (flock) — **`npx vitest run` 직접 호출 금지** |
| 사전 확인 | `ps aux \| grep -E '[v]itest run'` — 겹치면 대기 또는 **`./scripts/vitest-stop.sh`** |
| 회귀 테스트 | **`vitestConfig.test.js`** — pool·workers·fileParallelism·reporters assert |

> 관련: **`docs/qa/VITEST_CONCURRENCY.md`** · `.agents/rules.md` §5 · DEPLOYMENT_GUIDE §11-3 · CHANGELOG 296차

### Q611. **「모바일 촬영」** 버튼이 스마트폰에서 좁게 보입니다. (UXD-151)

**A.** **✅ FE Fixed (`9812ac4`, UXD-151, Q611)** — **`6bde24a`** 모바일 전폭 CSS가 존재하지 않는 **`.ds-button`** 클래스를 타깃해 **`StaffDocumentRepositoryPanel`** 의 **「모바일 촬영」**·**「업로드 선택」** 버튼에 적용되지 않던 **FE-16 회귀**를 해소했습니다. ogada **`Button`** 컴포넌트는 **`.ds-btn`** 을 렌더합니다.

| 항목 | 내용 |
|------|------|
| 수정 | **`components.css`** — **`.ds-staff-document-repository .ds-inline-actions .ds-btn { width: 100%; }`** |
| 범위 | **순수 CSS** — DOM·업로드 API·`capture="environment"` 동작 **불변** |
| 대상 화면 | **`/staff/{userId}?tab=files`** — **`StaffDocumentRepositoryPanel`** 미비 upload 슬롯 |
| 확인 | 스마트폰(또는 DevTools 모바일 뷰)에서 버튼이 **행 전체 너비**로 표시되는지 확인 |

> 관련: Q608 · Q606 · USER_MANUAL §5-3 · ADMIN_GUIDE §6-2-19 · CHANGELOG 297차

### Q612. 직원 **출퇴근 (8-4)** 은 어디서 기록하나요? (G-STAFF-WORK-ATTENDANCE)

**A.** **✅ BE+FE Full-stack Fixed (`a6eb8b7`·`5fd468b`, US-R03·케어포 PDF p.100 8-4, Q612)** — 케어포 **「8-4. 출퇴근·근무일지」** 패턴입니다. **`/staff/attendance`** 에서 지점 직원의 **일일 출근·퇴근**을 조회하고 **수동 체크인/아웃**을 기록합니다.

| API | 용도 |
|-----|------|
| `GET /api/v1/staff/work-attendance?date=YYYY-MM-DD&branchId=` | **일일 로스터** — `items[]` with `userName`·`roleCode`·`status`·`checkInAt`·`checkOutAt`·**`checkInMethod`** |
| `POST /api/v1/staff/work-attendance/check-in` | **출근** — `{ "userId": "<uuid>", "checkInMethod": "MANUAL" \| "MOBILE" \| "NFC" }` — **당일만** |
| `POST /api/v1/staff/work-attendance/check-out` | **퇴근** — `{ "userId": "<uuid>" }` — **당일·출근 후만** |

| UI 단계 | 내용 |
|---------|------|
| 1 | **`/staff/attendance`** — **`StaffContextNav`「출퇴근 (8-4)」** |
| 2 | **근무일** `DateInput` → **「조회」** — **`fetchStaffWorkAttendanceApi`** (`services.js`) |
| 3 | **「출근 방식」** select — **수동(`MANUAL`)** · **모바일(`MOBILE`)** · **NFC(`NFC`)** (`5fd468b`) |
| 4 | StatCard **출근/퇴근/미출근** · 테이블 **직원·역할·상태·출근 방식·시각** |
| 5 | **「출근」** → `staffWorkAttendanceCheckInApi({ checkInMethod })` · **「퇴근」** → `staffWorkAttendanceCheckOutApi` → 목록 자동 refresh |

| `checkInMethod` | UI 라벨 | 비고 |
|-----------------|---------|------|
| `MANUAL` | 수동 | 기본값 |
| `MOBILE` | 모바일 | 앱·QR 연동 **P3** |
| `NFC` | NFC | 단말 연동 **P3** |
...
| DB | **Flyway V169** — `staff_work_attendance` — `(organization_id, branch_id, user_id, work_date)` UNIQUE · `check_in_method` `MANUAL`/`MOBILE`/`NFC` |

> **이용자 출석**(`/attendance`, Q609)과 **직원 출퇴근**(`/staff/attendance`)은 **별도 API**입니다.  
> **P3 잔여**: NFC/MOBILE **하드웨어 단말 연동** · 월간 통계·CSV export  
> 관련: Q589 · Q614 · USER_MANUAL §5-3 · ADMIN_GUIDE §6-2-20 · **`StaffWorkAttendancePage.test`** · **`StaffWorkAttendanceLiveApiRoutingE2eTest`**

### Q613. 출석 통계 API와 화면 **`/attendance/stats`** contract는 무엇인가요? (G-ATTENDANCE-STATS)

**A.** **✅ BE + FE Full-stack (US-E05, Q613·Q615, FE `dffd726`)** — 백엔드 **`GET /api/v1/attendance/stats/monthly`** 와 프론트 **`AttendanceStatsPage`** 가 **동일 contract**로 연동됩니다.

| 계층 | 구현 | 상태 |
|------|------|------|
| BE API | **`AttendanceService.buildMonthlyAttendanceStats()`** — `MonthlyAttendanceStatsListResponse` | **✅** @ `6be0c79` |
| FE route | **`/attendance/stats`** — StatCard·6개월 추이·다지점 표 | **✅** @ `dffd726` |
| FE API client | **`fetchMonthlyAttendanceStatsApi({ yearMonth, branchId })`** → **`from`/`to`/`branchId`** | **✅** (`monthlyAttendanceStats.js`) |
| RBAC | **`hq_admin`·`branch_admin`·`social_worker`·`caregiver`** | **✅** |
...
| # | FE | BE | 매핑 |
|---|-----|-----|------|
| 1 | `yearMonth=2026-06` | `from=2026-06-01&to=2026-06-30` | **`yearMonthToPeriod()`** |
| 2 | `branchId` from JWT active branch | `branchId` optional query | **`user.activeBranchId`** |
| 3 | StatCard **활성 이용자** | `branches[].months[n].activeClientCount` | **`mapMonthlyAttendanceStatsResponse`** |
| 4 | StatCard **출석률 %** | `attendanceRate` 0~1 | **`attendanceRateToPercent()`** |
| 5 | 차트 **6개월 trend** | `months[]` in 6-month window | **`buildMonthlyStatsTrendPeriod()`** |
| 6 | 일별·이용자별 breakdown | *(없음)* | **P3 carry** — MVP 범위 외 |

> 관련: Q106·Q615 · USER_MANUAL §4-4 · ADMIN_GUIDE §6-2-21 · **`monthlyAttendanceStats.test.js`**

### Q614. **더 이른 월 미납**이 있는데 **신규 월 입금**이 거부됩니다. (G-BILLING-DEPOSIT-ORDER-GUARD)

**A.** **✅ BE Fixed (`a6eb8b7`, US-M03·케어포 PDF p.85 7-1 선행입금, Q614)** — 청구 생성 가드(Q571·Q310)와 **동일한 선행입금 원칙**을 **수납·은행 엑셀 import**에도 적용합니다. 동일 이용자에게 **더 이른 `CONFIRMED` 미납 청구**가 남아 있으면 **신규 월 입금**이 차단됩니다.

| 상황 | 응답·동작 |
|------|-----------|
| **5월 미납** 상태에서 **6월 청구**에 수동 수납 시도 | **`422 BUSINESS_RULE`** — **「이전 미납 청구(2026-05) 입금 선행이 필요합니다.」** |
| 은행 엑셀 import — 동일 이용자·금액이 **5월·6월** 청구에 모두 매칭 가능 | **가장 이른 청구월(5월) 우선** 자동 매칭 (`BankDepositImportService`) |
| 전월만 미납 — Q571과 동일 | **`/billing/payments`** 에서 **오래된 월부터** 입금일(`paidAt`) 입력 |

| 적용 API | 가드 |
|----------|------|
| `POST /api/v1/billing/claims/{claimId}/payments` | **`BillingService.assertPriorDepositOrderForClients()`** |
| `POST /api/v1/billing/imports/bank-deposits` | 동일 규칙 + **earliest-month auto-match** |

**현장 조치**:

1. **`/billing/overdue`** 또는 **`/billing/payments`** 에서 **가장 오래된 미납 월**부터 입금 처리합니다.
2. 대시보드 **「청구 생성 제한 (7-1 선행입금 가드)」** StatCard·**`ClaimGenerationGuardBanner`**(Q310)와 **동일한 월 순서**를 따릅니다.
3. 은행 엑셀 import 시 **미리보기 `yearMonth`·이용자명**을 확인하고, **이전 월 미납**이 있으면 먼저 수동 수납하세요.

> 관련: Q571 · Q310 · Q572 · USER_MANUAL §4-6 · ADMIN_GUIDE §6-2-22 · **`BillingServiceTest`** · **`BankDepositImportServiceTest`**

---

### Q615. 출석 통계 (`/attendance/stats`) 화면이 비어있거나 FE wire가 안 되어요. (G-ATTENDANCE-STATS contract)

**A.** **✅ BE + FE Full-stack Fixed** (`dffd726`·`6be0c79`, US-E05, Q615) — **`AttendanceStatsPage`** 가 백엔드 **`GET /api/v1/attendance/stats/monthly`** contract에 맞게 **월별 지점 집계**를 표시합니다.

| 항목 | 상태 | 내용 |
|------|------|------|
| **BE API** | **✅** | **`from`·`to`·`branchId`** — 응답 **`branches[].months[]`** (`activeClientCount`·`attendedDays`·`attendanceRate`) |
| **FE route** | **✅** | **`/attendance/stats`** — **`fetchMonthlyAttendanceStatsApi({ yearMonth, branchId })`** 가 **`monthlyAttendanceStats.js`** 로 **`from`/`to` 변환** |
| **화면** | **✅** | StatCard **활성 이용자·출석 일수·출석률** · **최근 6개월 추이 차트** · **다지점 표**(HQ) |
| **P3 한계** | **△** | **일별·이용자별 breakdown** 은 BE 미제공 — 월·지점 집계만 표시 (설계 범위) |

**사용 방법**:
1. SideNav **출석 → 출석 통계** 이동
2. **`MonthInput`** 으로 대상 월 선택 → **「조회」**
3. **`hq_admin`** 은 **BranchSwitcher** 전환 후 **재조회** (Q173)

**여전히 빈 화면일 때**:
- 해당 월·지점에 **활성 이용자 0명** 또는 **출석 기록 없음** — **`/attendance`** 당일 roster(Q609)로 먼저 출석 입력 확인
- **`400`/`422`** — 대상 월 형식(YYYY-MM) 확인

> 관련: Q106·Q613 · USER_MANUAL §4-4 · **`monthlyAttendanceStats.test.js`** · **`AttendanceStatsPage.test`**
...
**A.** **✅ BE + FE Fixed** (`dffd726`·`6be0c79`, US-E01) — **`/attendance`** 화면에 **「결석」** 버튼과 **`AttendanceAbsentModal`** 이 구현되어 **`POST /api/v1/attendance/absence`** 를 호출합니다.

| 항목 | 상태 | 내용 |
|------|------|------|
| **BE API** | **✅** | **`POST /api/v1/attendance/absence`** — `clientId`·`absenceReason` (FE `dffd726` contract 정정) |
| **FE 화면** | **✅** | **`/attendance`** 테이블 행별 **「결석」** → **`AttendanceAbsentModal`** |
| **RBAC** | **✅** | `caregiver`·`social_worker` 실행 권한 |

**사용 방법**:
1. **`/attendance`** 에서 미처리(`status=null`) 이용자 행의 **「결석」** 클릭
2. **결석 사유** 입력 후 확인
3. StatCard **「결석」** 카운트 갱신 확인

> 관련: Q94 · USER_MANUAL §4-2 · **`AttendancePage.test`** · **`attendanceServices.test.js`**

---

### Q618. 청구 보고서 필터 (월별·기간) 저장 API가 안 되는데요. (G-BILLING-REPORT-FILTER-PERSISTENCE, Q621)

**A.** **✅ BE + FE Full-stack Fixed** (`479995e`·`77b1ea8`·`fd0a3b3`, G-BILLING, Q621) — **`BillingReportPage`** 가 **마운트 시 저장된 필터를 불러와 UI에 반영**하고, **「조회」** 시 **`PUT /api/v1/billing/reports/filters`** 로 명시 저장합니다.

| 항목 | 상태 | 내용 |
|------|------|------|
| **BE GET API** | **✅** | **`GET /api/v1/billing/reports/filters?month=YYYY-MM&branchId=`** — charges/deposits/receipts/refunds 4-variant |
| **BE PUT API** | **✅** | **`PUT /api/v1/billing/reports/filters`** — `month`·`variant`·`q`·`depositPeriod`·`receiptBasis` — **write scope** |
| **BE auto-save** | **✅** | **`GET /billing/reports/{variant}`** 조회 시 **`recordFromReportQuery()`** silent upsert — **read scope** (`fd0a3b3`, Q623) |
| **DB** | **✅** | **V170** `billing_report_filters` · **V171** integrity CHECK |
| **FE 필터 UI** | **✅** | `/billing/reports/deposits` · `/billing/reports/receipts` — **`appliedFilters` echo** (Q587) |
| **FE bootstrap** | **✅** | **`fetchBillingReportFiltersApi`** · **`normalizeSavedFilters`** · variant 변경 시 hydrate (`77b1ea8`) |
| **RBAC** | **✅** | **`hq_admin`·`branch_admin`** |

**현장 사용**:
1. **입금대장**·**수납대장** 등 대장 화면 진입 → **저장된 입금 구간·집계 기준·검색어**가 자동 복원됩니다.
2. 조건 변경 후 **「조회」** → 서버 **auto-save** + FE **`PUT`** persist.
3. **통합 관리자**가 BranchSwitcher로 **다른 지점** 대장을 열어도 autosave **403** 없이 조회됩니다 (Q623).

**Swagger 확인**:
```
GET /api/v1/billing/reports/filters?month=2026-06&branchId=<uuid>
PUT /api/v1/billing/reports/filters
{ "month": "2026-06", "variant": "deposits", "depositPeriod": "FIRST_HALF", "q": "홍길동" }
```

> 관련: Q621·Q623 · Q601·Q585·Q587 · USER_MANUAL §4-6 · ADMIN_GUIDE §6-2-23 · **`BillingReportPage.test`**
...
### Q621. 청구 대장(입금·수납) 필터가 매번 초기화됩니다. 저장되나요? (G-BILLING-REPORT-FILTER-PERSISTENCE)

**A.** **✅ BE + FE Full-stack Fixed** (`77b1ea8`·`fd0a3b3`, G-BILLING, Q621) — **사용자·지점·월·대장 variant별** 마지막 조건이 **DB에 저장**되고, **화면 진입 시 자동 복원**됩니다.

| 항목 | API·동작 |
|------|----------|
| **조회(저장값)** | **`GET /api/v1/billing/reports/filters?month=YYYY-MM&branchId=`** — 4-variant(charges/deposits/receipts/refunds) |
| **명시 저장** | **`PUT /api/v1/billing/reports/filters`** — **「조회」** 클릭 시 FE **`saveBillingReportFilterApi`** (`77b1ea8`) |
| **자동저장** | **`GET /billing/reports/{variant}`** 호출 시 **`recordFromReportQuery()`** — **read scope** (`fd0a3b3`, Q623) |
| **FE hydrate** | **`BillingReportPage`** — **`loadSavedFilters()`** → **`setSearch`/`setDepositPeriod`/`setReceiptBasis`** → 자동 **「조회」** |
| **스코프** | **`UNIQUE(organization, branch, user, year_month, variant)`** |
| **RBAC** | **`hq_admin`·`branch_admin`** |

**저장되는 필드 예**:
- **입금대장** (`deposits`): **`depositPeriod`** — `FULL`/`FIRST_HALF`/`SECOND_HALF` (Q585)
- **수납대장** (`receipts`): **`receiptBasis`** — `PAYMENT`/`CLAIM` (Q585)
- **공통**: **`q`** — 이용자 검색어

**현장 팁**:
- **월 변경** 후에도 **variant별** 마지막 조건이 유지됩니다 — **입금대장 ↔ 수납대장** 탭 전환 시 각각 복원됩니다.
- **「적용 조건:」** summary와 **`appliedFilters`** echo로 서버 적용값을 교차 확인하세요 (Q587).

> 관련: Q618·Q623 · Q601·Q585 · USER_MANUAL §4-6 · ADMIN_GUIDE §6-2-23 · **`BillingReportPage.test`**

---

### Q622. 직원 출퇴근에서 출근 전 퇴근 처리가 거부됩니다. (G-STAFF-WORK-ATTENDANCE, Q612)

**A.** **✅ BE Fixed** (`35e6c52`, US-R03, Q622) — **출근 기록 없이 퇴근**하거나 **퇴근 시각이 출근 시각보다 이른** 경우 **`422 BUSINESS_RULE`** 로 거부됩니다.

| 상황 | 응답 |
|------|------|
| **미출근** 상태에서 **「퇴근」** | **`422`「출근 기록이 없어 퇴근 처리할 수 없습니다.」** |
| **출근 시각 > 퇴근 시각** (시계·데이터 불일치) | **`422`「퇴근 시각은 출근 시각보다 빠를 수 없습니다.」** |
| **이미 퇴근** | **`422`「이미 퇴근 처리된 직원입니다.」** |

**DB defense-in-depth**: **V171** `chk_staff_work_attendance_checkout_not_before_checkin` — 애플리케이션 guard와 동일 규칙.

**현장 조치**: **「출근」** → **「퇴근」** 순서로 처리하세요. 과거 **근무일 조회**는 가능하나 **당일만** 출근·퇴근 API가 동작합니다 (Q612).

> 관련: Q612·Q619 · USER_MANUAL §5-3 · ADMIN_GUIDE §6-2-20 · **`StaffWorkAttendanceServiceTest`**

---

### Q623. 통합 관리자가 다른 지점 청구 대장을 열 때 403이 나거나, 출퇴근·출석 통계 화면 접근성은? (UXD-152·filter autosave)

**A.** **✅ BE+FE Fixed** (`fd0a3b3`·`df7f308`, UXD-152·G-BILLING, Q623)

**청구 대장 필터 autosave (BE `fd0a3b3`)**:
- **`GET /billing/reports/{variant}`** 조회 시 필터 **auto-save**는 **read scope** — **`hq_admin`** 이 BranchSwitcher로 **비활성 지점** 대장을 열어도 **403 side-effect 없음**.
- **`PUT /billing/reports/filters`** 명시 저장은 **write scope** 유지 — 해당 지점 **수정 권한** 필요.

**접근성 (FE `df7f308`, UXD-152)**:

| 화면 | 개선 |
|------|------|
| **`/staff/attendance`** | **「조회」`aria-busy`** · StatCard **`role=group`·`aria-label`** · 출근/퇴근 버튼 **직원명 `aria-label`·`aria-busy`** · **`<time dateTime>`** |
| **`/attendance/stats`** | **「조회」`aria-busy`** · StatCard 그룹 **`aria-label`** |
| **`CmsDebitPanel`** | **`requestedAt`/`completedAt` `<time dateTime>`** · 상태 **`section aria-labelledby`** |

> 관련: Q621·Q612·Q615 · USER_MANUAL §4-4·§5-3 · **`StaffWorkAttendancePage.test`**

---

### Q624. 지점 QR 생성·이미지 저장은 어디서 하나요? (US-E03 ✅ full-stack)

**A.** **✅ BE+FE Full-stack Fixed (`250619e`, US-E03, Q624)** — **`/attendance/qr/generate`** **`QrGeneratePage`** 에서 **입소/귀가 QR**을 생성·**PNG 미리보기**·**「이미지 저장」**·**인쇄**까지 처리합니다.

| 단계 | 화면·API |
|------|----------|
| 1 | **`/attendance/qr/generate`** — **입소/귀가** 선택 → **「QR 생성」** |
| 2 | **`POST /api/v1/branches/{branchId}/qr`** — `{ "direction": "in"|"out", "expiresInMinutes": 480 }` → **`qrToken`** |
| 3 | FE **`branchQrCode.js`** — **`qrTokenToDataUrl()`** · **`qrcode` ^1.5.4** → PNG data URL |
| 4 | **「이미지 저장」** — `ogada-qr-{입소|귀가}.png` · **「인쇄」** — 입구 게시 |
| 5 | 보호자 **`/guardian/checkin`** — **`POST /attendance/qr/scan`** + `clientId` |

| 항목 | 상태 |
|------|------|
| BE **`qrToken` 발급** | ✅ |
| FE **PNG 렌더·저장·인쇄** | ✅ (`250619e`) |
| **카메라 스캔 UI** | **P3** — 토큰 수동 입력 또는 외부 QR 앱 (Q109) |

> 관련: Q590 · Q109 · Q616 · USER_MANUAL §5-7 · **`POST /api/v1/branches/{branchId}/qr`**

---

### Q625. Flyway **V171** 미적용 시 health·live E2E가 막히나요?

**A.** **✅ BE Fixed (`dce9bf1`/`175d9cb`, system, Q625)** — **`V171DefenseInDepthSchemaReadinessProbe`** 가 **5개 CHECK/FK** 존재 여부를 검사합니다.

| probe | 내용 |
|-------|------|
| **`GET /api/v1/system/health`** | **`v171DefenseInDepthIntegrityCheckReady`** (boolean) |
| **`GET /api/v1/system/live-e2e/probe`** | 동일 필드 + **`operationBlockers[]`** |
| blocker | **`v171-defense-in-depth-constraint-missing`** — V171 미적용·제약 누락 |

**V171 5항목**: `chk_staff_work_attendance_checkout_not_before_checkin` · `chk_billing_report_filters_updated_after_created` · `chk_billing_report_filters_search_query_nonempty` · `fk_billing_report_filters_branch_org` · `fk_billing_report_filters_user_org`

**조치**: 배포 후 Flyway **V171** 적용 확인 → health **`v171DefenseInDepthIntegrityCheckReady=true`** → `./scripts/run-live-e2e.sh` 재실행.

> 관련: DEPLOYMENT_GUIDE §1-3·§11-3 · ADMIN_GUIDE §1-4 · CHANGELOG 306차

---

### Q626. HQ admin이 **다른 지점 청구 대장**을 조회할 때 403이 나올 수 있나요?

**A.** **✅ BE Fixed (`99d03fa`/`fd0a3b3`, G-BILLING, Q626)** — 청구 대장 **GET 조회** 시 필터 autosave는 **`recordFromReportQuerySafely()`** 로 **non-blocking** 처리됩니다.

| 경로 | autosave | 실패 시 |
|------|----------|---------|
| **`GET /billing/reports/{variant}`** | **`recordFromReportQuerySafely()`** | **조회 200 유지** — persistence/validation 오류 **무시** |
| **`PUT /billing/reports/filters`** | **`saveFilterInternal(..., requireWriteScope=true)`** | **write scope** — 권한 없으면 **403** |

**현장 증상 (304차 이전)**: HQ admin **비활성/타 지점** 대장 조회 시 autosave side-effect **403** → **306차 이후 조회 자체는 정상**.

> 관련: Q621 · Q623 · USER_MANUAL §4-6 · **`BillingReportFilterServiceTest`**

---

### Q627. CMS 자동이체(7-4) 등록 시 **은행코드·계좌 뒷자리** 형식 오류는?

**A.** **✅ BE Fixed (`9db0bbb`, G2, Q627)** — **`CmsService`** 가 등록 입력을 **trim·pattern 검증**합니다.

| 필드 | 규칙 |
|------|------|
| **`payerName`** | trim · 빈 문자열 **422** |
| **`bankCode`** | trim · 허용 패턴만 |
| **`accountLast4`** | trim · **4자리 숫자** |

**FE**: **`/billing/cms`** **`CmsDebitPanel`** — stub FCMS 연동 (live FCMS **P3**, Q206–Q208).

> 관련: USER_MANUAL §4-6 · ADMIN_GUIDE §6-2-4 · **`CmsServiceTest`**

---

### Q628. 이용자 상세 **출석 탭**이 비어 있었는데, 지금은 어떻게 조회하나요? (US-D03 ✅ full-stack)

**A.** **✅ BE+FE Full-stack Fixed (`d058e43`, US-D03, Q628)** — 이전에는 존재하지 않는 `client.attendanceRecords` 필드를 읽어 **항상 빈 목록**이었습니다. **`ClientDetailPage`** 출석 탭이 **`GET /api/v1/clients/{clientId}/attendance`** 를 **`apiFetch`** 로 호출합니다.

| 단계 | 화면·API |
|------|----------|
| 1 | **`/clients/:id` → 「출석」** 탭 선택 — lazy load |
| 2 | **`fetchClientAttendanceHistoryApi(clientId, { from, to })`** — **`services.js`** |
| 3 | 응답 **`items[]`** — **`attendanceDate`·`status`·`checkInAt`·`checkOutAt`·`absenceReason`** |
| 4 | 테이블 — **`StatusBadge`** · **`<time dateTime>`** · 결석 사유 열 |
| 5 | **당일 입소·귀가** — **`/attendance`** 출석 관리 (Q609) |

| 항목 | 내용 |
|------|------|
| BE API | **`GET /clients/{id}/attendance?from=YYYY-MM-DD&to=YYYY-MM-DD`** — RBAC **`hq_admin`·`branch_admin`·`social_worker`·`caregiver`** |
| FE | **`clientAttendanceServices.test.js`** · **`ClientDetailPage.test`** — US-D03 regression |
| 잔여 | **건강 탭** `payload` flat 필드 갭(Q102) · **청구 탭** `claimStatus` 매핑(Q83) |

> 관련: Q102 · Q60 · USER_MANUAL §4-3 · DEPLOYMENT_GUIDE §1-4

---

### Q629. ogada **모니터링 15문항**과 **공단 평가지표 1~15** 번호가 다른데, 어디서 대응표를 보나요? (G30-LEGEND ✅ full-stack)

**A.** **✅ FE Fixed (`fdc135b`, G30-LEGEND, Q629)** — 이지케어 FAQ21836~21842 **공단 평가지표 1~15** 와 ogada **`MonitoringItemCatalog` 15문항** 은 **번호 순서가 1:1로 일치하지 않습니다**. **`/compliance/monitoring`** 화면 상단 **「공단 평가지표 ↔ ogada 문항 매핑 (G30-LEGEND)」** 패널에서 cross-walk 표를 확인하세요.

| ogada # | 공단 평가지표 | 매핑 종류 | FAQ 근거 |
|---------|--------------|-----------|----------|
| **6** | **15** (매월 자가진단·6개월 rolling) | **직접 매핑** | FAQ21842 |
| **15** | **12~14** (유선상담 5명·60% 만족) | **직접 매핑** | FAQ21841 |
| 1, 8 | 1~3 (직종별 업무·인력배치) | 부분 매핑 | FAQ21836 |
| 3, 7 | 6~10 (급여제공기록·별지 제24호) | 부분 매핑 | FAQ21838 |
| 4, 5, 9~14 | — (사례관리·고충·시설 등) | 직접 매핑 없음 | G32·G42 등 별도 화면 |

| 구현 | 내용 |
|------|------|
| FE | **`MonitoringOfficialIndicatorLegendPanel`** · **`monitoringOfficialIndicatorCrosswalk.js`** — **`buildMonitoringCrosswalkRows(templates)`** |
| API | **`GET /api/v1/compliance/monitoring/items`** — 15문항 **`inspectionDirection`** 템플릿(legend ogada 점검방향 열) |
| 테스트 | **`MonitoringOfficialIndicatorLegendPanel.test`** · **`monitoringOfficialIndicatorCrosswalk.test.js`** · **`MonitoringSelfDiagnosisPage.test`** |

> **운영 팁**: 통합 checklist **문항 6~15** 자동 집계(Q320)와 legend **ogada #** 을 함께 보면, 공단 평가 현장 점검 시 누락 문항을 빠르게 찾을 수 있습니다.

> 관련: Q314 · Q320 · USER_MANUAL §4-3 · ADMIN_GUIDE §1-4 4q2b

---

### Q630. **청구 대장 필터 복원**·**QR 유효시간** 화면에서 접근성(a11y)이 개선되었나요? (UXD-153 ✅)

**A.** **✅ FE Fixed (`da34daf`, UXD-153, Q630)** — UXD-153 재점검 결과 3화면 a11y가 정리되었습니다.

| 화면 | 개선 | WCAG |
|------|------|------|
| **`/billing/reports/*` `BillingReportPage`** | 마운트 시 **`hydrateFiltersAndFetch`** 선행 **`setLoading(true)`** — 필터 복원 비동기 구간 **`aria-busy`** 전달 | 4.1.3 |
| **`/attendance/qr/generate` `QrGeneratePage`** | **`validUntil`** 유효시각 **`<time dateTime>`** 래핑 | 1.3.1 |
| **`/clients/:id` 출석 탭 `ClientDetailPage`** | Spinner·Table·StatusBadge·`<time dateTime>` 표준 패턴 **이미 준수** — 추가 변경 없음 | — |

> 관련: Q621 · Q624 · Q628 · USER_MANUAL §2-2

---

### Q631. live E2E health·probe에서 **`staff-bootstrap-error`** 대신 **`staff-bootstrap-not-ready`** 만 보입니다. 정상인가요? (QA-B95 ✅)

**A.** **✅ BE Fixed (`24d25f1`, QA-B95, Q631)** — bootstrap 실패 시 **`operationBlockers`** 를 **`-not-ready` 코드만** 노출하도록 **정규화**했습니다. health·probe **동일 집합**이며, triage 시 **blocker 수가 줄어** credential·seed blocker와 혼동이 적습니다.

| 항목 | 309차 이전 (`018a781`~`9db0bbb`) | 310차+ (`24d25f1`) |
|------|----------------------------------|---------------------|
| bootstrap 내부 오류 시 blockers | `-not-ready` **+** `-error` **동시** | **`-not-ready`만** |
| primary (`operationBlocker`) | **`staff-bootstrap-error`** 우선 | **`staff-bootstrap-not-ready`** |
| HealthController exception | **`bootstrap-error`** 단일 | **`staff-bootstrap-not-ready` + `guardian-bootstrap-not-ready`** |
| root cause 확인 | blocker `-error` 코드 | **`liveE2eStatusDetail`** · **`liveE2eGuardianStatusDetail`** (`bootstrap=error` 등) |
| derived blocker 억제 | ✅ (Q596 carry) | ✅ **유지** |

**운영 triage 순서**:

1. **`GET /api/v1/system/live-e2e/probe`** — `operationBlockers[]` 확인
2. detail **`bootstrap=error`** / **`guardian-bootstrap=error`** 이면 **서버 로그·Flyway·seed** 우선
3. detail error **없고** `staff-credentials-missing` 등만 있으면 **`scripts/dev-live-e2e.env`** 확인

> 관련: Q448 · Q596 · DEPLOYMENT_GUIDE §11-3 · ADMIN_GUIDE §1-4 · **`LiveE2eOperationReadinessSupportTest`**

---

### Q632. **L03_M15 말기 돌봄(Palliative/End-of-Life Care)** 기록은 어디서 입력하나요? (말기 관리)

**A.** **현재 미구현** — backend/frontend 모두 **P2 roadmap** 상태입니다. 말기 돌봄 기록은 간호 서비스(`/nursing/*`) 화면 → **특이사항·응급·욕창** 등에 통합 기록합니다. 별도 L03_M15 화면은 **v1.2**에서 추가될 예정입니다 (REQUIREMENTS §3-8·L03 roadmap 참고).

> 관련: USER_MANUAL §3-6·ADMIN_GUIDE §2-8 · API_SPEC §15 (간호 기본 CRUD 구현)

---

### Q633. **청구 현금영수증 API (G-CASH-RECEIPT-NTS-API)** 는 언제 공개되나요?

**A.** **현재 미구현** — **P3 roadmap** 상태입니다. **NTS 세금계산서 API 연동**(국세청 현금영수증)은 v1.2 이후에 추가될 예정입니다. 현재 v1에서는 `/billing/cash-receipts` 기본 기록(수동 관리)만 지원합니다.

> 관련: API_SPEC §7-6 · DEPLOYMENT_GUIDE §7-2 환경 변수(NTS 인증)

---

### Q634. **직원 NFC/MOBILE 단말**(출퇴근 자동 태깅)은 언제 지원되나요?

**A.** **현재 미구현** — **P3 roadmap** 상태입니다. 현재 v1에서는 수기 입력(`POST /staff/work-attendance/check-in|check-out`)만 지원합니다. NFC·QR 단말 연동은 v1.2~v2에서 추가될 예정입니다.

> 관련: USER_MANUAL §3-10·ADMIN_GUIDE §8-5 · API_SPEC §9-1 (스탭 업무수행 일지)

---

### Q635. **G34-WORKFLOW-CATALOG**(ezCare 운영주기별 워크플로 카탈로그)는 어디서 보나요?

**A.** **✅ FE Fixed (`77cfc38`/`9f110a5`, G34-WORKFLOW-CATALOG, BNK-507)** — SideNav **운영 → 운영주기별 워크플로 (G34)** → **`/compliance/workflow-catalog`**.

| 항목 | 내용 |
|------|------|
| 화면 | **`WorkflowCatalogPage`** · **`EzcareWorkflowCatalogPanel`** |
| 데이터 | ezCare FAQ **21795–21828** 28건 — **`ezcareWorkflowCatalog.js`** 정적 cross-walk |
| coverage | **16/28 ogada 인용(57.1%)** · **미인용 12건** StatCard 표시 |
| 필터 | **운영 주기** FilterChips(일일/수시~비정기) · **인용 상태**(ogada 인용/미인용) |
| FAQ 링크 | 각 행 **ezCare FAQ 원문** 외부 링크 (`buildEzcareFaqUrl`) |
| ogada 링크 | route가 있는 항목은 **내부 `<Link>`** 로 해당 화면 이동 |

**잔여 P3 (별도 Epic)**: **선임 요양보호사 업무수행일지 재사용 템플릿** 설정 UI — `/staff/lead-caregiver-log` 기본 CRUD만 지원 · 템플릿 카탈로그는 후속.

> 관련: USER_MANUAL §3-5 · ADMIN_GUIDE §1-4 · REQUIREMENTS BNK-507

---

### Q636. 보호자가 **QR 코드를 카메라로 스캔**하는 기능이 있나요? (카메라 스캔 UI)

**A.** **현재 미구현** — **P3 roadmap** 상태입니다. 현재 v1에서는 `/guardian/checkin` 화면에 **QR 토큰 수기 입력**만 지원합니다. 보호자 모바일 카메라로 **자동 스캔·인식** 기능은 v1.1~v2에서 추가될 예정입니다 (UXD-2 `html5-qrcode` 라이브러리 검토 중).

> 관련: USER_MANUAL §2-7·API_SPEC §5·DESIGN_SYSTEM §UXD-2

---

### Q637. CMS **지점 등록 roster**는 어떻게 조회하나요? (케어포 7-4, G2)

**A.** **✅ BE+FE Fixed (`d361833`·`3ece965`, G2, Q637·Q638)** — **`GET /api/v1/billing/cms/enrollments`** 에서 **`clientId`를 생략**하면 **활성 지점(JWT 스코프)의 CMS 등록 목록**을 반환합니다.

| 파라미터 | 용도 |
|----------|------|
| `clientId` | *(생략)* — **지점 roster** 모드 |
| `branchId` | *(선택)* — HQ admin이 특정 지점 지정 · FE는 **`user.activeBranchId`** 를 명시 전송 (`3ece965`, Q638) · **생략** 시 BE **`TenantContext.activeBranchId` fallback** (`d0c0d12`, Q638) |
| `status` | *(선택)* — `ACTIVE` · `CANCELLED` · `PENDING` · 미지정 시 **전체** · **앞뒤 공백·소문자 허용** — BE가 trim·uppercase 후 조회 (`f1225b0`, Q662) |

| 응답 필드 | 설명 |
|-----------|------|
| `clientName` | 이용자명 enrichment (`d361833` 신규) |
| `payerName`·`bankCode`·`accountLast4`·`status`·`enrolledAt` | 기존과 동일 |

| UI | 내용 |
|----|------|
| **`/billing/cms` 등록 관리** | **이용자 미선택** 시 roster 로드 · **`BranchScopeNotice`** 로 활성 지점 표시 · **등록 상태 FilterChips** (전체·활성·해지·대기, `947312c`) · **`CmsEnrollmentTable`** **이용자** 열 — **이용자명 클릭 → `?clientId=` preselect** (`3ece965`) |
| **`/billing/payments`** | **CMS 등록** 열 — **ACTIVE** Badge 또는 **「미등록 · CMS 등록」** deep link (`df9ec6c`, Q638) |
| 해지 | roster에서 **ACTIVE** 행 **「해지」** 가능 — 이용자 선택 없이도 동작 |

> **활성 지점 없음**: FE **「활성 지점을 먼저 선택한 후 CMS roster를 조회하세요.」** · BE **`422`「활성 지점이 선택되지 않았습니다.」** (`3ece965`/`d0c0d12`, Q638)  
> **Swagger**: `GET /api/v1/billing/cms/enrollments?branchId=<uuid>&status=ACTIVE` — `clientId` 없음  
> 관련: Q207 · Q637 · Q638 · USER_MANUAL §4-6 · ADMIN_GUIDE §10-13 · DEPLOYMENT §4-6

---

### Q638. CMS roster와 **입금 처리** 화면은 어떻게 연결되나요? (G2-CMS-ENROLLMENT-ROSTER)

**A.** **✅ BE+FE Full-stack Fixed (`df9ec6c`/`947312c`/`3ece965`/`d0c0d12`/`d3d4d2d`, G2, Q638)** — 케어포 **7-4** CMS 등록 현황을 **입금·등록 화면 간 deep link**로 이어 줍니다.

| 화면 | 동작 |
|------|------|
| **`/billing/payments`** | 목록 로드 시 **`fetchCmsEnrollmentsApi({ status: "ACTIVE" })`** 병렬 조회 · **CMS 등록** 열 — **ACTIVE** → Badge · **미등록** → **「미등록 · CMS 등록」** 링크 (`/billing/cms?clientId=`) |
| **`/billing/cms`** | **`?clientId=<uuid>`** URL로 진입 시 **이용자 Select 자동 선택** (`947312c`) |
| **roster 이용자 열** | **`CmsEnrollmentTable`** — **`/billing/cms?clientId=`** 링크로 해당 이용자 등록 화면 이동 (`3ece965`) |
| **상태 필터** | roster 모드에서 **FilterChips** — 전체·활성·해지·대기 (`947312c`) |

| API | 비고 |
|-----|------|
| `GET …/enrollments?branchId=&status=ACTIVE` | FE가 **활성 지점 `branchId`** 명시 (`3ece965`) |
| `GET …/enrollments?status=ACTIVE` (`branchId` 생략) | BE **`TenantContext.activeBranchId`** fallback (`d0c0d12`) |

> 관련: Q637 · Q207 · USER_MANUAL §4-6 · ADMIN_GUIDE §10-13 · DEPLOYMENT §4-6

### Q639. 직원 **연차휴가 현황**은 어디서 조회·수정하나요? (G-STAFF-ANNUAL-LEAVE)

**A.** **✅ BE+FE Full-stack Fixed (`5353991`/`3902dba`/`41fb84f`, US-R03e, Q639)** — 이지케어 **worker-b100 tab01 「연차휴가 현황」** 과 동일 패턴입니다. **`/staff/annual-leaves`** 에서 지점 직원의 **연차 부여·월별 사용·잔여일**을 조회하고, 권한이 있으면 **월별 사용·부여·비고**를 저장합니다.

| 화면 | 동작 |
|------|------|
| **`/staff/annual-leaves`** | **`StaffContextNav`「연차휴가 (US-R03e)」** · **기준 연도** Select · **`StaffAnnualLeaveTable`** — 직원·소속·입사일·1~12월·부여·사용·**잔여**·비고 |
| **관련 화면 패널** | **`StaffAnnualLeaveRelatedSurfacesPanel`** — 스냅샷 안내 · **「출퇴근 기록」→ `/staff/attendance`** · **「연차·유급휴일 대장 (준비 중)」** (Q650, `0b0d7ba`) |
| **편집 Modal** | **`branch_admin`·`social_worker`** — 행 **「수정」** → **연차 부여(일)** · **월별 사용(일)** 12칸 · **비고** → **저장** |
| **요약 StatCard** | **대상 직원·사용 합계·잔여 합계** (`StaffAnnualLeaveTable` 상단) |

| API | 용도 |
|-----|------|
| `GET /api/v1/staff/annual-leaves/roster?year=2026&branchId=` | 지점 **활성 직원 전원** roster · **`monthlyUsage[]`·`totalEntitlement`·`totalUsed`·`remaining`·`memo`** · **`surfaceKind`·`relatedSurfaces[]`** US-R01 cross-link (Q648) |
| `GET /api/v1/staff/annual-leaves/roster?year=2026` | **`branchId` 생략** — JWT **활성 지점(`activeBranchId`)** roster (`6b84bcd`) |
| `GET /api/v1/staff/annual-leaves/users/{userId}?year=2026` | 1명 연간 레코드 조회 · **`surfaceKind`·`relatedSurfaces[]`** (Q650, `6ab3760`) |
| `PUT /api/v1/staff/annual-leaves/users/{userId}` | `{ year, monthlyUsage[], totalEntitlement, memo }` — **upsert** · 응답 **cross-link 메타** (Q650) |

| RBAC | roster GET | PUT 저장 |
|------|------------|----------|
| `hq_admin` | ✅ (조회만) | ❌ |
| `branch_admin` | ✅ | ✅ |
| `social_worker` | ✅ | ✅ |
...
| 검증·오류 | 내용 |
|-----------|------|
| `branchId` 생략 | **단일 활성 지점** JWT → **activeBranchId** fallback (`6b84bcd`) · **다지점 `branch_admin`** + 미선택 → **`422`** |
| 월별 음수 | **`422`「월별 사용일수는 0 이상이어야 합니다.」** (`a45745c`) · V172 DB CHECK `month_01`~`month_12` ≥0 |
| 월별 소수점 | **`422`「월별 사용일수는 정수여야 합니다.」** (`f88e8b1`, Q641) |
| 비고 31자 이상 | **`422`「비고는 30자 이하여야 합니다.」** (`6b35fb5`, Q642) |
| 사용 합계 > 부여일 | **`422`「월별 사용일 합계가 연차 총 부여일수를 초과할 수 없습니다.」** |
| 연도 | **2000~2100** (`leave_year` CHECK) |

> **DB**: **Flyway V172** — `staff_annual_leave_yearly` — **`UNIQUE(organization_id, user_id, leave_year)`** · org/branch/user Tenant FK · **V173** defense-in-depth CHECK/FK (Q645)  
> 관련: USER_MANUAL §4-7-0a · ADMIN_GUIDE §6-2-23 · DEPLOYMENT §1-4 · ezCare worker-b100 tab01 (BNK-516)

### Q640. live E2E에서 **`staff-credentials-default`** blocker가 남아 있는데 staff suite가 실행됩니다. 정상인가요? (QA-B95)

**A.** **✅ FE Fixed (`6fcd750`/`a170f9c`, QA-B95, Q640)** — health·probe에 **`staff-credentials-default`**·**`guardian-credentials-default`** 또는 **wording variant**(`default-staff-credentials`·`staff default credentials`·`staff credentials placeholder`·`default-guardian-credentials` 등)가 남아 있어도, **해당 suite가 이미 staff/guardian auth에 성공**(`staffAuthReady=true`·유효 access token)하면 **`liveConfig.getEffectiveOperationBlockers`** 가 **auth-class blocker를 recovered로 간주**해 **skip 사유에서 제외**합니다.

| 상황 | health `operationBlockers` | staff-only `liveDescribe` | guardian-only suite |
|------|---------------------------|---------------------------|---------------------|
| default staff creds + **`staffAuthReady=true`** | `staff-credentials-default` **유지 가능** | **skip 제외** — suite **RUN** (`6fcd750`) | guardian blocker는 **별도 contract** |
| default guardian creds + staff auth만 필요 | `guardian-credentials-default` | **제외** — unrelated guardian blocker filter (Q580) | guardian auth recovered 시 **제외** |
| auth 미복구 (`staffAuthReady=false`) | `staff-credentials-default` | **skip 유지** — Q490·Q542 동작 |

| 구현 | 내용 |
|------|------|
| **`isStaffAuthBlocker`** | **`staff credentials default`** · **`staff default credentials`** · **`default staff credentials`** · **`staff credentials placeholder`** · **`staff placeholder credentials`** (`6fcd750`/`a170f9c`) |
| **`isGuardianAuthBlocker`** | **`guardian credentials default`** · guardian 대칭 variant (`a170f9c`) |
| **recovery 조건** | **`requireStaffAuth` + `isLiveStaffAuthReady()`** 또는 guardian 대칭 |
| 테스트 | **`liveE2eHarness.test`** — **`ignores recovered staff default-credential blocker`** · **`ignores recovered staff auth blocker with default-credentials wording variants`** · **`ignores guardian default-credential blockers for staff-only suites`** |


> **주의**: 이 동작은 **FE suite gate** 입니다. health·probe의 **`liveE2eOperationReady`** 값 자체는 **변경하지 않습니다**. triage 시 health blocker 목록과 suite RUN/SKIP을 **별도로** 해석하세요.  
> 관련: Q580 · Q542 · Q490 · DEPLOYMENT §11-3 · ADMIN_GUIDE §1-4

---

### Q641. 연차휴가(G-STAFF-ANNUAL-LEAVE)에서 월별 사용일수가 **소수점**이거나 **음수**인데 저장되나요? (US-R03e)

**A.** **✅ BE+FE Fixed (`f88e8b1`/`80613c3`, G-STAFF-ANNUAL-LEAVE validation deepen)** — **월별 사용일수는 정수만 허용**합니다. 소수점 입력 시 **`422`「월별 사용일수는 정수여야 합니다.」** · 음수 입력 시 **`422`「월별 사용일수는 0 이상이어야 합니다.」** (Q639 deepen) 거부합니다.

| 검증 포인트 | 규칙 | 상태 |
|-----------|------|------|
| **FE form** | `monthlyUsage` input **`type="number" min="0" max="31"`** — 정수만 입력 가능 | **✅ FE wire** (`80613c3`) |
| **BE API** | `PUT /api/v1/staff/annual-leaves/users/{userId}` — **`monthlyUsage[]` 각 월별** 정수 범위 validate | **✅ BE guard** (`f88e8b1`) |
| **테스트** | **`StaffAnnualLeaveServiceTest.validateMonthlyUsageShouldRejectDecimal`** · **범위 boundary test** | ✅ |

> 관련: Q639 · Q642 · USER_MANUAL §4-7-0a · API_SPEC §16-1-2

### Q642. 연차휴가(G-STAFF-ANNUAL-LEAVE) **비고** 필드가 **30자 이상**이면 저장됩니까? (US-R03e)

**A.** **✅ BE Fixed (`6b35fb5`, G-STAFF-ANNUAL-LEAVE validation deepen)** — **비고(`memo`) 최대 길이 30자**입니다. 31자 이상 입력 시 **`422`「비고는 30자 이하여야 합니다.」** 거부합니다.

| 검증 포인트 | 규칙 | 상태 |
|-----------|------|------|
| **validation logic** | `StaffAnnualLeaveService.saveYearlyRecord()` — `memo.length() > 30` 거부 · **`validateMemoLength()`** | **✅ BE guard** (`6b35fb5`) |
| **테스트** | **`StaffAnnualLeaveServiceTest.saveYearlyRecordShouldRejectOlongMemo`** · exact 30 chars OK, 31 chars reject | ✅ |
| **FE UX** | textarea **`maxLength="30"`** · character counter 표시 (권장) | **서버 `422` → Modal 필드 오류** (`31ab1aa`, Q647) · FE 선검증은 Q647 |

> 관련: Q639 · Q641 · USER_MANUAL §4-7-0a · API_SPEC §16-1-2

### Q643. 연차휴가(G-STAFF-ANNUAL-LEAVE) 페이지 제목이 **pilot Must-page** 체크리스트에 포함되나요? (UXD-154·QA-B255)

**A.** **✅ FE Fixed (`8434435`/`971c7f1`/`61f670f`, G-STAFF-ANNUAL-LEAVE page title alignment)** — **`/staff/annual-leaves`** AppShell 제목은 **`STAFF_ANNUAL_LEAVE_PAGE_TITLE`** 상수(**「직원 연차휴가」**)로 통일되어 **pilot Must-page check** (`pilotPageFlows.test`)·단위 테스트·AppShell `<h1>` drift를 방지합니다 (QA-B255).

| 검증 포인트 | 내용 | 상태 |
|-----------|------|------|
| **상수** | **`staffAnnualLeave.js`** — **`STAFF_ANNUAL_LEAVE_PAGE_TITLE = "직원 연차휴가"`** | **✅ FE** (`971c7f1`) |
| **page meta** | **`StaffAnnualLeavePage`** — **`AppShell title={STAFF_ANNUAL_LEAVE_PAGE_TITLE}`** · `<h1>` 일치 | **✅ FE** (`971c7f1`) |
| **Must-page** | **`pilotPageFlows.test`** — **`["/staff/annual-leaves", STAFF_ANNUAL_LEAVE_PAGE_TITLE]`** | **✅ test** |
| **Nav label** | **`StaffContextNav`「연차휴가 (US-R03e)」** — SideNav·context nav 라벨(페이지 `<h1>`과 **별도**) | carry (Q639) |
| **test deepen** | **`StaffAnnualLeavePage.test`** — title regression + Modal validation (`61f670f`) | ✅ |

> 관련: Q639 · Q647 · UXD-154 · USER_MANUAL §4-7-0a · DESIGN_SYSTEM §77

### Q644. Solapi 자격증명에 **placeholder 예시값**을 넣으면 **`liveAlimtalkDispatchReady=true`** 로 나옵니까? (J03)

**A.** **✅ BE Fixed (`19ffa84`, J03, Q644)** — **`NOTIFICATION_PROVIDER=solapi`** 이더라도 **`SOLAPI_API_KEY`·`SOLAPI_API_SECRET`·`SOLAPI_SENDER_ID`·`KAKAO_PF_ID`·`KAKAO_TPL_*` mapped templateId`** 값에 아래 **placeholder marker**가 포함되면 **live-alimtalk ready로 간주하지 않습니다**. **`GET /api/v1/notifications/channel-status`** — **`liveAlimtalkDispatchReady=false`** · **`readinessBlockers[]`** 에 **`MISSING_SOLAPI_CONFIG`** 또는 **`MISSING_TEMPLATE_MAPPING`** 이 남습니다.

| placeholder marker (대소문자 무시·부분 일치) | 예시 거부 값 |
|---------------------------------------------|-------------|
| `stub` | `stub` |
| `default` | `default` |
...
| 계층 | 동작 |
|------|------|
| **readiness API** | **`NotificationChannelReadinessService.isLiveConfigured()`** — placeholder 포함 → **미설정** · **`missingSolapiConfigKeys[]`** 또는 **`missingTemplateCodes[]`** populate |
| **provider bootstrap** | **`NotificationConfig.validateSolapiConfiguration()`** — solapi 기동 시 **동일 guard** · placeholder credential/template → **`IllegalStateException` 기동 실패** |
| **내부 코드명 placeholder** | **`KAKAO_TPL_*` 값이 내부 코드명과 동일**(예: `ATTENDANCE_ARRIVAL` 그대로) — **기존 Q266 guard 유지** |

| 조치 | 내용 |
|------|------|
| dev/staging | **`NOTIFICATION_PROVIDER=stub`** 유지 권장 — Solapi live 키 없이도 앱·readiness 패널 동작 |
| live 전환 | Solapi 콘솔 **실제 API key/secret·발신번호·PF ID·승인 templateId** 로 교체 후 **`GET /notifications/channel-status`** 재확인 |
| 테스트 | **`NotificationChannelReadinessServiceTest.placeholderSolapiCredentialsShouldRemainNotReady`** · **`placeholderTemplateMappingShouldRemainNotReady`** · **`NotificationConfigTest`** |

> **주의**: readiness API는 **비밀값을 노출하지 않습니다**. blocker 코드와 **`missingSolapiConfigKeys[]`** 키 이름만 확인하세요.  
> 관련: Q266 · Q367 · Q318 · ADMIN_GUIDE §10-8 · DEPLOYMENT §4-3

### Q645. Flyway **V173**이 없으면 `/health`·live E2E에서 무엇이 막히나요? (G-STAFF-ANNUAL-LEAVE)

**A.** **✅ BE Fixed (`8c5dd65`, US-R03e, Q645)** — **V173**은 `staff_annual_leave_yearly`(V172)에 **defense-in-depth 무결성 제약** 5건을 추가합니다. 마이그레이션이 적용되지 않으면 readiness probe가 **false**를 반환하고 health·live-e2e에 blocker가 남습니다.

| V173 제약 | 내용 |
|-----------|------|
| `chk_staff_annual_leave_yearly_month_max` | 각 `month_01`~`month_12` **≤ 99.9** |
| `chk_staff_annual_leave_yearly_entitlement_max` | `total_entitlement` **≤ 999.9** |
| `chk_staff_annual_leave_yearly_used_within_entitlement` | **월별 사용 합계 ≤ 부여일** |
| `chk_staff_annual_leave_yearly_memo_nonempty` | `memo`가 있으면 **공백만 불가** |
| `fk_staff_annual_leave_yearly_user_branch_assignment` | `(org, user, branch)` → **`user_branches` 배정 일관성** |

| readiness | 내용 |
|-----------|------|
| probe | **`V173StaffAnnualLeaveYearlyIntegrityReadinessProbe.integrityCheckReady()`** — pg_constraint **5/5** 확인 |
| health | **`GET /api/v1/health`** — **`v173StaffAnnualLeaveYearlyIntegrityCheckReady=false`** |
| live-e2e | **`operationBlockers[]`** — **`v173-staff-annual-leave-yearly-constraint-missing`** |
| 테스트 | **`V173StaffAnnualLeaveYearlyIntegrityReadinessProbeTest`** · **`HealthControllerTest`** · **`LiveE2eOperationReadinessSupportTest`** |

> **배포**: staging/prod 기동 후 **`GET /health`** 에서 **`v173StaffAnnualLeaveYearlyIntegrityCheckReady=true`** 를 확인하세요. V172만 있고 V173이 없으면 **Flyway migrate**를 재실행합니다.  
> 관련: Q639 · DATA_RETENTION_POLICY §2·§3 · DEPLOYMENT §1-3 · ADMIN_GUIDE §6-2-23

### Q646. 연차휴가(G-STAFF-ANNUAL-LEAVE) 화면 **접근성(a11y)** 은 어떻게 보강되었나요? (UXD-155)

**A.** **✅ FE Fixed (`085a85a`, UXD-155, Q646)** — **`/staff/annual-leaves`** 에 디자인 시스템 패턴을 적용해 스크린리더·키보드 사용자가 **안내·로딩·월별 입력 그룹**을 구분하기 쉽게 했습니다.

| UI 요소 | 내용 |
|---------|------|
| **`ds-help-text`** | 카드 상단 **「지점 직원의 연차 부여·월별 사용·잔여일…」** 안내 |
| **`ds-fieldset` + `legend`** | Modal **「월별 사용(일)」** 12칸을 **하나의 fieldset**으로 그룹 |
| **`aria-busy`** | **「조회」** 버튼·roster **`section`** — 로딩 중 상태 |
| **`Field` + `htmlFor`** | 기준 연도·연차 부여·월별·비고 — label-input 연결 (UXD-154 carry) |
| CSS | **`components.css`** — `.ds-fieldset`·`.ds-help-text` 스타일 |

> 관련: Q639 · Q643 · USER_MANUAL §4-7-0a · DESIGN_SYSTEM

### Q647. 연차휴가 **편집 Modal**에서 저장 오류가 **필드별**로 표시되나요? (QA-B255 deepen)

**A.** **✅ FE Fixed (`61f670f`/`31ab1aa`, G-STAFF-ANNUAL-LEAVE, Q647)** — 예. 브라우저 기본 검증 대신 **`noValidate` + `validateStaffAnnualLeaveForm`** 으로 **월별 사용·부여일** 오류를 **해당 `Field` 아래**에 표시하고, API **`422`** 는 **`applyApiErrorToForm`** 으로 **Modal Alert 또는 필드 오류**로 노출합니다.

| 상황 | 화면 동작 |
|------|-----------|
| 월별 **소수점·음수·합계 초과** | **`editFieldErrors.monthlyUsed.N`·`totalEntitlement`** + 상단 Alert |
| 서버 **`422`「비고는 30자 이하여야 합니다.」** | **`editFieldErrors.memo`** 또는 form Alert |
| 저장 **네트워크/500** | **「연차휴가 저장에 실패했습니다.」** fallback Alert |
| 값 수정 | **`clearEditFieldError`** — 해당 필드 오류 **자동 제거** · **마지막 field error 제거 시 form Alert도 clear** (`31ab1aa`/`96e9d25`, Q647 deepen) |
| **비고(`memo`) 수정** | **`handleMemoChange` → `clearEditFieldError("memo")`** — 서버 **`422`** 후 비고 입력 시 **`aria-invalid` 해제** (`96e9d25`, Q647 deepen) |
| **QA-B255 title** | **`STAFF_ANNUAL_LEAVE_PAGE_TITLE`** regression test 유지 (`61f670f`) |

> **주의**: HTML5 **`step="0.5"`** number input이 있어도 **앱 검증이 정수만 허용**합니다 — 소수 입력 후 저장 시 **필드 오류**가 먼저 표시됩니다 (Q641 연계).  
> 관련: Q639 · Q641 · Q642 · USER_MANUAL §4-7-0a · **`StaffAnnualLeavePage.test`**

---

### Q648. 연차휴가 roster API의 **`surfaceKind`·`relatedSurfaces`** 는 무엇인가요? (US-R01 cross-link)

**A.** **✅ BE Fixed (`bbf333c`/`6ab3760`, US-R03e, Q648·Q650)** — **`GET /api/v1/staff/annual-leaves/roster`** 및 **`GET/PUT /api/v1/staff/annual-leaves/users/{userId}`** 응답에 **표면(surface) 메타데이터**가 포함됩니다. 이 화면은 **월별 사용 스냅샷**이며, canonical **연차·유급휴일 대장(US-R01)** 데이터를 **중복 저장하지 않음**을 API 계약으로 명시합니다.

| 필드 | 값·의미 |
|------|---------|
| **`surfaceKind`** | **`ANNUAL_LEAVE_USAGE_SNAPSHOT`** — ezCare worker-b100 tab01 월별 사용 그리드 성격 |
| **`relatedSurfaces[0]`** | **US-R01** · **「출퇴근 기록」** · **`/staff/attendance`** · **`AVAILABLE`** |
| **`relatedSurfaces[1]`** | **US-R01** · **「연차·유급휴일 대장」** · **`/staff/leave-ledger`** · **`AVAILABLE`** (`bb9df48`, Q663) |

| 운영 해석 | 내용 |
|-----------|------|
| **AVAILABLE** | 연관 화면·API 사용 가능 — **출퇴근(8-4)** · **대장(`/staff/leave-ledger`)** 병행 (`8057c1e`/`426d63a`, Q666·Q667) |
| **FE Route** | **`/staff/leave-ledger` ✅ full-stack** — 패널 링크 클릭 시 대장 화면 이동 (`8057c1e`, Q666) |
| **FE wire** | **`StaffAnnualLeaveRelatedSurfacesPanel`** — API **`relatedSurfaces`** 우선 (`0b0d7ba`, Q650) · default **`AVAILABLE`** (`426d63a`, BNK-551) |

> 관련: Q667 · Q639 · Q650 · USER_MANUAL §4-7-0a · ADMIN_GUIDE §6-2-23 · **`StaffAnnualLeaveSupportTest`**

### Q649. 연차휴가 **live API harness**와 **PILOT_CHECKLIST** 등록은 무엇인가요? (US-R03e·E05)

**A.** **✅ FE Fixed (`96e9d25`/`e296387`, G-STAFF-ANNUAL-LEAVE·E05, Q649)** — staging **`LIVE_E2E=1`** 환경에서 연차 roster·yearly API shape를 검증하고, Must API contract 목록에 등록했습니다.

| 항목 | 내용 |
|------|------|
| **live harness** | **`staffAnnualLeaveLiveApi.e2e.test.js`** — **`fetchStaffAnnualLeaveRosterApi`** · **`fetchStaffAnnualLeaveYearlyApi`** — roster `items[]`·`monthlyUsage` 12칸 shape · **401이 아닌 응답** 확인 |
| **PILOT_CHECKLIST** | **`R03e-a`** roster GET · **`R03e-b`** yearly GET · **`R03e-c`** yearly PUT · **`E05`** `GET /attendance/stats/monthly` (`e296387`) |
| **contract test** | **`pilotChecklist.test.js`** — annual leave·monthly stats service mock 분기 |

| 실행 | 명령 |
|------|------|
| live harness only | `LIVE_E2E=1 npm test -- staffAnnualLeaveLiveApi` (staging 전용) |
| full live E2E | `./scripts/run-live-e2e.sh` (DEPLOYMENT §11-3) |

> 관련: Q639 · Q615(E05) · DEPLOYMENT §4-3·§11-3 · ADMIN_GUIDE §1-4

### Q650. 연차휴가 화면에서 **「출퇴근 기록」·「연차·유급휴일 대장」** 은 무엇인가요? (US-R01 related surfaces panel)

**A.** **✅ BE+FE Full-stack Fixed (`6ab3760`/`0b0d7ba`/`8057c1e`, G-STAFF-ANNUAL-LEAVE, Q650·Q651·Q666)** — **`/staff/annual-leaves`** 상단 **`StaffAnnualLeaveRelatedSurfacesPanel`** 이 roster API의 **`surfaceKind`·`relatedSurfaces[]`** 를 표시합니다. 월별 사용 스냅샷과 canonical 대장·출퇴근 화면의 **역할 분리**를 현장에 안내합니다. **`/staff/attendance`** · **`/staff/leave-ledger`** 에서도 **역방향 패널** cross-link가 제공됩니다 (Q651·Q666).

| UI 요소 | 동작 |
|---------|------|
| **안내 문구** | 「이 화면은 월별 사용일 스냅샷입니다. 건별 휴가 기록은 연차·유급휴일 대장에서 관리합니다.」 (`STAFF_ANNUAL_LEAVE_SNAPSHOT_HELP`) |
| **출퇴근 기록** | **`availability=AVAILABLE`** — **`/staff/attendance`** 로 이동하는 **클릭 가능 링크** |
| **연차·유급휴일 대장** | **`AVAILABLE`** — **`/staff/leave-ledger`** 로 이동 (`8057c1e`, Q666) |
| **API fallback** | roster 응답에 메타가 없으면 **`STAFF_ANNUAL_LEAVE_DEFAULT_RELATED_SURFACES`** 로 표시 (`normalizeStaffAnnualLeaveRosterResponse`) |

| API | cross-link 포함 |
|-----|----------------|
| `GET /api/v1/staff/annual-leaves/roster` | **`surfaceKind`·`relatedSurfaces[]`** (`bbf333c`, Q648) |
| `GET /api/v1/staff/annual-leaves/users/{userId}` | **동일 메타** (`6ab3760`, Q650) |
| `PUT /api/v1/staff/annual-leaves/users/{userId}` | 저장 응답에도 **동일 메타** (`6ab3760`) |

> 관련: Q648 · Q639 · Q651 · USER_MANUAL §4-7-0a · **`StaffAnnualLeaveRelatedSurfacesPanel.test`** · **`StaffAnnualLeavePage.test`**

### Q651. 직원 출퇴근(8-4) 화면에서 **「연차휴가 현황」·「연차·유급휴일 대장」** 은 무엇인가요? (US-R01 reverse cross-links)

**A.** **✅ BE+FE Full-stack Fixed (`83a26e7`/`2040571`/`95f55aa`/`8057c1e`, G-STAFF-WORK-ATTENDANCE, Q651·Q653·Q666)** — **`/staff/attendance`** 상단에도 **`StaffAnnualLeaveRelatedSurfacesPanel`** 이 표시됩니다. Q650(연차 → 출퇴근)의 **역방향 링크**로, **일별 출퇴근(8-4)** 과 **월별 연차 스냅샷(US-R03e)** · **canonical 대장(US-R01)** 의 역할 분리를 현장에 안내합니다. **326차(`95f55aa`)부터** 패널 항목은 **`GET /staff/work-attendance`** 응답의 **`relatedSurfaces[]`** 를 우선 사용합니다 (Q653).

| 패널 항목 | `availability` | 동작 |
|-----------|----------------|------|
| **연차휴가 현황** | **`AVAILABLE`** | **`/staff/annual-leaves`** 로 이동하는 **클릭 가능 링크** (`US-R03e`) |
| **연차·유급휴일 대장** | **`AVAILABLE`** | **`/staff/leave-ledger`** 로 이동 (`8057c1e`, Q666) |

| 상수·컴포넌트 | 내용 |
|--------------|------|
| **`STAFF_WORK_ATTENDANCE_CROSS_LINK_HELP`** | 「일별 출퇴근은 이 화면에서 기록합니다. 연차 월별 사용·잔여는 연차휴가 현황에서 관리합니다.」 |
| **`normalizeStaffWorkAttendanceResponse`** | roster 조회 시 **`surfaceKind`·`relatedSurfaces[]`** 정규화 — API 메타 없으면 **`STAFF_WORK_ATTENDANCE_DEFAULT_RELATED_SURFACES`** fallback (`95f55aa`, Q653) |
| **`StaffAnnualLeaveRelatedSurfacesPanel` `helpText`** | 연차·출퇴근 화면별 **맞춤 안내 문구** 지원 (`2040571`) |

| API | cross-link 포함 |
|-----|----------------|
| `GET /api/v1/staff/work-attendance` | **`surfaceKind=DAILY_WORK_ATTENDANCE_ROSTER`** · **`relatedSurfaces[]`** 2건 (`83a26e7`, Q653) |

> **양방향 nav**: **`/staff/annual-leaves`** — **「출퇴근 기록」** (`0b0d7ba`, Q650) ↔ **`/staff/attendance`** — **「연차휴가 현황」** (`2040571`/`95f55aa`, Q651). **8-4 roster에 leave-ledger 데이터를 중복 저장하지 않습니다.**

> 관련: Q612 · Q648 · Q650 · Q653 · USER_MANUAL §4-7 · ADMIN_GUIDE §6-2-20 · **`StaffWorkAttendancePage.test`** · **`StaffWorkAttendanceSupportTest`** · **`StaffAnnualLeaveRelatedSurfacesPanel.test`**

### Q652. 직원 HR 화면(출퇴근·연차·대장)은 **각각 무엇을 기록**하나요? (US-R01·US-R03e 역할 분리)

**A.** **✅ BE+FE Full-stack Fixed (`83a26e7`/`6ab3760`/`0b0d7ba`/`95f55aa`, G-STAFF-WORK-ATTENDANCE·G-STAFF-ANNUAL-LEAVE, Q648·Q650·Q651·Q652·Q653)** — 세 화면은 **데이터를 중복하지 않습니다**. Q648·Q650·Q651을 **한 표**로 정리하면 현장 인수·교육 시 혼동을 줄일 수 있습니다.

| 화면 | 경로 | storyRef | 기록·조회 내용 | 패널 cross-link |
|------|------|----------|----------------|-----------------|
| **일별 출퇴근 (8-4)** | `/staff/attendance` | G-STAFF-WORK-ATTENDANCE | 당일 출근·퇴근 시각·`checkInMethod` | **연차휴가 현황** → `/staff/annual-leaves` (**AVAILABLE**, Q651) |
| **연차휴가 현황** | `/staff/annual-leaves` | US-R03e | 연차 부여·**월별 사용 스냅샷**·잔여 | **출퇴근 기록** → `/staff/attendance` (**AVAILABLE**, Q650) |
| **연차·유급휴일 대장** | `/staff/leave-ledger` | US-R01-c | 건별 휴가·유급휴일 **canonical** — **BE+FE ✅** (`8057c1e`, Q666) | 양 화면 **AVAILABLE** 링크 |

| 계층 | 계약 |
|------|------|
| **BE 연차** | roster·yearly GET/PUT — **`surfaceKind=ANNUAL_LEAVE_USAGE_SNAPSHOT`** · **`relatedSurfaces[]`** 2건 (`bbf333c`/`6ab3760`, Q648·Q650) |
| **BE 출퇴근** | **`GET /staff/work-attendance`** — **`surfaceKind=DAILY_WORK_ATTENDANCE_ROSTER`** · **`relatedSurfaces[]`** 2건 (`83a26e7`, Q653) · **`StaffWorkAttendanceSupport.relatedSurfaces()`** |
| **FE 연차** | **`StaffAnnualLeaveRelatedSurfacesPanel`** + API 응답 또는 **`STAFF_ANNUAL_LEAVE_DEFAULT_RELATED_SURFACES`** |
| **FE 출퇴근** | 동일 패널 + **`normalizeStaffWorkAttendanceResponse`** — API **`relatedSurfaces`** 우선 · fallback **`STAFF_WORK_ATTENDANCE_DEFAULT_RELATED_SURFACES`** · **`helpText`** (`95f55aa`, Q651·Q653) |

**현장 운영 체크리스트**

1. **오늘 출근/퇴근** → **`/staff/attendance`** 에서만 기록 (8-4 roster).
2. **연차 부여·월별 사용·잔여** → **`/staff/annual-leaves`** roster·Modal 편집 (US-R03e).
3. **건별 휴가 대장** → **`/staff/leave-ledger`** — roster·출퇴근에 **중복 입력하지 않음** (Q666).
4. **삼방향 링크** — 연차 ↔ 출퇴근 ↔ 대장 패널에서 상호 이동 smoke (DEPLOYMENT_GUIDE §11-3).

> **흔한 오해**: 월별 roster에 연차 사용을 입력했다고 **대장(US-R01)까지 완료된 것이 아닙니다**. API **`surfaceKind`** 가 스냅샷임을 명시합니다 (Q648).

> 관련: Q648 · Q650 · Q651 · Q653 · USER_MANUAL §4-7-0b · ADMIN_GUIDE §1-4 · **`StaffAnnualLeaveSupportTest`** · **`StaffWorkAttendanceSupportTest`** · **`staffAnnualLeave.test`**

### Q653. **`GET /staff/work-attendance`** 응답의 **`surfaceKind`·`relatedSurfaces[]`** 는 무엇인가요? (US-R01 API cross-link metadata)

**A.** **✅ BE+FE Full-stack Fixed (`83a26e7`/`95f55aa`, G-STAFF-WORK-ATTENDANCE, Q653)** — 연차 roster(`Q648`·`Q650`)와 **대칭 contract**입니다. **일별 출퇴근 roster**가 **월별 연차 스냅샷·canonical 대장**으로 어디로 이동해야 하는지 API가 명시합니다. **326차 이전(`2040571`)** 은 FE 정적 **`STAFF_WORK_ATTENDANCE_DEFAULT_RELATED_SURFACES`** 만 사용했으나, **이후** roster GET 응답 메타를 **authoritative source**로 사용합니다.

| 필드 | 값·의미 |
|------|---------|
| **`surfaceKind`** | **`DAILY_WORK_ATTENDANCE_ROSTER`** — 이 응답은 **당일 출퇴근 로스터**이며 leave-ledger canonical 데이터가 **아님** |
| **`relatedSurfaces[0]`** | **`US-R03e`** · **「연차휴가 현황」** · **`/staff/annual-leaves`** · **`AVAILABLE`** |
| **`relatedSurfaces[1]`** | **`US-R01`** · **「연차·유급휴일 대장」** · **`/staff/leave-ledger`** · **`AVAILABLE`** (`bb9df48`/`8057c1e`, Q653·Q666) |

| API | cross-link |
|-----|------------|
| `GET /api/v1/staff/work-attendance?date=&branchId=` | **`surfaceKind`·`relatedSurfaces[]`** (`83a26e7`) |
| `GET /api/v1/staff/annual-leaves/roster` | **역방향** — **`ANNUAL_LEAVE_USAGE_SNAPSHOT`** · **`/staff/attendance` AVAILABLE** (`bbf333c`, Q648) |

| FE 처리 | 내용 |
|---------|------|
| **`normalizeStaffWorkAttendanceResponse`** | roster JSON → **`surfaceKind`·`relatedSurfaces`** 정규화 (`95f55aa`) |
| **`normalizeStaffWorkAttendanceRelatedSurfaces`** | 빈·누락 시 **`STAFF_WORK_ATTENDANCE_DEFAULT_RELATED_SURFACES`** fallback |
| **`StaffWorkAttendancePage`** | 조회 성공 시 **`setRelatedSurfaces(normalized.relatedSurfaces)`** — 패널에 API 라벨·route 반영 |

| 테스트 | 내용 |
|--------|------|
| **`StaffWorkAttendanceSupportTest`** | **`relatedSurfaces()`** 2건·route·availability (`83a26e7`) |
| **`StaffWorkAttendanceLiveApiRoutingE2eTest`** | JSON **`surfaceKind`·`relatedSurfaces[0].route`** assert |
| **`StaffWorkAttendancePage.test`** | API 메타 제공 시 **「API 연차휴가」** 링크 렌더 (`95f55aa`) |
| **`staffAnnualLeave.test`** | **`normalizeStaffWorkAttendanceResponse`**·fallback |

> **운영 smoke**: `GET /staff/work-attendance` — **`relatedSurfaces[1].availability=AVAILABLE`** (`bb9df48`, Q653·Q663) · `GET /staff/leave-ledger` — **`surfaceKind=CANONICAL_LEAVE_LEDGER`** (DEPLOYMENT_GUIDE §11-3, Q663).

> 관련: Q651 · Q652 · Q650 · USER_MANUAL §5-3 · ADMIN_GUIDE §6-2-20 · **`MustApiEndpointRoutingTest`**

### Q654. ogada에서 **문자(SMS) 발신번호 본인인증**을 관리할 수 있나요? (G-COMM-CALLER-AUTH)

**A.** **△ P3 Planned · MVP out-of-scope** — ogada는 **알림 발송(Solapi 연동)** 과 **발신번호 등록·본인인증 workflow** 를 분리합니다. **2026-06-23 develop** 기준 **발신번호 self-verification 화면·API ❌** 입니다.

| 구분 | ogada | 운영자 조치 |
|------|-------|-------------|
| **SMS·알림톡 발송** | **✅** J03 — `NotificationService`·`SolapiKakaoAlimtalkProvider`·`SolapiSmsProvider` | `NOTIFICATION_PROVIDER=solapi` + §4-3 env |
| **발신번호 등록·본인인증** | **❌** ogada UI/API 없음 | **Solapi 콘솔**에서 `SOLAPI_SENDER_ID` 번호 **사전 등록·인증** |
| **경쟁사 정책 참고** | N/A (인텔로만 기록) | silverangel [notice 221562](https://www.silverangel.kr/silverangel/support/notice/view.do?pageNo=1&Tmp_idx=221562) — 미인증 휴대전화 발신번호 **2026-06-23 17:00 마감**·18:00 일괄 삭제 안내 |

**현장·IT 점검 순서**

1. 배포 env **`SOLAPI_SENDER_ID`** 가 실제 센터 발신번호인지 확인합니다 (placeholder 거부 — Q644).
2. **Solapi 콘솔** → 발신번호 관리에서 해당 번호가 **인증 완료** 상태인지 확인합니다.
3. **`GET /api/v1/notifications/channel-status`** — **`liveAlimtalkDispatchReady=true`** · **`readinessBlockers`** 에 **`MISSING_SOLAPI_CONFIG`** 없음을 확인합니다 (Q318·Q644).
4. stub 환경(`NOTIFICATION_PROVIDER=stub`)에서는 발신번호 검증 없이 **DB 이력만** 적재됩니다 — live 전환 전 Solapi 쪽 인증을 마칩니다.

> **planner 판정 (BNK-452)**: 경쟁사 마감일은 compliance 참고용이며, ogada 자체 SMS 벤더 정책 확정 전까지 **P3 backlog 유지**. ogada 후속 구현 시 **「부가서비스 > 문자발신번호관리」** 유사 self-service는 **US-J03** cluster와 범위 정렬 예정.

> 관련: Q644 · Q318 · ADMIN_GUIDE §10-8 · DEPLOYMENT_GUIDE §4-3 · USER_MANUAL §4-7-2(알림)

### Q655. **`/staff/leave-ledger`(연차·유급휴일 대장)** 는 언제·어떻게 쓰나요? (US-R01 v3 Must · BE+FE ✅)

**A.** **✅ v3 Must · BE+FE Full-stack Fixed (`bb9df48`/`8057c1e`, US-R01-c, Q663·Q666)** — **건별 canonical ledger**는 **`/staff/leave-ledger`** 화면과 REST API 모두에서 사용합니다. 연차·출퇴근 roster API는 **`relatedSurfaces[1].route=/staff/leave-ledger`·`availability=AVAILABLE`** 입니다 (Q648·Q653·`bb9df48`).

| 화면 | 경로 | 데이터 성격 | 현재 상태 |
|------|------|-------------|-----------|
| **일별 출퇴근** | `/staff/attendance` | 당일 `checkInAt`·`checkOutAt`·`checkInMethod` | **✅** G-STAFF-WORK-ATTENDANCE |
| **연차 월별 스냅샷** | `/staff/annual-leaves` | 연도별 부여·1~12월 사용·잔여·비고 (`ANNUAL_LEAVE_USAGE_SNAPSHOT`) | **✅** US-R03e |
| **연차·유급휴일 대장** | `/staff/leave-ledger` | **건별** 휴가·유급휴일·반차 등 **canonical ledger** (US-R01-c) | **✅ BE+FE** (`8057c1e`, Q666) |

**대장 운영 (중복 입력 금지)**

| 해야 할 일 | 하지 말아야 할 일 |
|------------|-------------------|
| 출근·퇴근은 **`/staff/attendance`** 에만 기록 | 연차 roster에 **출퇴근 시각**을 비고로 대체 입력 |
| 연차 **월별 사용일·부여**는 **`/staff/annual-leaves`** | 출퇴근 roster에 **연차 잔여일**을 별도 컬럼으로 관리 |
| **건별 휴가·유급휴일**은 **`/staff/leave-ledger`** 화면 또는 API (Q666) | 월별 스냅샷만으로 **법정 대장 전체**를 대체 |
| 양 화면 **cross-link 패널**으로 이동 (Q650·Q651) | 대장 API와 roster에 **동일 건을 이중 저장** |

**후속 Must 인수 조건 (USER_STORIES US-R01 · planner 정본)**

- [x] leave-ledger API — 건별 휴가·유급휴일 CRUD · annual-leaves·attendance와 **cross-link only** (`bb9df48`, Q663)
- [x] annual-leaves·work-attendance **`relatedSurfaces[1].availability=AVAILABLE`** (`bb9df48`)
- [x] **`/staff/leave-ledger`** Route + RBAC + **`StaffLeaveLedgerPage`** FE wire (`8057c1e`, Q666)
- [x] FE — 패널 링크 클릭 시 대장 화면 이동 (`8057c1e`, Q666)

| API cross-link | 출처 |
|----------------|------|
| annual-leaves roster·yearly | **`relatedSurfaces[1]`** → `/staff/leave-ledger` **AVAILABLE** (`bb9df48`) |
| work-attendance roster | **`relatedSurfaces[1]`** → `/staff/leave-ledger` **AVAILABLE** (`bb9df48`) |

> 관련: Q666 · Q663 · Q652 · Q648 · Q650 · Q653 · USER_MANUAL §4-7-0b·§4-7-0c·§4-7-0d · ADMIN_GUIDE §1-4·§6-2-24 · REQUIREMENTS US-R01

### Q656. 다지점 **`branch_admin`** 이 연차 roster 조회 시 **「지점을 선택해 주세요.」** 가 나옵니다. (G-STAFF-ANNUAL-LEAVE activeBranch fallback)

**A.** **✅ BE Fixed (`40ab9e7`, US-R01, Q656)** — **`branch_admin`** 이 **2개 이상 지점** scope를 가지고 **`GET /staff/annual-leaves/roster`** 에 **`branchId`를 생략**하면, 백엔드가 **`TenantContext.activeBranchId()`** 를 fallback으로 사용합니다. 상단 **BranchSwitcher**로 선택한 활성 지점과 roster가 일치해야 합니다.

| 상황 | 이전 동작 | 현재 동작 (`40ab9e7`) |
|------|----------|----------------------|
| `branch_admin`·다지점·`branchId` 생략 | **`422`「지점을 선택해 주세요.」** (오탐) | **`activeBranchId` roster** 반환 |
| `hq_admin`·`branchId` 생략 | **`422`「지점을 선택해 주세요.」** (의도적) | 동일 — **`hq_admin`은 `branchId` 필수** |
| `branchId` 명시 | 해당 지점 roster | 동일 |

**현장 점검**

1. 다지점 **`branch_admin`** — 상단 **지점 선택기**에서 조회할 지점을 먼저 선택합니다.
2. **`/staff/annual-leaves`** — **「조회 지점」** `BranchScopeNotice`와 roster 직원 목록이 같은 지점인지 확인합니다 (Q657).
3. API smoke — `GET /staff/annual-leaves/roster?year=2026` ( **`branchId` 생략** ) — **`items[]`** 가 활성 지점 직원만 포함하는지 확인합니다.

| 테스트 | 내용 |
|--------|------|
| **`StaffAnnualLeaveServiceTest.getRosterShouldUseTenantActiveBranchForMultiBranchScope`** | multi-branch scope + `TenantContext.setActiveBranchId` fallback (`40ab9e7`) |
| **`StaffAnnualLeaveServiceTest.getRosterShouldUseActiveBranchWhenBranchIdMissing`** | 단일 지점 scope carry (`6b84bcd`, Q639) |

> 관련: Q639 · Q657 · USER_MANUAL §4-7-0a · ADMIN_GUIDE §1-4 · DEPLOYMENT_GUIDE §11-3

### Q657. 연차·출퇴근 roster 화면의 **「조회 지점」** 과 **관련 화면 패널** 은 어떻게 표시되나요? (US-R01 HR roster UX · UXD-156)

**A.** **✅ BE+FE Full-stack Fixed (`40ab9e7`/`c183ebd`/`949e9bf`, US-R01, Q657)** — **`/staff/annual-leaves`** · **`/staff/attendance`** 에 **지점 범위 안내**와 **API 기반 cross-link 패널**이 함께 표시됩니다.
...
| **PLANNED 항목** | 대장 (준비 중) | neutral **`Badge`「준비 중」** · **`aria-label="{라벨}, 준비 중"`** |
| **nav 라벨** | cross-link 목록 | **`STAFF_HR_RELATED_NAV_LABEL`** = **「직원 HR 관련 화면」** |

**다지점 `hq_admin` 운영 순서**

1. 상단 **BranchSwitcher**로 지점을 선택합니다.
2. **`BranchScopeNotice`** 의 **조회 지점**이 의도한 지점인지 확인합니다.
3. **「조회」** — roster·cross-link 패널이 **같은 `branchId`** 기준으로 로드되는지 확인합니다.
...
> 관련: Q656 · Q650 · Q651 · Q652 · Q653 · USER_MANUAL §4-7-0a·§5-3 · ADMIN_GUIDE §1-4 · DEPLOYMENT_GUIDE §11-3

### Q658. develop `npm test` 에서 **`StaffAnnualLeavePage` branch scope** 1건이 실패하는데, 현장 UI는 정상입니다. (QA-B266)

**A.** **✅ QA Fixed (TSR 1311차, Q658)** — FE develop `@949e9bf` pre-merge **2049/2049 PASS** · post-merge **2049/2049 PASS** · isolated **8/8 PASS** — 1310차 **2048/2049 FAIL** 은 **non-reproducible flake** (QA-B262 vitest pollution lineage). **현장 UI·API와 무관**했으며 **develop→test merge 완료**되었습니다.

| 항목 | 내용 |
|------|------|
| 1310차 증상 | full suite만 `StaffAnnualLeavePage` branch scope 1 FAIL · isolated **8/8 PASS** |
| 1311차 해소 | pre-merge·post-merge **2049/2049 PASS** · merge **EXECUTED** (`95f55aa`→`949e9bf`) |
| 재발 방지 | **`npm test` 1회만** — `scripts/npm-test-locked.sh` · hang 시 `vitest-stop.sh` (Q661) |

**운영·인수 시**: **`/staff/annual-leaves`** **「조회 지점」** `BranchScopeNotice`는 Q657대로 동작합니다.

> 관련: Q657 · QA_FEEDBACK QA-20260623-B266 · **Q661** · `docs/qa/VITEST_CONCURRENCY.md` · DEPLOYMENT_GUIDE §11-3

### Q659. **연차·유급휴일 대장**(`/staff/leave-ledger`)은 언제 열리고, 연차 roster와 무엇이 다른가요? (US-R01-c · BE+FE ✅)

**A.** **✅ US-R01-c · BE+FE Full-stack Fixed (`bb9df48`/`8057c1e`, Q663·Q666)** — **건별 canonical ledger** 화면·REST API가 모두 사용 가능합니다. 연차 roster(`/staff/annual-leaves`)는 **월별 사용 스냅샷**만 담고, **건별 휴가·유급휴일**은 **`/staff/leave-ledger`** 에 저장합니다.

| 화면 | `surfaceKind` / story | 저장 데이터 | 상태 |
|------|----------------------|-------------|------|
| `/staff/annual-leaves` | `ANNUAL_LEAVE_USAGE_SNAPSHOT` (US-R03e) | 연·월별 **사용일 합계**·부여·잔여·비고 | **✅ LIVE** |
| `/staff/attendance` | `DAILY_WORK_ATTENDANCE_ROSTER` (8-4) | 당일 **출근·퇴근** | **✅ LIVE** |
| `/staff/leave-ledger` | `CANONICAL_LEAVE_LEDGER` (US-R01-c, 8-13) | **건별** 휴가·유급휴일 원장 | **✅ LIVE** (`8057c1e`, Q666) |

**API contract**

- 대장 list — **`surfaceKind=CANONICAL_LEAVE_LEDGER`** · **`relatedSurfaces`** → annual-leaves·attendance **AVAILABLE** (Q663).
- 연차·출퇴근 roster — **`relatedSurfaces[1].availability=AVAILABLE`** 승격 (`bb9df48`).
- **roster·출퇴근에 건별 대장 데이터를 중복 입력하지 마세요** (Q655·USER_MANUAL §4-7-0c).

**벤치마크**: 이지케어 worker-b100 tab01(연차 grid) ↔ ogada **분리 surface** trade-off — mobile·RBAC·검색 우위 vs 단일 deep-dive 화면.
...
**A.** **✅ 운영 규칙 고정 (Q610·Q661, `docs/qa/VITEST_CONCURRENCY.md`)** — **`npm test` / `npm run test:live-e2e`는 한 번에 하나만** 실행합니다. **`npx vitest run` 직접 호출 금지** — `package.json`의 `test`는 **`scripts/npm-test-locked.sh`(flock)** 를 경유합니다.

| 상황 | 조치 |
|------|------|
| 새 테스트 시작 전 | `ps aux \| grep -E '[v]itest run'` — **기존 실행 0건** 확인 |
| **2개 이상** `npm test` / vitest 프로세스 | **`./scripts/vitest-stop.sh`** 로 정리 후 **1회만** 재실행 |
| hang (5분 무출력·전체 15분+) | 새 실행 **시작 금지** — 정리 후 원인 기록 |
| **QA-B266 (1310차)** — full suite **2048/2049 FAIL** · isolated **8/8 PASS** | **non-repro flake** — **1311차 Fixed** @ `949e9bf` · **2049/2049 PASS** (Q658) |

**QA-B266 재발 방지 (운영·이관)**

1. **병렬 vitest 없음** 확인 — `./scripts/vitest-stop.sh` 필요 시 실행.
2. **격리 재현** — `cd src/frontend && npx vitest run src/pages/StaffAnnualLeavePage.test.jsx` → **8/8 PASS** 기대.
3. **full suite** — `npm test` **1회만** — FAIL 시 pollution 의심 → 정리 후 재시도.
4. **인수·배포 판단** — **`/staff/annual-leaves`** **「조회 지점」** `BranchScopeNotice`는 Q657대로 **UI 정상**.

| 도구 | 용도 |
|------|------|
| **`scripts/npm-test-locked.sh`** | flock으로 동시 실행 차단 |
| **`scripts/vitest-stop.sh`** | 잔여 vitest·npm 프로세스 정리 |
| **`docs/qa/VITEST_CONCURRENCY.md`** | 에이전트·개발자 공통 규칙 |

> 관련: Q658 · Q610 · `.agents/rules.md` §5 · DEPLOYMENT_GUIDE §11-3 · ADMIN_GUIDE §1-4

### Q662. CMS 지점 roster **`status` 필터**에 공백·소문자를 넣어도 되나요? (G2, Q637 deepen)

**A.** **✅ BE Fixed (`f1225b0`, G2, Q662)** — **`GET /api/v1/billing/cms/enrollments`** 의 **`status` 쿼리**는 서버에서 **trim·uppercase** 후 조회합니다. Swagger·FilterChips는 대문자 enum을 쓰지만, **수동 API 호출·연동** 시 **` pending `·`active`** 도 허용됩니다.

| 입력 `status` | 정규화 결과 | 동작 |
|---------------|-------------|------|
| `ACTIVE` · `active` · ` active ` | `ACTIVE` | 해당 상태 roster |
| `PENDING` · ` pending ` | `PENDING` | 대기 등록 roster |
| `CANCELLED` | `CANCELLED` | 해지 이력 roster |
| *(생략·빈 문자열)* | `null` | **전체** 상태 |
| `EXPIRED` 등 미지원 | — | **`422`「CMS 등록 상태 필터는 ACTIVE, CANCELLED, PENDING만 허용됩니다.」** |

| 검증 | 내용 |
|------|------|
| 지점 스코프 | **`jwtScopeResolver.validateBranchReadScope(branchId)`** — roster 조회 시 지점 읽기 권한 확인 |
| 테스트 | **`CmsServiceTest.listBranchEnrollmentsShouldNormalizeStatusFilterAndReturnPendingRoster`** · **`listBranchEnrollmentsShouldRejectUnsupportedStatusFilter`** |

> **현장 UI**: **`/billing/cms`** FilterChips는 **`ACTIVE`/`CANCELLED`/`PENDING`** 을 그대로 전송합니다 — 정규화는 **API 방어** 목적입니다.  
> 관련: Q637 · Q638 · USER_MANUAL §4-6 · ADMIN_GUIDE §10-13 · DEPLOYMENT §1-4

---

### Q663. **연차·유급휴일 대장** API·화면은 어떻게 쓰나요? (US-R01-c · G-STAFF-LEAVE-LEDGER, BE `bb9df48`/FE `426d63a`)

**A.** **✅ BE+FE Full-stack Fixed (`bb9df48`·`8057c1e`·`bd1d0ad`, US-R01-c, Q663·Q666·Q667)** — 케어포 **8-13** 건별 canonical ledger REST API와 **`/staff/leave-ledger`** 화면이 연동되었습니다.

| API | 권한 | 용도 |
|-----|------|------|
| `GET /api/v1/staff/leave-ledger?year=&branchId=` | `hq_admin`·`branch_admin`·`social_worker` | 지점·연도별 **건별 대장 목록** |
| `GET /api/v1/staff/leave-ledger/users/{userId}?year=` | 동일 | 직원 1명 연간 대장 |
| `POST /api/v1/staff/leave-ledger/users/{userId}` | `branch_admin`·`social_worker` | **건별 행 생성** |
| `PUT /api/v1/staff/leave-ledger/entries/{entryId}` | 동일 | 기존 행 수정 |
| `DELETE /api/v1/staff/leave-ledger/entries/{entryId}` | 동일 | 행 삭제 (**204**) |

**요청 본문 (POST/PUT)**

| 필드 | 규칙 |
|------|------|
| `leaveType` | **`ANNUAL_LEAVE`**(연차) · **`PAID_HOLIDAY`**(유급휴일) — trim·uppercase · 미지원 → **`422`** |
| `leaveDate` | 시작일 (필수) |
| `endDate` | 종료일 (선택) — `endDate < leaveDate` → **`422`「종료일은 시작일보다 빠를 수 없습니다.」** |
| `daysUsed` | 생략 시 `endDate` 없으면 **1** · 있으면 **포함 일수 자동 계산** · **0 초과·≤99.9** · 소수 **첫째 자리**까지 |
| `memo` | 최대 **2000자** |
...
| 필드 | 값 |
|------|-----|
| `surfaceKind` | **`CANONICAL_LEAVE_LEDGER`** |
| `relatedSurfaces[0]` | **US-R03e** · **「연차휴가 현황」** · `/staff/annual-leaves` · **`AVAILABLE`** |
| `relatedSurfaces[1]` | **US-R01** · **「출퇴근 기록」** · `/staff/attendance` · **`AVAILABLE`** |

**연차·출퇴근 roster cross-link 승격 (`bb9df48`)**

| roster API | 변경 |
|------------|------|
| `GET /staff/annual-leaves/roster` · yearly GET/PUT | **`relatedSurfaces[1].availability`** **`PLANNED` → `AVAILABLE`** |
| `GET /staff/work-attendance` | 동일 |

> **FE 화면 (`8057c1e`/`bd1d0ad`, Q666·Q667)**: **`/staff/leave-ledger`** — **`StaffContextNav`「연차·유급휴일 대장」** · 연도 조회·**`StaffLeaveLedgerTable`** · **`StaffLeaveLedgerDeleteModal`** · **`DateInput`** 등록 Modal · **`branch_admin`/`social_worker`「항목 등록」** · **`hq_admin` 조회만** · 연차·출퇴근 패널 **「연차·유급휴일 대장」** 링크 **AVAILABLE**.

> **FE 주의**: roster 로드 시 **API `relatedSurfaces` 우선** — BE·FE default 모두 **`AVAILABLE`** (`426d63a`, BNK-551). Swagger-only fallback은 API 장애 시에만 해당.

**DB**: Flyway **V174** `staff_leave_ledger_entries` — `leave_type` CHECK · `days_used` CHECK · org/branch/user FK · **V175** memo nonempty CHECK · **user_branches FK** (`c4e6bcb`, Q668).

> 관련: Q667 · Q666 · Q665 · Q655 · Q659 · Q652 · USER_MANUAL §4-7-0c·§4-7-0d · ADMIN_GUIDE §1-4·§6-2-24 · DEPLOYMENT §1-4·§11-3

---

### Q664. live E2E **`liveCashReceiptDescribe`** suite가 guard 없이 실행될 수 있나요? (QA-B268)

**A.** **✅ FE Fixed (`b7101d5`, QA-B268, Q664)** — `src/test/liveE2eSuiteGuard.test.js`가 **`liveCashReceiptDescribe`** import·호출을 검사합니다. single/double-quote import 모두 허용합니다.

| 검사 | 내용 |
|------|------|
| 대상 | `src/e2e/*.e2e.test.js(x)` 전체 |
| 허용 guard | `liveDescribe` · `liveClientDescribe` · `guardianLiveDescribe` · `liveG21Describe` · `liveWriteDescribe` · **`liveCashReceiptDescribe`** |
| 금지 | raw `describe(` — health probe·credential blocker 우회 방지 |

> 관련: Q580 · Q597 · Q661 · DEPLOYMENT_GUIDE §11-3 · `docs/qa/VITEST_CONCURRENCY.md`

---

### Q665. **연차·유급휴일 대장** API는 역할별로 어떤 작업이 허용되나요? (US-R01-c RBAC, BE `62fce23`)

**A.** **✅ BE Fixed (`62fce23`, US-R01-c, Q665)** — `@PreAuthorize` 와 **`RoleBasedControllerAccessTest.StaffLeaveLedgerAccess`** 로 역할별 접근이 고정되었습니다. **조회(GET)** 와 **등록·수정·삭제(CUD)** 권한이 다릅니다.

| 역할 | list GET · user list GET | POST · PUT · DELETE |
|------|--------------------------|---------------------|
| **`hq_admin`** | ✅ | ❌ **403** — 통합 조회만, 현장 등록은 `branch_admin`·`social_worker` 담당 |
| **`branch_admin`** | ✅ | ✅ |
| **`social_worker`** | ✅ | ✅ |
| **`caregiver`** | ❌ **403** | ❌ **403** |
| **`guardian`** | ❌ **403** | ❌ **403** |

**pilot flow 검증 (`StaffLeaveLedgerPilotServiceFlowE2eTest`)**

| 단계 | 확인 |
|------|------|
| 1 | `GET /staff/leave-ledger?year=2026` — 빈 roster · **`surfaceKind=CANONICAL_LEAVE_LEDGER`** · **`relatedSurfaces`** → annual-leaves·attendance |
| 2 | `POST /users/{userId}` — `ANNUAL_LEAVE` 3일(기간) 생성 |
| 3 | `PUT /entries/{id}` — `PAID_HOLIDAY` 0.5일 반차로 수정 |
| 4 | `DELETE /entries/{id}` — **204** · roster **entryCount=0** |

> **운영**: 통합 관리자는 **대장 조회·감사**만 가능합니다. 현장 등록은 **센터장·사회복지사** 계정으로 **`/staff/leave-ledger`** 화면을 사용하세요.  
> 관련: Q666 · Q663 · Q652 · ADMIN_GUIDE §6-2-24 · DEPLOYMENT §11-3 · USER_MANUAL §4-7-0c·§4-7-0d

---

### Q666. **`/staff/leave-ledger`** 화면은 어떻게 쓰나요? (US-R01-c FE full-stack, FE `8057c1e`/`bd1d0ad`)

**A.** **✅ FE+BE Full-stack Fixed (`8057c1e`·`bd1d0ad`·`5fd12dd`, US-R01-c, Q666·Q667)** — 연차·출퇴근과 **데이터를 중복하지 않는** 건별 휴가·유급휴일 대장 화면이 추가되었습니다.

| 단계 | 화면·동작 |
|------|----------|
| 1 | **`/staff/leave-ledger`** — SideNav **「직원」** 또는 **`StaffContextNav`「연차·유급휴일 대장」** · 연차·출퇴근 패널 cross-link |
| 2 | **「기준 연도」** 선택 → **「조회」** — **`BranchScopeNotice`** 로 지점 확인 |
| 3 | **`branch_admin`/`social_worker`** — **「항목 등록」** → 직원·휴가 유형(연차/유급휴일)·**`DateInput` 시작일·종료일**·사용일수·비고 |
| 4 | **`StaffLeaveLedgerTable`** **「수정」/「삭제」** — 삭제 시 **`StaffLeaveLedgerDeleteModal`** 확인 (Q667) |
| 5 | **`hq_admin`** — 목록 **조회만** (등록·수정·삭제 버튼 없음, Q665) · **`caregiver`** — API **403** (화면 Route guard) |

**FE API client (`services.js`)**

| 함수 | API |
|------|-----|
| `fetchStaffLeaveLedgerApi` | `GET /api/v1/staff/leave-ledger?year=&branchId=` |
| `createStaffLeaveLedgerEntryApi` | `POST …/users/{userId}` |
| `updateStaffLeaveLedgerEntryApi` | `PUT …/entries/{entryId}` |
| `deleteStaffLeaveLedgerEntryApi` | `DELETE …/entries/{entryId}` |

**pilot checklist (`pilotChecklist.js`)**: **R01c-a** list · **R01c-b** create · **R01c-c** update · **R01c-d** delete

**BE live contract harness (`5fd12dd`)**: **`StaffLeaveLedgerLiveApiRoutingE2eTest`** — list·create JSON shape — FE wire 회귀 방지

> **중복 입력 금지**: 월별 잔여는 **`/staff/annual-leaves`**, 당일 출퇴근은 **`/staff/attendance`** — 대장은 **건별 이벤트만** 기록 (Q652).

> 관련: Q667 · Q663 · Q665 · Q652 · USER_MANUAL §4-7-0c · ADMIN_GUIDE §6-2-24 · DEPLOYMENT §11-3

---

### Q667. **연차·유급휴일 대장** 화면 접근성(UXD-157)과 cross-link **AVAILABLE** 동기화는 무엇이 바뀌었나요? (US-R01-c, FE `bd1d0ad`/`426d63a`)

**A.** **✅ FE deepen Fixed (`bd1d0ad`·`426d63a`, US-R01-c·BNK-551, Q667)** — 대장 화면의 **키보드·스크린리더 사용성**과 BE **`relatedSurfaces` contract** 정합이 강화되었습니다.

**UXD-157 접근성 (`bd1d0ad`)**

| 항목 | 변경 |
|------|------|
| **`StaffLeaveLedgerTable`** | 표 **`caption`**(시각 숨김) · **`scope="col"`** 헤더 · 시작일·종료일 **`<time dateTime>`** 시맨틱 날짜 · 직원명 **`aria-label`** · 행 **「수정」/「삭제」** 버튼 contextual label |
| **`StaffLeaveLedgerDeleteModal`** | **`window.confirm` 대체** — WCAG 4.1.2 준수 · 삭제 대상 직원·날짜 안내 · **「삭제한 항목은 복구할 수 없습니다」** 경고 |
| **등록·수정 Modal** | **`DateInput`** 으로 시작일·종료일 입력 — 브라우저 기본 date picker·label 연결 |
| **레이아웃** | compact grid CSS — 좁은 화면에서 Table 가독성 유지 |

**BNK-551 AVAILABLE 동기화 (`426d63a`)**

| 항목 | 내용 |
|------|------|
| **문제** | BE `GET /staff/annual-leaves/roster` — leave-ledger **`relatedSurfaces[1].availability=AVAILABLE`** (`bb9df48`) · FE 테스트가 **PLANNED** 를 기대해 full-suite 회귀 위험 |
| **해결** | **`staffAnnualLeaveServices.test.js`** — **`AVAILABLE`** 기대로 정정 · **`STAFF_ANNUAL_LEAVE_DEFAULT_RELATED_SURFACES`**·연차·출퇴근 패널 **실제 링크**와 일치 |

> **운영**: 연차·출퇴근 패널의 **「연차·유급휴일 대장」** 은 **클릭 가능한 링크**입니다 — 「(준비 중)」 배지가 아닙니다 (Q655·Q663 갱신).

> 관련: Q666 · Q663 · Q650 · USER_MANUAL §4-7-0c · ADMIN_GUIDE §1-4 · DEPLOYMENT §11-3

---

### Q668. **연차·유급휴일 대장** DB 무결성(V175)은 무엇을 막나요? (US-R01-c, BE `c4e6bcb`)

**A.** **✅ BE Fixed (`c4e6bcb`, US-R01-c·V175, Q668)** — Flyway **V175** 가 V174 `staff_leave_ledger_entries` 에 **defense-in-depth** 제약을 추가했습니다. 앱(`StaffLeaveLedgerService`) 검증을 **raw SQL·일괄 import 우회**로도 강제합니다.

| 제약 | 내용 | 위반 시 |
|------|------|---------|
| **`chk_staff_leave_ledger_entries_memo_nonempty`** | `memo` 가 있으면 **공백만** 저장 불가 — 앱 `trimToNull(memo)` DB 미러 | INSERT/UPDATE **DB 오류** |
| **`fk_staff_leave_ledger_entries_user_branch_assignment`** | `(organization_id, user_id, branch_id)` → **`user_branches`** — 직원이 **본인 배정 지점**의 휴가만 기록 | FK 위반 **DB 오류** |

**현장 조치**

| 증상 | 확인·조치 |
|------|----------|
| 대장 등록 실패·500 | 직원이 **해당 지점에 배정**(`user_branches`)되어 있는지 **`/staff`** 에서 확인 |
| 비고 저장 실패 | **공백만** 입력하지 말고 내용을 입력하거나 **비워 두세요** |
| 마이그레이션 | 배포 시 Flyway **V175 자동 적용** — V174 테이블 **backfill 불요**(신규 테이블) |

> 관련: Q663 · Q666 · Q667 · ADMIN_GUIDE §6-2-24 · DEPLOYMENT §1-3·§11-3 · ERD V175
...
| API | `social_worker` | `branch_admin`/`hq_admin` |
|-----|-----------------|---------------------------|
| **`GET /api/v1/users`** | **✅ 200** (지점 스코프) | **✅ 200** |
| **`GET /api/v1/users/{userId}`** | **✅ 200** | **✅ 200** |
| **`POST /api/v1/users`** | **403** | **✅** |
| **`PATCH /api/v1/users/{userId}`** | **403** | **✅** |
...
### Q670. live E2E **테넌트 UUID**가 바뀌었나요? dev seed와 충돌하지 않나요? (BE `c4e6bcb`)

**A.** **✅ BE Fixed (`c4e6bcb`, Q670)** — live E2E bootstrap 기본 UUID가 **`e2e00001-*`** prefix 로 **dev seed fixture**(`00000001-*`, `scripts/seed-dev-fixtures.sql`)와 **분리**되었습니다.

| 항목 | 이전 기본값 | 현재 기본값 (`application.yml`) |
|------|------------|--------------------------------|
| Organization | `00000001-0001-4000-8000-000000000001` | **`e2e00001-0001-4000-8000-000000000001`** |
| Branch | `00010101-0001-4000-8000-000000000001` | **`e2e10101-0001-4000-8000-000000000001`** |
| Staff user | `00000002-0002-4000-8000-000000000001` | **`e2e00002-0002-4000-8000-000000000001`** |

**운영·스테이징 조치**

1. **`scripts/dev-live-e2e.env`** 에 **커스텀 UUID**를 하드코딩했다면 **`e2e*`** 계열로 **갱신**하거나 env에서 **제거**(기본값 사용).
2. **dev + live E2E 동시 기동** 시 — seed fixture tenant와 E2E tenant가 **다른 UUID** 이므로 **데이터 오염·full-suite pollution** 위험이 줄어듭니다.
