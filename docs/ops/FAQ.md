...
### 구현 상태 안내 (2026-06-21 develop HEAD `a6eb8b7` / frontend `5fd468b` 기준 — 196차 baseline)

| 영역 | 상태 | FAQ에서의 의미 |
|------|------|----------------|
| 백엔드 API | **Must + … + V169 ✅** @ `a6eb8b7` · **G-BILLING-DEPOSIT-ORDER-GUARD ✅** · **G-STAFF-WORK-ATTENDANCE ✅** · **G-ATTENDANCE-ROSTER-STATUS ✅** · … | **선행입금 입금 순서 가드** (Q614) · **`GET /staff/work-attendance`** roster (Q612) |
| 데이터베이스 | Flyway **V1–V169** | **V169** `staff_work_attendance` · **V168** overdue integrity · **V166** `ON_LEAVE` |
| 프론트엔드 | **109 route · 87 page** @ `5fd468b` · **G-STAFF 출근방식 FE wire ✅** · **G-ATTENDANCE-ROSTER-STATUS FE wire ✅** · **G-ATTENDANCE-STATS FE contract △** · … | **`/staff/attendance`** 출근 방식 select (Q612) · **`/attendance/stats`** contract 갭 (Q613) · QR **Must 갭** (Q590) |
| 본 FAQ | **Q614 신규** (301차) · **Q612 갱신** (301차) · **Q613** (299차) · **Q609·Q610** (296차) | **선행입금 입금 순서** · **직원 출퇴근 8-4** · **출석 통계 API contract** |
...
> **placeholder 자격증명**은 여전히 **SKIP** (`fffc2c1`, Q578). **stale access token** 은 **credential login fallback** (`b60c622`, Q583). auth recovery·feature-scoped filter·placeholder skip·stale token recovery는 **별개**입니다.

> 관련: Q578 · Q583 · Q548 · DEPLOYMENT_GUIDE §3-7 · `./scripts/run-live-e2e.sh`
...
**A.** **✅ BE Fixed (`0c9518a`·`20485f1`·`018a781`, QA-B95, Q596)** — **`bootstrap=error`** 또는 **`guardian-bootstrap=error`** 이면 **`LiveE2eOperationReadinessSupport`** 가 **derived blocker**(visit-schedule·NHIS·G32·V159 등)를 **생략**합니다. **`20485f1`부터** detail에 **접미사(`: …`)** 가 붙어도 동일하게 error로 인식합니다. **`018a781`부터** **`operationBlocker` primary** 는 **`-error` 코드**가 **`-not-ready`·credential·G21 seed blocker보다 우선**합니다 (`resolveOperationBlocker` priority).

| `operationBlockers` (bootstrap error) | primary (`operationBlocker`) | 의미 |
|---------------------------------------|-------------------------------|------|
| `staff-bootstrap-not-ready` + `staff-bootstrap-error` | **`staff-bootstrap-error`** | staff bootstrap **내부 오류** 우선 |
| `guardian-bootstrap-not-ready` + `guardian-bootstrap-error` | **`guardian-bootstrap-error`** | guardian bootstrap **내부 오류** 우선 |

**조치**: env placeholder보다 **서버 로그·Flyway·live-e2e seed** 를 먼저 확인하세요. credential·G21 seed blocker는 **bootstrap 복구 후**에만 의미 있습니다.

> 관련: Q448 · Q580 · DEPLOYMENT_GUIDE §11-3 · **`LiveE2eOperationReadinessSupportTest`** · **`LiveE2eControllerTest`**
...
**A.** **△ Must UI + BE API 갭 (US-E05, Q613)** — **`AttendanceStatsPage`**(`53d65a0` carry)는 등록되어 **`MonthInput`**·**`AttendanceRateChart`**·StatCard를 표시하지만, **`fetchMonthlyAttendanceStatsApi(yearMonth)`** 가 백엔드 **`AttendanceController.monthlyStats(from, to, branchId)`** 와 **쿼리·응답 contract 불일치**로 StatCard·차트·이용자별 표가 **비거나 `—`** 로 남습니다.

| 항목 | 프론트 (`services.js`·`AttendanceStatsPage`) | 백엔드 (`560057f`) |
|------|---------------------------------------------|---------------------|
| 쿼리 | `?yearMonth=YYYY-MM` (**`branchId` 미전송**) | `?from=YYYY-MM-DD&to=YYYY-MM-DD&branchId=` — **`yearMonth` 미지원** |
| StatCard | `totalClients` · `avgAttendanceRate` · `totalAttendanceDays` | **`branches[].months[]`** — `activeClientCount` · `attendedDays` · `attendanceRate` |
| 차트 | `dailyRates[]` — `{ day, attendanceRate }` | **일별 breakdown 없음** — **월별 `months[]`만** |
| 표 | `clientStats[]` — 이용자별 출석/결석 일수 | **이용자별 breakdown API 없음** |

**증상**: **「총 이용자」「평균 출석률」「총 출석 일수」** StatCard **`—`** · 막대 차트 **빈 상태** · **「출석 데이터 없음」** EmptyState — API는 **200** 이지만 FE가 필드를 읽지 못함.

**백엔드 집계 의미** (`MonthlyAttendanceStatItem`, 지점·월 단위):

| 필드 | 의미 |
|------|------|
| `activeClientCount` | 해당 월 **활성 이용자** 수 (`active=true`) |
| `attendedDays` | 해당 월·지점 **출석 행 전체** (`check_in_at IS NOT NULL` 건수, 이용자별 합 아님) |
| `attendanceRate` | `attendedDays ÷ (activeClientCount × 해당 월 일수)` — **0~1** (×100 = %) |

`from`/`to` 생략 시 `to`=오늘, `from`=6개월 전 월 1일이 기본입니다.

**Swagger 우회 (IT·센터장)** — JWT 로그인 후:

`GET /api/v1/attendance/stats/monthly?branchId=<uuid>&from=2026-06-01&to=2026-06-30`
...
**월 출석률 %** = `attendanceRate × 100` (예: `0.50` → **50.0%**).

> **상세 contract·coder 정합 매트릭스**: FAQ **Q613** · USER_MANUAL §4-4 · ADMIN_GUIDE §6-2-21
...
| `liveE2eOperationBlocker` | 차단 사유 — `none` · `staff-bootstrap-not-ready` · **`staff-bootstrap-error`** · `staff-credentials-missing` · `guardian-credentials-missing` · `seed-client-missing` · `guardian-bootstrap-not-ready` · **`guardian-bootstrap-error`** · **`bootstrap-disabled`** (Q448·Q472, `d7f1a9a`/`8cf09d8`) |
...
| **`operationBlockers`** | **미충족 prerequisite 전체 목록** (Q447, `c5dd4f2`) — bootstrap 내부 오류 시 **`-not-ready`와 `-error` 동시** (Q448, `d7f1a9a`) |

| **`operationBlocker` / `operationBlockers` 값** | 의미 |
|--------------------------------------------------|------|
| **`none`** / **`[]`** | operation ready |
| **`staff-bootstrap-not-ready`** | org·branch·user·mapping 미완 |
| **`staff-bootstrap-error`** | staff bootstrap 내부 오류 (`bootstrap=error`) |
...
| **`guardian-bootstrap-not-ready`** | guardian mapping·user 미완 |
| **`guardian-bootstrap-error`** | guardian bootstrap 내부 오류 |
...
**A.** **Fixed (BE `d7f1a9a`·`0c9518a`·`20485f1`, QA-B135, Q448·Q596)** — bootstrap **시드가 아직 미완**이면 **`staff-bootstrap-not-ready`**(또는 guardian 쪽 **`guardian-bootstrap-not-ready`**)만 나옵니다. bootstrap **상태 조회 자체가 내부 오류**(`bootstrap=error`·`guardian-bootstrap=error`·**`: …` 접미사 포함**)이면 **`operationBlockers`** 에 **`-error`** 코드가 **추가**되어 credential 누락과 구분됩니다. **`0c9518a`부터** bootstrap error 시 **derived seed/schema blocker는 생략** · **`20485f1`부터** detail suffix도 error로 인식 (Q596).

| `liveE2eStatusDetail` / guardian status | 추가 blocker |
|----------------------------------------|--------------|
| **`bootstrap=error`** | **`staff-bootstrap-error`** (`staff-bootstrap-not-ready`와 **동시**) |
| **`guardian-bootstrap=error`** | **`guardian-bootstrap-error`** (`guardian-bootstrap-not-ready`와 **동시**) |

| 예시 `operationBlockers` | 의미 |
|---------------------------|------|
| `["staff-bootstrap-not-ready","staff-bootstrap-error","guardian-bootstrap-not-ready","guardian-bootstrap-error"]` | staff·guardian bootstrap **probe 모두 내부 오류** — env보다 **서버 로그·DB seed** 우선 점검 |
| `["staff-credentials-missing","seed-client-missing"]` | credential·시드 이용자 **미설정** — **`scripts/dev-live-e2e.env`** 확인 |

| 노출 | **`GET /api/v1/system/live-e2e/probe`** · **`GET /api/v1/health`** — **`liveE2eOperationBlockers`**
| 테스트 | **`HealthControllerTest.healthShouldSurfaceLiveE2eBootstrapErrorsWhenStatusProbesFail`** · **`LiveE2eControllerTest.probeShouldSurfaceBootstrapErrorsInOperationBlockers`**

> 관련: Q441 · Q447 · Q409 · DEPLOYMENT §3-6 · ADMIN_GUIDE §1-4
...
| PII | **`hq_admin`·`platform_admin`·`sysadmin`** 만 전체 주소 · 그 외 역할은 **마스킹** (`서울시 중구 ***` 패턴, SEC-D9) |
...
**A.** **△ Must UI + BE API — FE wire 미완 (US-E05, Q106·Q613)** — 백엔드 **`GET /api/v1/attendance/stats/monthly`** 는 **지점·월 집계**를 반환하지만, 프론트 **`AttendanceStatsPage`** 는 **다른 필드명·일별·이용자별 breakdown** 을 기대합니다.

| 계층 | 구현 | 상태 |
|------|------|------|
| BE API | **`AttendanceService.monthlyStats()`** — `MonthlyAttendanceStatsListResponse` | **✅ Must** @ `560057f` |
| FE route | **`/attendance/stats`** — **`AttendanceStatsPage`** + **`AttendanceRateChart`** | **△ Partial** @ `53d65a0` |
| FE API client | **`fetchMonthlyAttendanceStatsApi(yearMonth)`** — `?yearMonth=` | **❌ contract mismatch** |
| RBAC | **`hq_admin`·`branch_admin`·`social_worker`·`caregiver`** | **✅** |

**coder 정합 체크리스트** (FE wire 시):

| # | FE (현재) | BE (정식) | 조치 |
|---|-----------|-----------|------|
| 1 | `yearMonth=2026-06` | `from=2026-06-01&to=2026-06-30` | **`YearMonth.parse`** → 월 1일~말일 변환 |
| 2 | `branchId` 미전송 | `branchId` optional (JWT active branch) | **`user.activeBranchId`** 전송 |
| 3 | `stats.totalClients` | `branches[0].months[n].activeClientCount` | 월 일치 row 매핑 |
| 4 | `stats.avgAttendanceRate` | `months[n].attendanceRate × 100` | **0~1 → %** |
| 5 | `stats.totalAttendanceDays` | `months[n].attendedDays` | 직접 매핑 |
| 6 | `stats.dailyRates[]` | *(없음)* | **API 확장 P3** 또는 차트 **월별 bar** 로 변경 |
| 7 | `stats.clientStats[]` | *(없음)* | **API 확장 P3** 또는 표 **숨김/EmptyState** |

**현장 우회** — 화면 숫자가 비면 **지점 대시보드**(`/dashboard`) StatCard·**당일 출석**(`/attendance`, Q609)로 운영 확인 후, 월간 집계는 **Swagger**로 `GET …/stats/monthly?branchId=&from=&to=` 호출 (Q106).

> 관련: Q106 · USER_MANUAL §4-4 · ADMIN_GUIDE §6-2-21 · **`AttendanceStatsPage`** · **`fetchMonthlyAttendanceStatsApi`**
