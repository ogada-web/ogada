...
| 데이터베이스 | PostgreSQL | `pgcrypto` 확장, Flyway V1–V169 |
| 인증 | JWT (RS256) + RBAC | access 30분, refresh 7일 |
| 멀티테넌트 | Organization → Branch | `organization_id` 강제 격리 |

> **구현 상태 (2026-06-21 develop HEAD `a6eb8b7` / frontend `5fd468b` 기준 — 196차 baseline)**:
> - **백엔드**: Must API + **V1–V169**. **G-BILLING-DEPOSIT-ORDER-GUARD ✅** (`a6eb8b7`, Q614) · **G-STAFF-WORK-ATTENDANCE ✅** (`a6eb8b7`/`10c0daf`, Q612) · **G-ATTENDANCE-ROSTER-STATUS ✅** (Q609) · …
> - **프론트엔드**: **109 route · 87 page**. **G-STAFF 출근방식 FE wire ✅** (`5fd468b`, Q612) · **G-ATTENDANCE-ROSTER-STATUS FE wire ✅** (Q609) · **G-ATTENDANCE-STATS FE contract 갭 △** (Q613) · **Vitest serial pool ✅** (Q610) · …
> - **merge gate**: develop→test **~657** · **BE @ `a6eb8b7`** · **FE @ `5fd468b`** · **staging `./scripts/run-live-e2e.sh` RUN 확인 권장**
> - **마이그레이션**: V43 … **V168** overdue integrity · **V169** `staff_work_attendance` (G-STAFF-WORK-ATTENDANCE). Flyway 자동 실행. **V165 미적용 시 live E2E `user-account-request-integrity-check-missing`** (Q575).
...
| G-ATTENDANCE-STATS (Q613) | `GET /api/v1/attendance/stats/monthly?branchId=&from=YYYY-MM-01&to=YYYY-MM-last` | 200, `branches[].months[]` with `yearMonth`·`activeClientCount`·`attendanceRate` — **FE `/attendance/stats` wire 갭** |
...
| `liveE2eOperationBlocker` | … · **`g32-compliance-field-missing`·`g32-dashboard-field-missing`·`g32-v157-constraint-missing`** · **`cash-receipt-identifier-check-missing`** (**health·probe**, Q543·Q548) · **`g21-branch-missing-or-inactive`** (**health·probe**, Q505·Q511) · **`bootstrap-disabled`** | QA-B95 preflight 진단 · **health·probe credential/G21/G32/G-CASH-RECEIPT blocker 집합** (Q525·Q528·Q548) |
...
- [ ] **`liveE2eHarness.test`** — **ignores recovered staff/guardian auth blockers** when auth ready (`9105332`·`06c6bb5`·**`33e9e1a` snake_case**, Q580) · **filters unrelated auth blockers by suite contract** (`5f1815f`, Q580 deepen) · **filters feature-scoped blockers** (`cb3fe3d` — G21·G32·cash-receipt·V165 when not required) · **`liveCashReceiptDescribe` suite guard** (`cd6891f`, Q597) · **stale token credential fallback** (`b60c622`, Q583) · **placeholder token bootstrap probing** (`82a542c`, Q578 deepen)
- [ ] **`LiveE2eOperationReadinessSupportTest`** — **bootstrap error suppresses derived blockers** (`0c9518a`, Q596) · **bootstrap error detail suffix match** (`20485f1`, Q596 deepen) · **hard blocker priority** — **`staff-bootstrap-error` primary** (`018a781`/`56cb5d9`, Q596 deepen)
...
- [ ] **`StaffWorkAttendancePage.test`** · **`staffWorkAttendance.test`** — roster load·**checkInMethod MANUAL/MOBILE**·check-out·403 error (`5fd468b`, Q612)
- [ ] **`/staff/attendance`** — **근무일 조회** · **출근 방식 select** · StatCard **출근/퇴근/미출근** · **「출근」/「퇴근」** 버튼 (`5fd468b`, Q612)
...
- [ ] **`npm test`** — **`scripts/npm-test-locked.sh`** 경유 · **동시 2개 이상 실행 금지** (`docs/qa/VITEST_CONCURRENCY.md`, Q610)
...
- [ ] **`GET /api/v1/system/live-e2e/probe`** — **`operationBlockers`** includes **`staff-bootstrap-error`**·**`guardian-bootstrap-error`** when status detail is bootstrap error (`d7f1a9a`, Q448)
...
- [ ] **`GET /api/v1/health`** — **`liveE2eOperationBlockers`** mirrors probe bootstrap-error codes (`d7f1a9a`, Q448)
