...
| 데이터베이스 | PostgreSQL | `pgcrypto` 확장, Flyway V1–V148 |
| 인증 | JWT (RS256) + RBAC | access 30분, refresh 7일 |
| 멀티테넌트 | Organization → Branch | `organization_id` 강제 격리 |

> **구현 상태 (2026-06-17 develop HEAD `aa42b9c` / frontend `6a18dfd` 기준)**:
> - **백엔드**: Must API + **V1–V148**. **G15** **`GET/PUT /transport/runs/{runId}/service-log`** + **`TRANSPORT_SERVICE_LOG_UPSERT` audit** (`aa42b9c`, Q411) · **monthly reports 2-7/2-8** (`5d27ad3`, Q410) · QA-B95 **probe seed guard** (`0b5657a`, Q409) · **`ensureClientWithFallback`** (`c13800c`) · **BE WT CLEAN**.
> - **프론트엔드**: **106 route**. **G15 `TransportServiceLogPanel` archive** (`088e906`, Q411) · **`TransportMonthlyReportsPage`** (`6a18dfd`, Q410) · QA-B116 service log a11y (`dff2f32`) · **FE WT CLEAN**.
> - **merge gate**: develop→test **~377 pending** (346 BE + 31 FE) · **BE WT CLEAN @ `aa42b9c`** · **FE WT CLEAN @ `6a18dfd`** · **operation 승격**: harness + G15 monthly reports smoke — **staging `./scripts/run-live-e2e.sh` RUN 확인 권장** (QA-B95).
> - **마이그레이션**: V43 … **V147** guardian trigger · **V148** service log compliance columns. Flyway 자동 실행.
...
| **Live E2E credential probe (Q360, QA-B96)** | **`LiveE2eControllerTest`**·**`HealthControllerTest`** — `credentialsConfigured` (`c5f1325`/`8a92179`) · FE **`liveE2eHarness.test`** placeholder guard (`7106106`) |
...
`npm run test:live-e2e`(FE `eb16734`·`0122bfe`·`5f17beb`·`5c24e4e`·`10f32c4`·`61141a6`·`b3bd0cc`·`ddd4489`·`6f2a4eb`)는 **git root**의 **`scripts/dev-live-e2e.env`** 를 자동 source합니다 (`frontend-test` checkout 포함). 파일이 없으면 **`.example`** 을 fallback으로 읽습니다. **`LIVE_E2E_GUARDIAN_EMAIL`·`LIVE_E2E_GUARDIAN_PASSWORD`** 미설정 시 **`b3bd0cc`** — bootstrap enabled면 **`POST /api/v1/system/live-e2e/bootstrap-guardian`** 으로 guardian token·`LIVE_E2E_CLIENT_ID` 자동 발급. bootstrap endpoint **HTTP 500/503** 이더라도 env creds가 유효하면 **`ddd4489`** — **`login-fallback-ok`** 로 재로그인 시도. **example placeholder** 값 그대로면 authenticated suite도 **skip** (`5c24e4e`/`7106106`).
...
# 또는 프론트엔드 package script (env auto-source)
...
>
> **readiness 점검 (`de25b3e`, FAQ Q367)**: **`GET /api/v1/notifications/channel-status`** 응답 **`readinessBlockers[]`** 로 live 전환 전 누락 항목을 확인합니다 — `ALIMTALK_PROVIDER_NOT_SOLAPI` · `MISSING_SOLAPI_CONFIG` · `MISSING_TEMPLATE_MAPPING` · `EMAIL_PROVIDER_NOT_SMTP` · `MISSING_SMTP_CONFIG`. 비밀값은 노출하지 않습니다.
>
...
- **route-preview**: `hq_admin`·직원 4역할 JWT → `POST /transport/route-preview` `{ branch, stops[] }` — 정차 **`pickupLat`/`pickupLng`** 또는 geocode 좌표 → `path[]`·`distanceMeters`·`durationSeconds` (`e8b8398`). FE **`KakaoTransportMapView`** — **`mapEnabled=true`** 이후 overlay 렌더 · route-preview **전** 좌표 seed markers · 경로 요약 **`role="status" aria-live="polite"`** · canvas **`role="application"`** (`5ebaade`). route-preview 실패 시 **마커만** 렌더 (SVG fallback **제거**).
- **스테이징 스모크**: (1) geocode — `POST /transport/geocode` `{ "address": "서울특별시 …" }` → `lat`/`lng` (2) route — 정차 2곳 이상 좌표로 route-preview → `status: OK` (3) UI — `/transport/runs/new` 지도 **「지도 로딩 중」** → 마커·폴리라인 표시.
...
| `databaseStatusDetail` | `SELECT_1_OK` | `SELECT_1_FAILED` |
...
로드밸런서는 **liveness**(`GET /api/v1/health/ping`)와 **readiness**(`GET /api/v1/health`, `ready === true`)를 분리하세요. live E2E harness는 **`ready === true`** (또는 레거시 호환 `status === "UP"` + HTTP 200)일 때만 테스트를 허용합니다. DB만 일시 중단된 경우 **503** 으로 인스턴스를 드레인할 수 있습니다 (FAQ **Q360**).
...
- [ ] **`POST /api/v1/transport/runs`** — `stops[]` with **`stopKind=BRANCH`** 지점 경유 · `direction=DROPOFF` (`f5b2b42`/`114411f`, Q401)
