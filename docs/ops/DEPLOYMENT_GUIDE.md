<!-- doc:owner=TWR doc:audience=PLN,COD updated=2026-06-06T20:00:00+09:00 -->
...
| 데이터베이스 | PostgreSQL | `pgcrypto` 확장, Flyway V1~V39 |
| 인증 | JWT (RS256) + RBAC | access 30분, refresh 7일 |
| 멀티테넌트 | Organization → Branch | `organization_id` 강제 격리 |

> **구현 상태 (2026-06-06)**: 백엔드 Must API·Flyway **V39**·설정·백업 스케줄러·인증 rate limit·**대시보드 API** 구현됨. 테스트 **25클래스/86건** PASS. 프론트엔드는 **JWT·REST API 연동 Must 핵심 화면** 21페이지·24 라우트·`SideNav`. **`/branches`·`/attendance/stats` 라우트 등록**(Q104·Q106). QR·보호자 체크인·건강 추이·수가표·설정 — **UI 골격·API 미연동**(Q105). **일부 `services.js` 경로·응답·본문 불일치**(Q83·Q94–Q101). **Dockerfile·docker-compose는 저장소에 아직 없음** — 본 문서 §6은 권장 구성 초안입니다.
...
| Flyway | 애플리케이션 로그 | `V1`~`V39` 마이그레이션 성공 |
...
cd src/frontend
npm ci
npm test          # Vitest — ProtectedRoute·roleHomePaths 등
npm run build     # dist/ 산출물
```

프론트 단위 테스트는 **라우팅·역할 홈 경로** 위주이며, Must 화면 E2E는 아직 CI에 포함되지 않습니다. 스테이징 스모크 시 FAQ Q83·Q104–Q105의 **API 경로 불일치·UI 골격 미연동·`/branches` 라우트 미등록**을 확인하세요.
...
| 단계 | 검증 |
|------|------|
| 빌드 | `mvn -B test` (24 테스트 클래스, 82 tests), `npm ci && npm run build` |
| 스테이징 | Flyway V1~V39 적용, E2E 로그인·Tenant 격리·청구 상태 필터·NHIS 수동 매칭·`primaryGuardian` 등록·actor backstop·rate limit |
