...
### 1-4. 구현 상태 안내 (2026-06-17 develop HEAD `aa42b9c` / frontend `6a18dfd`, 208차)
...
3. **G41 기관 교육일지 준수**: `GET /api/v1/staff/training-logs/compliance`의 반기/연간 미준수 확인
4. **7-5 간편결제 상태**: `GET /api/v1/billing/easy-pay/claims/{claimId}/payment`의 실패·가드 케이스 확인
5. **G15 이동서비스 수칙·계약**: `/transport/compliance` — 미서명 이용자·**`guardianContact` 빈 값** 확인 (Q397·Q398, `35e03ef`/`84e75ec`)
6. **G15 별지 제22호 일지**: 확정 루트 **`GET/PUT /transport/runs/{runId}/service-log`** — 준수 기록 저장·**`summary.onTime`** · **`TRANSPORT_SERVICE_LOG_UPSERT` 감사 로그** 확인 (Q407·Q411, `aa42b9c`/`088e906`)
7. **G15 월간 리포트 (2-7/2-8)**: `/reports/transport-monthly` — 당월 **변동·계약 미서명**·**확정 배차** 집계 확인 (Q410, `5d27ad3`/`6a18dfd`)
8. **배차 승차·하차**: `/transport` — **DROPOFF** 루트·명단 **희망 시각**·지점 경유 geocode·**DRAFT 삭제** BE·FE (Q399~Q404, `1d1a71f`/`45bd923` carry)
9. **live E2E harness (QA-B95)**: bootstrap enabled + **`GET /api/v1/system/live-e2e/probe`** — **`clientReady`·`seedClientId`** (seed lookup 실패 시 **500 아님**, `0b5657a`) · **`GET /api/v1/health`** — **`liveE2eClientReady`·`liveE2eSeedClientId`** · reused dev DB **`ensureClientWithFallback`** (`c13800c`, Q409) · staging **`./scripts/run-live-e2e.sh`** RUN · harness env 오염 **Q408** (`b69c8ae`)
...
| FE UI | **`MonitoringSelfDiagnosisPage`** — **`MonitoringIntegratedChecklistPanel`**(증빙 기간 표시, `73094f9`) · **`navConfig`** **「모니터링 자가진단 (G30)」** · **`monitoringCompliance.js`** · FAQ21836 **`DEFAULT_MONITORING_RELATED_BASIS`** (`0da41c6`) |
...
| guards | 노인인권 — **referenceHalf·trainingContent 필수** · 신규 오리엔테이션 — **`newHireUserId` + hiredAt+7일** · **`trainedAt.year = referenceYear`** · **`trainingType` `Locale.ROOT` uppercase** · **V129**: `ELDERLY_HUMAN_RIGHTS` 외 **`reference_half` NULL** |
...
| FE UI | **`StaffTrainingLogPage`**(`/staff/training-logs`) — StatCard·필터·Modal CRUD · **`StaffContextNav`** **「교육일지 (8-7)」** · **`staffTrainingLogs.js`** — **드롭다운 5종**(V129 23종 wire **P2**) |
...
| FE UI | **`CareServiceWeeklyRecordPage`**(`/care/weekly-service-records`) — **`CareServiceWeeklyRecordForm`** (`41b2123`) |
...
| 항목 | 내용 |
|------|------|
| BE API | **`BathingScheduleController`** — **`/api/v1/care/bathing-schedules`** (`e703252`) |
...
| FE UI | **`BathingSchedulePage`**(`/care/bathing-schedules`) — **`BathingScheduleForm`** (`950415d`) |
| 테스트 | **`BathingScheduleServiceTest`** · **`BathingSchedulePilotServiceFlowE2eTest`** · **`BathingSchedulePage.test`** · **`bathingScheduleLiveApi.e2e.test.js`** |

> 현장: USER_MANUAL §5-26 · FAQ Q363
...
| FE UI | **`MealAssistanceRecordPage`**(`/care/meal-assistance-records`) · **`MealAssistanceRecordForm`** Field render-prop (`9ad8346`) |
| 테스트 | **`MealAssistanceRecordServiceTest`** · **`MealAssistanceRecordPilotServiceFlowE2eTest`** · **`MealAssistanceRecordPage.test`** · **`mealAssistanceRecordLiveApi.e2e.test.js`** |
...
| FE UI | **`CareNursingServiceReportPage`** — 3 route variant · **`CareNursingServiceReportNav`** 3탭 (`.ds-context-nav--sub`, `58ee122`/`8ed937c`) |
| SideNav | **`navConfig.js`** — **통합 간호제공 리포트**·**병의원 진료내역**·**투약제공** → `/care/reports/*` |
| 레거시 | **`/nursing/service/reports/*`** · **`NursingServiceReportsPage`** — 간호 모듈 컨텍스트 유지 (Q350) |
| 테스트 | **`CareReportServiceTest`** · **`CareReportLiveApiRoutingE2eTest`** · **`CareNursingReportsPilotServiceFlowE2eTest`** (`2ba2761`) · **`RoleBasedControllerAccessTest$CareReportAccess`** · FE **`CareNursingServiceReportPage.test`** · **`careReportServices.test`** · **`pilotPageFlows.test`** (`6b34d31`) |

> 현장: USER_MANUAL §5-36 · FAQ Q386·Q389~Q392 · REQUIREMENTS L02/L03 parity
...
| **route preview (Q370·Q394·Q395, BNK-253·BNK-285)** | **`TransportRoutePreviewService`** · **`KakaoDirectionsClient`** — `/v1/directions`·waypoints · FE **`fetchTransportRoutePreviewApi`** · **`kakaoMapInstance.js`**·**`useKakaoMap`**·**`loadKakaoMapSdk`** + **`KakaoTransportMapView`** (`5ebaade` — seed markers·**`mapEnabled`**·canvas **`role="application"`**·**`.ds-transport-map__summary` `aria-live`**) · **`kakaoMapInstance.test`** · **`KakaoTransportMap.test.jsx`** · **`TransportRoutePreviewServiceTest`** · **`KakaoDirectionsClientTest`** |
...
| **RFID split-view (Q377·Q387, BNK-262/273, FE `55fdbd0`/`4c9103d`/`cb457b7`)** | |
|--------------------------------------------------|--|
| UI | **`VisitsPage`** — **`Switch`「계획·청구 분할 비교 (RFID split-view)」** · **`ds-visits-split-compare`** dual table · **청구반영 현황 chip** · **후속 확인 목록** |
...
> **FE UI**: **`VisitsPage`** — **`VisitNhisImportPanel`** import 업로드 **Fixed** (UXD-63, Q189) · 확정 일정 존재 시 **UI 차단** (BNK-26, Q195) · **RFID split-view** (Q377, `55fdbd0`). USER_MANUAL §5-11.
...
| 메서드 | 경로 | 설명 | 권한 |
|--------|------|------|------|
| **GET** | **`/billing/reports/medical-deduction`** | 지점·귀속연도별 **이용자별 PAID copay** 합계·건수 (페이지네이션) | `hq_admin`, `branch_admin` |
...
| 규칙 | 내용 |
|------|------|
| medical-deduction 쿼리 | **`taxYear`** (필수) · **`branchId`** (선택) · **`q`** (이름·인정번호) · **`page`/`size`** |
| medical-deduction 집계 | Q252·Q254와 **동일** — `PAID`+`paidAt` · `CASH`·`BANK_TRANSFER`만 · CMS·EASY_PAY 제외 |
...
| FE | **`BillingStatisticsReportPage`** — **`/billing/reports/statistics`** · **`BillingReportsContextNav`** · SideNav **청구 → 본인부담 통계** · **`fetchMedicalExpenseDeductionReportApi`**·**`fetchCopayMonthlyStatisticsApi`**·**`fetchTransportServiceFeeStatisticsApi`** 3-fetch (`09e4ec1`) · a11y **`Field error`·StatCard groups** (`31544cf`, Q381) |
| 테스트 | **`BillingServiceTest`** · **`TransportServiceFeeServiceTest`** · **`MedicalExpenseDeductionReportLiveApiRoutingE2eTest`** · **`CopayMonthlyStatisticsReportLiveApiRoutingE2eTest`** · **`G26StatisticsReportsLiveApiRoutingE2eTest`** — **3-endpoint** harness·invalid year **`422`** (`92ae60b`/`3672bbe`) · **`G26StatisticsReportsPilotServiceFlowE2eTest`** — **3-function** service flow (`30f03e8`) · **`BillingStatisticsReportPage.test`** · **`g26StatisticsReports.test`** · **`MustApiEndpointRoutingTest`** · **`RoleBasedControllerAccessTest`** |
