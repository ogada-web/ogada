...
### 1-4. 구현 상태 안내 (2026-06-06 기준, Flyway V39)
...
| 인증 API | **구현됨** | 로그인·갱신·로그아웃·`active-branch`·비밀번호 재설정, **IP·계정 rate limit** (`429 RATE_LIMITED`) |
| 청구 API | **구현됨** | `BillingController` — 청구 생성·상태 변경·**목록 상태 필터**(`?status=`), **NHIS 수동 매칭**(`candidates`, `rows/{id}/match`) |
| PII 암호화 | **구현됨** | `PiiCryptoService` — 주민번호·연락처·주소 AES-GCM, **복호화 열람 API** + `audit_log` |
| Actor 감사 | **구현됨** | `DbSessionContext` + V32–V35 트리거 — 출석·건강·청구·NHIS import·**수가표·QR** `*_by` 컬럼 자동 기록 |
...
| 프로덕션 시크릿 | **구현됨** | `ProductionSecretValidator` — `prod` 프로필 기동 시 JWT·QR·DB_PASSWORD 필수 |
| `/platform` 프론트 | **부분 연동** | Tenant 등록 모달 동작. **목록 `{items}` vs `List` 불일치**(Q97), **hq_admin 발급 `{email}` vs `password`+`displayName` 필수**(Q97), 검색 `?q=` vs `?query=`(Q83). 행별「관리」후속 |
| `/billing/nhis-import` | **API 연동** | 업로드·이력 UI. **목록 필드 `targetMonth`/`totalRows`/`branchName` 불일치**(Q100) |
| `/billing/nhis-import/:batchId` | **API 연동** | `ReconciliationPage` — UNMATCHED 수동 매칭. **행 필드 `name`/`billingDays`/`billingAmount` 불일치**(Q101) |
| `/clients/:id` 상세 | **부분** | 기본정보·청구 탭·SSN [열람]. **건강·출석 탭 스텁**(Q102), **청구 `claimStatus`/`yearMonth` 필드**(Q102), API 경로 Q83 |
| `/clients/:id/edit` | **부분** | `ClientFormPage` 수정 모드 등록됨. **동의·보호자 본문 불일치** — FAQ Q92·Q103 |
| `/clients/new` 등록 | **부분** | 폼 UI 있음. **`primaryGuardian`·동의 필드명 불일치** — FAQ Q92 (V39) |
| `/guardian` 포털 | **부분** | 탭 UI. 백엔드 `checkin-targets`·`daily-records`와 경로 불일치 |
| `/dashboard`·`/dashboard/hq` | **데모 UI** | StatCard 하드코딩. **`GET /dashboard/*` API는 동작** — FAQ Q85 |
| `/attendance/qr/generate` | **UI 골격** | `QrGeneratePage` — 유형·인쇄 레이아웃. **QR API 미연동** (FAQ Q105) |
| `/guardian/checkin` | **UI 골격** | `GuardianCheckinPage` — 대상 선택·스캔 버튼. **카메라·API 미연동** (FAQ Q105) |
| `/health/:clientId` | **UI 골격** | `HealthDetailPage` — 차트 플레이스홀더 (FAQ Q105) |
| `/billing/fee-schedules`·`/copay-rates` | **UI 골격** | 입력 폼 있음. **저장·조회 API 미연동** (FAQ Q91) |
| `/settings` 프론트 | **UI 골격·역할 불일치** | 탭 레이아웃. API **`SYSADMIN`만** (FAQ Q87). 본문 TODO |
| `/branches` | **구현됨** | `BranchesPage` — 목록·등록/수정 모달·`GET/POST/PATCH /branches` 연동 (FAQ Q104) |
| `/attendance/stats` | **부분** | `AttendanceStatsPage` — UI·API 호출. **쿼리·응답 불일치** (FAQ Q106) |
| `/users` | **미구현** | 페이지 파일 없음 — `/users` CRUD는 Swagger (FAQ Q88) |
...
> **UI (2026-06-07)**: `/platform`에서 Tenant **등록 모달**은 동작합니다. 다만 **목록이 빈 테이블**로 보이거나 **hq_admin 발급이 실패**할 수 있습니다 — 프론트가 페이지 래퍼(`items`)·발급 본문(`email`만)을 기대하는데 API는 `List` 직접 반환·`password`+`displayName` 필수입니다 (FAQ **Q97**). **검색**은 `?q=` vs `?query=` 불일치(Q83). 행별 **「관리」** 상세는 후속입니다.
...
> **역할 불일치 (2026-06-07, FAQ Q87)**: `App.jsx`는 `/settings`에 `hq_admin`·`platform_admin`도 허용하지만, **설정 API는 `sysadmin` 전용**입니다. `hq_admin`이 SideNav에서 「시스템 설정」을 눌러도 감사·백업 API는 `403`입니다. Tenant **운영 정책**(`allow_client_self_checkin`)은 §6-3의 `PATCH /organization/settings`(`hq_admin`)를 사용하세요.
...
### 6-3-1. 수가표·본인부담 비율 (API-only, `hq_admin`)

`/billing/fee-schedules`·`/billing/copay-rates` 화면은 **플레이스홀더**입니다 (FAQ Q91). 월말 청구 전 `hq_admin` JWT로 Swagger에서 등록합니다.

| 작업 | API | 본문 예시 |
|------|-----|----------|
| 수가 조회 | `GET /billing/fee-schedules` | — |
| 수가 등록 | `POST /billing/fee-schedules` | `{ "year": 2026, "ltcGrade": 3, "dailyRate": 68000 }` |
| 수가 수정 | `PATCH /billing/fee-schedules/{id}` | 이력 보존 — 새 버전 생성 |
| 부담 비율 조회 | `GET /billing/copay-rates` | 4구분 기본값 |
| 부담 비율 수정 | `PATCH /billing/copay-rates/{copayType}` | `{ "rate": 0.15 }` |

등록 시 `fee_schedules.created_by`에 행위자 UUID가 V35 트리거로 기록됩니다. **지점 관리**(`/branches`)는 `BranchesPage`·SideNav에서 접근 가능합니다 (FAQ Q104). **직원 관리**(`/users`)는 페이지 없음 — Swagger 사용 (FAQ Q88).
...
| 2 | `POST /clients` — `consentToCollectResidentRegistrationNo: true` + **`primaryGuardian` 필수** → `guardian_link_status=LINKED` (V39) | `branch_admin`, `social_worker` |
| 3 | (선택) `POST /clients/{id}/guardians` — 추가 보호자 연결 | 동일 |

프론트 `/clients/new`는 2~3단계를 **자동 수행하지 않습니다**. 파일럿 센터 온보딩 시 Swagger 체크리스트로 검증하세요.
...
| POST | `/billing/imports/nhis` | 공단 엑셀 import |
...
### 10-6. Must API-only (프론트 미연동·미등록)

| 기능 | API | 비고 |
|------|-----|------|
| 지점·통합 대시보드 | `GET /dashboard/branch`, `/hq`, `/hq/alerts` | UI 데모 (FAQ Q85) |
| 결석 사유 | `POST /attendance/absence` | UI 없음 (FAQ Q86) |
| 지점 QR | `POST/GET /branches/{id}/qr` | **UI 있음·버튼 미연동** (FAQ Q105) |
| QR 스캔 | `GET /guardian/checkin-targets`, `POST /attendance/qr/scan` | **UI 있음·카메라·API 미연동** (FAQ Q105) |
| 월별 출석 통계 | `GET /attendance/stats/monthly` | **UI 있음·쿼리·응답 불일치** (FAQ Q106) |
| 지점 CRUD | `/branches` | **`BranchesPage` 연동·라우트 등록** (FAQ Q104) |
| 직원 CRUD | `/users` | **페이지 없음** (FAQ Q88) |
| 수가표·본인부담 | `/billing/fee-schedules`, `/copay-rates` | **UI 골격·저장 미연동** (FAQ Q91) |
| 시스템 설정 | `/settings/*` | **UI 골격·`SYSADMIN`만** (FAQ Q87) |
| 건강 추이 | `GET /clients/{id}/health?days=` | **`/health/:clientId` 플레이스홀더** (FAQ Q105) |
| 이용자 퇴소·사진 | `POST /clients/{id}/discharge`, `/photo` | UI 없음 |
