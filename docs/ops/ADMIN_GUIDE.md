...
| 화면 | 경로 | storyRef | canonical 데이터 | cross-link (반대편) |
|------|------|----------|------------------|---------------------|
| **일별 출퇴근** | `/staff/attendance` | G-STAFF-WORK-ATTENDANCE | 당일 `checkInAt`·`checkOutAt`·`checkInMethod` | **연차휴가 현황** → `/staff/annual-leaves` (**AVAILABLE**, Q651) |
| **연차휴가 현황** | `/staff/annual-leaves` | US-R03e | 월별 사용 스냅샷 (`surfaceKind=ANNUAL_LEAVE_USAGE_SNAPSHOT`) | **출퇴근 기록** → `/staff/attendance` (**AVAILABLE**, Q650) |
| **연차·유급휴일 대장** | `/staff/leave-ledger` | US-R01-c | 건별 휴가·유급휴일 canonical (**BE+FE ✅**, Q663·Q666) | **API `relatedSurfaces`** → annual-leaves·attendance (**AVAILABLE**) |
...
| 항목 | ogada | 외부(Solapi) |
|------|-------|--------------|
| 알림 발송·이력 | **✅** J03 `NotificationService` | Provider API 호출 |
| 발신번호 등록·본인인증 UI | **❌ P3 Planned** | **Solapi 콘솔** — `SOLAPI_SENDER_ID` 사전 인증 |
| readiness | **`GET /notifications/channel-status`** — placeholder·누락 key 거부 (Q644) | 인증 미완료 번호는 Solapi 측 발송 실패 가능 |

**sysadmin 체크리스트 (live 전환 전)**

1. env **`SOLAPI_SENDER_ID`** = 센터 실제 발신번호 (placeholder 아님 — Q644).
2. Solapi 콘솔에서 해당 번호 **인증 완료** 확인.
3. **`liveAlimtalkDispatchReady=true`** · blocker 없음 확인 (Q318).

> 경쟁사 silverangel [notice 221562](https://www.silverangel.kr/silverangel/support/notice/view.do?pageNo=1&Tmp_idx=221562) 발신번호 마감(2026-06-23)은 **참고 인텔**이며, ogada MVP에는 **발신번호 self-service 미포함** (planner P3).
...
4n. **G-STAFF-WORK-ATTENDANCE (Q612·Q622, BE `a6eb8b7`·`10c0daf`·`35e6c52`·FE `5fd468b`/`df7f308`)**: **`GET /staff/work-attendance`** — 지점 **활성 직원 전원** + **`userName`·`roleCode`·`status`·`checkInAt`·`checkOutAt`·`checkInMethod`** · **`POST …/check-in`** — **`MANUAL`/`MOBILE`/`NFC`** · **`POST …/check-out`** — **checkout-before-checkin guard** · **V169** `staff_work_attendance` · **`/staff/attendance` `StaffWorkAttendancePage`** — **UXD-152 a11y** · RBAC **`hq_admin`·`branch_admin`·`social_worker`**
...
4p. **G-ATTENDANCE-STATS monthly stats (Q615, BE `6be0c79`·FE `dffd726`)**: **`GET /attendance/stats/monthly?from=&to=&branchId=`** — **`MonthlyAttendanceStatsListResponse`** — **`branches[].months[]`** · FE **`/attendance/stats`** — **`fetchMonthlyAttendanceStatsApi`** + **`monthlyAttendanceStats.js`** — StatCard·6개월 추이·다지점 표 · **`markAbsentApi`** **`POST /attendance/absence`** + **`absenceReason`** · **`checkOutApi`** **`transportType`** 정규화
4q. **G-BILLING-REPORT-FILTER-PERSISTENCE (Q621·Q623·Q626, BE `479995e`/`fd0a3b3`/`99d03fa`·FE `77b1ea8`)**: **V170** `billing_report_filters` · **`GET/PUT /billing/reports/filters`** — **`BillingReportPage` hydrate** · **`recordFromReportQuery()` read-scope autosave** · **`recordFromReportQuerySafely()` non-blocking** (Q626) · RBAC **`hq_admin`·`branch_admin`**
4q1. **V171 defense-in-depth schema readiness (Q625, BE `dce9bf1`/`175d9cb`)**: **`V171DefenseInDepthSchemaReadinessProbe`** — health **`v171DefenseInDepthIntegrityCheckReady`** · blocker **`v171-defense-in-depth-constraint-missing`** · Flyway V171 5 CHECK/FK
4q2. **US-E03 QR image FE wire (Q624, FE `250619e`)**: **`QrGeneratePage`** — **`branchQrCode.js`**·**`qrcode`** · PNG preview·download·print · **`POST /branches/{branchId}/qr`**
4q2a. **US-D03 client detail attendance tab FE wire (Q628, FE `d058e43`)**: **`ClientDetailPage`** — **`fetchClientAttendanceHistoryApi`** · **`GET /clients/{id}/attendance`** · 출석 탭 테이블 · **`clientAttendanceServices.test.js`**
4q2b. **G30-LEGEND official indicator cross-walk legend (Q629, FE `fdc135b`)**: **`MonitoringOfficialIndicatorLegendPanel`** — **`monitoringOfficialIndicatorCrosswalk.js`** — **`/compliance/monitoring`** — ogada 15문항 ↔ 공단 1~15 · direct **#6↔15** · **#15↔12-14** · **`GET /compliance/monitoring/items`** template titles · **`MonitoringOfficialIndicatorLegendPanel.test`**
4q2c. **UXD-153 a11y closure (Q630, FE `da34daf`)**: **`BillingReportPage`** hydrate **`aria-busy`** · **`QrGeneratePage`** **`validUntil` `<time dateTime>`** · **`ClientDetailPage`** 출석 탭 확인(변경 불요)
4q2d. **G34-WORKFLOW-CATALOG ezCare workflow cross-walk (Q635, FE `77cfc38`/`9f110a5`)**: **`WorkflowCatalogPage`** — **`/compliance/workflow-catalog`** · **`EzcareWorkflowCatalogPanel`** — FAQ **21795–21828** · **16/28 verbatim** · cycle·citation **FilterChips** · **`ezcareWorkflowCatalog.js`** · SideNav **「운영주기별 워크플로 (G34)」** · **`EzcareWorkflowCatalogPanel.test`** · **`pilotPageFlows`**
4q3. **G2 CMS enrollment input normalization (Q627, BE `9db0bbb`)**: **`CmsService.normalizePayerName()`** · **`normalizeBankCode()`** · **`normalizeAccountLast4()`**
4q3a. **G2 CMS branch enrollment roster API (Q637, BE `d361833`·FE `CmsPage`)**: **`GET /billing/cms/enrollments?branchId=&status=`** — **`clientId` 생략** 시 **`listBranchEnrollments()`** · **`clientName`** enrichment · **`CmsEnrollmentResponse.clientName`** · **`CmsPage`** roster·상태 필터 · **`CmsServiceTest.listBranchEnrollments*`**
4q3b. **G2-CMS-ENROLLMENT-ROSTER full-stack deepen (Q638, BE `d0c0d12`/`d3d4d2d`·FE `df9ec6c`/`947312c`/`3ece965`)**: **`branchId` 생략 → TenantContext.activeBranchId fallback** · **`CmsPage` FilterChips·`BranchScopeNotice`·`?clientId=` preselect** · **`CmsEnrollmentTable` 이용자 링크** · **`PaymentPage` CMS 등록 열·미등록 deep link** · **`CmsPilotServiceFlowE2eTest`** roster e2e
...
4q3j. **QA-B266 develop pre-merge npm flake (Q658, FE `949e9bf`)**: **Fixed @ TSR 1311** — 1310차 **2048/2049 FAIL** non-repro · post-merge **2049/2049 PASS** · Vitest pollution 재발 방지는 Q661
4q3k. **Vitest 동시 실행·full-suite pollution 운영 (Q661)**: **`npm test` 1회만** — **`scripts/npm-test-locked.sh`** · **`./scripts/vitest-stop.sh`** 정리 · **`docs/qa/VITEST_CONCURRENCY.md`**
4q3l. **G2 CMS branch roster status filter normalization (Q662, BE `f1225b0`)**: **`CmsService.normalizeEnrollmentStatusFilter()`** — **`status` trim·uppercase** · **`ACTIVE`/`CANCELLED`/`PENDING`** · unsupported → **`422`** · **`CmsServiceTest.listBranchEnrollmentsShouldNormalizeStatusFilterAndReturnPendingRoster`** · **`validateBranchReadScope`**
...
| **Live E2E bootstrap blocker normalize (Q631, QA-B95, BE `24d25f1`)** | **BE Fixed** | **`LiveE2eOperationReadinessSupport`** — bootstrap error 시 **`staff-bootstrap-not-ready`·`guardian-bootstrap-not-ready`만** · **`-error`/`bootstrap-error` blocker 제거** · root cause는 **`liveE2eStatusDetail`** · **`HealthController`** health·probe **동일 집합** · **`LiveE2eOperationReadinessSupportTest`** · **`HealthControllerTest`** · **`LiveE2eControllerTest`**
| **Live E2E bootstrap error blockers (Q448, QA-B135, BE `d7f1a9a`)** | **BE Fixed (310차 superseded by Q631)** | *(309차까지)* status detail **`bootstrap=error`** 시 **`-error` blocker 추가** — **310차 `24d25f1`부터 `-not-ready` only** (Q631)
...
| list | **`GET …?date=&branchId=`** — **`StaffWorkAttendanceListResponse`** — `workDate`·`branchId`·`items[]`·**`surfaceKind`**·**`relatedSurfaces[]`** (Q653, `83a26e7`) |
...
| guards | 중복 출근 · 미출근 퇴근 · 중복 퇴근 · **checkout < check-in** → **`422 BUSINESS_RULE`** (`35e6c52`, Q622) |
| DB | **Flyway V169** — `staff_work_attendance` — org/branch/user/date UNIQUE · **V171** checkout-after-checkin CHECK · Tenant FK 3쌍 |
| RBAC | **`hq_admin`·`branch_admin`·`social_worker`** — **`RoleBasedControllerAccessTest$StaffWorkAttendanceAccess`** |
| FE UI | **`StaffWorkAttendancePage`**(`/staff/attendance`) · **`StaffContextNav`「출퇴근 (8-4)」** · **`StaffAnnualLeaveRelatedSurfacesPanel`** — API **`relatedSurfaces`** 우선 · **US-R03e `/staff/annual-leaves` AVAILABLE** · **US-R01 `/staff/leave-ledger` AVAILABLE** (BE, `bb9df48`) · **`normalizeStaffWorkAttendanceResponse`** (`95f55aa`, Q653) · **`STAFF_WORK_ATTENDANCE_CROSS_LINK_HELP`** (Q651, `2040571`) · **「출근 방식」** select · **UXD-152 a11y** (`df7f308`, Q623) · **`fetchStaffWorkAttendanceApi`** · **`staffWorkAttendanceCheckInApi({ checkInMethod })`/`CheckOutApi`** (`5fd468b`) |
| cross-link | **양방향 US-R01 nav** — **`/staff/annual-leaves` → `/staff/attendance`** (Q650, `0b0d7ba`) · **`/staff/attendance` → `/staff/annual-leaves`** (Q651, `2040571`/`95f55aa`) — **BE `GET /staff/work-attendance` `relatedSurfaces`** (Q653, `83a26e7`) — **8-4 roster에 leave-ledger 데이터 중복 없음** |
| 테스트 | **`StaffWorkAttendanceServiceTest`** (+ surfaceKind/relatedSurfaces `83a26e7`, Q653 · check-in method validation `10c0daf` · checkout guard `35e6c52`) · **`StaffWorkAttendanceSupportTest`** (Q653) · **`StaffWorkAttendanceLiveApiRoutingE2eTest`** · **`StaffWorkAttendancePage.test`** (+ API metadata cross-links `95f55aa`, Q653) |
...
이지케어 **worker-b100 tab01 「연차휴가 현황」** — 지점 **활성 직원**의 **연차 부여·월별 사용·잔여** 그리드.

| 항목 | 내용 |
|------|------|
| BE API | **`StaffAnnualLeaveController`** — Base **`/api/v1/staff/annual-leaves`** (`41fb84f`/`6b84bcd`/`a45745c`) |
| roster | **`GET …/roster?year=&branchId=`** — **`StaffAnnualLeaveRosterResponse`** — `year`·`branchId`·`items[]`·`staffCount`·**`surfaceKind`**·**`relatedSurfaces[]`** (Q648, `bbf333c`) |
| item fields | `userId`·`displayName`·`roleCode`·`hiredAt`·`monthlyUsage[12]`·`totalEntitlement`·`totalUsed`·`remaining`·`memo`·`persisted` |
| yearly GET | **`GET …/users/{userId}?year=`** — 1명 레코드 (미저장 시 empty defaults) · **`surfaceKind`·`relatedSurfaces[]`** (Q650, `6ab3760`) |
| yearly PUT | **`PUT …/users/{userId}`** — **`SaveStaffAnnualLeaveRequest`** — `{ year, monthlyUsage[], totalEntitlement, memo }` — **upsert** · 응답 **`StaffAnnualLeaveYearlyResponse`** 에 **cross-link 메타** (Q650) |
| guards | **월별 사용일수 < 0** → **`422`「월별 사용일수는 0 이상이어야 합니다.」** (`a45745c`) · **소수점** → **`422`「월별 사용일수는 정수여야 합니다.」** (`f88e8b1`, Q641) · **`memo` > 30자** → **`422`「비고는 30자 이하여야 합니다.」** (`6b35fb5`, Q642) · **월별 사용 합계 > totalEntitlement** → **`422`「월별 사용일 합계가 연차 총 부여일수를 초과할 수 없습니다.」** · **leave_year 2000~2100** |
| roster scope | **지점 배정 활성 직원** · **`lifecycle != TERMINATED`** · **`branchId` 생략 → TenantContext.activeBranchId** (`6b84bcd` test deepen) · **다지점 `branch_admin` + `branchId` 생략 → `422`** |
| DB | **Flyway V172** — `staff_annual_leave_yearly` — **`month_01`~`month_12` NUMERIC(4,1)≥0** · **`total_entitlement`** · **`memo`** · **`UNIQUE(org, user, leave_year)`** · Tenant FK 4쌍 · non-negative CHECK · **V173** (`8c5dd65`, Q645) — month≤99.9 · entitlement≤999.9 · sum≤entitlement · memo nonempty · user_branch_assignment FK |
| RBAC | roster/yearly GET — **`hq_admin`·`branch_admin`·`social_worker`** · PUT — **`branch_admin`·`social_worker` only** (`hq_admin` **403**) |
| FE UI | **`StaffAnnualLeavePage`**(`/staff/annual-leaves`) · **`StaffContextNav`「연차휴가 (US-R03e)」** · **`STAFF_ANNUAL_LEAVE_PAGE_TITLE`** = **「직원 연차휴가」** AppShell `<h1>` (`971c7f1`, Q643) · **`StaffAnnualLeaveRelatedSurfacesPanel`** — 스냅샷 안내·**`/staff/attendance` 링크**·**`/staff/leave-ledger` AVAILABLE 링크** (`0b0d7ba`/`8057c1e`, Q650·Q666) · **`ds-help-text`·`ds-fieldset`** (UXD-155, Q646) · **`StaffAnnualLeaveTable`** — StatCard·1~12월·**잔여 derived** · **편집 Modal `noValidate` + `Field error`** · **memo field error clear on edit** (`96e9d25`, Q647 deepen) · **`applyApiErrorToForm`** (Q647) · **`fetchStaffAnnualLeaveRosterApi`/`saveStaffAnnualLeaveYearlyApi`** · **`normalizeStaffAnnualLeaveRosterResponse`** (`0b0d7ba`) |
| cross-link | **`surfaceKind=ANNUAL_LEAVE_USAGE_SNAPSHOT`** · **`relatedSurfaces`** — **US-R01 `/staff/attendance` AVAILABLE** · **US-R01 `/staff/leave-ledger` AVAILABLE** (BE `bb9df48`, Q663) — roster·yearly GET/PUT **동일 contract** (`bbf333c`/`6ab3760`, Q648·Q650) · **FE panel wire ✅** (`0b0d7ba`) · **역방향 FE+BE wire ✅** (`2040571`/`83a26e7`/`95f55aa`, Q651·Q653) |
| live harness | **`staffAnnualLeaveLiveApi.e2e.test.js`** (`96e9d25`, Q649) · **`pilotChecklist` R03e-a/b/c + E05** (`e296387`, Q649) |
| readiness | **`GET /health`** · **`GET /live-e2e/probe`** — **`v173StaffAnnualLeaveYearlyIntegrityCheckReady`** · blocker **`v173-staff-annual-leave-yearly-constraint-missing`** (Q645) |
| 테스트 | **`StaffAnnualLeaveServiceTest`** (+ activeBranch fallback `6b84bcd` · negative usage `a45745c` · decimal reject `f88e8b1` · overlong memo `6b35fb5` · **surfaceKind/relatedSurfaces roster+yearly** `bbf333c`/`6ab3760`) · **`StaffAnnualLeaveSupportTest`** (Q648) · **`V173StaffAnnualLeaveYearlyIntegrityReadinessProbeTest`** · **`RoleBasedControllerAccessTest.StaffAnnualLeaveAccess`** (RBAC 5건) · **`StaffAnnualLeavePilotServiceFlowE2eTest`** · **`StaffAnnualLeavePage.test`** (+ related surfaces link Q650) · **`StaffAnnualLeaveRelatedSurfacesPanel.test`** (Q650) · **`staffAnnualLeaveLiveApi.e2e.test.js`** (Q649) · **`pilotChecklist.test`** R03e/E05 (Q649) · **`pilotPageFlows.test`** Must title (`971c7f1`) |
| P3 잔존 | **연차 자동 부여 규칙**(근속연수별) · **CSV export** · **ezCare worker-b100 tab02~05**(건강검진·교육 분리 surface) |

> ~~**V175 leave-ledger integrity** (DBA dirty)~~ — **✅ closure @ `c4e6bcb` (Q668)**

> 현장: USER_MANUAL §4-7-0a·§4-7-0b·§4-7-0c·§4-7-0d · FAQ **Q639·Q641·Q642·Q643·Q645·Q646·Q647·Q648·Q649·Q650·Q651·Q652·Q653·Q663** · **직원 출퇴근** §6-2-20 · **연차·유급휴일 대장** §6-2-24과 **양방향 cross-link**

### 6-2-24. US-R01-c 연차·유급휴일 canonical 대장 (케어포 8-13, BNK-532, BE+FE Fixed)

건별 휴가·유급휴일 **canonical ledger** — 월별 스냅샷(§6-2-23)·일별 출퇴근(§6-2-20)과 **데이터 중복 없이** cross-link만 공유.

| 항목 | 내용 |
|------|------|
| BE API | **`StaffLeaveLedgerController`** — base **`/api/v1/staff/leave-ledger`** (`bb9df48`, Q663) |
| list | **`GET ?year=&branchId=`** — 지점·연도별 **`items[]`** · **`entryCount`** · **`surfaceKind=CANONICAL_LEAVE_LEDGER`** |
| user list | **`GET /users/{userId}?year=`** — 직원 1명 연간 대장 |
| create | **`POST /users/{userId}`** — `leaveType`·`leaveDate`·`endDate`·`daysUsed`·`memo` |
| update | **`PUT /entries/{entryId}`** | 
| delete | **`DELETE /entries/{entryId}`** — **204** |
| `leaveType` | **`ANNUAL_LEAVE`** · **`PAID_HOLIDAY`** — trim·uppercase · 미지원 → **`422`** |
| `daysUsed` | **0 < ≤99.9** · 소수 **첫째 자리** · `endDate` 생략 시 기본 **1** · 기간 지정 시 **포함 일수 자동 계산** |
...
### 6-2-21. G-ATTENDANCE-STATS 월별 출석 통계 (US-E05, BNK-493, BE+FE Fixed)

케어포 **func 2-3 출석관리** 월간 집계 — **지점·월** 출석률·출석 일수. **당일 roster**는 §6-2-4와 **별도 API**.

| 항목 | 내용 |
|------|------|
| BE API | **`AttendanceController`** — **`GET /api/v1/attendance/stats/monthly`** |
| query | **`from`** · **`to`** (ISO date) · **`branchId`** (optional — JWT active branch) |
...
| RBAC | **`hq_admin`·`branch_admin`·`social_worker`·`caregiver`** |
| FE UI | **`AttendanceStatsPage`**(`/attendance/stats`) · **`fetchMonthlyAttendanceStatsApi({ yearMonth, branchId })`** (`dffd726`) |
| FE mapping | **`monthlyAttendanceStats.js`** — `yearMonthToPeriod`·`buildMonthlyStatsTrendPeriod`·`mapMonthlyAttendanceStatsResponse` |
| FE 표시 | StatCard **활성 이용자·출석 일수·출석률** · **6개월 line chart** · **다지점 HQ 표** |
| 관련 fix | **`markAbsentApi`** — **`POST /attendance/absence`** + **`absenceReason`** · **`checkOutApi`** — **`transportMethod` → `transportType`** |
| P3 | 일별 `dailyRates` · 이용자별 `clientStats` · CSV export |

> 현장: USER_MANUAL §4-4 · FAQ Q106·Q613·Q615 · **당일 roster** §6-2-4(Q609)

### 6-2-23. G-BILLING-REPORT-FILTER-PERSISTENCE 청구 대장 필터 저장 (G-BILLING, BNK-493, BE·FE Fixed)

케어포 **PDF p.91~92** 입금·수납 대장 — **월·지점·사용자별** 마지막 조회 조건 저장·복원.

| 항목 | 내용 |
|------|------|
| DB | **V170** `billing_report_filters` · **V171** integrity CHECK — **`UNIQUE(org, branch, user, year_month, variant)`** |
| GET API | **`GET /api/v1/billing/reports/filters?month=YYYY-MM&branchId=`** — charges/deposits/receipts/refunds 4-variant |
| PUT API | **`PUT /api/v1/billing/reports/filters`** — `SaveBillingReportFilterRequest` — **write scope** |
| auto-save | **`BillingReportFilterService.recordFromReportQuery()`** — **`GET /billing/reports/{variant}`** — **read scope** (`fd0a3b3`, Q623) |
| deposit filter | **`depositPeriod`**: `FULL`/`FIRST_HALF`/`SECOND_HALF` (Q585) — deposits variant only |
| receipt filter | **`receiptBasis`**: `PAYMENT`/`CLAIM` (Q585) — receipts variant only |
| RBAC | **`hq_admin`·`branch_admin`** — **`RoleBasedControllerAccessTest`** |
| FE wire | **`BillingReportPage`** — **`fetchBillingReportFiltersApi`** hydrate · **`saveBillingReportFilterApi`** on **「조회」** · **`normalizeSavedFilters`** (`77b1ea8`, Q621) |
| 테스트 | **`BillingReportFilterServiceTest`** (+ read-scope autosave, `fd0a3b3`) · **`BillingReportPage.test`** · **`MustApiEndpointRoutingTest`** |

> 현장: USER_MANUAL §4-6 · FAQ Q618·Q621·Q623 · **appliedFilters echo** §6-2-6g(Q587)
...
| 테스트 | **`NotificationServiceTest`**(+quiet-hours boundary·**manual dispatch reject**, Q329·Q539) · **`NotificationServiceTest`**(+91 primary dispatch, Q272) · **`NotificationChannelReadinessServiceTest`**(+placeholder credential/template, Q644) · **`NotificationConfigTest`** · `SolapiKakaoAlimtalkProviderTest` · **`AlimtalkTemplateVariablesTest`** · **`AlimtalkFallbackTextTest`** · **`NotificationAlimtalkDispatchE2eTest`** · **`J03AlimtalkServiceFlowE2eTest`** · **`J03EmailServiceFlowE2eTest`** · **`J03GuardianDocumentManualNotifyQuietHoursE2eTest`** (Q539) · **`EmailNotificationContentTest`** · **`GuardianDocumentNotificationServiceTest`** · **`StubEmailProviderTest`** · **`SmtpEmailProviderTest`** · **`NotificationHistoryServiceTest`** · FE **`notificationQuietHours.test`** · **`BillingDetailPage.test`** · **`OverduePage.test`** |
...
| **GET** | **`/billing/cms/enrollments?branchId=&status=`** | **`clientId` 생략** 시 **지점 CMS roster** · **`clientName`** enrichment (`d361833`, Q637) | `hq_admin`, `branch_admin` |
...
| roster | **`clientId` 생략** + `status`/`branchId` — 지점 단위 목록 · **`clientName`** (`d361833`, Q637) · **`branchId` 생략** 시 **`TenantContext.activeBranchId` fallback** (`d0c0d12`, Q638) · **`status` trim·uppercase** — `ACTIVE`/`CANCELLED`/`PENDING` (`f1225b0`, Q662) · 미지원 `status` → **`422`** |
...
| FE | **`CmsPage`** · **`BranchScopeNotice`** · roster **FilterChips** · **`?clientId=` preselect** · **`CmsEnrollmentTable`** 이용자 링크 · **`PaymentPage`** CMS 등록 열 · **「해지」**·Modal · **`BillingContextNav`** CMS 링크 · 출금 후 **확정 목록 갱신** (`c0a01b4`) |
| 테스트 | **`CmsServiceTest`** — cancel·duplicate guardian·amount mismatch·**branch roster fallback** (`d0c0d12`/`d3d4d2d`, Q638) · **`CmsCopayLifecycleE2eTest`** · **`CmsPilotServiceFlowE2eTest`** cancel lifecycle·**roster e2e** · **`MustApiEndpointRoutingTest$CmsRouting`** · **`RoleBasedControllerAccessTest$CmsAccess`** · **`CmsPage.test`** · **`CmsEnrollmentTable.test`** · **`PaymentPage.test`** (`df9ec6c`/`3ece965`, Q638) · **`StubFcmsClientTest`** |
...
> FAQ Q206–Q208·**Q299**·**Q637**·**Q638**·**Q662** · USER_MANUAL §4-6 · DEPLOYMENT §4-6·§8-1.
...
| 날짜 | 변경 내용 |
|------|----------|
| 2026-06-22 | **310차** — §1-4 live E2E harness **bootstrap blocker normalize**·**Q631**·**Q596·Q448 갱신**·baseline **`24d25f1`/`fdc135b`** |
