...
### 1-3. 구현 상태 안내 (2026-06-21 develop HEAD `a6eb8b7` / frontend `5fd468b` 기준 — 196차 baseline)

| 영역 | 상태 | 비고 |
|------|------|------|
| 백엔드 API | **Must + … + V169 ✅** @ `a6eb8b7` · **G-BILLING-DEPOSIT-ORDER-GUARD ✅** · **G-STAFF-WORK-ATTENDANCE ✅** · **G-ATTENDANCE-ROSTER-STATUS ✅** · … | **선행입금 입금 순서** (Q614) · **`GET /staff/work-attendance`** (Q612) |
| 데이터베이스 | Flyway **V1–V169** | **V169** `staff_work_attendance` · **V168** overdue defense-in-depth · **V166** `ON_LEAVE` |
| 프론트엔드 | **109 route · 87 page** @ `5fd468b` · **G-STAFF 출근방식 FE wire ✅** · **G-ATTENDANCE-ROSTER-STATUS FE wire ✅** · **UXD-151 mobile capture CSS ✅** · … | **`/staff/attendance`** — 출근 방식 select (Q612) · **`/attendance`** — 이용자 roster (Q609) |
| UI 연동 완료 | **… + G-BILLING-DEPOSIT-ORDER-GUARD · G-STAFF-WORK-ATTENDANCE full-stack · G-ATTENDANCE-ROSTER-STATUS · …** | **Q572·Q576–Q614** · staging `./scripts/run-live-e2e.sh` RUN 확인 권장 |
| UI API 갭 | **출석 QR·통계·본인부담 비율 저장·보호자 식사 API·공단 포털 자동 제출·NTS live API·수납 동시 발급 체크박스·국세청 자동 제출** | QR 생성 payload·이미지(Q109·Q590) · **출석 통계 FE/BE contract**(Q106·Q613) — **출석 roster ✅** (Q609) |
| **P2 Planned** | **L03 간호급여 잔여 5 leaf·7-5 live PG·J03 Solapi live dispatch·LCMS CMS 3-method·G34 SMS live·G-Payroll·G30 live E2E** | **G-STAFF-WELFARE P3**(FAQ21796) · **8-12 PDF 공식 서식**(Q315) · 건강검진 checklist **자동 반영**(Q444) |
...
| **직원 출퇴근 (G-STAFF-WORK-ATTENDANCE, 8-4)** | `/staff/attendance` | `GET /api/v1/staff/work-attendance` · `POST …/check-in` · `POST …/check-out` | **활성 직원 전원** · **출근 방식 `MANUAL`/`MOBILE`/`NFC`** · **수동 출근/퇴근** · **당일만** (`5fd468b`/`a6eb8b7`, Q612) |
...
| **월별 출석 통계 (G-ATTENDANCE-STATS, US-E05)** | `/attendance/stats` | `GET /api/v1/attendance/stats/monthly?from=&to=&branchId=` | **지점·월 집계** BE ✅ · FE **`yearMonth`/`dailyRates`/`clientStats` contract 갭** (Q613) — Swagger 우회 |
...
> **현재 UI 상태 (FE `5fd468b`)**: **109 route · 87 page** · **`/staff/attendance`** — **「출퇴근 (8-4)」** · **출근 방식 select**·테이블 출근 방식 열 (`5fd468b`, Q612) · **`/staff/{userId}?tab=files`** — **「서류관리 repository」** · **「모바일 촬영」** 전폭 버튼 (`9812ac4`, Q611) · **`/attendance`** roster API 단일 호출 (`8383f8d`, Q609) · **`/billing/payments`** — 선행입금 입금 순서 가드 (Q614) · **`/care/bathing-schedules`** — **「전월 일정 복사」** (`751c593`, Q606) · **`/billing/overdue`** — **「관리」** Modal (Q607) · **`/dashboard`·`/dashboard/hq`** — StatCard (Q594·Q595·Q603)
...
> **SideNav 준수 지표 라벨 (Q553, FE `acc5933`)**: 대시보드 StatCard와 구분하기 위해 SideNav 항목명에 **준수 지표 코드**가 붙습니다 — **운영 → 「모니터링 자가진단 (G30)」** · **기록 → 「급여제공결과 평가 (G39)」**. 화면 URL·기능은 **`/compliance/monitoring`**·**`/programs/provision-result-evaluations`** 그대로입니다.
...
> **직원 모듈 in-page 네비 (`StaffContextNav`, UXD-86·UXD-92)**: **직원 관리** · **출퇴근 (8-4 G-STAFF-WORK-ATTENDANCE)** · **교육일지 (8-7 G41)** · **보수교육 (8-7-1)** · **건강검진 (8-10)** · **직원현황 리포트 (8-12 US-R02)** · **고충상담 (8-8 G42)** · **선임 업무수행일지** 8화면 상단에서 **서브 탭**으로 전환합니다 (Q612·Q321·Q294·Q296·Q305·Q308·`02cbd05`). **출퇴근**은 **`/staff/attendance`** · **교육일지**는 **`/staff/training-logs`** · **직원현황 리포트**는 **`/staff/reports/status`** · **고충상담**은 **`/staff/grievance-counselings`** · **건강검진**은 **`/staff/health-checkups`** · **보수교육**은 **`/staff/training`** · **선임 업무수행일지**는 **`/staff/lead-caregiver-log`** 로 직접 진입합니다.
...
3. **통합 모니터링 checklist** 패널에서 선택 **연·월** 기준 **FAQ21838~21842 문항 6~15** 준수 현황을 확인합니다 (Q320). **증빙 수집 기간(FAQ21838 전전월 ±2개월)** 이 패널 상단에 표시됩니다 — 예: 2026년 6월 모니터링 → **2026-02-01 ~ 2026-06-30** (`MonitoringEvidenceWindow`, BE `73df04d` / FE `73094f9`). **충족·일부 충족·미충족·수동 점검** StatCard와 8행 표가 표시됩니다. **RFID(문항 10)** 는 **수동 점검(MANUAL)** 로만 표시됩니다.
4. **연·월** 선택 후 **조회**합니다. **`BranchSwitcher`** 로 선택한 지점 기준 데이터가 표시됩니다.
5. **6개월 자가진단 준수** StatCard — FAQ21842 **직전 6개월** 중 기록이 없는 월 수를 확인합니다.
6. **월별 자가진단 (15문항)** — **점검 문항** 선택 시 **점검방향·관련근거·점검기준·점검결과·점검방법** 5필드가 템플릿으로 채워집니다. **관련근거**는 FAQ21836 **고시·세부사항 조문** fallback이 기본값으로 들어갑니다 (`0da41c6`).
7. 등록된 행 **「수정」** 으로 같은 월·문항을 갱신할 수 있습니다. **문항당 월 1건**만 허용됩니다 (V100 UK).
8. **유선상담 (월 5명·60% 만족, FAQ21841)** — **유선상담 실시**·**만족(Y) 비율** StatCard로 **등록 건수·만족 비율**을 확인합니다 (`9ad8346`, Q365). 5명 미달·60% 미달 시 **warning Alert**가 표시됩니다. **추천 대상** 목록에서 이용자를 고른 뒤 **이용자·상담일·상담 내용·만족(Y/N)** 을 입력해 등록합니다. **이용자×월 1건** 제한 · **퇴소·비활성 이용자**는 등록 불가 (V101·V138).
9. **`caregiver`·`guardian`** 은 이 화면에 접근할 수 없습니다.
...
#### 잔여 갭 — 출석 QR·통계 (Q109·Q106)

| 증상 | 원인 | 조치 |
|------|------|------|
| QR **미리보기 빈 칸** | `qrDataUrl` 미반환 | Swagger·토큰 수동 입력 (Q109) |
| **`/attendance/stats` 숫자 비음** | FE `yearMonth`/`dailyRates`/`clientStats` vs BE `from`/`to`/`branches[].months[]` | FAQ **Q106·Q613** — Swagger 월간 집계 우회 |
| 귀가 **교통편 미저장** | FE `transportMethod` vs BE `transportType` | FAQ Q96 |
...
| `/attendance/stats` | **`AttendanceStatsPage`** + `MonthInput` + **`AttendanceRateChart`** + **`BranchScopeNotice`** (US-E05, UXD-48·53) | **`yearMonth` vs `from`/`to`·`dailyRates`/`clientStats` vs `branches[].months[]`** (Q106·Q613) |
| `/attendance/qr/generate` | **`QrGeneratePage`** — `generateBranchQrApi(branchId)`·**이미지 저장·인쇄**(US-E03) | **경로 Fixed** · payload·`qrDataUrl` **갭 잔존** (Q109 Partial) — Swagger **`direction`/`expiresInMinutes`** 우회 |
...
#### 월별 출석 통계 (`/attendance/stats`, US-E05, Q613)
...
3. StatCard **「총 이용자」「평균 출석률」「총 출석 일수」** 와 **일별 막대 차트**·**이용자별 표**가 표시되도록 설계되어 있습니다.

> **⚠ Must API 갭 (Q613)**: 현재 FE는 **`?yearMonth=`** 를 보내지만 BE는 **`?from=&to=&branchId=`** 만 인식합니다. 또한 BE는 **월별 지점 집계**(`activeClientCount`·`attendedDays`·`attendanceRate`)만 반환하고 **일별·이용자별 breakdown** 은 없어 화면이 **비거나 `—`** 로 보일 수 있습니다.

**현장 우회 (월간 출석률 확인)**:

| 단계 | 조치 |
|------|------|
| 1 | **당일·주간 운영** — **`/attendance`** roster (Q609) · **지점 대시보드** StatCard |
| 2 | **월간 집계** — Swagger **`GET /api/v1/attendance/stats/monthly?branchId=<uuid>&from=2026-06-01&to=2026-06-30`** |
| 3 | 응답 **`branches[0].months[]`** 에서 **`yearMonth`** 일치 row의 **`attendanceRate × 100`** = 출석률(%) |

| API 필드 | 의미 |
|----------|------|
| `activeClientCount` | 해당 월 활성 이용자 수 |
| `attendedDays` | 해당 월·지점 전체 출석 행 수 |
| `attendanceRate` | 0~1 (50% = `0.50`) |

> 관련: FAQ **Q106·Q613** · ADMIN_GUIDE §6-2-21

#### Must 갭 현장 대응 — QR (Q109)

| 구분 | 증상 | 즉시 조치 |
|------|------|----------|
| **생성** | QR **미리보기 빈 칸** | BE가 **`qrDataUrl` 미반환** — Swagger로 `qrToken` 발급 후 **토큰 수동 입력·인쇄** (FAQ Q109) |
| **생성** | **`400` 오류** | FE `{ type, validMinutes }` → Swagger **`{ "direction": "in", "expiresInMinutes": 480 }`** |
| **생성** | 버튼 비활성 | **BranchSwitcher**로 지점 선택 |
| **체크인** | 스캔 실패 | **`/guardian/checkin`** — 이용자 선택 + `qrToken` · `guardian_link_status=LINKED` 확인 (Q110) |

> coder 후속: `QrGeneratePage` payload·`qrDataUrl` 렌더링 정합.
...
| 화면 | UI | 백엔드 |
|------|-----|--------|
| 입금 처리 (`/billing/payments`) | **`BillingContextNav`** + **`PaymentPage`** — **이용자명**·월·검색·**`Pagination`** · **`PaymentRecordModal`** · **현금 수납 후 `CashReceiptRegisterModal`** (Q531) | **`POST /claims/{id}/payments` Fixed** · 목록은 **`GET /billing/claims` 클라이언트 필터** (전용 목록 API 미구현, Q174·Q198) |
...
2. **일일 출퇴근 로스터** 카드에서 **근무일**을 선택하고 **「조회」** 를 누릅니다 — **`fetchStaffWorkAttendanceApi`** (`services.js`)가 **`GET /api/v1/staff/work-attendance?date=&branchId=`** 를 호출합니다.
3. **「출근 방식」** 에서 **수동·모바일·NFC** 중 하나를 선택합니다 — 출근 시 **`checkInMethod`** 로 전송됩니다 (`5fd468b`).
4. 상단 StatCard에서 **출근·퇴근·미출근** 건수를 확인합니다.
5. 직원 목록 테이블에서 **직원명**(상세 링크)·**역할**·**상태 Badge**·**출근 방식**·**출근/퇴근 시각**을 확인합니다.
...
5. **「출근」** — **`staffWorkAttendanceCheckInApi`** → **`POST /api/v1/staff/work-attendance/check-in`** — `{ userId, checkInMethod }` — 선택한 **출근 방식**(`MANUAL`/`MOBILE`/`NFC`)이 전송됩니다.
6. **「퇴근」** — **`staffWorkAttendanceCheckOutApi`** → **`POST /api/v1/staff/work-attendance/check-out`** — `{ userId }`.
7. 처리 성공 시 목록이 **자동 refresh** 됩니다. 처리 중에는 버튼에 **Spinner**가 표시됩니다.
...
| API | 용도 |
|-----|------|
| `GET /api/v1/staff/work-attendance?date=&branchId=` | 일일 로스터 |
...
> 관련: FAQ **Q612·Q589** · ADMIN_GUIDE §6-2-20
