...
> **작성**: tech_writer 에이전트  
> **최초 작성일**: 2026-06-05  
> **최종 갱신**: 2026-06-15 (138차 자동 동기화 — **v1.3-B ✅ full · L03_M01/M06 △ BE · merge 555 FULLY UNBLOCKED** · BE `a4352a8`/FE `3845f0c` · Vitest 1275/1275 · `mvn test` 246/246 PASS)  
...
### 1-3. 구현 상태 안내 (2026-06-15 develop HEAD `a4352a8` / frontend `3845f0c` 기준)

| 영역 | 상태 | 비고 |
|------|------|------|
| 백엔드 API | **Must + … + L03_M01·L03_M11~M14·M13·M04 + v1.3-B suggest + G-NURSING + G17b + G21 + G2/7-5 + G41/G41b + G30 + G34-QUAL + J03 + 8-12 + G42 + #44 + G9-COG** | **L03_M01** V123 @ `9bd1660` · **v1.3-B** suggest @ `db94a65` · **G21** date guard @ `230659a` · **L03** V115–V119 |
| 데이터베이스 | Flyway **V1–V123** | **V120** transport v1.3-B · **V121** nursing integrity · **V122** transport integrity · **V123** nursing service records |
| 프론트엔드 | **77 라우트 · v1.3-B transport FE + L03_M11~M14·M13·M04 + G-NURSING + G17b + G21 + G2/7-5 + G41 + G30 + G34-QUAL + J03 + 8-12 + G42 · Vitest 1274/1274** | **`TransportSuggestPanel`**·**`BranchTransportSettingsPanel`** @ `2ffe59f` · **SideNav** manual expand @ `edfba7f` |
| UI 연동 완료 | **… + v1.3-B transport·L03_M13~M04·L03_M11·L03_M14·G-NURSING·G17b·G21·G42·US-UX-05·G2/7-5·G41·G30·J03·8-12·FAQ21824·G9-COG** | **Q340~Q348** · **P2**: L03_M01 FE wire · 나머지 L03 leaf · **7-5 live PG** |
| UI API 갭 | **L03_M01 간호급여 제공기록 FE·출석 roster·QR·통계·본인부담 비율 저장·보호자 식사 API·가정통신문·공단 포털 자동 제출** | L03_M01(Q348) · 출석 roster(Q94)·QR(Q109) |
| **P2 Planned** | **L03 간호급여 9 leaf·G-ONBOARD-SUPPORT·7-5 live PG·J03 Solapi live dispatch·건강검진 결과통보서·LCMS CMS 3-method·G34 SMS live·G-Payroll·G30 live E2E** | **G-STAFF-WELFARE P3**(FAQ21796) · **8-12 PDF 공식 서식**(Q315) |
| JWT 저장 | **메모리 전용** (SEC-005) | 새로고침 시 재로그인. **30분 idle** 시 SessionTimeoutModal (Q112) |
...
> **현재 UI 상태 (FE `edfba7f`)**: **77 라우트** · **`/transport`** — **v1.3-B 자동 배차 제안·배차 최적화 설정**(Q347, `2ffe59f`) · **`/nursing/vital-checks`** — **L03_M11 통합 바이탈**(Q340~Q342) · **`/nursing/weight-records`** — **L03_M14 체중 기록**(Q343~Q344) · **`/nursing/oral-care-checks`** — **L03_M13 구강상태**(Q345) · **`/nursing/emergency-records`** — **L03_M04 응급상황**(Q346) · **`/nursing/pressure-ulcer/*`** — **G-NURSING 욕창 케어 4단 lifecycle**(Q336~Q339) · **`/programs`** — **G17b**(Q333~Q334) · **`/visits`** — **G21 일괄확정**(Q330) · **SideNav** — **US-UX-05**(Q331, 수동 펼침 유지 @ `edfba7f`) · **`/billing/easy-pay`** — **7-5**(Q326~Q328) · **J03 quiet-hours**(Q329) · **`/staff/training-logs`** — **G41/G41b** · **`/compliance/monitoring`** — **G30** · **P2**: L03_M01 FE wire · 나머지 L03 leaf · **7-5 live PG**

> **로그아웃 방법**: AppShell **로그아웃** 버튼 · 브라우저 탭 종료 · 페이지 새로고침(토큰 소멸).
...
4. **페이지를 새로고침하면** JWT가 메모리에서 사라져 **다시 로그인**해야 합니다 (FAQ Q82). 공용 PC 사용 후에는 **로그아웃**하거나 브라우저를 닫으세요.
...
> **그룹 토글 (US-UX-05 Fixed, FE `1e111be`·`4ba7ea6`, Q331)**: **운영·이동·출석·기록·청구** 5개 그룹명을 클릭하면 하위 메뉴가 **접히거나 펼쳐집니다**.
> - **그룹 헤더 클릭** → 해당 그룹 하위 메뉴 표시/숨김 (`aria-expanded`·`aria-controls`)
> - **초기 상태**: 모든 그룹 **접혀 있음** — **현재 route가 속한 부모 그룹만** 자동 펼침 (예: `/clients` → **운영**만 펼침)
> - **route 변경 시** 활성 그룹 펼침 상태 **재계산** — 다른 그룹은 사용자가 펼친 상태 유지
> - **다중 펼침 허용**: 여러 그룹을 **동시에** 펼칠 수 있음 (한 그룹을 열어도 다른 그룹이 자동으로 닫히지 않음)
> - **모바일·PC 동일** · **`SideNav.test`** · FAQ **Q331**

| 그룹 | 주요 항목 | `branch_admin` | `hq_admin` | `staff` | `guardian` |
|------|----------|:--------------:|:----------:|:-------:|:----------:|
| **운영** | 대시보드·지점·이용자·**급여계획 통보 (G38)**·**모니터링 자가진단 (G30)**·**정기욕구평가 현황 (G40b)**·보호자·**직원** | ✅ | ✅ | ✅(G38·G30·G40b·보호자 제외) | 포털만 |

> **직원 모듈 in-page 네비 (`StaffContextNav`, UXD-86·UXD-92)**: **직원 관리** · **교육일지 (8-7 G41)** · **보수교육 (8-7-1)** · **건강검진 (8-10)** · **직원현황 리포트 (8-12 US-R02)** · **고충상담 (8-8 G42)** · **선임 업무수행일지** 7화면 상단에서 **서브 탭**으로 전환합니다 (Q321·Q294·Q296·Q305·Q308·`02cbd05`). **교육일지**는 **`/staff/training-logs`** · **직원현황 리포트**는 **`/staff/reports/status`** · **고충상담**은 **`/staff/grievance-counselings`** · **건강검진**은 **`/staff/health-checkups`** · **보수교육**은 **`/staff/training`** · **선임 업무수행일지**는 **`/staff/lead-caregiver-log`** 로 직접 진입합니다.
| **출석** | 현황·**탑승(차량)**·**현장 출석**·수기 체크인·통계·QR | ✅ | ✅ | ✅ | — |
| **기록** | 건강·**식사**·**프로그램**·**기능회복훈련**·**급여제공결과 평가**·**방문 일정**·**사례관리 회의록**·**통합 바이탈 (L03_M11)**·**체중 기록 (L03_M14)**·**구강상태 점검 (L03_M13)**·**응급상황 기록 (L03_M04)**·**욕창 케어 (US-O03)**·**선임 업무수행일지** | ✅ | ✅ | ✅(사례관리 제외) | — |
| **청구** | 청구·NHIS·입금·미납·**간편결제**·CMS·수가표 | ✅ | ✅ | — | — |
| **이동** | **배차** · **차량 관리** · **이동서비스비 청구** · **외출 관리** · **외출 리포트** (`/transport`, `/transport/vehicles`, `/transport/service-fees`, `/transport/outings`, `/reports/client-outings`) | ✅ | ✅ | ✅ | — |
...
> **이동 모듈 in-page 네비 (`TransportContextNav`)**: 배차·차량·이동서비스비 청구·외출 관리·외출 리포트 화면 상단에서 **서브 탭**으로 전환합니다. SideNav **이동** 그룹에도 **「이동서비스비 청구」**(`/transport/service-fees`)가 **직접 등록**되어 있습니다 (Q239, `a03a9f9`).
...
> **API 경로 갭**: 지점·플랫폼·수가표·설정·QR·등록 폼 등 — FAQ **Q151**. **Recharts 차트**는 UXD-48에서 **연동 Fixed**(Q118). 수기 출석 **버튼은 US-E01·E02에서 제공** — roster·status 갭은 Q94.
...
| **`BranchScopeNotice`** (Q173) | 출석·출석 통계·QR 생성·배차 화면 상단 **「조회 지점: ○○점」** — `role="status"` · `hq_admin`은 **BranchSwitcher** 전환 후 재조회 (UXD-53/54) |
...
| **`DateInput` 날짜 필드** (Q201, UXD-64) | 이용자 등록·수납일·기간 필터·**가산율 제공일(G11)**·**이동계약 서명일(G15)** 등 — **`Field` + `DateInput`** 로 너비·라벨 일관 (raw `<input type="date">` 금지, Q281) |
...
| **급여계약 (US-T10, G14)** | **「급여계약」** 탭 — **`ClientBenefitContractPanel`** — lifecycle 안내·**계약서 파일함** | **`GET/POST/DELETE …/benefit-contract-attachments*`** (Q285) |
...
| **건강·출석** | **「건강」·「출석」** 탭 — 요약·최근 이력 (상세 입력은 `/health`·`/attendance` 메뉴) | `GET /clients/{id}/health` · `GET /clients/{id}/attendance` |
...
#### 기초평가 탭 (US-T09, G24 Fixed · FAQ21800)

1. 이용자 상세에서 **「기초평가」** 탭을 선택합니다.
2. 상단 **`LifecycleWorkflowPanel`** 에서 연간 욕구사정 단계(가정방문·8항목 기록·이전 연도 비교)를 확인합니다.
3. **가정방문 일자**와 **8항목**(신체·인지·가족·경제·사회·서비스 필요·방문 기록·만족도)을 **`ClientNeedsAssessmentForm`** 에 입력합니다.
4. **저장** — `PUT /api/v1/clients/{clientId}/needs-assessments` 로 **회계연도별 upsert** 됩니다. 동일 연도 기록이 있으면 갱신됩니다.
5. **이전 회계연도 비교** — 전년도 기록이 있으면 **`ClientNeedsAssessmentCompare`** 가 항목별 변화를 표시합니다.
6. **회계연도·가정방문 일자** — FE **`4e4bdf6`·`479e064`** 에서 fiscal-year 파싱이 강화되어, 선택 연도와 가정방문 일자·비교 연도가 일관됩니다.
7. **가정방문 일자**는 해당 **회계연도(1/1~12/31)** 범위를 벗어나면 서버가 `422`(`가정방문 일자는 회계연도… 범위 내여야 합니다.`)로 거부합니다 (BE `b238779`).

> **권한**: **조회** — `hq_admin`·`branch_admin`·`social_worker`·`caregiver`. **작성·저장** — `branch_admin`·`social_worker`만. **P2 잔여**: FAQ21800 **연간 리포트 일괄 출력**·지표15 대시보드 자동 집계 (Q286).
...
| 영역 | 설명 |
|------|------|
| 운영 위젯 | 오늘 **입소**·**결석**·**미처리**·건강 알림·**미매칭 NHIS**·**NHIS 대기(보류)** 6종 + **`hq_admin`·`branch_admin`·`social_worker` 전용 「사례관리 30일 미반영」** (`0821ce8`, Q263) · **「사례관리 평가 미실시」**·**「사례관리 평가실시」** (`7f2289b`, Q266) · **「기능회복 제공·기록 미충족」**·**「급여시작 전 계획 미수립」**·상태 StatCard (`f1c60fe`, Q271) · **지표44 gap 4종** — **「주간 상태변화 미기록」**·**「월간 기록지 미제공」**·**「급여제공결과 평가 미등록」**·**「평가 30일 미반영」** + **상태 StatCard 4종** (`8e66ae8`, Q276) · **「급여 5개월 경과」**·**「급여 11개월 경과」**·**「이용계획서 재발급 미반영」** (`4b2b082`, Q277) · **「신규입소 위험도평가 미완료」** (`2f5af63`, Q301) · **「반기 기초평가 위험도 미완료」** (`22325f4`, Q302) + **`branch_admin`·`hq_admin` 전용 「미납 본인부담」** (US-M02·US-L02, Q202) |
| 건강 이상 알림 | `healthAlerts[]` 목록 — 이용자명·이상 유형·기록 시각 (**지점 Badge 없음** — 단일 지점 스코프) |
| API | `GET /api/v1/dashboard/branch` — **`nhisUnmatchedCount`·`pendingReviewCount`·`overdueCount`·`monthlyCapWarningCount`** (BE `15b3c7e`, Q282) · **G17/G32/G38/G39 준수지표 스냅샷 count·met 필드 전체** (BE `7ba18c1`, Q280) — **도메인별** 스냅샷 있으면 해당 compliance API **생략** · 누락 도메인만 **`Promise.all` 병렬 폴백** (`f72da41`) · NHIS·미납·월한도는 현재 **병렬 API 조회** (Q85) |
...
#### 이용자 목록·상세 (BNK-19 + BE `a49e496`)

1. **이용자** 메뉴 → 목록(`/clients`) — `GET /api/v1/clients` — **8열 Table**: 이름·나이·성별·지역·등급·보호자·상태·등록일 (Q191).
2. 상단 **검색**(`q`) — 이용자명·**지역**·**보호자명**·성별 필터.
3. 이름 링크 → **이용자 상세**(`/clients/:id`) — 기본 정보·**보호자 초대**(§3-3).
...
| 기본 픽업 시각 | `defaultPickupTime` | `type="time"` — 배차 명단 기본 시각 |
...
2. 상단 **통합 모니터링 checklist** 패널에서 선택 **연·월** 기준 **FAQ21838~21842 문항 6~15** 준수 현황을 확인합니다 (Q320). **충족·일부 충족·미충족·수동 점검** StatCard와 8행 표가 표시됩니다. **RFID(문항 10)** 는 **수동 점검(MANUAL)** 로만 표시됩니다.
3. **연·월** 선택 후 **조회**합니다. **`BranchSwitcher`** 로 선택한 지점 기준 데이터가 표시됩니다.
4. **6개월 자가진단 준수** StatCard — FAQ21842 **직전 6개월** 중 기록이 없는 월 수를 확인합니다.
5. **월별 자가진단 (15문항)** — **점검 문항** 선택 시 **점검방향·관련근거·점검기준·점검결과·점검방법** 5필드가 템플릿으로 채워집니다. **관련근거**는 FAQ21836 **고시·세부사항 조문** fallback이 기본값으로 들어갑니다 (`0da41c6`).
6. 등록된 행 **「수정」** 으로 같은 월·문항을 갱신할 수 있습니다. **문항당 월 1건**만 허용됩니다 (V100 UK).
7. **유선상담 (월 5명)** — **유선상담 준수** StatCard와 **추천 대상** 목록을 확인합니다. **이용자·상담일·상담 내용**을 입력해 등록합니다. **이용자×월 1건** 제한 · **퇴소·비활성 이용자**는 등록 불가 (V101).
8. **`caregiver`·`guardian`** 은 이 화면에 접근할 수 없습니다.
...
**FE `6c4c151` + UXD-73 `420b4d7` (US-E01·E02 + G15 transportMode + UXD-53)** — 당일 출석 **목록 + 수기 처리 UI**가 제공됩니다. 화면 상단 **`AttendanceContextNav`** 로 출석 현황·탑승·현장·수기 체크인·통계·QR을 전환합니다 (Q242). 카드 상단에 **`BranchScopeNotice`** 로 **조회 지점**이 표시됩니다 (Q173).
...
| 작업 | UI | API | 비고 |
|------|-----|-----|------|
| 목록·요약 | ✅ StatCard(입소·귀가·결석·미처리) + 테이블 | `GET /attendance?transportMode=` | **`clientName`·`status` 미제공** 시 UUID·버튼 미노출 (Q94) |
| 체크인 | ✅ **입소** 버튼 | `POST /attendance/check-in` | ✅ |
| 체크아웃 | ✅ **귀가** → `CheckoutModal`(교통편) | `POST /attendance/check-out` | **`transportMethod` vs `transportType`** (Q96) |
| 결석 | ✅ **결석** → `AttendanceAbsentModal` | `POST /attendance/absence` | ✅ |

> **roster 갭 (Q94)**: API는 **당일 출석 기록이 있는 행만** 반환하고, **`clientName`·`status` 필드가 없어** UI에서 이름·버튼이 비활성일 수 있습니다. **미처리 이용자**는 목록에 아예 없을 수 있습니다.

**Swagger 우회 — 파일럿 수기 출석 (Must)**

1. **지점 선택기**로 작업 지점을 확인합니다.
2. `/clients`에서 이용자 UUID를 확인합니다.
3. Swagger **Authorize**에 `caregiver` 또는 `branch_admin` JWT를 입력합니다.
4. **입소**: `POST /api/v1/attendance/check-in` — `{ "clientId": "<uuid>" }`
5. **귀가**: `POST /api/v1/attendance/check-out` — `{ "clientId": "<uuid>", "transportType": "SELF" }` (Q96)
6. **결석**: `POST /api/v1/attendance/absence` — `{ "clientId": "<uuid>", "reason": "개인사정" }`

> 화면 버튼이 보이지 않아도 위 API가 **정식 Must 출석** 경로입니다. 상세는 FAQ **Q94**.
...
| `/attendance/stats` | **`AttendanceStatsPage`** + `MonthInput` + **`AttendanceRateChart`** + **`BranchScopeNotice`** (US-E05, UXD-48·53) | `yearMonth` vs 백엔드 `from`/`to`·`dailyRates` **불일치** (Q106) |
| `/attendance/qr/generate` | **`QrGeneratePage`** — `generateBranchQrApi(branchId)`·**이미지 저장·인쇄**(US-E03) | **경로 Fixed** · payload·`qrDataUrl` **갭 잔존** (Q109 Partial) |
| `/attendance/checkin/qr` | **`GuardianCheckinPage`** — `checkin-targets`·`scanQrAttendanceApi`·입소/귀가 `FilterChips` (US-E04) | **Fixed** (Q109) — 카메라 스캔 **준비 중** |

### 4-5. 건강 기록 (`/health`)

**FE `95b92b9` (UXD-39·UXD-40·UXD-41, US-F01·F02·F03)** — 이용자 선택 후 **일일 건강·투약·낙상·특이사항 입력·저장 UI**가 제공됩니다 (파일럿 P3).

1. **기록 → 건강 기록**(`/health`)으로 이동합니다.
2. **이용자** 드롭다운에서 대상을 선택합니다 (`GET /clients`).
3. 비정상 수치가 있으면 **`HealthAbnormalBanner`**가 표시됩니다.
4. 탭으로 기록 유형을 선택합니다.
...
| 화면 | UI | 백엔드 |
|------|-----|--------|
| 입금 처리 (`/billing/payments`) | **`BillingContextNav`** + **`PaymentPage`** — **이용자명**·월·검색·**`Pagination`** · **`PaymentRecordModal`** | **`POST /claims/{id}/payments` Fixed** · 목록은 **`GET /billing/claims` 클라이언트 필터** (전용 목록 API 미구현, Q174·Q198) |
| 미납 관리 (`/billing/overdue`) | **`BillingContextNav`** + **`OverdueSummaryBar`** + 보호자 **`MaskedPhone`** + **「안내 발송」** + **`Pagination`** | **`GET /billing/overdue?page&size&q=` Fixed** (Q197) · **`POST /claims/{id}/notify` Fixed** (Q196) |
...
| 청구 상세 (`/billing/claims/:id`) | **`BillingDetailPage`** — **`DRAFT`** 시 **「공단 명세 비교」** (`BillingNhisComparisonPanel`, Q264) · **「보호자 발송」** (`CONFIRMED`/`PAID`) · **「납부확인서 발송」** (`PAID`+`paidAt`+`paymentMethod`, Q221·Q249) · **「환불 처리」** (`PAID`+`paidAt`, Q261) | **`GET /claims/{id}/nhis-comparison`** · **`POST /claims/{id}/notify`** · **`POST /claims/{id}/payment-receipt-notify`** · **`POST /claims/{id}/refunds`** (G2·US-M03 7-9) |
...
1. **청구 → 입금 처리**로 이동합니다.
2. **기준 월**·이용자 검색 후 **조회**합니다. 표에 **이용자명**·청구액·본인부담금·수납 상태가 표시됩니다 (Q198). *(전용 목록 API 없음 — 확정·수납 청구를 claims에서 필터, Q174)*
...
**은행 거래내역 엑셀 일괄입금 (케어포 7-2, US-L01 — Q227)**

> **UI Fixed (`f4bb171`)** — **`/billing/payments`** 하단 **`BankDepositImportPanel`** 이 JWT **활성 지점(`activeBranchId`)** 을 **`branchId`** 로 전송합니다. **다지점 `hq_admin`** 은 업로드 전 **BranchSwitcher**로 작업 지점을 선택하세요 (Q228).
...
4. 은행 인터넷뱅킹에서 받은 **입금 내역 xlsx**(.xlsx)를 선택합니다.
   - **「지원 은행 8종 엑셀 형식 (케어포 7-2 p.88)」** 을 펼치면 KB국민·우리·NH농협·신한·하나·부산·대구·광주 **컬럼 예시**를 확인할 수 있습니다 (Q258).
   - 각 은행 행의 **「샘플」** 버튼으로 **최소 컬럼 레이아웃** `.xlsx`를 내려받아 형식을 확인할 수 있습니다 (`b9845ac`, Q258).
5. **「엑셀 일괄 등록」**을 클릭합니다. 성공 시 **적용·미매칭·건너뜀 건수**(`appliedCount` 등)가 표시되고 수납 목록이 갱신됩니다.
6. (IT 담당) Swagger: `POST /api/v1/billing/imports/bank-deposits` — `multipart/form-data` — **`branchId`** + `file` + `yearMonth`.
7. 응답 `rows[]`에서 행별 **`status`** 를 확인합니다.
...
> **조용한 시간대 (Q329, `111f056`)**: **22:00~08:00 (Asia/Seoul)** 에는 **「보호자 발송」·「안내 발송」·「납부확인서 발송」** 버튼이 **비활성**됩니다. BE가 non-emergency 알림을 skip하므로 UI에서도 사전 안내합니다. **긴급(`EMERGENCY`)** 알림(예: 낙상)은 **우회**됩니다 (Q147). **08:00 이후** 재시도하세요.
...
**CMS 등록 절차 (UI, FE `9a6fdb6`)**

1. **청구 → CMS 자동이체**로 이동합니다.
2. **등록 관리** 탭에서 **이용자**를 선택합니다.
3. **연결 보호자**·**예금주명**·**은행**·**계좌번호 끝 4자리**를 입력합니다 — ogada는 **전체 계좌번호를 저장하지 않습니다** (Q207).
4. **CMS 자동이체 등록**을 클릭합니다. 성공 시 하단 **등록 이력**에 **등록완료** Badge가 표시됩니다.
5. 보호자 **서면 자동이체 동의서**는 센터가 별도 보관합니다.
6. 동일 이용자에 **다른 보호자**로 이미 **ACTIVE** 등록이 있으면 **거부**됩니다 — 기존 등록을 **해지**한 뒤 재등록하세요 (Q299).
...
> **provider 값**: API는 **`CARD`·`KAKAO_PAY`** 를 기대합니다. **허용 alias** — 대소문자·공백·하이픈(`" card "`, `"kakao pay"`, `"kakao-pay"`, `"KAKAOPAY"`, 전각 `ｋａｋａｏ pay`) → canonical 저장. **거부** — `card1`·`kakao/pay`·`카카오페이` 등 (Q328, `745a2f6`·`7ec7cd4`·`82054f1`). FE **`normalizeEasyPayProvider`** 가 전송 전 정규화·검증합니다.
...
청구 **상세**(`/billing/claims/:claimId`) — **`BillingDetailPage`** — **`DRAFT`** 시 **「공단 명세 비교」** (`BillingNhisComparisonPanel`, Q264) · 요약·급여 항목에 **이용시간대** 표시 — **`resolveClaimDurationBand`** 가 **라인 `durationBandSnapshot` → 청구 헤더 밴드** 순으로 해석 (FE `eb3f0fd`, Q213) · 금액은 **`formatWon`** 으로 **콤마·문자열 JSON**도 안전 표시 (Q220) · 확정·수납 시 **「보호자 발송」**(Q196)·상태 변경·인쇄.
...
> **UI (FE `c30aaac`·`42f48e1`·`190b2b1`·`1220bfb`·`2b6024a`·`0abf164`, BE `9a97a1c`·`970c7af`·`8f208e4`)**: `/billing/imports/nhis` — 월·파일 업로드 + **`NhisImportGuidePanel`**(Chrome/Edge·4단계 + **`GET /billing/imports/nhis/guidance` 실서버 안내**, QA-B24, Q133) + **`NhisScheduleConfirmLockGuide`** + **`FeeScheduleYearGuardBanner`**(US-G04, Q260) + **`fetchFeeScheduleYearCoverageApi`** 서버 사전 점검 + 배치 목록·대사 (`ReconciliationPage`). guidance API 일시 실패 시 **기존 실서버 안내 문구 유지** (`Promise.allSettled`).
...
0-1. **수가표 연도 가드 (US-G04, Q260)**: 선택 **청구 월의 연도**에 **등급(1~5)×이용시간대 25칸** 수가표가 모두 등록되지 않으면 **`FeeScheduleYearGuardBanner`** warning이 표시되고 **업로드가 비활성**됩니다. **인지지원등급(ltcGrade 0) 5칸**은 **별도 집계**(`cognitiveRegistered`/`cognitiveComplete`, Q311) — **NHIS import gate는 표준 25칸만** 검사합니다. 화면은 **`GET /billing/fee-schedules/year-coverage?year=`** 로 서버 검증 결과를 우선 표시하며, API **로딩 중·오류** 시에도 업로드를 차단합니다. **「수가표 등록」** 링크(`/billing/fee-schedules`)에서 **「공단 2026 수가 시드」** 또는 셀별 등록 후 재시도하세요. **`POST /billing/imports/nhis`** 도 동일 규칙으로 **`422`**를 반환합니다 (`970c7af`·`8f208e4`).
...
센터장·통합 관리자는 SideNav **운영 → 직원 관리**(`/staff`)에서 소속 지점 **직원 계정 조회·생성**이 가능합니다 (FE `73f7d39`, FAQ **Q162**).

1. **직원 관리** 화면에서 등록된 직원 목록(이름·이메일·역할·소속 지점)을 확인합니다.
2. **+ 직원 등록**을 눌러 모달에서 **이름**·**이메일**·**역할**(`StaffRoleSelect` — 요양보호사·사회복지사·지점장·통합 관리자)을 입력합니다. **소속 지점 ID**는 선택 입력(미입력 시 현재 활성 지점)입니다.
3. 이름·이메일 미입력 시 **필드 아래 오류**가 표시됩니다(UXD-47). 저장 중에는 버튼에 **저장 중** 스피너가 표시됩니다.
4. 퇴직 처리·역할 변경은 **아직 UI 없음** — Swagger `PATCH /api/v1/users/{id}` 사용 (ADMIN_GUIDE §6-4).

| API | 용도 |
|-----|------|
| `GET /api/v1/users?page=0&size=50` | 직원 목록 |
| `POST /api/v1/users` | 신규 직원 등록 (`hq_admin`, `branch_admin`) |

> **API 필드 갭**: UI는 `name`·`role`을 전송하나 백엔드는 **`displayName`·`roleCode`** 를 기대합니다 — 목록에 `-`가 보이거나 등록이 실패하면 Swagger 본문을 사용하세요 (Q162). **`platform_admin` 역할은 생성 불가**합니다.
...
케어포·엔젤 패리티 **보호자 서류 5종** 중 **노인학대예방 지침**은 이용자 상세 UI에서, **급여제공기록지**는 **조회 UI Fixed**(§3-3, Q243)·**발송은 Swagger**, **가정통신문**은 Swagger에서 수동 발송합니다 (FAQ **Q216–Q217·Q222**). **납부확인서**는 청구 상세 **「납부확인서 발송」**(Q221).

| 서류 | 화면 | API | 템플릿 |
|------|------|-----|--------|
| 급여제공기록지 | **「급여제공」탭 조회 Fixed** · **발송 Swagger** (Q216) | `GET …/care-provision-records/{yearMonth}` · `POST …/notifications/care-provision-record` | `CARE_PROVISION_RECORD` |
| 가정통신문 | **Swagger** (UI 후속) | `POST /api/v1/clients/{clientId}/notifications/home-newsletter` | `HOME_NEWSLETTER` |
| **노인학대예방 및 대응지침** | **`/clients/:id` → 기본정보** — **「노인학대예방 지침 발송」** 카드 · **`GuardianDocumentNotifyPanel`** | `POST /api/v1/clients/{clientId}/notifications/elder-abuse-prevention-guideline` | `ELDER_ABUSE_PREVENTION_GUIDELINE` |
...
**노인학대예방 지침 발송 절차 (UI, FE `eedcc80`)**

1. **이용자 상세**(`/clients/:id`) → **기본정보** 탭 하단 **「노인학대예방 지침 발송 (G2)」** 카드를 엽니다.
2. **대상 연월**(`MonthInput`)을 선택하고, 필요 시 **요약**(최대 500자)을 입력합니다.
3. **지침 이메일 발송**을 클릭합니다 — 성공 시 초록색 안내가 표시됩니다.
4. 보호자 **`channelEmail=true`** · **`users.email`** 등록이 필요합니다 (Q204). 동의는 **`notify_daily_care`** preference를 재사용합니다 (Q222).

**급여제공기록지·가정통신문 발송 절차 (Swagger)**

1. 이용자 상세에서 **이용자 UUID**를 확인합니다.
2. Swagger **Authorize**에 `branch_admin` 또는 `social_worker` JWT를 입력합니다.
3. 연결 보호자 preference **`channelEmail=true`** · 보호자 **`users.email`** 등록을 확인합니다 (Q204).
4. 위 API 중 하나를 호출합니다 — 성공 시 `notifications` 이력에 **`channel=email`** 1건이 추가됩니다.
5. **stub** 환경에서는 실제 메일 없이 DB 이력만 생성됩니다. **live SMTP** — DEPLOYMENT §4-8.
...
| 영역 | 설명 |
|------|------|
| 전 지점 출석 카드 | 지점별 오늘 입소·결석·퇴소 요약 + **NHIS 대기(보류)**·미매칭·**미납 본인부담** 집계 (US-M02-b·US-L02, Q183·Q202) |
...
**활용 팁**: 아침에 통합 대시보드로 2지점 이상 현황을 한눈에 확인한 뒤, 건강 이상 Badge의 **지점명**으로 우선순위를 정하고, 상세 작업이 필요한 지점을 **지점 선택기**로 전환하세요.

> **현재 UI 상태 (US-M02 + BNK-19, Q177·Q183·Q202·Q280·Q282)**: `/dashboard/hq`가 **`GET /api/v1/dashboard/hq`**·**`/dashboard/hq/alerts`** 와 연동되어 전 지점 `todayAttendance`·건강 알림 집계를 표시합니다. **G17/G32/G38/G39 준수지표 위젯**은 **`/dashboard/hq` 스냅샷**에서 **도메인별 우선** 로드됩니다 (FE `8fa9f3d`). BE `15b3c7e`는 **전 지점 NHIS·미납·월한도** 집계를 스냅샷에 포함하나, FE는 현재 NHIS 배치·미납·월한도 API **병렬 조회**를 유지합니다 (`hq_admin`만 미납 위젯). **`BranchCompareChart`**·건강 이상 **`branchName` Badge** (Q167).
...
5. **`INTEGRATED_HOME`(통합재가)** 선택 시 **warning 안내**가 표시됩니다 — **주·야간보호형 월 가산(100,000원/인, 고시 제55조의2)** 은 v1에서 **자동 청구·정산하지 않음** (G19, Q223).
...
2. 목록 **상단** **「입사 처리 compliance 현황」** 패널(`StaffOnboardingCompliancePanel`, Q300)에서 **대상·완료·진행 중·기한 초과** StatCard와 **FAQ21806 6단계 workflow**를 확인합니다. **기한 초과** 직원이 있으면 warning Alert가 표시됩니다 — 표에서 **이름**을 눌러 상세로 이동해 **HR 파일함**·**입사~퇴사** 탭에서 서류를 보완합니다.
3. **+ 직원 등록**으로 요양보호사(`caregiver`)·사회복지사(`social_worker`)·센터장(`branch_admin`) 등 **역할 코드**와 **소속 지점**을 지정합니다.
4. `platform_admin` 역할은 생성할 수 없습니다.
5. **이메일은 Tenant 내에서 대소문자 구분 없이 유일**해야 합니다 (V30, FAQ Q146).
6. 목록 **「lifecycle」** 열에서 **입사 진행·재직·퇴사 진행·퇴사 완료** `StatusBadge`로 진행 상태를 확인합니다 (`b3e59e2`, Q290).
7. 상단 **lifecycle 상태** `Select`로 목록을 **입사 진행·재직·퇴사 진행·퇴사 완료**별로 필터링할 수 있습니다 (`37dc785`).
8. 목록에서 직원 **이름** 또는 **「상세」** 링크를 눌러 **`/staff/{userId}`** 직원 상세로 이동합니다 (US-R03, `e7c289e`). 상세 **기본정보** 탭에도 동일 Badge가 표시됩니다.

> **Swagger 대체**: UI 등록 실패 시 `POST /api/v1/users` — `{ "email", "displayName", "roleCode", "branchIds": ["<uuid>"] }` (Q162).
...
3. **진행 상태**를 선택합니다 — `입사 진행` · `재직` · `퇴사 진행` · `퇴사 완료`.
4. **입사일**·**퇴사일**(해당 시)·**근로계약서 서명일**을 입력합니다 — 서명일은 **`DateInput`** 표준 컴포넌트를 사용합니다 (`b3e59e2`).
5. 단계별 체크리스트를 표시합니다.
...
6. **입사 완료**·**신고 완료** 토글을 필요에 따라 켭니다.
7. **저장** — `PATCH /api/v1/users/{userId}` 로 서버에 반영됩니다. 저장 중 패널에 **`aria-busy="true"`** 가 설정되어 스크린리더에 진행 상태가 전달됩니다 (`b3e59e2`).
...
> **P2 잔여**: 급여명세·퇴직금 **G-Payroll 연동**, G34 수준 **전자서명 잠금**, **건강검진 결과통보서** 별도 파일함, **대시보드 입사 compliance 위젯** — FAQ **Q290·Q296·Q298·Q300** 참고.
...
3. **직원 일반 파일함 (US-R03)** — 문서 유형을 고른 뒤 **PDF/PNG/JPEG(≤10MB)** 파일을 업로드합니다.
...
| 자격증 | `certificate` | |
| 통장 사본 | `bank-account` | |
| 건강검진 결과 | `health-check` | checklist 연동 — **결과통보서 전용 파일함은 P2**(Q296) |
| 치매교육 이수 | `dementia-training` | |
| 근로계약서 | `employment-contract` | 서명일은 **「입사~퇴사」** 탭에서 별도 입력 |
| 범죄경력조회 | `criminal-record` | |
| 사직서 | `resignation` | 퇴사 ④ |

4. **같은 문서 유형**을 다시 업로드하면 **기존 파일을 교체**합니다(유형당 1건).
5. 목록에서 **미리보기**·**삭제**할 수 있습니다. 삭제 시 해당 checklist 항목이 **false**로 되돌아갑니다.
6. 하단 **보수교육 이수증**·**건강검진 이력** 패널에서 직원별 증빙·기록을 관리합니다 — 목록 화면(`/staff/training`·`/staff/health-checkups`)과 **동일 API**입니다.
...
7. **권한**: **`hq_admin`·`branch_admin`·`social_worker`** 가 조회·미리보기 가능합니다. **업로드·삭제**는 **`branch_admin`·`social_worker`만** (`51477bd`). 대상 역할은 **요양보호사·사회복지사·지점장**입니다.
...
> **P2 잔여**: **교육기관 API 연동** · **대시보드 위젯** — FAQ **Q294·Q295** 참고.
...
2. **연도·교육 유형** 필터로 목록을 좁힙니다. G41b **재난·소화·직원권익** 3종도 필터·등록 가능합니다 (V106).
3. 상단 **StatCard**에서 **노인인권 상·하반기** · **운영규정 연간** · **G41b 3종 연간** · **신규직원 7일 오리엔테이션** compliance를 확인합니다 (`GET …/compliance`, BNK-185 `0f11158`). G41b StatCard는 **BE compliance API 우선** · API 필드 누락 시에만 목록 건수 fallback (`38d24b6`).
4. **「교육일지 등록」** — Modal에서 **교육 유형·교육일·방법·강사·참석자·교육내용**을 입력합니다.
...
5. 목록 **「수정」** — 동일 Modal에서 **PATCH** 저장합니다.
6. **신규직원 오리엔테이션 현황** 표(해당 연도·**지점 스코프** 신규 채용자)에서 **기한·충족 여부**를 확인합니다. 동일 직원에 여러 오리엔테이션 기록이 있으면 **가장 이른 교육일** 기준으로 집계됩니다 (BNK-187).
7. **권한**: **`hq_admin`·`branch_admin`·`social_worker`** 가 조회·등록·수정 가능합니다 (`6191b91`).
...
> **P2 잔여**: **대시보드 위젯** · **인쇄** · **G-STAFF-WELFARE(8-6 포상)** — FAQ **Q321·Q325** 참고.

#### 직원 건강검진 (8-10, US-R02, FAQ21799, Q296)

케어포 **8-10** 「건강검진관리」·이지케어 FAQ21799 **지표9** — 직원 **건강검진 실시일**과 **5개 영역** 이수 여부를 관리합니다.

1. SideNav **운영 → 직원**(`/staff`) 진입 후 **`StaffContextNav`** **「건강검진 (8-10)」** 또는 URL **`/staff/health-checkups`** 으로 이동합니다 (`604787f`).
2. 상단 **StatCard**에서 **대상 직원·검진 완료·검진 필요** 건수를 확인합니다. **검진 필요** 직원이 있으면 warning **`Alert`** 가 표시됩니다.
3. **`LifecycleWorkflowPanel`** — 건강검진 **2단계 lifecycle**(주기 확인 → 실시 기록)을 확인합니다.
4. 목록에서 **직원·역할·최근 검진일·다음 검진 기준·주기·5영역 진행·상태 Badge**를 확인합니다.
5. **「검진 기록」** — Modal에서 **실시일**·**5개 영역 Checkbox**(신체계측·요검사·혈액·영상·결과판정)·**사무직 여부**·**소견**을 입력합니다. **최소 1개 영역** 체크가 필요합니다.
6. **「이력」** — 해당 직원의 과거 검진 기록 목록을 조회합니다.
7. **주기**: **일반 직원 1년** · **사무직 2년** (입사일·최근 검진일 기준).
8. **권한**: **`hq_admin`·`branch_admin`·`social_worker`** 가 조회·이력 확인 가능합니다. **기록 등록**은 **`branch_admin`·`social_worker`만** (`f1268c6`).
...
> **P2 잔여**: **결과통보서 PDF·이미지 파일함 업로드**(FAQ21799·FAQ21806) · **대시보드 위젯** — FAQ **Q296** 참고.
...
5. **출력물 7종** — 케어포 PDF p.106 대응 **명부·사진게시·연락처·월간명부·입퇴사**는 **인쇄**, **엑셀**은 **서버 CSV 다운로드**합니다 (`exportStaffStatusReportCsvApi`, Q315). **인쇄 레이아웃 가독성**은 `e3e6964`에서 개선되었습니다 — 표·제목 간격·페이지 나눔을 확인한 뒤 출력하세요. 연락처·입퇴사·사진게시·인쇄 5종은 **`GET /users`를 페이지 단위로 순회**(size=200, 최대 50페이지)해 **전 직원 프로필**을 불러온 뒤 enrich합니다 (`ff173af`). 인쇄 전 행 데이터가 안정화됩니다 (`5bba7a2`).
...
> **`/settings` vs `/organization/settings` (Q178·Q201)**: **`sysadmin`** — SideNav **시스템 설정**(`/settings`) — **4탭**(보안·로그인 이력·감사·백업) · **보안** 탭 **「비밀번호 변경」** → **`PasswordChangeModal`** (`POST /auth/change-password`, Q122). **`hq_admin`** — SideNav **조직 설정**(`/organization/settings`) — 셀프 체크인·**청구 생성 기준**. `hq_admin`이 `/settings`에 접근하면 **권한 없음**(`/forbidden`)입니다.
...
센터장·통합 관리자는 보호자 셀프 체크인용 **입소·귀가 QR**을 발급·게시합니다 (US-E03).
...
3. **입소** 또는 **귀가** 유형과 **유효 시간(분)**을 선택합니다.
4. **생성**을 누르면 QR 이미지가 표시됩니다.
5. **이미지 저장** 또는 **인쇄**로 지점 입구에 게시합니다.
6. 유효 시간이 지나면 새 QR을 생성해 교체합니다.

> **API 갭 (Q109)**: UI는 `POST /branches/current/qr`·`{ type, validMinutes }`·`qrDataUrl`을 사용하나, 백엔드는 `POST /branches/{branchId}/qr`·`{ direction, expiresInMinutes }`·`qrToken`을 기대합니다. **QR 이미지 미리보기·저장·인쇄**가 비활성일 수 있습니다.

**Swagger 우회 — QR 발급 (Must US-E03)**

1. JWT `active_branch_id` 또는 Branch Switcher로 **지점 UUID**를 확인합니다.
2. Swagger **Authorize**에 `branch_admin` 또는 `hq_admin` JWT를 입력합니다.
3. **입소 QR**:
...
4. **귀가 QR** — `"direction": "out"` 으로 동일 호출.
5. 응답 **`qrToken`** 을 복사합니다 — 백엔드는 **이미지 URL을 반환하지 않습니다**.
6. 보호자는 `/guardian/checkin` **토큰 입력란**에 붙여 넣거나, `POST /api/v1/attendance/qr/scan` — `{ "qrToken": "...", "clientId": "<uuid>" }` 로 처리합니다 (카메라 스캔 UI **미구현**).

> 체크인 대상 목록이 빈 라벨이면 `GET /guardian/checkin-targets` 응답 필드(`clientId`·`clientName`) 불일치입니다 — FAQ **Q109**.

### 5-8. 배차·이동경로 (`/transport`, v1.3-A)

통합 관리자·직원이 **픽업 배차 명단·운행 루트·차량**을 관리하는 화면입니다 (BE `53a1ffe` + FE `7ef1083`, US-T01~T03). **백엔드 `/api/v1/transport/*` API가 구현**되어 FE와 연동됩니다 (FAQ **Q159**). 화면 상단 **`TransportContextNav`** 로 **배차·차량·이동서비스비 청구·외출 관리·외출 리포트**를 전환합니다.

| 화면 | 경로 | 역할 | 조작 |
|------|------|------|------|
| 배차 홈 | `/transport` | 직원 4역할 | **운행일** 선택 → 당일 픽업 명단·운행 루트 목록 |
| **차량 관리** | `/transport/vehicles` | 조회: 직원 4역할 · 등록/수정: `hq_admin`·`branch_admin` | §5-8-3 (G16, Q241) |
...
| 새 픽업 배차 | `/transport/runs/new` | **`hq_admin` only** | 명단 **다중 선택** → **차량 선택** → 수동 순서 편집 → **지도** 탭 미리보기 → **임시 저장(DRAFT)** |
| 루트 상세 | `/transport/runs/:runId` | `hq_admin`(편집·확정 취소) · 직원(**확정 루트만** 조회) | 정차 목록·지도·**차량**·**배차 확정**·**확정 취소** |

1. SideNav **이동 → 배차·이동경로**로 이동합니다 (또는 **`TransportContextNav`** 탭).
2. **`BranchScopeNotice`** 로 **조회 지점**을 확인합니다 (Q173).
3. 상단 **`TransportDisclaimer`** 를 읽습니다 — **운영 편의용**이며 이동서비스비 청구(G15)는 포함되지 않습니다. **v1.3-B 자동 배차 제안**은 아래 카드에서 실행합니다 (Q347, FE `2ffe59f`).
4. **`hq_admin` only — 배차 최적화 설정 (v1.3-B)`** — **`BranchTransportSettingsPanel`**:
   - **픽업 시간 허용 오차**(분, 기본 15) · **담당 안정성(w1)** · **거리 공정성(w2)** · **총 거리(w3)** 가중치(0~1)
   - **저장** — `PUT /api/v1/transport/settings` · `Field` 단위 서버 오류 표시
5. **`hq_admin` only — 자동 배차 제안 (v1.3-B)`** — **`TransportSuggestPanel`**:
   - **「자동 배차 제안」** — `POST /api/v1/transport/runs/suggest` · 당일 **PICKUP** roster·활성 차량 기준 **DRAFT 루트** 생성
   - **일 10회** 상한 표시 — 초과 시 버튼 비활성 · roster 비어 있으면 안내
   - 제안 요약(생성 루트 수·담당 변경·거리 편차) · **DRAFT 검토** 링크 → `/transport/runs/{runId}`
   - **기존 DRAFT 루트는 새 제안 시 대체** — 확정 전 검토·수정 필수
6. **`TransportCompliancePanel`** (G15, Q230·Q231) — 2026 평가 지표 **41·42** 대응 **이동서비스 수칙 체크리스트**(5항목) · **계약서 템플릿** Modal. **이동서비스 이용(`usesTransport=true`)·활성·미퇴소** 이용자만 저장 가능 (V65, Q235). **이용자 선택** → 5항목 체크 → **보호자·기관 서명**·**서명일(`DateInput`, Q281)** 입력 → **저장**(`PUT /api/v1/transport/contracts/{clientId}`). **`hq_admin`·`branch_admin`·`social_worker`** 만 저장 가능 · **`caregiver`** 는 **조회만**.
7. **`TransportForm18GuidePanel`** (G15, Q237, FE `fcf713a`) — **별지 제18·19·20호** 공단 포털 **선행 신청 5단계** 안내 + **3분리 신청 유형**(적용·변경·중단) 표 + **등록상태 4단**(작성중·진행중·적용·적용불가). ogada는 **자동 제출하지 않습니다** — 수칙·계약(Q231) → **제22호 일지**(Q236) → **공단 포털 제18호** → **제19·20호 변경·통보** → **월간 대조·G16 청구 생성**(Q239) 순서를 따르세요.
8. **`hq_admin`** 은 **새 픽업 배차**로 명단을 선택합니다. **`TransportVehicleSelect`** 로 **지점 차량**을 지정합니다 — 정차 수가 차량 **정원(capacity)** 을 초과하면 경고가 표시됩니다 (Q241). 차량이 없으면 §5-8-3에서 먼저 등록하세요.
9. 정차는 **최대 15명**(`MAX_TRANSPORT_STOPS`)까지입니다.
10. **지도 제외 주소**(geocode `FAILED` 또는 **좌표 미보유**)가 있으면 상단 **경고 Alert**가 표시되고 **임시 저장·순서 저장·배차 확정**이 **차단**됩니다 (QA-B19, Q233). 이용자 **픽업 주소**를 수정한 뒤 다시 시도하세요.
11. **임시 저장** 후 루트 상세에서 순서·지도·**차량**을 확인하고 **배차 확정**(`CONFIRMED`)합니다.
12. 확정 루트 상세 **`TransportServiceLogPanel`** (G15, Q236) — **별지 제22호 이동서비스일지**:
   - **차량번호**·**운전자**·**동승 직원** 입력
   - 정차별 **실제 픽업 시각**·**동승 여부** 기록 — 계획 픽업 대비 **15분 이내** **「준수」** / 초과 **「지연」** (평가 지표 42)
   - **「인쇄」** — 일지 본문만 출력 (`ds-transport-log-printing`) · **「다운로드」** — `.txt` 파일
   - **미확정(DRAFT)** 루트는 미리보기만 — 제출용 출력은 **확정 후** 권장
   - 실제 픽업·동승 입력은 **브라우저 메모리** — 새로고침 시 초기화 (서버 저장 **미구현**)
13. **`hq_admin`** 이 확정 루트를 되돌려야 하면 루트 상세 하단 **「확정 취소」** → **`TransportUnconfirmModal`** 확인으로 `DRAFT` 복원 후 **순서 저장** → **배차 확정** 재실행합니다 (UXD-47, FAQ **Q163**).
14. 직원(`branch_admin`·요양보호사·사회복지사)은 **`CONFIRMED`(확정)** 상태 루트만 조회합니다 — `DRAFT`는 목록에서 숨겨집니다. 확정 루트 상세에는 **「읽기 전용」** 안내(US-T03)가 표시되며, 정차 목록·지도·geocode **Badge**로 좌표 변환 상태를 확인할 수 있습니다 (UXD-48). **픽업 주소**는 **`hq_admin`이 아닌 역할**에게 **시·구 수준 마스킹**(`e7d4cf6`, `서울시 중구 ***` 등, Q169). **픽업 연락처**는 **`c7941e9`** 백엔드 마스킹 + **`TransportPickupContact`** UI — **`hq_admin`** 만 전체 번호·**`tel:` 링크**, 그 외 역할은 **`010-****-5678`** 마스킹·링크 없음 (UXD-52, Q171·Q172).
15. **Windows 고대비 모드**(Q115·Q168) — 지도 캔버스·정차 카드·마커에 **경계선**이 표시되고, 활성 마커·순번 Badge·강조 정차는 **Highlight outline**으로 식별됩니다. 색상만으로 순서를 구분하지 않습니다 (UXD-50).

> **G16 Partial Fixed (Q239)**: ogada는 **확정 배차 기반 이동서비스비 청구 기록 생성·확정**까지 지원합니다 — **공단 포털 제18·19·20호 자동 제출**은 **미지원** (Q237). §5-8-1 참고.

| API | 용도 |
|-----|------|
| `GET /api/v1/transport/roster?runDate=&direction=PICKUP` | 당일 배차 명단 |
| `POST /api/v1/transport/runs` | DRAFT 루트 생성 (`hq_admin`) — optional `vehicleId` (Q241) |
| `PATCH /api/v1/transport/runs/{id}` | DRAFT 정차 순서 수정 |
...
| `POST /api/v1/transport/runs/suggest` | **v1.3-B** 자동 배차 제안 — **`hq_admin` only** · **PICKUP** · **`TransportSuggestPanel`** (Q347, FE `2ffe59f`) |
...
>
> **지도가 안 보일 때**: 서버에 **`KAKAO_REST_KEY`** 가 설정되어야 geocode·지도 마커가 동작합니다 (DEPLOYMENT §4-6).
...
2. **월** 선택 → 지점 전체 외출 목록(이용자명·일시·사유·상태) 확인.
3. **「인쇄」** — 브라우저 인쇄(`window.print()`). `CANCELLED` 행은 **제외**됩니다.
...
#### 5-8-3. 차량 마스터 (`/transport/vehicles`, G16, Q241)

**케어포 2-4** 대응 — 지점별 **차량번호·정원**을 등록하고 배차·급여제공 기록에 연결합니다 (BE `bd375e6` + FE `107bfb3`).

1. **`TransportContextNav`** → **「차량 관리」** (또는 SideNav **이동 → 차량 관리**).
2. **`BranchScopeNotice`** 로 조회 지점을 확인합니다.
3. **`hq_admin`·`branch_admin`** — **차량 등록** Modal: **차량번호** · **정원**(1–15) · **별칭**(선택) · **활성** 토글.
4. 목록에서 **수정** — 동일 필드 편집. 스크린리더는 **`{차량번호} 차량 수정`** 으로 읽습니다 (UXD-76, Q253). **비활성** 차량은 배차 선택 목록(`activeOnly`)에서 제외됩니다.
5. 등록한 차량은 **새 픽업 배차**·**루트 상세** **`TransportVehicleSelect`** 에 표시됩니다.
6. 확정 배차 + `vehicleId` → 이용자 **「급여제공」** 탭 일별 **차량번호** · 이메일 발송 payload (Q243).

| API | 용도 |
|-----|------|
| `GET /api/v1/transport/vehicles?activeOnly=true` | 활성 차량 목록 (배차 선택) |
| `POST /api/v1/transport/vehicles` | 신규 등록 |
...
2. 상단 **지표43·지표29 준수 현황** — 분기 충족·참석자 2인 이상·30일 급여반영·**지표29 평가 실시** StatCard 4종을 확인합니다 (UXD-79, Q266).
...
5. **`hq_admin`·`branch_admin`·`social_worker`** — **사례관리 회의록 등록** 폼에서 다음 **7필드**를 입력합니다 (FE `443f379`, Q265 Fixed).
   - 이용자 · **회의 일자** · **선정 사유** · **회의 내용** · **회의 결과** · **사례관리 계획** · **참석자**(**쉼표·세미콜론·슬래시** 구분, **최소 2인**)
6. **저장** 시 `POST /api/v1/case-management/meetings` 가 호출됩니다. FE·BE 모두 참석자 구분자를 정규화합니다 (`7eebd8c`·`2225a7a`, QA-B40). 동일 이용자·분기에 이미 회의록이 있으면 **422**(`해당 분기 사례관리 회의 기록이 이미 존재합니다.`)가 반환됩니다.
7. 목록에서 **수정**을 누르면 동일 폼으로 기존 회의록이 로드되고 **`PATCH …/{meetingId}`** 가 호출됩니다 — **이용자 필드는 잠금** · 요청 본문에 **`clientId` 미포함** (`26499b3`·`2cd2cd8`, Q279·Q283).
...
1. SideNav **청구** 그룹에서 **청구대장**·**입금대장**·**수납대장**·**환불대장** 중 하나를 선택합니다. 각 대장·**간편계산기** 화면 상단 **`BillingReportsContextNav`** 로 **5개 화면을 서브 탭** 전환할 수 있습니다 (UXD-81, Q273).
2. **`MonthInput`**으로 **대상 월**을 선택하고 **조회**합니다.
3. (선택) **이용자 검색**(`q`) — 이름·인정번호로 필터합니다.
4. 상단 **StatCard** 요약을 확인합니다.
   - **청구대장**: 청구 건수·총 급여·본인부담 합계·공단부담 합계
   - **입금·수납대장**: 내역 건수·입금(수납) 합계
   - **환불대장**: 환불 건수·환불 금액 합계
5. **`BillingLedgerTable`**에 이용자·청구월·금액·상태·수납일·결제수단·**환불일·환불 사유**(환불대장) 등이 표시됩니다.
   - **청구대장** — **적용 시작월** 조회 시 **「청구시작 기준금액 (G33)」** opening balance 행이 **맨 위**에 표시됩니다 (Q269). 메모는 **인정번호** 열에 표시될 수 있습니다.
6. **인쇄** 버튼으로 대장만 인쇄합니다 (`ds-billing-report-print-zone`).
7. 건수가 많으면 하단 **페이지네이션**으로 이동합니다.

> **API (BE `de49b21`·FE `212e010`, Q179·Q261 Fixed)**: `GET /api/v1/billing/reports/{charges|deposits|receipts|refunds}?month=YYYY-MM&branchId=&q=&page=&size=` — **`hq_admin`·`branch_admin`** 전용.
...
| **계획/청구 탭** | `PLAN`(계획 일정) \| `BILLING`(청구 일정) — 탭 전환 시 해당 `scheduleKind`만 조회 |
| **당일 목록** | 선택 날짜의 이용자·시간·상태·처리 버튼 |
...
3. Modal에서 **사전 점검** 결과를 확인합니다 — **DRAFT 건수**·**이미 확정 건수**·**담당자 미배정**·**페어 불일치** 건수.
4. **차단 사유(blockers)** 가 있으면 — **DRAFT 없음** · **페어 PLAN/BILLING 확정 상태 불일치** — 목록·개별 **확정**으로 먼저 정리한 뒤 재시도합니다. **담당자 미배정**은 경고로 표시되나 **일괄확정 자체는 가능**합니다 (배정된 일정만 담당자 검증).
5. 아래 **확인 Checkbox 2종**을 **모두** 체크합니다 (UXD-102 **`aria-describedby`** 연결):
   - **「공단 청구명세서와 비교했습니다.」** (`nhisComparisonAcknowledged`)
   - **「공단조회 변경이력을 확인했습니다.」** (`changeHistoryChecked`)
6. **「일괄확정 실행」** — **`POST /api/v1/visits/batch-confirm`** — 해당 월·종류의 **모든 DRAFT** 가 **`CONFIRMED`** 로 전환됩니다 (페어 확정 규칙은 개별 확정과 동일, Q259).
7. Checkbox 미체크·사전 점검 미충족·**`fromDate`/`toDate` 누락** 시 **`422 BUSINESS_RULE`** — Modal에 **「일괄확정 기간(시작일/종료일)을 입력해주세요.」** 등 오류가 표시됩니다 (`230659a`, Q330).

| API | 용도 |
|-----|------|
| `GET /api/v1/visits/confirm-readiness?from=&to=&scheduleKind=&branchId=` | 일괄확정 **사전 점검** — `ready`·`blockers[]`·`draftVisitIds[]` |
| `POST /api/v1/visits/batch-confirm` | `{ fromDate, toDate, scheduleKind, branchId, nhisComparisonAcknowledged: true, changeHistoryChecked: true }` |

> **권한**: **`branch_admin`·`social_worker`** — **`caregiver`** 는 batch-confirm **거부** (Q330). **P2**: G21 **live E2E** 잔여.
...
2. **`assignedUserId`(담당 직원)** 가 배정된 일정에서 **`caregiver`** 가 **다른 직원 배정 일정**을 체크인·체크아웃하려 하면 **「배정된 요양보호사만 체크인·체크아웃할 수 있습니다.」** 오류가 표시됩니다 (Q307, `b459f4c`). **`hq_admin`·`branch_admin`·`social_worker`** 는 감독 역할로 예외 없이 가능합니다.
3. 방문 완료 후 **체크아웃**을 눌러 종료 시각을 기록합니다 (`POST …/check-out`).
4. **페어(계획↔청구)** 일정이 연결되어 있으면, 한쪽 체크인·체크아웃 시 **페어 일정의 상태·시각도 동기화**됩니다 (G21, Q238) — **페어가 `CONFIRMED`→`IN_PROGRESS`→`COMPLETED` 허용 전이에 해당할 때만** (`6bfc745`). **페어 일정이 없거나 상태가 맞지 않으면** 「연결된 계획/청구 일정…」 **`422` 오류**로 **한쪽만 진행되지 않습니다** (`45b8147`).
5. 완료된 일정은 **완료**로 표시되며 추가 버튼이 없습니다.

| API | 용도 |
|-----|------|
| `GET /api/v1/visits?from=&to=&scheduleKind=` | 월간 일정 조회 |
...
**활용 팁**: 모니터링 체크리스트의 **integrated checklist panel**에서 **8문항 자동 집계**를 확인하고, 부족한 항목을 지점별로 보완하세요.
...
| 영역 | 설명 |
|------|------|
| **연간 이수 현황** | StatCard — 총 직원·이수 완료·미이수·기한 초과 |
| **교육 유형별 필터** | 드롭다운 — 5가지 유형별로 목록 필터링 |
| **직원별 교육 기록** | 테이블 — 이름·직책·교육 유형·교육일·교육 기관·시간·비고 |
| **등록/수정 모달** | 직원 선택 → 유형·교육일·내용·기관 입력 → 저장 |
...
상단 **`NursingContextNav`** — **8탭** 서브 네비(통합 바이탈·체중·구강·응급 포함) · **`LifecycleWorkflowPanel`** — 4단 진행 상태(위험평가·예방계획·일별 기록·분기 리포트) · **`aria-label="간호급여 하위 메뉴"`**
...
> **P2 잔여**: L03 간호급여 **나머지 leaf**(배설관리·병의원·투약 rpt 등) · **G-NURSING live E2E** (`LIVE_E2E=1`) · **PDF 공식 서식**
...
| 영역 | 설명 |
|------|------|
| **`NursingContextNav`** | **통합 바이탈** · **체중 기록** · **구강상태 점검** · **응급상황 기록** · 위험평가 · 예방계획 · 간호 기록 · 분기 리포트 **8탭** |
| **등록·수정 폼** | **`NursingVitalCheckForm`** — `Field` render-prop으로 label·id·필드 오류 연결 |
| **최근 기록 표** | 기본 **최근 30일** 목록 — 이용자·일시·혈압·맥박·호흡·체온·SpO₂ |
...
1. 우측 목록에서 **수정**을 클릭하면 좌측 폼에 기존 값이 채워집니다 — 이용자·점검일·시각은 수정 시 **변경 불가**.
...
| 영역 | 설명 |
|------|------|
| **`NursingContextNav`** | §5-16과 동일 **8탭** — **체중 기록** 탭이 두 번째 |
...
| 영역 | 설명 |
|------|------|
| **`NursingContextNav`** | §5-16과 동일 **8탭** — **구강상태 점검** 탭이 세 번째 |
...
| 영역 | 설명 |
|------|------|
| **`NursingContextNav`** | §5-16과 동일 **8탭** — **응급상황 기록** 탭이 네 번째 |
...
### 5-20. 간호급여 제공기록 (L03_M01, BE-only)

케어포 **view.nursing_service** (3-1 간호급여 제공기록)에 대응합니다. **이용자×제공일** 단위로 **간호·투약·진료(외래)** 제공 여부를 기록합니다.

> **현재 UI 없음 (BE `9bd1660`, V123, Q348)** — 화면·SideNav 메뉴는 **미구현**입니다. Swagger(`POST/GET/PATCH /api/v1/nursing/service-records`) 또는 연동 스크립트로만 등록·조회할 수 있습니다. **P2**: FE wire.
...
| 엔드포인트 | 메서드 | 용도 |
|-----------|:------:|------|
| `/api/v1/nursing/service-records` | GET | `fromDate`·`toDate`·`clientId` 필터 목록 (기본 최근 90일) |
| — | POST | `{ clientId, serviceDate, nursingProvided, … }` 등록 |
| `/{recordId}` | PATCH | 동일 필드 수정 |

> 관련: FAQ **Q348** · ADMIN_GUIDE §6-2-9f · BENCHMARK_REPORT L03_M01 · **P2**: `/nursing/service-records` FE wire
...
| **간호급여 제공기록 (L03_M01)** | ✅ (API) | ❌ UI — Swagger only (§5-20, Q348) |
...
2. **`CONFIRMED`** 행의 **체크인** → 방문 시작.
3. 서비스 종료 후 **체크아웃** → 완료 처리.
...
3. **저장**합니다. **기록 이력** 탭에서 당일 입력을 확인할 수 있습니다.
...
> **현재 UI (`eedcc80`)** — **`GuardianDailySummary`**(출석·건강·**식사**, UXD-62) + **명세 CONFIRMED/PAID 필터·더 보기·금액 정규화·이용시간대 스냅샷**(Q212·Q213·Q220) + **`GuardianBillingDetailModal`** 상세·인쇄 **Fixed**. **알림 발송 이력 탭 Fixed**(BNK-22). **식사 실데이터**는 **`daily-records` BE `meals[]` 후속**(Q188). 직원 측 **노인학대예방 지침·납부확인서**는 §4-7-3·§4-6 UI **Fixed**(Q221·Q222) — **급여제공기록지·가정통신문 UI**는 **후속**(Q216·Q217). **CMS·알림톡 실발송**은 **stub/후속** — **`/billing/cms`** (Q207·Q208).
...
2. **QR 체크인**(`/attendance/checkin/qr`)으로 이동합니다. *(FE `a627c6d`: **입소/귀가 `FilterChips`** 선택 후 토큰 입력 — 카메라 스캔 **준비 중**, FAQ Q109)*
...
| 결석 버튼이 없어요 | Q94 roster·`status`·`clientName` 불일치 — Swagger `POST /attendance/absence` 우회 |
...
| QR·체크인이 API 오류로 실패해요 | 입소·귀가·결석 **버튼은 US-E01·E02에서 제공**. API 본문·필드·roster 불일치 시 FAQ **Q94·Q96·Q109** |
