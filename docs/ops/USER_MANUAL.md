...
### 1-3. 구현 상태 안내 (2026-06-17 develop HEAD `aa42b9c` / frontend `6a18dfd` 기준)

| 영역 | 상태 | 비고 |
|------|------|------|
| 백엔드 API | **Must + … + G15 service log audit @ `aa42b9c` · monthly reports 2-7/2-8 @ `5d27ad3` · QA-B95 probe guard @ `0b5657a` · V148 · US-T02·…** | **`TRANSPORT_SERVICE_LOG_UPSERT` audit** · **`GET /transport/reports/monthly-*`** |
| 데이터베이스 | Flyway **V1–V148** | **V148** service log compliance · **V147** guardian trigger · **V146** DROPOFF · 월간 리포트는 **기존 roster·runs·contracts 집계**(신규 마이그레이션 없음) |
| 프론트엔드 | **106 route · G15 `TransportServiceLogPanel` archive @ `088e906` · `TransportMonthlyReportsPage` @ `6a18dfd` · QA-B116 a11y @ `dff2f32` · US-E04·US-T02·compliance·SEC-005·L02·G26·G21** | **일지 보관·감사 추적** · **`/reports/transport-monthly`** |
| UI 연동 완료 | **… + G15 별지 제22호 일지 서버 저장·감사·보관 UX·월간 리포트 2-7/2-8(full stack) · US-T02·transport IA 154차·compliance·tab session·…** | **Q407·Q410·Q411** · **P2**: CSV export · **staging `./scripts/run-live-e2e.sh` RUN 확인 권장** (Q409) |
| UI API 갭 | **출석 roster·QR·통계·본인부담 비율 저장·보호자 식사 API·가정통신문·공단 포털 자동 제출** | 출석 roster(Q94)·QR(Q109) |
| **P2 Planned** | **L03 간호급여 잔여 5 leaf·7-5 live PG·J03 Solapi live dispatch·건강검진 결과통보서·LCMS CMS 3-method·G34 SMS live·G-Payroll·G30 live E2E** | **G-STAFF-WELFARE P3**(FAQ21796) · **8-12 PDF 공식 서식**(Q315) |
...
> **현재 UI 상태 (FE `6a18dfd`)**: **106 route** · **`/transport`** — **승차/하차** 방향별 운행 루트·명단·수동 배차 (Q399) · **희망 탑승·하차 시각** (Q400) · **`TransportRouteSplitView`** 지점 경유·번호 마커·**지도 범례·지점 핀 SR** (Q401·Q404) · **`/transport/compliance`** (Q397) · **`/reports/transport-monthly`** — **월간 변동·입소자 현황** (Q410) · **SEC-005 tab session** (Q396) · **`/transport/runs/:runId`** — **G15 별지 제22호 일지 API 저장·감사 추적** (Q407·Q411) · **DRAFT 삭제** (Q403) · **`/transport/vehicles`** **기본 운전자명** (Q402) · **`/care/reports/nursing-service`** 3탭 · **`/billing/reports/statistics`** G26 · **`/visits`** G21 split-view
...
| **청구** | 청구·NHIS·입금·미납·**간편결제**·CMS·**본인부담 통계 (G26)**·수가표·**청구/입금/수납/환불 대장** | ✅ | ✅ | — | — |

> **청구 리포트 in-page 네비 (`BillingReportsContextNav`, G26, Q379·Q380)**: **청구대장** · **입금대장** · **수납대장** · **환불대장** · **본인부담 통계** 5화면 상단에서 **서브 탭**으로 전환합니다 — SideNav **청구 → 본인부담 통계**(`/billing/reports/statistics`)와 병행합니다.
...
2. 상단 **통합 모니터링 checklist** 패널에서 선택 **연·월** 기준 **FAQ21838~21842 문항 6~15** 준수 현황을 확인합니다 (Q320). **증빙 수집 기간(FAQ21838 전전월 ±2개월)** 이 패널 상단에 표시됩니다 — 예: 2026년 6월 모니터링 → **2026-02-01 ~ 2026-06-30** (`MonitoringEvidenceWindow`, BE `73df04d` / FE `73094f9`). **충족·일부 충족·미충족·수동 점검** StatCard와 8행 표가 표시됩니다. **RFID(문항 10)** 는 **수동 점검(MANUAL)** 로만 표시됩니다.
...
7. **유선상담 (월 5명·60% 만족, FAQ21841)** — **유선상담 실시**·**만족(Y) 비율** StatCard로 **등록 건수·만족 비율**을 확인합니다 (`9ad8346`, Q365). 5명 미달·60% 미달 시 **warning Alert**가 표시됩니다. **추천 대상** 목록에서 이용자를 고른 뒤 **이용자·상담일·상담 내용·만족(Y/N)** 을 입력해 등록합니다. **이용자×월 1건** 제한 · **퇴소·비활성 이용자**는 등록 불가 (V101·V138).
...
| 청구 상세 (`/billing/claims/:id`) | **`BillingDetailPage`** — **`DRAFT`** 시 **「공단 명세 비교」** (`BillingNhisComparisonPanel`, Q264) · **`CONFIRMED`/`PAID`** 시 **「명세 4채널 발송」** (`BillingStatementDispatchPanel`, Q364) · **「보호자 발송」** · **「납부확인서 발송」** (`PAID`+`paidAt`+`paymentMethod`, Q221·Q249) · **「환불 처리」** (`PAID`+`paidAt`, Q261) | **`GET/POST …/statement-dispatches`** · **`PATCH …/statement-dispatches/{id}`** · **`GET /claims/{id}/nhis-comparison`** · **`POST /claims/{id}/notify`** · **`POST /claims/{id}/payment-receipt-notify`** · **`POST /claims/{id}/refunds`** |
...
청구 **상세**(`/billing/claims/:claimId`) — **`BillingDetailPage`** — **`DRAFT`** 시 **「공단 명세 비교」** (`BillingNhisComparisonPanel`, Q264) · **`CONFIRMED`/`PAID`** 시 **「명세 4채널 발송」** 카드 — **`BillingStatementDispatchPanel`** (케어포 PDF p.87, G-7-1-4CHANNEL, Q364) · 요약·급여 항목에 **이용시간대** 표시 — **`resolveClaimDurationBand`** · 확정·수납 시 **「보호자 발송」**(Q196)·상태 변경·인쇄.
...
6. **「배차 신청 인원 명단」** — 당일 roster · **연락처**(`contact`)·**보호자 연락처**(`guardianContact`, Q398) · **희망 탑승 시각**·**희망 하차 시각** (Q400) · **`TransportPickupContact`** — **`hq_admin`** 만 전체 번호·`tel:` 링크, 그 외 **`010-****-5678`** 마스킹 (Q171·Q172).
7. **`hq_admin` only — 자동 배차 (승차, v1.3-B)** — **승차** 방향일 때만 **`BranchTransportSettingsPanel`** + **`TransportSuggestPanel`** — `POST /api/v1/transport/runs/suggest` · **일 10회** 상한 · 제안 DRAFT는 **운행 루트**에서 검토 (Q347).
8. **`hq_admin` only — 수동 배차** — **「승차/하차 수동 배차 생성」** → `/transport/runs/new` (방향·운행일 state 전달) · 해당 방향에 **이미 확정 배차에 포함된 이용자**는 선택 불가.
9. 수동 배차 화면 **`TransportRouteSplitView`** (Q401) — 왼쪽 **경로 지도**(번호 마커·정차 클릭 시 하이라이트 동기화) · 오른쪽 **정차 목록**(드래그 순서 변경) · **`지점 추가`** 로 센터(depot) **BRANCH** 경유 정차 삽입(US-T02). 이용자 정차는 **최대 15명**(`MAX_TRANSPORT_STOPS`), 지점 포함 **최대 17** 정차까지 가능합니다.
10. **`TransportVehicleSelect`** 로 **지점 차량**을 지정합니다 — 차량 **기본 운전자명**이 라벨에 표시됩니다 (Q402). 정차 수가 차량 **정원(capacity)** 을 초과하면 경고가 표시됩니다 (Q241).
11. **지도 제외 주소**(geocode `FAILED` 또는 **좌표 미보유**)가 있으면 상단 **경고 Alert**가 표시되고 **임시 저장·순서 저장·배차 확정**이 **차단**됩니다 (QA-B19, Q233). **`KakaoGeocodeClient`**·**`KoreanGeocodeUtil`**·**`kakaoMapGeocoder.js`** 가 주소→좌표를 해석합니다 (BE `114411f` / FE `d3bef42`).
12. **임시 저장** 후 루트 상세에서 순서·지도·**차량**을 확인하고 **배차 확정**(`CONFIRMED`)합니다.
13. 확정 루트 상세 **`TransportServiceLogPanel`** (G15, Q236·**Q407·Q411**, BE `0cfa970`/`aaaeb10`/`aa42b9c` / FE `088e906`/`dff2f32`) — **별지 제22호 이동서비스일지**:
...
14. **`hq_admin`** 이 확정 루트를 되돌려야 하면 루트 상세 하단 **「확정 취소」** → **`TransportUnconfirmModal`** 확인으로 `DRAFT` 복원 후 **순서 저장** → **배차 확정** 재실행합니다 (UXD-47, FAQ **Q163**).
15. **`hq_admin`** 이 **임시 저장(DRAFT)** 루트를 버리려면 루트 상세 **「삭제」** → **`TransportDeleteRunModal`** 확인으로 삭제합니다 (Q403, BE `1d1a71f` / FE `45bd923`). 모달에는 **운행일**·**정차 수**가 표시되고 **「삭제한 배차는 복구할 수 없습니다」** 경고가 나타납니다. **확정 루트는 삭제할 수 없습니다** — 확정 취소 후 편집하세요.
16. 직원(`branch_admin`·요양보호사·사회복지사)은 **`CONFIRMED`(확정)** 상태 루트만 조회합니다 — `DRAFT`는 목록에서 숨겨집니다. 확정 루트 상세에는 **「읽기 전용」** 안내(US-T03)가 표시되며, 정차 목록·지도·geocode **Badge**로 좌표 변환 상태를 확인할 수 있습니다 (UXD-48). **픽업 주소**는 **`hq_admin`이 아닌 역할**에게 **시·구 수준 마스킹**(`e7d4cf6`, `서울시 중구 ***` 등, Q169).
17. **Windows 고대비 모드**(Q115·Q168) — 지도 캔버스·정차 카드·마커에 **경계선**이 표시되고, 활성 마커·순번 Badge·강조 정차는 **Highlight outline**으로 식별됩니다. 색상만으로 순서를 구분하지 않습니다 (UXD-50).
18. **도로 경로 미리보기 (Q370·Q394·Q395·Q401·Q404, FE `10489a7`/`5ebaade`)** — **`KakaoTransportMap`** 이 **`loadKakaoMapSdk`** 로 Kakao Maps **JavaScript SDK**를 로드하고, **`kakaoMapInstance.js`**·**`useKakaoMap`** 으로 지도 인스턴스를 관리합니다. **`KakaoTransportMapView`** 에서 **번호 마커**·정차 **도로 폴리라인**·**하이라이트 동기화**를 표시하며, **route-preview 응답 전**에도 좌표가 있으면 **seed markers**로 마커를 먼저 표시합니다. **지점 핀**은 스크린리더에 **「지점(센터) 정차」** 로 읽히고, 하단 **범례**에 **파란 선=도로 경로·회색 점선=정차 안내선·「센」=지점** 이 표시됩니다 (Q404). **`POST /api/v1/transport/route-preview`** 로 **실제 도로 경로**를 받으며, 정차 좌표는 **`pickupLat`/`pickupLng`** 또는 geocode 결과를 사용합니다 — **건물번호 일치** 시 Kakao 후보 우선 (`1d1a71f`). 지도 캔버스는 **`role="application"`**·**`aria-label`** 로 스크린리더에 구역을 알리고, 상단 **거리·예상 소요 시간** 요약은 **`role="status" aria-live="polite"`** 라이브 영역입니다. **`VITE_KAKAO_MAP_JS_KEY`** 미설정·SDK 로드 실패 시 **Spinner 대신 경고 Alert**가 표시되고 **정차 목록은 계속** 확인할 수 있습니다 (FAQ **Q394·Q395**).
...
| `POST /api/v1/transport/runs` | DRAFT 루트 생성 (`hq_admin`) — `direction` · `stops[]` with **`stopKind` CLIENT/BRANCH** (US-T02) · optional `vehicleId` |
| `PATCH /api/v1/transport/runs/{id}` | DRAFT 정차 순서·지점 경유 수정 |
...
| `POST /api/v1/transport/route-preview` | **도로 경로 미리보기** — 지점·정차 좌표 → polyline·거리·시간 (Q370, `e8b8398`) |
...
>
> **지도가 안 보일 때**: (1) 서버 **`KAKAO_REST_KEY`** — geocode·경로 API (2) 프론트 **`VITE_KAKAO_MAP_JS_KEY`** — **`loadKakaoMapSdk`** (3) JS 키 **Web 도메인**에 접속 URL 등록 (FAQ **Q370·Q394·Q395**, DEPLOYMENT §4-7).
...
3. **`TransportCompliancePanel`** (Q230·Q231) — **이동서비스 수칙 체크리스트**(5항목) · **계약서 템플릿** Modal · **별지 제22호 일지 예시** Modal. **이동서비스 이용(`usesTransport=true`)·활성·미퇴소** 이용자만 저장 가능 (V65, Q235). **이용자 선택** → 5항목 체크 → **보호자·기관 서명**·**서명일(`DateInput`, Q281)** → **저장**(`PUT /api/v1/transport/contracts/{clientId}`). **`hq_admin`·`branch_admin`·`social_worker`** 만 저장 · **`caregiver`** 조회만.
4. **`TransportForm18GuidePanel`** (Q237) — **별지 제18·19·20호** 공단 포털 **선행 신청 5단계** · **3분리 신청 유형** · **등록상태 4단**. ogada는 **자동 제출하지 않습니다** — 수칙·계약 → **제22호 일지**(§5-8 step 12) → **공단 포털 제18호** → **제19·20호** → **G16 청구**(§5-8-1) 순서.

> **G15 일지 (Q407/Q411, v1.3-C full stack)**: 확정 루트 **`TransportServiceLogPanel`** 에서 **서버 저장·감사 추적·보관 안내·인쇄·텍스트 다운로드**가 모두 지원됩니다. 수칙·계약(§5-8-0) 완료 후 일지를 기록하세요.

#### 5-8-0-1. 이동서비스 일지 기록 (별지 제22호, G15, Q407·Q411)

**2026 평가 지표 41·42** 대응 **이동서비스 일지(별지 제22호)** 기록·저장·보관·인쇄입니다 (BE `0cfa970`·`aaaeb10`·`aa42b9c` + FE `088e906`/`dff2f32`, **v1.3-C G15 full stack**, Q407·Q411).
...
4. **`TransportServiceLogPanel`** — **별지 제22호 형식 표 진입**:
   - **운행 정보**: 운행일, 차량, 기사(read-only)
   - **정차별 실적**: 각 정차(이용자) 행마다:
     - **「동행자명」**: 동행인 이름(선택 입력)
     - **「실제 탑승 시간」**: 실제 픽업 시각 (시:분)
     - **「탑승 여부」**: 예/아니오 (FE 라벨: 「동행자 부착」)
     - **「실제 하차 시간」**: 실제 드롭오프 시각 (시:분)
   - **15분 준수**: 각 행 아래 **`TransportTimeCompliance`** 표시 — 「예정 시간 ±15분 범위 준수」 상태 표시.

5. **저장**: **`PUT /api/v1/transport/runs/{runId}/service-log`** — **CONFIRMED 운행만** 저장 가능. 수정/추가 시 기존 레코드 **upsert**(동일 `runId` 기존 기록 덮어쓰기). 저장 성공 시 **`TRANSPORT_SERVICE_LOG_UPSERT`** 감사 로그가 기록됩니다 — **`/settings` → 감사 로그** 탭에서 확인 (Q411).

6. **보관·출력**:
   - **「일지 보관·감사 추적」** 패널에서 **배차 확정 시각**·**DB 저장 건수**·**마지막 저장**·**저장 상태** 확인
   - **인쇄** — **미저장 변경** 또는 **일부 정차 미기록** 시 경고 후 **차단** · 저장 완료 후 Print Dialog (`Ctrl+P` / `Cmd+P`) 또는 **「인쇄」** 버튼 (`ds-transport-log-printing`)
   - **텍스트 다운로드** — 동일 가드 적용 후 **`.txt`** 파일 (시간 준수 요약 포함)

**조회·확인**

- **`GET /api/v1/transport/runs/{runId}/service-log`** — 저장된 일지 조회 (응답: 전체 정차·시간 정보·준수 요약)
...
#### 지점별 연말정산·본인부담 통계 (G26 / L07_M09, Q379·Q380·Q381·Q382 — FE `09e4ec1`·BE `903f462`/`6d10e0d`/`3672bbe`)
...
4. **「인쇄」** 로 현재 집계를 인쇄합니다 (`ds-billing-report-print-zone`).
...
| 집계 규칙 | Q252·Q254와 동일 — **`PAID`+`paidAt`** · **`CASH`·`BANK_TRANSFER`만** · **CMS·간편결제 제외** |
...
> **개별 이용자 상세**는 이용자 상세 **「청구」** 탭 **`MedicalExpenseDeductionPanel`** (Q252)을 사용하세요. **국세청 엑셀 export**는 이용자 상세(Q258)에서 제공하며, **지점 통계 화면 CSV export**는 후속(P2)입니다.

| API (참고) | 용도 |
|------------|------|
| **`GET /api/v1/billing/reports/medical-deduction?taxYear=&branchId=&q=&page=&size=`** | ① 이용자별 PAID copay 집계 |
...
| **RFID split-view (Q377, FE `55fdbd0`)** | **`Switch`「계획·청구 분할 비교 (RFID split-view)」** — 켜면 탭 숨김 · 선택 날짜 **계획·청구 일정 나란히** (`ds-visits-split-compare`) · 청구 열에만 **「청구반영」** 표시 |
...
1. **`/visits`** 상단 **`Switch`「계획·청구 분할 비교 (RFID split-view)」** 를 켭니다.
...
7. 분할 비교를 끄면 기존 **계획/청구 탭** UI로 돌아갑니다.

> **접근성**: split-view 안내 Alert는 **텍스트 라벨**(청구반영·미반영·페어 없음)로 상태를 설명합니다 — 색상(검은/빨간)만으로 의미를 전달하지 않습니다 (`25291b3`, DESIGN_SYSTEM §1-2).
...
케어포 **2-4 신체제재**(`view.care_sanction`)에 대응하는 **요양기록** 화면입니다 (FE `14a2bb9`, BE `ea6092a`, **V131**). SideNav **기록 → 신체제재 기록 (L02_M07)** 또는 **`/care/body-restraint`** 으로 이동합니다 (FAQ **Q361**).
...
> 관련: FAQ **Q362** · ADMIN_GUIDE §6-2-15 · REQUIREMENTS L02 v3.1 Must

### 5-26. 목욕 일정·제공현황 (L02_M03, v3.1 Must — **화면 연동 완료**)

케어포 **2-3 목욕일정 및 제공현황**(`view.care_bath_manage`)에 대응합니다 (FE `950415d`, BE `e703252`/`47a4e25`, **V136**). SideNav **기록 → 목욕 일정·제공현황 (L02_M03)** 또는 **`/care/bathing-schedules`** (FAQ **Q363**).
...
2. **「새 목욕 일정」** 카드에서 **`BathingScheduleForm`** 을 작성합니다.
3. 이용자·예정일·목욕 유형·제공 상태를 입력하고 **저장**합니다.
4. 상태를 **제공 완료**로 바꿀 때는 **제공 내용**(`provisionNotes`)이 필수입니다.
5. **취소**·**미제공** 처리 시 **비고(사유)**(`notes`)가 필수입니다 (`47a4e25`).
...
> 관련: FAQ **Q363** · ADMIN_GUIDE §6-2-16 · REQUIREMENTS L02 v3.1 Must

### 5-27. 통합식사도움기록 (L02_M13, v3.1 Must — **화면 연동 완료**)

케어포 **2-1-1 통합식사도움기록**(`view.total_meal`)에 대응합니다 (BE `81a2223` / FE `9ad8346`, **V140**). SideNav **기록 → 통합식사도움기록 (L02_M13)** 또는 **`/care/meal-assistance-records`** 로 이동합니다 (FAQ **Q366**).
...
> 관련: FAQ **Q386·Q389·Q390** · ADMIN_GUIDE §6-2-26 · §5-22(간호 모듈 경로) · REQUIREMENTS L02/L03 parity
